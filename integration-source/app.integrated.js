...
const mainNav = document.getElementById('mainNav');
    ...
    if(recordingPlayer){
      ...
      if(findatProtectedPosterPath){recordingPlayer.poster=findatProtectedPosterPath;if(startRecordingLesson){startRecordingLesson.classList.add('has-thumbnail');startRecordingLesson.style.backgroundImage=`url("${findatProtectedPosterPath}")`}}
    ...
    function syncRecordingLearningState(){
      ...
      if(recordingStartLabel) recordingStartLabel.textContent=complete?'Replay session':hasResume?'Resume session':'Start session';
...
(()=>{
      ...
      let socialCache={reactions:[],bookmarks:[],follows:[],comments:[],messages:[],clears:[]};
      let socialAvailable=true;
      let socialError='';
      let socialActiveTab='feed';
      let socialCommentArticleId='';
      let socialCommentReplyId='';
      let socialChatUserId='';
      let socialNetworkQuery='';
      let socialNetworkPage=1;
      const SOCIAL_NETWORK_PAGE_SIZE=4;
      let socialChannel=null;
      let socialPresenceChannel=null;
      let socialTypingChannel=null;
      let socialPresenceByUser=new Map();
      let socialTypingByUser=new Map();
      let socialTypingTimers=new Map();
      let socialPresenceIdleTimer=0;
      let socialTypingStopTimer=0;
      let socialPresenceSessionId='';
      let socialPresenceOnlineAt='';
      let socialPresenceStatus='offline';
      let socialPresenceReady=false;
      let socialPresenceListenersBound=false;
      let socialTypingChannelReady=false;
      let socialTypingSent=false;
      let socialTypingLastSentAt=0;
      ...
      const socialEmojiSet=['😀','😄','😊','🙂','😂','😍','🤝','👏','👍','👎','🙏','💡','✅','🎉','🔥','💯','📌','📊','📈','📉','💼','📝','🔍','⚠️','❤️','⭐','🌍','🚀','👀','🤔','🙌','💬'];
      const SOCIAL_AWAY_DELAY=5*60*1000;
      const SOCIAL_TYPING_IDLE_DELAY=1800;
      const SOCIAL_TYPING_EXPIRY=4200;

      function profileFromRow(row){
        if(!row)return null;
        const displayName=String(row.display_name||'').trim();
        return{id:row.id,displayName,firstName:row.first_name||'',lastName:row.last_name||'',email:row.email||'',username:row.username||'',phone:row.phone||'',organisation:row.organisation||'',role:normaliseRole(row.role),country:row.country||'',avatarUrl:row.avatar_url||'',coverUrl:row.cover_url||'',qualifications:row.qualifications||'',jobTitle:row.job_title||'',placeOfWork:row.place_of_work||'',active:row.active!==false,emailConfirmed:row.emailConfirmed===true||Boolean(row.email_confirmed_at),emailConfirmedAt:row.email_confirmed_at||'',lastSignInAt:row.last_sign_in_at||'',createdAt:row.created_at||'',updatedAt:row.updated_at||''}
      }
      ...
      async function waitForOwnProfile(userId,attempts=10){for(let attempt=0;attempt<attempts;attempt+=1){const account=await fetchOwnProfile(userId);if(account)return account;await new Promise(resolve=>setTimeout(resolve,120+attempt*40))}return null}
      ...
      async function clearSession(){stopCollaborationRealtime();stopSocialRealtime();if(supabaseClient)await supabaseClient.auth.signOut();activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];collaborationActivity=[];notificationFeed=[];notificationCommentArticleId='';notificationCommentReplyId='';contributorLayoutDraft=[];contributorLayoutArticleId='';contributorLayoutDirty=false;reviewSelection.clear();workspaceCache={articles:[],audit:[]};resetSocialState()}
      function readableError(value,fallback='The request could not be completed.'){
        ...
        const read=item=>{
          ...
          visited.add(item);
          for(const key of ['message','error_description','error','msg','details','hint','reason','code']){
            const text=read(item[key]);
      ...
      function friendlySignupError(error){
        const raw=readableError(error,'The account could not be created.');
        const value=raw.toLowerCase();
        const code=String(error?.code||error?.error_code||'').toLowerCase();
        if(value.includes('email rate limit exceeded')||value.includes('rate limit')&&value.includes('email')||code.includes('email_send_rate'))return'The registration email service has reached its temporary sending limit. Please try again later or contact the Administrator.';
        if(value.includes('user already registered')||value.includes('already been registered')||code==='user_already_exists'||code==='email_exists')return'An account already uses this email address. Log in or use password recovery.';
        if(value.includes('invalid email')||code==='email_address_invalid')return'Enter a valid email address.';
        if(value.includes('password')&&value.includes('weak'))return'Choose a stronger password containing at least 8 characters.';
        return raw;
      }
      function setStatus(id,message,type=''){const el=document.getElementById(id);if(!el)return;el.textContent=message===null||message===undefined?'':typeof message==='string'?message:readableError(message,'The request could not be completed.');el.classList.remove('is-success','is-error');if(type==='success')el.classList.add('is-success');if(type==='error')el.classList.add('is-error')}
      ...
      function profileAvatarPath(userId=activeAccount?.id){return userId?`findat-v1/profiles/${userId}/avatar.jpg`:''}
      function profileCoverPath(userId=activeAccount?.id){return userId?`findat-v1/profiles/${userId}/cover.jpg`:''}
      ...
      function resizeCoverFile(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(new Error('The cover picture could not be read.'));reader.onload=()=>{const image=new Image();image.onerror=()=>reject(new Error('Choose a valid cover image.'));image.onload=()=>{const width=1400,height=420,canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const context=canvas.getContext('2d'),scale=Math.max(width/image.naturalWidth,height/image.naturalHeight),drawWidth=image.naturalWidth*scale,drawHeight=image.naturalHeight*scale;context.drawImage(image,(width-drawWidth)/2,(height-drawHeight)/2,drawWidth,drawHeight);canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('The cover picture could not be prepared.')),'image/jpeg',.84)};image.src=String(reader.result)};reader.readAsDataURL(file)})}
      function renderProfileCoverModal(){const preview=$('#fdProfileCoverPreview');if(!preview||!activeAccount)return;preview.style.backgroundImage=activeAccount.coverUrl?`linear-gradient(rgba(24,37,54,.1),rgba(24,37,54,.22)),url("${String(activeAccount.coverUrl).replace(/"/g,'')}")`:'';preview.innerHTML=activeAccount.coverUrl?'':'<span><i class="fas fa-image"></i> Profile cover</span>';if($('#fdRemoveProfileCover'))$('#fdRemoveProfileCover').disabled=!activeAccount.coverUrl}
      async function persistProfileCover(url){const result=await supabaseClient.rpc('findat_set_profile_cover',{p_cover_url:url});if(result.error)throw result.error;return String(result.data||url||'')}
      function updateCachedCover(url){activeAccount.coverUrl=url;accountCache=accountCache.map(item=>item.id===activeAccount.id?{...item,coverUrl:url}:item);directoryCache=directoryCache.map(item=>item.id===activeAccount.id?{...item,coverUrl:url}:item);renderProfileCoverModal();renderAll()}
      async function uploadProfileCover(file){if(!file||!activeAccount)return;if(file.size>15*1024*1024){setStatus('fdProfilePhotoStatus','Choose a cover image smaller than 15 MB.','error');return}setStatus('fdProfilePhotoStatus','Preparing and uploading your cover picture…','');try{const blob=await resizeCoverFile(file),path=profileCoverPath();let url='',savedInStorage=false;try{const upload=await supabaseClient.storage.from('findat-documents').upload(path,blob,{upsert:true,contentType:'image/jpeg',cacheControl:'3600'});if(upload.error)throw upload.error;const publicData=supabaseClient.storage.from('findat-documents').getPublicUrl(path).data;if(!publicData?.publicUrl)throw new Error('The cover picture address was not returned.');url=`${publicData.publicUrl}?v=${Date.now()}`;savedInStorage=true}catch(error){url=await blobAsDataURL(blob)}const saved=await persistProfileCover(url);updateCachedCover(saved);setStatus('fdProfilePhotoStatus',savedInStorage?'Profile cover saved.':'Profile cover saved using the available profile storage.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The profile cover could not be uploaded.','error')}finally{if($('#fdProfileCoverInput'))$('#fdProfileCoverInput').value=''}}
      async function removeProfileCover(){if(!activeAccount)return;setStatus('fdProfilePhotoStatus','Removing profile cover…','');try{try{await supabaseClient.storage.from('findat-documents').remove([profileCoverPath()])}catch(error){}const result=await supabaseClient.rpc('findat_clear_profile_cover');if(result.error)throw result.error;updateCachedCover('');setStatus('fdProfilePhotoStatus','Profile cover removed.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The profile cover could not be removed.','error')}}
      async function uploadRegistrationAvatar(file,userId){if(!file||!userId)throw new Error('Choose a profile picture.');if(file.size>12*1024*1024)throw new Error('Choose a profile picture smaller than 12 MB.');const blob=await resizeAvatarFile(file),path=profileAvatarPath(userId);let url='';try{const upload=await supabaseClient.storage.from('findat-documents').upload(path,blob,{upsert:true,contentType:'image/jpeg',cacheControl:'3600'});if(upload.error)throw upload.error;const publicData=supabaseClient.storage.from('findat-documents').getPublicUrl(path).data;if(!publicData?.publicUrl)throw new Error('The profile picture address was not returned.');url=`${publicData.publicUrl}?v=${Date.now()}`}catch(error){url=await blobAsDataURL(blob)}const result=await supabaseClient.rpc('findat_set_profile_avatar',{p_avatar_url:url});if(result.error)throw result.error;return String(result.data||url)}

      function renderProfilePhotoModal(){const preview=$('#fdProfilePhotoPreview');if(!preview||!activeAccount)return;preview.innerHTML=activeAccount.avatarUrl?`<img src="${escapeHTML(activeAccount.avatarUrl)}" alt="${escapeHTML(accountName(activeAccount))} profile picture">`:escapeHTML(initials(activeAccount));if($('#fdRemoveProfilePhoto'))$('#fdRemoveProfilePhoto').disabled=!activeAccount.avatarUrl;renderProfileCoverModal();setStatus('fdProfilePhotoStatus','')}
      ...
      async function uploadProfilePhoto(file){
        ...
        try{
          ...
          setStatus('fdProfilePhotoStatus',savedInStorage?'Profile picture saved and updated across FINDAT.':`Profile picture saved using the available profile storage${storageError?.message?`: ${storageError.message}`:''}.`,'success');
        }catch(error){
      ...
      async function enterDashboard(account){if(!account||account.active===false)return;activeAccount=account;accountCache=[account];closeAllAuthModals();document.body.classList.add('dashboard-mode');dashboard?.setAttribute('aria-hidden','false');history.replaceState(null,'','#writing');syncPublicAuthBar(account);hydrateUser();applyRoleVisibility();renderAll();try{await loadAuthenticatedWorkspace();await migrateLegacyWorkspaceIfEmpty();populateCollaborators();renderAll();startCollaborationRealtime();startSocialRealtime();if(activeAccount.role==='consultant'){const assigned=visibleWritingArticles()[0];if(assigned)openEditor(assigned.id);else setStatus('fdArticleStatus','No collaborative or publication-assigned article is available yet.','')}}catch(error){console.error(error);setStatus('fdArticleStatus',error.message||'The workspace could not be loaded.','error')}showView('writing');clearInterval(collaborationRefreshTimer);collaborationRefreshTimer=setInterval(async()=>{try{await refreshCollaborationState();await refreshArticles();await refreshSocialState({quiet:true});renderAll()}catch(error){console.warn('Collaboration refresh failed',error)}},10000)}
      ...
      function renderCollaborationInbox(){
        ...
        list.innerHTML=rows.length?rows.map(request=>{
          ...
          const accountApproval=request.kind==='system'&&request.status==='pending'&&/^Account approval required:/i.test(String(request.message||''));
          ...
          const viewButton=accountApproval?`<button class="fd-notification-open" data-account-approval-open="true" type="button"><i class="fas fa-user-check"></i> Review account</button>`:viewLabel?`<button class="fd-notification-open" data-notification-open="${escapeHTML(request.article_id||'')}" data-notification-open-kind="${escapeHTML(request.kind||'')}" type="button"><i class="fas ${request.kind==='article_comment'?'fa-comments':'fa-arrow-right'}"></i> ${viewLabel}</button>`:'';
          const statusLabel=accountApproval?'Pending account approval':collaborationStatusLabel(request.status);
          return `<article class="fd-collaboration-request is-${escapeHTML(request.status||'none')} ${request.is_read===false?'is-unread':''} ${request.article_id||accountApproval?'is-clickable':''}" data-notification-article="${escapeHTML(request.article_id||'')}" data-notification-kind="${escapeHTML(request.kind||'')}">${avatarMarkup(person,'fd-collaboration-avatar')}<div><p>${sentence}</p><div class="fd-collaboration-meta"><span class="fd-collaboration-status is-${escapeHTML(request.status||'none')}">${escapeHTML(statusLabel)}</span><time>${escapeHTML(formatDate(request.updated_at||request.created_at))}</time>${viewButton}</div>${actions}</div></article>`
      ...
      function updateReviewSelection(){
        const valid=new Set(workspaceCache.articles.filter(article=>article.status==='Pending approval'&&(!isAdmin()||!article.publisherId)).map(article=>article.id));for(const id of [...reviewSelection])if(!valid.has(id))reviewSelection.delete(id);
      ...
      function renderAll(){renderHomepagePublications();window.FINDAT_X1_PUBLICATIONS=workspaceCache.articles.filter(article=>article.status==='Published'&&(article.ownerId===activeAccount?.id||article.createdBy===activeAccount?.id)).map(article=>({id:article.id,title:article.title||'',subtitle:article.subtitle||'',category:article.category||'',content:stripHTML(article.content||'').slice(0,7000),publishedAt:article.publishedAt||article.updatedAt||article.createdAt||''}));window.FINDAT_SOCIAL_STATE={activeAccount:activeAccount?{...activeAccount}:null,profiles:accountCache.map(item=>({...item})),articles:workspaceCache.articles.map(item=>({...item})),social:{reactions:[...socialCache.reactions],bookmarks:[...socialCache.bookmarks],follows:[...socialCache.follows],comments:[...socialCache.comments]}};window.dispatchEvent(new CustomEvent('findat:x1-publications-updated'));if(!activeAccount)return;renderCounters();renderLibrary();renderReview();renderPublications();renderCover();renderCollaborationInbox();renderSocialWorkspace();const current=loadWorkspace().articles.find(a=>a.id===editingArticleId);renderStatusBanner(current);renderContributorBoard(current);renderArticleActivity(current)}

      function socialProfileById(id){return accountById(id)||directoryCache.find(person=>person.id===id)||null}
      function socialOwnsArticle(article){return Boolean(article&&activeAccount&&(article.ownerId||article.createdBy)===activeAccount.id)}
      function socialCanEditArticle(article){return Boolean(article&&activeAccount&&(isAdmin()||socialOwnsArticle(article)||article.publisherId===activeAccount.id||isAcceptedCollaborator(article.id)))}
      function socialSetupError(error){const message=String(error?.message||error?.details||'').toLowerCase();return error?.code==='42P01'||error?.code==='PGRST205'||message.includes('findat_article_reactions')||message.includes('findat_direct_messages')||message.includes('findat_message_clears')||message.includes('does not exist')}
      function resetSocialState(){socialCache={reactions:[],bookmarks:[],follows:[],comments:[],messages:[],clears:[]};socialAvailable=true;socialError='';socialActiveTab='feed';socialCommentArticleId='';socialCommentReplyId='';socialChatUserId='';socialNetworkQuery='';socialNetworkPage=1;socialPresenceByUser=new Map();clearSocialTypingUsers();socialPresenceReady=false;socialPresenceStatus='offline'}
      async function refreshSocialState({quiet=false}={}){
        if(!supabaseClient||!activeAccount){resetSocialState();return}
        const articleIds=workspaceCache.articles.map(article=>article.id).filter(Boolean);
        const empty=Promise.resolve({data:[],error:null});
        const results=await Promise.all([
          articleIds.length?supabaseClient.from('findat_article_reactions').select('id,article_id,user_id,reaction_type,created_at').in('article_id',articleIds).limit(2000):empty,
          articleIds.length?supabaseClient.from('findat_article_bookmarks').select('article_id,user_id,created_at').in('article_id',articleIds).limit(1000):empty,
          supabaseClient.from('findat_user_follows').select('follower_id,following_id,created_at').limit(3000),
          articleIds.length?supabaseClient.from('findat_social_comments').select('id,article_id,author_id,parent_id,body,created_at,updated_at').in('article_id',articleIds).order('created_at',{ascending:true}).limit(3000):empty,
          supabaseClient.from('findat_direct_messages').select('id,sender_id,recipient_id,body,created_at,read_at').order('created_at',{ascending:false}).limit(800),
          supabaseClient.from('findat_message_clears').select('user_id,other_user_id,cleared_at').eq('user_id',activeAccount.id).limit(500)
        ]);
        const firstError=results.map(result=>result.error).find(Boolean);
        if(firstError){
          if(socialSetupError(firstError)){socialAvailable=false;socialError='Run the latest FINDAT social and x1 usage SQL migration in Supabase to activate reactions, discussions, follows, private messages and chat clearing.';if(!quiet)console.warn('FINDAT social workspace is awaiting its database upgrade.',firstError)}
          else{socialError=firstError.message||'The social workspace could not refresh.';if(!quiet)console.warn('FINDAT social refresh failed',firstError)}
          renderSocialWorkspace();return
        }
        socialAvailable=true;socialError='';
        socialCache={reactions:results[0].data||[],bookmarks:results[1].data||[],follows:results[2].data||[],comments:results[3].data||[],messages:results[4].data||[],clears:results[5].data||[]};
        renderSocialWorkspace()
      }

      function socialPresenceState(userId){if(!userId)return'offline';if(userId===activeAccount?.id&&socialPresenceStatus!=='offline')return socialPresenceStatus;return socialPresenceByUser.get(userId)?.status||'offline'}
      function socialPresenceLabel(userId){const status=socialPresenceState(userId);return status==='online'?'Online':status==='away'?'Away':'Offline'}
      function socialPresenceBadge(userId){const status=socialPresenceState(userId);return `<span class="fd-presence-badge is-${status}"><i></i>${socialPresenceLabel(userId)}</span>`}
      function socialAvatarMarkup(account,className='fd-avatar'){const name=accountName(account),url=String(account?.avatarUrl||'').trim(),status=socialPresenceState(account?.id);return `<span class="${className} fd-presence-avatar" data-presence="${status}" ${account?.id?`data-social-view-profile="${escapeHTML(account.id)}"`:''} title="${escapeHTML(`${name} · ${socialPresenceLabel(account?.id)}`)}">${url?`<img src="${escapeHTML(url)}" alt="">`:escapeHTML(initials(account))}<i class="fd-presence-dot is-${status}" aria-hidden="true"></i></span>`}
      function syncSocialPresence(){if(!socialPresenceChannel)return;const state=socialPresenceChannel.presenceState?.()||{},next=new Map();Object.entries(state).forEach(([key,presences])=>{(Array.isArray(presences)?presences:[presences]).filter(Boolean).forEach(presence=>{const userId=String(presence.user_id||presence.userId||String(key).split(':')[0]||'');if(!userId)return;const status=presence.status==='away'?'away':'online',updatedAt=presence.updated_at||presence.online_at||'';const current=next.get(userId);if(!current||current.status==='away'&&status==='online'||current.status===status&&new Date(updatedAt||0)>new Date(current.updatedAt||0))next.set(userId,{status,updatedAt})})});socialPresenceByUser=next;socialPresenceReady=true;renderSocialWorkspace()}
      async function authoriseSocialRealtime(){const session=await getCurrentSession();if(session?.access_token&&typeof supabaseClient?.realtime?.setAuth==='function')await supabaseClient.realtime.setAuth(session.access_token)}
      async function publishSocialPresence(status){if(!activeAccount)return;const next=status==='away'?'away':'online';if(socialPresenceStatus===next&&socialPresenceByUser.get(activeAccount.id)?.status===next)return;socialPresenceStatus=next;socialPresenceByUser.set(activeAccount.id,{status:next,updatedAt:nowISO()});renderSocialWorkspace();if(!socialPresenceChannel)return;try{await socialPresenceChannel.track({user_id:activeAccount.id,status:next,online_at:socialPresenceOnlineAt||nowISO(),updated_at:nowISO()})}catch(error){console.warn('FINDAT presence update failed',error)}}
      function scheduleSocialAway(){clearTimeout(socialPresenceIdleTimer);if(!activeAccount||document.hidden)return;socialPresenceIdleTimer=setTimeout(()=>publishSocialPresence('away'),SOCIAL_AWAY_DELAY)}
      function noteSocialActivity(){if(!activeAccount||document.hidden)return;if(socialPresenceStatus!=='online')publishSocialPresence('online');scheduleSocialAway()}
      function bindSocialPresenceActivity(){if(socialPresenceListenersBound)return;socialPresenceListenersBound=true;['pointerdown','keydown','touchstart'].forEach(type=>document.addEventListener(type,noteSocialActivity,{capture:true,passive:type!=='keydown'}));document.addEventListener('visibilitychange',()=>{if(!activeAccount)return;if(document.hidden)publishSocialPresence('away');else{publishSocialPresence('online');scheduleSocialAway()}});window.addEventListener('focus',()=>{if(activeAccount&&!document.hidden){publishSocialPresence('online');scheduleSocialAway()}});window.addEventListener('blur',()=>{if(activeAccount)publishSocialPresence('away')});window.addEventListener('pagehide',()=>{try{socialPresenceChannel?.untrack?.()}catch{}})}
      async function startSocialPresence(){if(!supabaseClient||!activeAccount||typeof supabaseClient.channel!=='function')return;try{await authoriseSocialRealtime()}catch(error){console.warn('FINDAT Realtime authentication could not be refreshed.',error)}socialPresenceSessionId=socialPresenceSessionId||uid();socialPresenceOnlineAt=nowISO();socialPresenceStatus=document.hidden?'away':'online';socialPresenceChannel=supabaseClient.channel('findat:workspace:presence',{config:{private:true,presence:{key:`${activeAccount.id}:${socialPresenceSessionId}`}}}).on('presence',{event:'sync'},syncSocialPresence).on('presence',{event:'join'},syncSocialPresence).on('presence',{event:'leave'},syncSocialPresence).subscribe(async(status,error)=>{if(status==='SUBSCRIBED'){socialPresenceReady=true;await publishSocialPresence(document.hidden?'away':'online');scheduleSocialAway()}else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'){socialPresenceReady=false;socialPresenceStatus='offline';socialPresenceByUser.delete(activeAccount?.id);console.warn('FINDAT online status is unavailable until the Realtime policies are deployed.',error||status);renderSocialWorkspace()}});bindSocialPresenceActivity()}
      function socialChatTopic(userId){if(!activeAccount?.id||!userId)return'';return `findat:chat:${[activeAccount.id,userId].sort().join(':')}`}
      function clearSocialTypingUser(userId){const timer=socialTypingTimers.get(userId);if(timer)clearTimeout(timer);socialTypingTimers.delete(userId);socialTypingByUser.delete(userId)}
      function receiveSocialTyping(event,expectedUserId){const payload=event?.payload||event||{},senderId=String(payload.sender_id||''),recipientId=String(payload.recipient_id||'');if(senderId!==expectedUserId||recipientId!==activeAccount?.id)return;clearSocialTypingUser(senderId);if(payload.typing===true){socialTypingByUser.set(senderId,true);socialTypingTimers.set(senderId,setTimeout(()=>{clearSocialTypingUser(senderId);renderSocialChatPeople();renderSocialChatThread()},SOCIAL_TYPING_EXPIRY))}renderSocialChatPeople();renderSocialChatThread()}
      async function broadcastSocialTyping(typing){if(!socialTypingChannel||!socialTypingChannelReady||!socialChatUserId||!activeAccount)return;try{await socialTypingChannel.send({type:'broadcast',event:'typing',payload:{sender_id:activeAccount.id,recipient_id:socialChatUserId,typing:Boolean(typing),sent_at:nowISO()}})}catch(error){console.warn('FINDAT typing signal failed',error)}}
      function stopSocialTyping(){clearTimeout(socialTypingStopTimer);socialTypingStopTimer=0;if(socialTypingSent)broadcastSocialTyping(false);socialTypingSent=false;socialTypingLastSentAt=0}
      function queueSocialTyping(){const input=$('#fdSocialChatInput');if(!socialChatUserId||input?.disabled)return;const hasText=Boolean(String(input?.value||'').trim()),now=Date.now();if(!hasText){stopSocialTyping();return}if(!socialTypingSent||now-socialTypingLastSentAt>1400){broadcastSocialTyping(true);socialTypingSent=true;socialTypingLastSentAt=now}clearTimeout(socialTypingStopTimer);socialTypingStopTimer=setTimeout(stopSocialTyping,SOCIAL_TYPING_IDLE_DELAY)}
      function stopSocialTypingChannel({notify=true}={}){if(notify)stopSocialTyping();else{clearTimeout(socialTypingStopTimer);socialTypingStopTimer=0;socialTypingSent=false;socialTypingLastSentAt=0}if(socialTypingChannel&&supabaseClient){try{supabaseClient.removeChannel(socialTypingChannel)}catch(error){console.warn('Could not remove typing channel',error)}}socialTypingChannel=null;socialTypingChannelReady=false}
      async function startSocialTypingChannel(userId){stopSocialTypingChannel();clearSocialTypingUsers();if(!supabaseClient||!activeAccount||!userId||userId===activeAccount.id||typeof supabaseClient.channel!=='function')return;try{await authoriseSocialRealtime()}catch(error){console.warn('FINDAT Realtime authentication could not be refreshed.',error)}const topic=socialChatTopic(userId),expectedUserId=userId;socialTypingChannel=supabaseClient.channel(topic,{config:{private:true,broadcast:{self:false,ack:true}}}).on('broadcast',{event:'typing'},event=>receiveSocialTyping(event,expectedUserId)).subscribe((status,error)=>{socialTypingChannelReady=status==='SUBSCRIBED';if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')console.warn('FINDAT typing indicators are unavailable until the Realtime policies are deployed.',error||status)})}
      function stopSocialRealtime(){stopSocialTypingChannel();clearSocialTypingUsers();clearTimeout(socialPresenceIdleTimer);socialPresenceIdleTimer=0;if(socialPresenceChannel&&supabaseClient){try{socialPresenceChannel.untrack?.();supabaseClient.removeChannel(socialPresenceChannel)}catch(error){console.warn('Could not remove presence channel',error)}}socialPresenceChannel=null;socialPresenceByUser=new Map();socialPresenceReady=false;socialPresenceStatus='offline';if(socialChannel&&supabaseClient){try{supabaseClient.removeChannel(socialChannel)}catch(error){console.warn('Could not remove social channel',error)}}socialChannel=null}
      function startSocialRealtime(){
        stopSocialRealtime();if(!supabaseClient||!activeAccount||typeof supabaseClient.channel!=='function')return;
        const refresh=()=>refreshSocialState({quiet:true}).catch(error=>console.warn('Social realtime refresh failed',error));
        socialChannel=supabaseClient.channel(`findat-social-${activeAccount.id}`)
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_article_reactions'},refresh)
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_article_bookmarks',filter:`user_id=eq.${activeAccount.id}`},refresh)
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_user_follows'},refresh)
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_social_comments'},refresh)
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_direct_messages'},refresh)
          .subscribe((status,error)=>{if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')console.warn('Social realtime unavailable; timed refresh remains active.',error||status)});
        startSocialPresence().catch(error=>console.warn('FINDAT presence could not start.',error))
      }
      function socialIsFollowing(userId){return socialCache.follows.some(row=>row.follower_id===activeAccount?.id&&row.following_id===userId)}
      function socialReactionCount(articleId,type){return socialCache.reactions.filter(row=>row.article_id===articleId&&row.reaction_type===type).length}
      function socialHasReaction(articleId,type){return socialCache.reactions.some(row=>row.article_id===articleId&&row.user_id===activeAccount?.id&&row.reaction_type===type)}
      function socialIsBookmarked(articleId){return socialCache.bookmarks.some(row=>row.article_id===articleId&&row.user_id===activeAccount?.id)}
      function socialCommentCount(articleId){return socialCache.comments.filter(row=>row.article_id===articleId).length}
      function socialTrendScore(article){const ageHours=Math.max(0,(Date.now()-new Date(article.publishedAt||article.updatedAt||article.createdAt||Date.now()).getTime())/36e5),likes=socialReactionCount(article.id,'like'),reposts=socialReactionCount(article.id,'repost'),comments=socialCommentCount(article.id),saves=socialCache.bookmarks.filter(row=>row.article_id===article.id).length,ownerId=article.ownerId||article.createdBy,networkBoost=ownerId&&socialIsFollowing(ownerId)?10:0,freshness=45*Math.exp(-ageHours/72),quality=Math.min(18,(String(article.title||'').length+stripHTML(article.content||'').length)/180);return Number((likes*3+comments*5+reposts*7+saves*2+networkBoost+freshness+quality).toFixed(2))}

      function socialUnreadCount(userId=''){const cutoff=userId?new Date(socialCache.clears.find(row=>row.user_id===activeAccount?.id&&row.other_user_id===userId)?.cleared_at||0).getTime():0;return socialCache.messages.filter(row=>row.recipient_id===activeAccount?.id&&!row.read_at&&(!userId||row.sender_id===userId)&&new Date(row.created_at).getTime()>cutoff).length}
      function socialLatestMessage(userId){const cutoff=new Date(socialCache.clears.find(row=>row.user_id===activeAccount?.id&&row.other_user_id===userId)?.cleared_at||0).getTime();return socialCache.messages.filter(row=>((row.sender_id===activeAccount?.id&&row.recipient_id===userId)||(row.recipient_id===activeAccount?.id&&row.sender_id===userId))&&new Date(row.created_at).getTime()>cutoff).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0]||null}
      function socialSetTab(tab='feed'){
        socialActiveTab=['feed','work','saved','network'].includes(tab)?tab:'feed';
        if(socialActiveTab==='network')socialNetworkPage=1;
        $$('[data-social-tab]').forEach(button=>button.classList.toggle('is-active',button.dataset.socialTab===socialActiveTab));
        renderSocialFeed();
        $('#fdSocialFeed')?.scrollIntoView({behavior:'smooth',block:'start'})
      }
      function setSocialEditorOpen(open,heading='New article'){
        const panel=$('#fdSocialEditorPanel'),layout=$('.fd-social-network-layout');if(!panel||!layout)return;
        panel.classList.toggle('is-open',Boolean(open));panel.setAttribute('aria-hidden',open?'false':'true');layout.classList.toggle('is-editing',Boolean(open));
        if($('#fdSocialEditorHeading'))$('#fdSocialEditorHeading').textContent=heading||'Article editor';
        if(open)setTimeout(()=>panel.scrollIntoView({behavior:'smooth',block:'start'}),40)
      }
      function startSocialComposition(template='classic'){
        if(activeAccount?.role==='consultant'){setStatus('fdSocialSystemStatus','Consultants can edit collaborative or assigned articles but cannot create a new article.','error');socialSetTab('work');return}
        clearEditor(true);const chosen=['classic','research','report','newsletter'].includes(template)?template:'classic';
        $('#fdArticleTemplate').value=chosen;$('#fdArticleEditor').className=`fd-editor-canvas template-${chosen}`;$('#fdArticleEditor').innerHTML=templateStarters[chosen]||templateStarters.classic;$('#fdTemplateHint').textContent=templateHints[chosen]||templateHints.classic;
        setSocialEditorOpen(true,chosen==='research'?'New research paper':chosen==='report'?'New professional report':'New article');
        if(template==='photo')setTimeout(()=>$('#fdArticleImage')?.click(),120);else $('#fdArticleTitle')?.focus()
      }
      function renderSocialProfile(){
        if(!activeAccount)return;const following=socialCache.follows.filter(row=>row.follower_id===activeAccount.id).length,followers=socialCache.follows.filter(row=>row.following_id===activeAccount.id).length;
        if($('#fdSocialFollowingCount'))$('#fdSocialFollowingCount').textContent=String(following);if($('#fdSocialFollowerCount'))$('#fdSocialFollowerCount').textContent=String(followers);if($('#fdSocialProfileFollowerCount'))$('#fdSocialProfileFollowerCount').textContent=String(followers);if($('#fdSocialConnectionCount'))$('#fdSocialConnectionCount').textContent=String(Math.max(1,accountCache.length));if($('#fdSocialSavedCount'))$('#fdSocialSavedCount').textContent=String(socialCache.bookmarks.length);if($('#fdSocialUnreadCount')){$('#fdSocialUnreadCount').textContent=String(socialUnreadCount());$('#fdSocialUnreadCount').dataset.count=String(socialUnreadCount())}if($('#fdSocialProfileOrganisation'))$('#fdSocialProfileOrganisation').textContent=activeAccount.organisation||activeAccount.placeOfWork||'';const profileCover=$('#fdSocialProfileCover');if(profileCover){profileCover.style.backgroundImage=activeAccount.coverUrl?`linear-gradient(rgba(22,36,54,.08),rgba(22,36,54,.22)),url("${String(activeAccount.coverUrl).replace(/"/g,'')}")`:''}
      }
      function renderSocialPeople(limit=2){
        const host=$('#fdSocialPeople');if(!host)return;const people=accountCache.filter(person=>person.id!==activeAccount?.id&&person.active!==false).sort((a,b)=>Number(socialPresenceState(b.id)==='online')-Number(socialPresenceState(a.id)==='online')||Number(socialIsFollowing(b.id))-Number(socialIsFollowing(a.id))||accountName(a).localeCompare(accountName(b))).slice(0,limit);
        host.innerHTML=people.length?people.map(person=>`<div class="fd-social-person-row">${socialAvatarMarkup(person,'fd-avatar')}<div><strong>${escapeHTML(accountName(person))}</strong><span>${escapeHTML(person.jobTitle||person.organisation||roleLabel(person.role))}</span>${socialPresenceBadge(person.id)}</div><div class="fd-social-person-actions"><button class="${socialIsFollowing(person.id)?'is-following':''}" data-social-follow="${person.id}" title="${socialIsFollowing(person.id)?'Unfollow':'Follow'}" type="button"><i class="fas ${socialIsFollowing(person.id)?'fa-user-check':'fa-user-plus'}"></i></button><button data-social-view-profile="${person.id}" title="View profile" type="button"><i class="fas fa-user-circle"></i></button><button data-social-message="${person.id}" title="Message" type="button"><i class="fas fa-comment-dots"></i></button></div></div>`).join(''):'<div class="fd-empty-compact"><i class="fas fa-users"></i>No other active members are available.</div>'
      }
      function socialPostMarkup(article){
        const owner=socialProfileById(article.ownerId||article.createdBy)||articleOwner(article),following=owner?.id&&owner.id!==activeAccount?.id&&socialIsFollowing(owner.id),published=article.status==='Published',excerpt=(article.subtitle||stripHTML(article.content)||'').slice(0,360),likes=socialReactionCount(article.id,'like'),reposts=socialReactionCount(article.id,'repost'),comments=socialCommentCount(article.id),mine=socialOwnsArticle(article),canEdit=socialCanEditArticle(article),trendScore=socialTrendScore(article),trending=published&&trendScore>=42;
        return `<article class="fd-social-post ${published?'':'is-private'}" data-social-post="${article.id}"><header class="fd-social-post-head">${socialAvatarMarkup(owner,'fd-avatar fd-social-post-avatar')}<div class="fd-social-post-author"><div class="fd-social-post-author-line"><strong>${escapeHTML(accountName(owner))}</strong><small>${owner?.username?`@${escapeHTML(owner.username)}`:escapeHTML(roleLabel(owner?.role))}</small></div><p>${escapeHTML(roleLabel(owner?.role))} · ${escapeHTML(formatTime(article.publishedAt||article.updatedAt||article.createdAt))}</p></div>${owner?.id&&owner.id!==activeAccount?.id?`<button class="fd-social-post-follow ${following?'is-following':''}" data-social-follow="${owner.id}" type="button">${following?'Following':'Follow'}</button>`:''}</header><div class="fd-social-post-body"><span class="fd-social-post-category"><i class="fas ${published?'fa-globe-africa':'fa-lock'}"></i>${escapeHTML(article.category||'Article')} · ${escapeHTML(published?'Published':article.status||'Draft')}</span>${trending?'<span class="fd-social-trending-badge"><i class="fas fa-fire"></i> Trending</span>':''}<h4>${escapeHTML(article.title||'Untitled article')}</h4><p class="fd-social-post-copy">${escapeHTML(excerpt)}${(article.subtitle||stripHTML(article.content)||'').length>360?'…':''}</p>${article.image?.data?`<div class="fd-social-post-media" data-article-preview="${article.id}"><img src="${article.image.data}" alt="${escapeHTML(article.title||'Article cover')}"></div>`:''}<div class="fd-social-post-meta"><span>${likes} like${likes===1?'':'s'} · ${comments} comment${comments===1?'':'s'} · ${reposts} repost${reposts===1?'':'s'}</span><span>${escapeHTML(contributorSummary(article))}</span></div><div class="fd-social-post-actions"><button class="${socialHasReaction(article.id,'like')?'is-active':''}" data-social-like="${article.id}" type="button"><i class="fas fa-thumbs-up"></i><span>Like</span><b>${likes||''}</b></button><button data-social-comment="${article.id}" type="button"><i class="fas fa-comment"></i><span>Comment</span><b>${comments||''}</b></button><button class="${socialHasReaction(article.id,'repost')?'is-active':''}" data-social-repost="${article.id}" type="button"><i class="fas fa-retweet"></i><span>Repost</span><b>${reposts||''}</b></button><button class="${socialIsBookmarked(article.id)?'is-active':''}" data-social-bookmark="${article.id}" type="button"><i class="fas fa-bookmark"></i><span>Save</span></button><button data-social-share="${article.id}" type="button"><i class="fas fa-share-alt"></i><span>Share</span></button></div>${published?`<div class="fd-library-item-actions"><button class="fd-mini-btn" data-article-preview="${article.id}" type="button">Read article</button>${mine?`<button class="fd-mini-btn" data-social-analytics="${article.id}" type="button"><i class="fas fa-chart-line"></i> Analytics</button>`:''}${canEdit?`<button class="fd-mini-btn" data-article-open-editor="${article.id}" type="button">Edit</button>`:''}</div>`:`<div class="fd-social-post-private-note"><i class="fas fa-shield-alt"></i>This work is visible only to authorised participants.${canEdit?` <button class="fd-mini-btn" data-article-open-editor="${article.id}" type="button">Continue writing</button>`:''}</div>`}</div></article>`
      }
      function renderSocialNetworkDirectory(){
        const host=$('#fdSocialFeed');if(!host)return;
        const query=normal(socialNetworkQuery);
        const people=accountCache.filter(person=>person.id!==activeAccount?.id&&person.active!==false)
          .filter(person=>!query||normal(`${accountName(person)} ${person.username} ${person.organisation} ${person.jobTitle} ${person.country}`).includes(query))
          .sort((a,b)=>Number(socialPresenceState(b.id)==='online')-Number(socialPresenceState(a.id)==='online')||Number(socialIsFollowing(b.id))-Number(socialIsFollowing(a.id))||accountName(a).localeCompare(accountName(b)));
        const totalPages=Math.max(1,Math.ceil(people.length/SOCIAL_NETWORK_PAGE_SIZE));socialNetworkPage=Math.max(1,Math.min(totalPages,socialNetworkPage));
        const startIndex=(socialNetworkPage-1)*SOCIAL_NETWORK_PAGE_SIZE,pagePeople=people.slice(startIndex,startIndex+SOCIAL_NETWORK_PAGE_SIZE);
        const cards=pagePeople.length?pagePeople.map(person=>`<article class="fd-social-network-profile">${socialAvatarMarkup(person,'fd-avatar')}<div><h4>${escapeHTML(accountName(person))}</h4><p>${escapeHTML(person.jobTitle||roleLabel(person.role))}${person.organisation?` · ${escapeHTML(person.organisation)}`:''}</p>${socialPresenceBadge(person.id)}</div><div class="fd-social-network-profile-actions"><button class="${socialIsFollowing(person.id)?'is-following':''}" data-social-follow="${person.id}" type="button"><i class="fas ${socialIsFollowing(person.id)?'fa-user-check':'fa-user-plus'}"></i> ${socialIsFollowing(person.id)?'Following':'Follow'}</button><button data-social-view-profile="${person.id}" type="button"><i class="fas fa-user-circle"></i> Profile</button><button data-social-message="${person.id}" type="button"><i class="fas fa-comment-dots"></i> Message</button></div></article>`).join(''):'<div class="fd-social-feed-empty"><i class="fas fa-user-friends"></i>No members match this search.</div>';
        const pageButtons=Array.from({length:totalPages},(_,index)=>index+1).map(page=>`<button class="${page===socialNetworkPage?'is-active':''}" data-social-network-page="${page}" type="button" aria-label="Network page ${page}">${page}</button>`).join('');
        host.innerHTML=`<div class="fd-social-network-toolbar"><label class="fd-social-network-search"><i class="fas fa-search"></i><input id="fdSocialNetworkSearch" type="search" autocomplete="off" placeholder="Search people by name, company or location" value="${escapeHTML(socialNetworkQuery)}"></label><button class="fd-social-network-search-button" id="fdSocialNetworkSearchButton" type="button" aria-label="Search FINDAT network"><i class="fas fa-search"></i></button></div><div class="fd-social-network-summary">${people.length} member${people.length===1?'':'s'} found · showing ${people.length?startIndex+1:0}-${Math.min(startIndex+SOCIAL_NETWORK_PAGE_SIZE,people.length)}</div><div class="fd-social-network-directory">${cards}</div><nav class="fd-social-network-pagination" aria-label="Network pages"><button data-social-network-step="-1" type="button" aria-label="Previous network page" ${socialNetworkPage<=1?'disabled':''}><i class="fas fa-chevron-left"></i></button>${pageButtons}<button data-social-network-step="1" type="button" aria-label="Next network page" ${socialNetworkPage>=totalPages?'disabled':''}><i class="fas fa-chevron-right"></i></button></nav>`
      }

      function renderSocialFeed(){
        const host=$('#fdSocialFeed');if(!host||!activeAccount)return;const query=normal($('#fdDashboardSearch')?.value),title=$('#fdSocialFeedTitle'),eyebrow=$('#fdSocialFeedEyebrow');
        if(socialActiveTab==='network'){if(title)title.textContent='Your FINDAT network';if(eyebrow)eyebrow.textContent='People and connections';renderSocialNetworkDirectory();return}
        let articles=[];
        if(socialActiveTab==='work')articles=visibleWritingArticles();
        else if(socialActiveTab==='saved'){const saved=new Set(socialCache.bookmarks.map(row=>row.article_id));articles=workspaceCache.articles.filter(article=>saved.has(article.id))}
        else articles=workspaceCache.articles.filter(article=>article.status==='Published'||socialCanEditArticle(article));
        articles=articles.filter(article=>!query||normal(`${article.title} ${article.subtitle} ${article.category} ${stripHTML(article.content)} ${accountName(articleOwner(article))}`).includes(query)).sort((a,b)=>socialActiveTab==='feed'?(socialTrendScore(b)-socialTrendScore(a)||new Date(b.publishedAt||b.updatedAt||b.createdAt)-new Date(a.publishedAt||a.updatedAt||a.createdAt)):new Date(b.publishedAt||b.updatedAt||b.createdAt)-new Date(a.publishedAt||a.updatedAt||a.createdAt));
        if(title)title.textContent=socialActiveTab==='work'?'My writing and collaborations':socialActiveTab==='saved'?'Saved publications':'Latest from FINDAT';if(eyebrow)eyebrow.textContent=socialActiveTab==='work'?'Private workbench':socialActiveTab==='saved'?'Reading list':'Professional feed';
        host.innerHTML=articles.length?articles.map(socialPostMarkup).join(''):`<div class="fd-social-feed-empty"><i class="fas ${socialActiveTab==='saved'?'fa-bookmark':'fa-newspaper'}"></i>${socialActiveTab==='saved'?'No saved publications yet.':'No posts are available in this view.'}</div>`;setTimeout(observeSocialPosts,0)
      }
      function renderSocialWorkspace(){
        if(!activeAccount)return;renderSocialProfile();renderSocialPeople();renderSocialFeed();const status=$('#fdSocialSystemStatus');if(status)status.textContent=socialError||'';
        if($('#fdSocialChatModal')?.classList.contains('is-open')){renderSocialChatPeople();renderSocialChatThread()}
        if($('#fdSocialDiscussionModal')?.classList.contains('is-open'))renderSocialDiscussion()
      }
      async function toggleSocialReaction(articleId,type){
        if(!socialAvailable){$('#fdSocialSystemStatus').textContent=socialError;return}const existing=socialCache.reactions.find(row=>row.article_id===articleId&&row.user_id===activeAccount.id&&row.reaction_type===type);let result;
        if(existing)result=await supabaseClient.from('findat_article_reactions').delete().eq('article_id',articleId).eq('user_id',activeAccount.id).eq('reaction_type',type);else result=await supabaseClient.from('findat_article_reactions').insert({article_id:articleId,user_id:activeAccount.id,reaction_type:type});if(result.error){$('#fdSocialSystemStatus').textContent=result.error.message||'The reaction could not be saved.';return}await refreshSocialState({quiet:true})
      }
      async function toggleSocialBookmark(articleId){
        if(!socialAvailable){$('#fdSocialSystemStatus').textContent=socialError;return}const existing=socialIsBookmarked(articleId);const result=existing?await supabaseClient.from('findat_article_bookmarks').delete().eq('article_id',articleId).eq('user_id',activeAccount.id):await supabaseClient.from('findat_article_bookmarks').insert({article_id:articleId,user_id:activeAccount.id});if(result.error){$('#fdSocialSystemStatus').textContent=result.error.message||'The bookmark could not be saved.';return}await refreshSocialState({quiet:true})
      }
      async function toggleSocialFollow(userId){
        if(!socialAvailable||!userId||userId===activeAccount?.id){if(!socialAvailable)$('#fdSocialSystemStatus').textContent=socialError;return}const following=socialIsFollowing(userId);const result=following?await supabaseClient.from('findat_user_follows').delete().eq('follower_id',activeAccount.id).eq('following_id',userId):await supabaseClient.from('findat_user_follows').insert({follower_id:activeAccount.id,following_id:userId});if(result.error){$('#fdSocialSystemStatus').textContent=result.error.message||'The connection could not be updated.';return}await refreshSocialState({quiet:true})
      }
      async function shareSocialArticle(articleId){
        const article=workspaceCache.articles.find(row=>row.id===articleId);if(!article)return;const url=article.status==='Published'?`${location.origin}${location.pathname}#publication-${article.id}`:location.href;const shareData={title:article.title||'FINDAT article',text:article.subtitle||stripHTML(article.content).slice(0,160),url};
        try{if(navigator.share&&article.status==='Published')await navigator.share(shareData);else{await navigator.clipboard.writeText(article.status==='Published'?url:`${article.title}\n\n${shareData.text}`);$('#fdSocialSystemStatus').textContent=article.status==='Published'?'Publication link copied.':'Article summary copied.';setTimeout(()=>{if($('#fdSocialSystemStatus')?.textContent.includes('copied'))$('#fdSocialSystemStatus').textContent=''},2500)}}catch(error){if(error?.name!=='AbortError')$('#fdSocialSystemStatus').textContent='The share action could not be completed.'}
      }
      function openSocialDiscussion(articleId){socialCommentArticleId=articleId;socialCommentReplyId='';renderSocialDiscussion();window.toggleModal?.('fdSocialDiscussionModal',true);setTimeout(()=>$('#fdSocialDiscussionInput')?.focus(),60)}
      function renderSocialDiscussion(){
        const article=workspaceCache.articles.find(row=>row.id===socialCommentArticleId),host=$('#fdSocialDiscussionList');if(!host)return;if(!article){host.innerHTML='<div class="fd-social-feed-empty"><i class="fas fa-comments"></i>Article unavailable.</div>';return}$('#fdSocialDiscussionTitle').textContent=article.title||'Article discussion';$('#fdSocialDiscussionSubtitle').textContent=`${socialCommentCount(article.id)} comment${socialCommentCount(article.id)===1?'':'s'} · ${accountName(articleOwner(article))}`;
        const comments=socialCache.comments.filter(row=>row.article_id===article.id),roots=comments.filter(row=>!row.parent_id),replies=new Map();comments.filter(row=>row.parent_id).forEach(row=>{const list=replies.get(row.parent_id)||[];list.push(row);replies.set(row.parent_id,list)});
        const commentMarkup=row=>{const author=socialProfileById(row.author_id)||{id:row.author_id,displayName:'FINDAT Member'};return `<div class="fd-social-comment">${socialAvatarMarkup(author,'fd-avatar')}<div><div class="fd-social-comment-bubble"><strong>${escapeHTML(accountName(author))}</strong><p>${escapeHTML(row.body||'')}</p></div><div class="fd-social-comment-meta"><time>${escapeHTML(formatTime(row.created_at))}</time><button data-social-comment-reply="${row.id}" data-social-comment-author="${escapeHTML(accountName(author))}" type="button">Reply</button>${row.author_id===activeAccount?.id||isAdmin()?`<button data-social-comment-delete="${row.id}" type="button">Remove</button>`:''}</div></div>${(replies.get(row.id)||[]).length?`<div class="fd-social-comment-replies">${(replies.get(row.id)||[]).map(reply=>commentMarkup(reply)).join('')}</div>`:''}</div>`};
        host.innerHTML=roots.length?roots.map(commentMarkup).join(''):'<div class="fd-social-feed-empty"><i class="fas fa-comment-alt"></i>Start the discussion.</div>';host.scrollTop=host.scrollHeight
      }
      function setSocialDiscussionReply(id='',author=''){socialCommentReplyId=id||'';$('#fdSocialDiscussionReplyLabel').textContent=id?`Replying to ${author||'comment'}`:'';$('#fdSocialDiscussionReplyCancel').hidden=!id;$('#fdSocialDiscussionInput')?.focus()}
      async function sendSocialComment(){
        const input=$('#fdSocialDiscussionInput'),body=String(input?.value||'').trim();if(!socialAvailable){setStatus('fdSocialDiscussionStatus',socialError,'error');return}if(!socialCommentArticleId||!body){setStatus('fdSocialDiscussionStatus','Write a comment first.','error');return}const button=$('#fdSocialDiscussionSend');button.disabled=true;setStatus('fdSocialDiscussionStatus','Sending…','');const result=await supabaseClient.from('findat_social_comments').insert({article_id:socialCommentArticleId,author_id:activeAccount.id,parent_id:socialCommentReplyId||null,body});button.disabled=false;if(result.error){setStatus('fdSocialDiscussionStatus',result.error.message||'The comment could not be sent.','error');return}input.value='';setSocialDiscussionReply();await refreshSocialState({quiet:true});setStatus('fdSocialDiscussionStatus','Comment posted.','success')
      }
      async function deleteSocialComment(id){if(!id||!confirm('Remove this comment?'))return;const result=await supabaseClient.from('findat_social_comments').delete().eq('id',id);if(result.error){setStatus('fdSocialDiscussionStatus',result.error.message||'The comment could not be removed.','error');return}await refreshSocialState({quiet:true})}
      function socialConversationPeople(){return accountCache.filter(person=>person.id!==activeAccount?.id&&person.active!==false).sort((a,b)=>{const typing=Number(Boolean(socialTypingByUser.get(b.id)))-Number(Boolean(socialTypingByUser.get(a.id)));if(typing)return typing;const unread=socialUnreadCount(b.id)-socialUnreadCount(a.id);if(unread)return unread;const online=Number(socialPresenceState(b.id)==='online')-Number(socialPresenceState(a.id)==='online');if(online)return online;const am=socialLatestMessage(a.id),bm=socialLatestMessage(b.id);return new Date(bm?.created_at||0)-new Date(am?.created_at||0)||accountName(a).localeCompare(accountName(b))})}
      function openSocialChat(userId=''){if(!socialAvailable){$('#fdSocialSystemStatus').textContent=socialError;return}socialChatUserId=userId||socialChatUserId||socialConversationPeople()[0]?.id||'';renderSocialChatPeople();renderSocialChatThread();window.toggleModal?.('fdSocialChatModal',true);if(socialChatUserId){startSocialTypingChannel(socialChatUserId);markSocialMessagesRead(socialChatUserId)}}
      function renderSocialChatPeople(){
        const host=$('#fdSocialChatPeople');if(!host)return;const query=normal($('#fdSocialChatSearch')?.value);const people=socialConversationPeople().filter(person=>!query||normal(`${accountName(person)} ${person.username} ${person.organisation}`).includes(query));host.innerHTML=people.length?people.map(person=>{const latest=socialLatestMessage(person.id),unread=socialUnreadCount(person.id),typing=Boolean(socialTypingByUser.get(person.id));return `<button class="fd-social-chat-person ${socialChatUserId===person.id?'is-active':''}" data-social-chat-person="${person.id}" type="button">${socialAvatarMarkup(person,'fd-avatar')}<div><strong>${escapeHTML(accountName(person))}</strong><p class="${typing?'is-typing':''}">${typing?`${escapeHTML(accountName(person))} is typing…`:escapeHTML(latest?.body||person.jobTitle||person.organisation||'Start a conversation')}</p><span class="fd-chat-person-presence is-${socialPresenceState(person.id)}">${socialPresenceLabel(person.id)}</span></div>${unread?`<b>${unread}</b>`:''}</button>`}).join(''):'<div class="fd-empty-compact"><i class="fas fa-users"></i>No members found.</div>'
      }
      function socialChatClearedAt(userId){return socialCache.clears.find(row=>row.user_id===activeAccount?.id&&row.other_user_id===userId)?.cleared_at||''}
      function renderSocialChatThread(){
        const host=$('#fdSocialChatMessages'),person=socialProfileById(socialChatUserId),input=$('#fdSocialChatInput'),send=$('#fdSocialChatSend'),clear=$('#fdSocialChatClear');if(!host)return;
        if(!person){$('#fdSocialChatTitle').textContent='Select a conversation';$('#fdSocialChatPresence').textContent='Choose a FINDAT member to begin.';$('#fdSocialChatPresence').className='';host.innerHTML='<div class="fd-social-chat-empty"><i class="fas fa-comments"></i><strong>Your FINDAT messages</strong><span>Private conversations are visible only to the participants.</span></div>';input.disabled=true;send.disabled=true;if(clear)clear.disabled=true;return}
        const header=$('#fdSocialChatHeader'),avatar=header.querySelector('.fd-avatar');if(avatar)avatar.outerHTML=socialAvatarMarkup(person,'fd-avatar');
        const typing=Boolean(socialTypingByUser.get(person.id)),presence=socialPresenceState(person.id),detail=person.jobTitle||person.organisation||roleLabel(person.role);$('#fdSocialChatTitle').textContent=accountName(person);const presenceEl=$('#fdSocialChatPresence');presenceEl.textContent=typing?`${accountName(person)} is typing…`:`${socialPresenceLabel(person.id)}${detail?` · ${detail}`:''}`;presenceEl.className=`is-${typing?'typing':presence}`;input.disabled=false;send.disabled=false;if(clear)clear.disabled=false;
        const clearedAt=socialChatClearedAt(person.id);const cutoff=clearedAt?new Date(clearedAt).getTime():0;
        const messages=socialCache.messages.filter(row=>((row.sender_id===activeAccount.id&&row.recipient_id===person.id)||(row.recipient_id===activeAccount.id&&row.sender_id===person.id))&&new Date(row.created_at).getTime()>cutoff).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
        const messageHTML=messages.length?messages.map(row=>`<div class="fd-social-chat-message ${row.sender_id===activeAccount.id?'is-mine':''}"><div class="fd-social-chat-bubble">${escapeHTML(row.body||'')}<time>${escapeHTML(formatTime(row.created_at))}${row.sender_id===activeAccount.id&&row.read_at?' · Read':''}</time></div></div>`).join(''):'<div class="fd-social-chat-empty"><i class="fas fa-comment-medical"></i><strong>Start the conversation</strong><span>Send a private message to this FINDAT member.</span></div>';
        const typingHTML=typing?`<div class="fd-social-chat-message is-typing" aria-live="polite" aria-label="${escapeHTML(accountName(person))} is typing"><div class="fd-social-chat-bubble fd-social-typing-bubble"><span></span><span></span><span></span></div></div>`:'';host.innerHTML=messageHTML+typingHTML;host.scrollTop=host.scrollHeight
      }
      async function clearSocialChat(){
        if(!activeAccount?.id||!socialChatUserId)return;
        if(!confirm('Clear this conversation from your view? The other member will keep their copy.'))return;
        const button=$('#fdSocialChatClear');if(button)button.disabled=true;setStatus('fdSocialChatStatus','Clearing conversation…','');
        const row={user_id:activeAccount.id,other_user_id:socialChatUserId,cleared_at:nowISO()};const result=await supabaseClient.from('findat_message_clears').upsert(row,{onConflict:'user_id,other_user_id'});
        if(result.error){if(button)button.disabled=false;setStatus('fdSocialChatStatus',result.error.message||'The conversation could not be cleared.','error');return}
        const existing=socialCache.clears.find(item=>item.user_id===row.user_id&&item.other_user_id===row.other_user_id);if(existing)existing.cleared_at=row.cleared_at;else socialCache.clears.push(row);renderSocialChatThread();renderSocialChatPeople();setStatus('fdSocialChatStatus','Conversation cleared from your view.','success')
      }

      async function markSocialMessagesRead(userId){if(!socialAvailable||!userId)return;const now=nowISO();const result=await supabaseClient.from('findat_direct_messages').update({read_at:now}).eq('recipient_id',activeAccount.id).eq('sender_id',userId).is('read_at',null);if(!result.error){socialCache.messages.forEach(row=>{if(row.recipient_id===activeAccount.id&&row.sender_id===userId&&!row.read_at)row.read_at=now});renderSocialProfile();renderSocialChatPeople();renderSocialChatThread()}}
      async function selectSocialChat(userId){stopSocialTyping();socialChatUserId=userId;renderSocialChatPeople();renderSocialChatThread();await startSocialTypingChannel(userId);await markSocialMessagesRead(userId);setTimeout(()=>$('#fdSocialChatInput')?.focus(),50)}
      async function sendSocialMessage(){
        const input=$('#fdSocialChatInput'),body=String(input?.value||'').trim();if(!socialAvailable){setStatus('fdSocialChatStatus',socialError,'error');return}if(!socialChatUserId||!body){setStatus('fdSocialChatStatus','Choose a member and write a message.','error');return}stopSocialTyping();const button=$('#fdSocialChatSend');button.disabled=true;setStatus('fdSocialChatStatus','Sending…','');const result=await supabaseClient.from('findat_direct_messages').insert({sender_id:activeAccount.id,recipient_id:socialChatUserId,body});button.disabled=false;if(result.error){setStatus('fdSocialChatStatus',result.error.message||'The message could not be sent.','error');return}input.value='';await refreshSocialState({quiet:true});renderSocialChatThread();setStatus('fdSocialChatStatus','','')
      }


      function renderSocialEmojiPickers(){$$('[data-social-emoji-picker]').forEach(picker=>{picker.innerHTML=socialEmojiSet.map(emoji=>`<button type="button" data-social-emoji="${escapeHTML(emoji)}" title="Add ${escapeHTML(emoji)}">${escapeHTML(emoji)}</button>`).join('')})}
      function closeSocialEmojiPickers(except=null){$$('[data-social-emoji-picker]').forEach(picker=>{if(picker!==except)picker.hidden=true});$$('[data-social-emoji-trigger]').forEach(button=>button.setAttribute('aria-expanded',String(Boolean(except&&button.closest('.fd-social-emoji-control')?.contains(except)))))}
      function toggleSocialEmojiPicker(button){const control=button?.closest('.fd-social-emoji-control'),picker=control?.querySelector('[data-social-emoji-picker]');if(!picker)return;const opening=picker.hidden;closeSocialEmojiPickers(opening?picker:null);picker.hidden=!opening;button.setAttribute('aria-expanded',String(opening))}
      function insertSocialEmoji(targetId,emoji){const input=document.getElementById(targetId);if(!input||input.disabled)return;const start=Number.isInteger(input.selectionStart)?input.selectionStart:input.value.length,end=Number.isInteger(input.selectionEnd)?input.selectionEnd:start;if(typeof input.setRangeText==='function')input.setRangeText(emoji,start,end,'end');else input.value=`${input.value.slice(0,start)}${emoji}${input.value.slice(end)}`;input.dispatchEvent(new Event('input',{bubbles:true}));input.focus()}
      ...
      async function saveArticle(status='Draft'){
        ...
        const successMessage=collaborativeEdit?'Collaborative changes saved.':status==='Published'?'Article published successfully.':status==='Pending approval'?'Article sent to the administrator for approval.':'Draft saved.';clearEditor(false);setStatus('fdArticleStatus',successMessage,'success');const socialStatus=$('#fdSocialSystemStatus');if(socialStatus){socialStatus.textContent=successMessage;setTimeout(()=>{if($('#fdSocialSystemStatus')?.textContent===successMessage)$('#fdSocialSystemStatus').textContent=''},3200)}await refreshCollaborationState();renderAll()
      }
      ...
      let fdReaderSpeechChunks=[];
      let fdReaderSpeechIndex=0;
      let fdReaderSpeaking=false;
      let fdReaderPaused=false;
      function fdReaderPreferredVoice(){
        const voices=window.speechSynthesis?.getVoices?.()||[],english=voices.filter(voice=>/^en[-_]/i.test(voice.lang||''));
        return english.find(voice=>/(david|daniel|george|alex|james|ryan|eric|male)/i.test(voice.name||''))||english.find(voice=>/(natural|neural|premium)/i.test(voice.name||''))||english[0]||voices[0]||null
      }
      function fdReaderSpeechText(){return [$('#fdReaderTitle')?.textContent,$('#fdReaderSubtitle')?.textContent,stripHTML($('#fdReaderContent')?.innerHTML||'')].filter(Boolean).join('. ').replace(/\s+/g,' ').trim()}
      function fdReaderSpeechParts(text,max=900){const sentences=String(text||'').match(/[^.!?]+[.!?]+|[^.!?]+$/g)||[];const parts=[];let current='';for(const sentence of sentences){if((current+' '+sentence).trim().length>max&&current){parts.push(current.trim());current=sentence}else current=(current+' '+sentence).trim()}if(current)parts.push(current.trim());return parts}
      function updateReaderSpeechUi(){const play=$('#fdReaderSpeak'),wave=$('#fdReaderWave');if(play){play.innerHTML=`<i class="fas fa-${fdReaderSpeaking&&!fdReaderPaused?'pause':'play'}"></i>`;play.title=fdReaderSpeaking&&!fdReaderPaused?'Pause reading':'Read or resume'}wave?.classList.toggle('is-speaking',fdReaderSpeaking&&!fdReaderPaused)}
      function stopReaderSpeech(){try{window.speechSynthesis?.cancel()}catch(error){}fdReaderSpeechChunks=[];fdReaderSpeechIndex=0;fdReaderSpeaking=false;fdReaderPaused=false;updateReaderSpeechUi()}
      function speakReaderChunk(){
        if(!window.speechSynthesis||fdReaderSpeechIndex>=fdReaderSpeechChunks.length){stopReaderSpeech();return}
        const utterance=new SpeechSynthesisUtterance(fdReaderSpeechChunks[fdReaderSpeechIndex]);utterance.voice=fdReaderPreferredVoice();utterance.rate=.86;utterance.pitch=.78;utterance.volume=Math.max(.1,Math.min(1,Number($('#fdReaderVolume')?.value||.8)));utterance.onend=()=>{fdReaderSpeechIndex+=1;speakReaderChunk()};utterance.onerror=()=>stopReaderSpeech();fdReaderSpeaking=true;fdReaderPaused=false;updateReaderSpeechUi();window.speechSynthesis.speak(utterance)
      }
      function toggleReaderSpeech(){
        if(!('speechSynthesis'in window)){showToast('Audio reading is not supported by this browser.');return}
        if(fdReaderSpeaking&&fdReaderPaused){window.speechSynthesis.resume();fdReaderPaused=false;updateReaderSpeechUi();return}
        if(fdReaderSpeaking){window.speechSynthesis.pause();fdReaderPaused=true;updateReaderSpeechUi();return}
        const text=fdReaderSpeechText();if(!text){showToast('This publication has no readable text.');return}window.speechSynthesis.cancel();fdReaderSpeechChunks=fdReaderSpeechParts(text);fdReaderSpeechIndex=0;speakReaderChunk()
      }
      function openReader(id,{updateHash=true}={}){
        ...
        currentReaderArticleId=id;if(article.status==='Published')recordSocialImpression(article.id,'reader');
      ...
      async function beginGoogleAuthentication(statusId='loginStatus',remember=true){
        ...
        const buttons=['#googleLoginButton','#googleSignupButton','#utilityGoogleLogin'].map(selector=>$(selector)).filter(Boolean);
      ...
      $('#utilityGoogleLogin')?.addEventListener('click',event=>{event.preventDefault();if(activeAccount){showView('writing');document.body.classList.add('dashboard-mode');return}beginGoogleAuthentication('loginStatus',true)});
      $('#signupForm')?.addEventListener('submit',async event=>{
        event.preventDefault();
        if(!requireSupabase('signupStatus'))return;
        const form=event.currentTarget;
        if(form.dataset.submitting==='true')return;
        const data=Object.fromEntries(new FormData(form).entries()),avatarFile=$('#signupAvatar')?.files?.[0]||null;
        setStatus('signupStatus','');
        if(!data.firstName||!data.lastName||!data.email||!data.username||!data.password||!data.confirmPassword||!avatarFile){setStatus('signupStatus','Complete all required fields and choose a profile picture.','error');return}
        const username=String(data.username).trim();
        if(!/^[A-Za-z0-9][A-Za-z0-9._-]{2,29}$/.test(username)){setStatus('signupStatus','Use 3–30 letters, numbers, dots, underscores or hyphens for the username.','error');return}
        if(String(data.password).length<8){setStatus('signupStatus','Password must contain at least 8 characters.','error');return}
        if(data.password!==data.confirmPassword){setStatus('signupStatus','The two passwords do not match.','error');return}
        const submit=form.querySelector('[type="submit"]');
        form.dataset.submitting='true';
        form.setAttribute('aria-busy','true');
        if(submit)submit.disabled=true;
        setStatus('signupStatus','Creating and activating your secure Client account…','');
        try{
          const available=await supabaseClient.rpc('findat_username_available',{candidate:username});
          if(available.error)throw available.error;
          if(available.data!==true){setStatus('signupStatus','This username is already in use.','error');return}
          rememberSupabaseSession=true;
          authTransitionInProgress=true;
          const{data:authData,error}=await supabaseClient.auth.signUp({email:String(data.email).trim(),password:String(data.password),options:{emailRedirectTo:`${location.origin}${location.pathname}#login`,data:{username,first_name:String(data.firstName).trim(),last_name:String(data.lastName).trim(),phone:String(data.phone||'').trim(),organisation:String(data.organisation||'').trim(),country:'',role_request:'client',registration_source:'public'}}});
          if(error)throw error;
          if(authData.session?.user?.id)await uploadRegistrationAvatar(avatarFile,authData.session.user.id);
          form.reset();$('#signupAvatarPreview').innerHTML='<i class="fas fa-user"></i>';
          $('#signupRole').value='client';
          setStatus('signupStatus','Account created and activated successfully.','success');
          window.toggleModal?.('signupPopup',false);
          authTransitionInProgress=false;
          const successHeading=$('#signupSuccessHeading'),successMessage=$('#signupSuccessMessage');
          if(authData.session?.user?.id){
            if(successHeading)successHeading.textContent='Your Client account is ready';
            if(successMessage)successMessage.textContent='Your account was created and activated successfully. You can use FINDAT immediately without waiting for Administrator confirmation.';
            const account=await waitForOwnProfile(authData.session.user.id);
            if(account?.active!==false)await enterDashboard(account);
          }else{
            if(successHeading)successHeading.textContent='Your Client account was created';
            if(successMessage)successMessage.textContent='Administrator approval is not required. Complete any email confirmation requested by your Supabase authentication settings, then sign in with your username or email and password.';
          }
          window.toggleModal?.('signupApprovalPopup',true);
        }catch(error){
          setStatus('signupStatus',friendlySignupError(error),'error');
        }finally{
          authTransitionInProgress=false;
      ...
      function activationDeliveryNotice(response,base='Account activated and confirmed.'){
        const delivery=response?.delivery;if(!delivery)return base;
        const label=channel=>{if(!channel)return'not reported';if(channel.status==='sent')return'sent';if(channel.status==='skipped')return`not sent — ${channel.detail||'provider or contact details unavailable'}`;return`failed — ${channel.detail||'provider error'}`};
        return `${base}
Email: ${label(delivery.email)}
WhatsApp: ${label(delivery.whatsapp)}
For security, the password itself is never sent; the user keeps the registration password and receives a secure password setup/reset link.`
      }
      function accountStateBadges(user){const active=`<span class="fd-account-status ${user.active?'is-active':'is-suspended'}">${user.active?'Active':'Inactive / suspended'}</span>`;const email=user.emailConfirmed?'<span class="fd-account-status is-confirmed"><i class="fas fa-envelope-circle-check"></i> Email confirmed</span>':'<span class="fd-account-status is-unconfirmed"><i class="fas fa-envelope-open"></i> Email unconfirmed</span>';return`${active}${email}`}
      async function refreshAdminAccounts(){if(!isAdmin())return;const body=$('#fdAccountTableBody');if(body)body.innerHTML='<tr><td colspan="4">Loading accounts…</td></tr>';try{const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'list'},true),users=(response.users||[]).map(profileFromRow);accountCache=[...new Map([activeAccount,...users].filter(Boolean).map(item=>[item.id,item])).values()];if($('#fdAccountNavCount'))$('#fdAccountNavCount').textContent=String(users.length);if(!body)return;body.innerHTML=users.length?users.map(user=>`<tr class="${user.role==='admin'?'is-admin-row':''}" data-account-search="${escapeHTML(normal(`${accountName(user)} ${user.username} ${user.email} ${user.organisation} ${user.phone} ${roleLabel(user.role)}`))}"><td><strong>${escapeHTML(accountName(user))}</strong><small>@${escapeHTML(user.username)} · ${escapeHTML(user.email)}</small>${user.organisation||user.phone?`<small>${escapeHTML([user.organisation,user.phone].filter(Boolean).join(' · '))}</small>`:''}</td><td>${adminRoleBadge(user.role)}</td><td><div class="fd-account-status-stack">${accountStateBadges(user)}</div></td><td><div class="fd-account-controls">${user.role==='admin'?'<button class="fd-mini-btn is-locked" disabled type="button"><i class="fas fa-lock"></i> Protected</button>':`${user.emailConfirmed?'':`<button class="fd-mini-btn is-success" data-account-confirm="${user.id}" type="button"><i class="fas fa-envelope-circle-check"></i> Confirm email</button>`}<button class="fd-mini-btn" data-account-password="${user.id}" type="button"><i class="fas fa-key"></i> Password</button>${user.role==='consultant'?`<button class="fd-mini-btn is-admin-promote" data-account-promote="${user.id}" data-account-name="${escapeHTML(accountName(user))}" type="button"><i class="fas fa-user-shield"></i> Make Administrator</button>`:''}<button class="fd-mini-btn ${user.active?'is-danger':'is-success'}" data-account-active="${user.id}" data-next-active="${user.active?'false':'true'}" type="button">${user.active?'Suspend':'Activate'}</button>`}</div></td></tr>`).join(''):'<tr><td colspan="4">No accounts found.</td></tr>';setTimeout(()=>$('#fdAccountSearch')?.dispatchEvent(new Event('input')),0)}catch(error){if(body)body.innerHTML=`<tr><td colspan="4">${escapeHTML(error.message||'Accounts could not be loaded.')}</td></tr>`}}
      ...
      $('#fdAccountTableBody')?.addEventListener('click',async event=>{const confirmButton=event.target.closest('[data-account-confirm]'),passwordButton=event.target.closest('[data-account-password]'),promoteButton=event.target.closest('[data-account-promote]'),activeButton=event.target.closest('[data-account-active]');if(confirmButton){if(!confirm('Confirm this Client or Consultant email, activate the account, and send activation notifications?'))return;try{const result=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'confirm_email',userId:confirmButton.dataset.accountConfirm},true);alert(activationDeliveryNotice(result,'The account details and email were confirmed, and the account was activated.'));await refreshAdminAccounts()}catch(error){alert(error.message||'The email could not be confirmed.')}return}if(passwordButton){const password=prompt('Enter the new password (at least 8 characters):');if(!password)return;if(password.length<8){alert('Password must contain at least 8 characters.');return}try{const result=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'change_password',userId:passwordButton.dataset.accountPassword,password},true);alert(activationDeliveryNotice(result,'Password changed, email confirmed, and the account is active.'));await refreshAdminAccounts()}catch(error){alert(error.message||'The password could not be changed.')}return}if(promoteButton){const target=accountCache.find(item=>item.id===promoteButton.dataset.accountPromote);if(target?.role!=='consultant'){alert('Only a Consultant account can be promoted to Administrator.');return}const name=promoteButton.dataset.accountName||'this Consultant';if(!confirm(`Promote ${name} to Administrator? This grants full FINDAT privileges and the account will become protected.`))return;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'promote_admin',userId:promoteButton.dataset.accountPromote},true);alert(`${name} is now an Administrator.`);await refreshAdminAccounts();await refreshDirectory()}catch(error){alert(error.message||'The account could not be promoted.')}return}if(activeButton){const active=activeButton.dataset.nextActive==='true';try{const result=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'set_active',userId:activeButton.dataset.accountActive,active},true);if(active)alert(activationDeliveryNotice(result,'The account was approved, activated, and email-confirmed.'));else alert('The account was suspended.');await refreshAdminAccounts()}catch(error){alert(error.message||'The account status could not be changed.')}}});
      ...
      function syncToolPanelLabel(panel){
        const label=panel?.querySelector('summary small');
        if(label)label.textContent=panel.open?'Close':'Open';
      }
      ['fdDataTools','fdPythonTools'].forEach(id=>{
        const panel=$(`#${id}`);
        if(!panel)return;
        syncToolPanelLabel(panel);
        panel.addEventListener('toggle',()=>{
          if(panel.open){
            const other=$(`#${id==='fdDataTools'?'fdPythonTools':'fdDataTools'}`);
            if(other?.open)other.open=false;
          }
          syncToolPanelLabel(panel);
          const other=$(`#${id==='fdDataTools'?'fdPythonTools':'fdDataTools'}`);
          if(other)syncToolPanelLabel(other);
        });
      });


      let viewedSocialProfileId='';
      const socialImpressionSeen=new Set();
      let socialImpressionObserver=null;
      function profileFollowerCount(userId){return socialCache.follows.filter(row=>row.following_id===userId).length}
      function profileFollowingCount(userId){return socialCache.follows.filter(row=>row.follower_id===userId).length}
      function openSocialProfile(userId){
        const profile=socialProfileById(userId)||accountById(userId);if(!profile)return;
        viewedSocialProfileId=profile.id;const own=profile.id===activeAccount?.id,modal=$('#fdPublicProfileModal'),cover=$('#fdPublicProfileCover'),avatar=$('#fdPublicProfileAvatar');
        if(cover)cover.style.backgroundImage=profile.coverUrl?`linear-gradient(rgba(20,32,48,.08),rgba(20,32,48,.24)),url("${String(profile.coverUrl).replace(/"/g,'')}")`:'';
        if(avatar)avatar.innerHTML=profile.avatarUrl?`<img src="${escapeHTML(profile.avatarUrl)}" alt="${escapeHTML(accountName(profile))} profile picture">`:escapeHTML(initials(profile));
        if($('#fdPublicProfileName'))$('#fdPublicProfileName').textContent=accountName(profile);if($('#fdPublicProfileHandle'))$('#fdPublicProfileHandle').textContent=profile.username?`@${profile.username}`:roleLabel(profile.role);
        if($('#fdPublicProfileProfessional'))$('#fdPublicProfileProfessional').textContent=[profile.jobTitle,profile.organisation||profile.placeOfWork].filter(Boolean).join(' · ')||roleLabel(profile.role);
        if($('#fdPublicProfileLocation'))$('#fdPublicProfileLocation').textContent=[profile.country,profile.phone].filter(Boolean).join(' · ');
        if($('#fdPublicProfileQualifications'))$('#fdPublicProfileQualifications').textContent=profile.qualifications||'';
        if($('#fdPublicProfilePosts'))$('#fdPublicProfilePosts').textContent=String(workspaceCache.articles.filter(article=>article.status==='Published'&&(article.ownerId===profile.id||article.createdBy===profile.id)).length);
        if($('#fdPublicProfileFollowers'))$('#fdPublicProfileFollowers').textContent=String(profileFollowerCount(profile.id));if($('#fdPublicProfileFollowing'))$('#fdPublicProfileFollowing').textContent=String(profileFollowingCount(profile.id));
        const follow=$('#fdPublicProfileFollow'),message=$('#fdPublicProfileMessage'),edit=$('#fdPublicProfileEditOwn');
        if(follow){follow.hidden=own;follow.dataset.socialFollow=own?'':profile.id;follow.innerHTML=`<i class="fas ${socialIsFollowing(profile.id)?'fa-user-check':'fa-user-plus'}"></i> ${socialIsFollowing(profile.id)?'Following':'Follow'}`}
        if(message){message.hidden=own;message.dataset.socialMessage=own?'':profile.id}if(edit)edit.hidden=!own;
        window.toggleModal?.('fdPublicProfileModal',true)
      }
      function regionFromCountry(country){const value=normal(country);if(!value)return'Unspecified';const africa=['zambia','zimbabwe','south africa','nigeria','kenya','ghana','uganda','tanzania','rwanda','botswana','namibia','malawi','mozambique','angola','ethiopia','egypt','morocco','tunisia','algeria','senegal','cameroon','congo','lesotho','eswatini','mauritius'];const europe=['united kingdom','uk','england','scotland','wales','france','germany','italy','spain','portugal','netherlands','belgium','sweden','norway','denmark','finland','poland','ireland','switzerland','austria','greece'];const asia=['china','india','japan','singapore','malaysia','indonesia','philippines','thailand','vietnam','pakistan','bangladesh','united arab emirates','uae','saudi arabia','qatar','israel','turkey','south korea'];const americas=['united states','usa','us','canada','mexico','brazil','argentina','chile','colombia','peru','jamaica','bahamas','barbados'];const oceania=['australia','new zealand','fiji','papua new guinea'];if(africa.some(x=>value.includes(x)))return'Africa';if(europe.some(x=>value.includes(x)))return'Europe';if(asia.some(x=>value.includes(x)))return'Asia';if(americas.some(x=>value.includes(x)))return'Americas';if(oceania.some(x=>value.includes(x)))return'Oceania';return'Other'}
      function analyticsRows(host,rows){if(!host)return;const values=Object.entries(rows||{}).sort((a,b)=>b[1]-a[1]),max=Math.max(1,...values.map(row=>row[1]));host.innerHTML=values.length?values.map(([label,count])=>`<div class="fd-social-analytics-row"><span>${escapeHTML(label||'Unspecified')}</span><span class="fd-social-analytics-bar"><i style="width:${Math.max(4,count/max*100)}%"></i></span><b>${count}</b></div>`).join(''):'<div class="fd-empty-compact">No audience data is available yet.</div>'}
      function renderAudienceMap(regions){const host=$('#fdSocialAnalyticsMap');if(!host)return;const max=Math.max(1,...Object.values(regions||{})),level=name=>Math.max(0,Math.min(4,Math.ceil(((regions?.[name]||0)/max)*4)));host.innerHTML=`<svg viewBox="0 0 760 330" role="img" aria-label="Simplified world audience map"><path class="fd-map-region" data-level="${level('Americas')}" d="M55 55h135l55 55-34 54-39 13-18 70-51 22-32-72 20-60-50-35z"/><path class="fd-map-region" data-level="${level('Europe')}" d="M320 62h100l27 42-45 35-87-11-25-35z"/><path class="fd-map-region" data-level="${level('Africa')}" d="M310 144l112-10 43 66-42 102-70-14-45-84z"/><path class="fd-map-region" data-level="${level('Asia')}" d="M425 55h235l55 77-52 65-87-10-51 52-80-53 20-64-38-34z"/><path class="fd-map-region" data-level="${level('Oceania')}" d="M590 239l96-7 32 43-45 37-83-18z"/><text class="fd-map-label" x="130" y="155">Americas</text><text class="fd-map-label" x="365" y="103">Europe</text><text class="fd-map-label" x="375" y="220">Africa</text><text class="fd-map-label" x="565" y="130">Asia</text><text class="fd-map-label" x="650" y="275">Oceania</text></svg>`}
      async function openSocialAnalytics(articleId){
        const article=workspaceCache.articles.find(item=>item.id===articleId);if(!article||!socialOwnsArticle(article)&&!isAdmin())return;
        window.toggleModal?.('fdSocialAnalyticsModal',true);$('#fdSocialAnalyticsTitle').textContent=article.title||'Publication analytics';$('#fdSocialAnalyticsSubtitle').textContent='Weighted reach, engagement and follower audience insights.';setStatus('fdSocialAnalyticsStatus','Loading analytics…','');
        const likes=socialReactionCount(articleId,'like'),reposts=socialReactionCount(articleId,'repost'),comments=socialCommentCount(articleId),saves=socialCache.bookmarks.filter(row=>row.article_id===articleId).length,score=socialTrendScore(article);let views=0,unique=0,analyticsData={};
        try{const result=await supabaseClient.rpc('findat_article_analytics',{p_article_id:articleId});if(result.error)throw result.error;analyticsData=result.data||{};views=Number(analyticsData.views||0);unique=Number(analyticsData.unique_viewers||0)}catch(error){setStatus('fdSocialAnalyticsStatus','Engagement is available. Deploy the supplied social analytics SQL to add view tracking.','')}
        const metrics=$('#fdSocialAnalyticsMetrics');if(metrics)metrics.innerHTML=[['Views',views],['Unique viewers',unique],['Likes',likes],['Comments',comments],['Reposts',reposts],['Trend score',Math.round(score)]].map(([label,value])=>`<article><strong>${value}</strong><span>${label}</span></article>`).join('');
        const articleOwnerId=article.ownerId||article.createdBy||activeAccount.id,followers=socialCache.follows.filter(row=>row.following_id===articleOwnerId).map(row=>socialProfileById(row.follower_id)).filter(Boolean),companies={...(analyticsData.follower_companies||{})},regions={};Object.entries(analyticsData.follower_regions||{}).forEach(([country,count])=>{const region=regionFromCountry(country);regions[region]=(regions[region]||0)+Number(count||0)});if(!Object.keys(companies).length||!Object.keys(regions).length)followers.forEach(person=>{if(!Object.keys(companies).length){const company=person.organisation||person.placeOfWork||'Unspecified';companies[company]=(companies[company]||0)+1}if(!Object.keys(regions).length){const region=regionFromCountry(person.country);regions[region]=(regions[region]||0)+1}});analyticsRows($('#fdSocialAnalyticsCompanies'),companies);analyticsRows($('#fdSocialAnalyticsRegions'),regions);renderAudienceMap(regions);const followerTotal=Number(analyticsData.followers??followers.length);if(!$('#fdSocialAnalyticsStatus')?.textContent)setStatus('fdSocialAnalyticsStatus',`${followerTotal} follower profile${followerTotal===1?'':'s'} included in audience demographics.`,'success')
      }
      async function recordSocialImpression(articleId,source='feed'){if(!activeAccount||!articleId||socialImpressionSeen.has(articleId))return;socialImpressionSeen.add(articleId);try{await supabaseClient.rpc('findat_record_article_impression',{p_article_id:articleId,p_session_id:sessionStorage.getItem('findat_social_view_session')||(()=>{const id=uid();sessionStorage.setItem('findat_social_view_session',id);return id})(),p_source:source})}catch(error){console.warn('FINDAT impression tracking unavailable',error)}}
      function observeSocialPosts(){if(!('IntersectionObserver'in window))return;if(!socialImpressionObserver)socialImpressionObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting&&entry.intersectionRatio>=.45){recordSocialImpression(entry.target.dataset.socialPost,'feed');socialImpressionObserver.unobserve(entry.target)}}),{threshold:[.45]});$$('#fdSocialFeed .fd-social-post[data-social-post]').forEach(post=>socialImpressionObserver.observe(post))}
      function socialX1WelcomeMarkup(){return '<div class="fd-social-x1-welcome"><span aria-hidden="true" class="x1-side-logo fd-x1-welcome-logo">x1<small>|</small></span><strong>x1 is ready</strong><span>OpenAI is used when available; Ollama and embedded FINDAT knowledge remain as fallbacks.</span></div>'}
      function resetSocialX1Thread(){const thread=$('#fdSocialX1Thread');if(thread)thread.innerHTML=socialX1WelcomeMarkup();const input=$('#fdSocialX1Input');if(input)input.value='';setStatus('fdSocialX1Status','','');input?.focus()}
      function socialX1Message(role,text,thinking=false){const thread=$('#fdSocialX1Thread');if(!thread)return null;thread.querySelector('.fd-social-x1-welcome')?.remove();const node=document.createElement('div');node.className=`fd-social-x1-message ${role==='user'?'is-user':''} ${thinking?'is-thinking':''}`;node.textContent=text;thread.appendChild(node);thread.scrollTop=thread.scrollHeight;return node}
      async function askSocialX1(prompt){const text=String(prompt||'').trim();if(!text)return;socialX1Message('user',text);const thinking=socialX1Message('assistant','x1 is analyzing…',true);setStatus('fdSocialX1Status','');try{const current={title:$('#fdArticleTitle')?.value||'',subtitle:$('#fdArticleSubtitle')?.value||'',content:stripHTML($('#fdArticleEditor')?.innerHTML||'').slice(0,9000),template:$('#fdArticleTemplate')?.value||''};const result=await window.X1PublicAgent?.ask?.(text,{surface:'Writing Desk',currentDraft:current});if(!result?.text)throw new Error('x1 did not return an answer.');thinking?.remove();const answer=socialX1Message('assistant',result.text);if(answer&&(current.title||current.content)){const use=document.createElement('button');use.type='button';use.className='fd-mini-btn';use.innerHTML='<i class="fas fa-file-import"></i> Insert answer into current editor';use.addEventListener('click',()=>{setSocialEditorOpen(true,'x1-assisted publication');const editor=$('#fdArticleEditor');if(editor)editor.innerHTML=sanitiseHTML(`<p>${escapeHTML(result.text).replace(/\n\n/g,'</p><p>').replace(/\n/g,'<br>')}</p>`);setStatus('fdArticleStatus','x1 answer inserted for your review. It has not been saved or published.','success')});answer.append(document.createElement('br'),use)}setStatus('fdSocialX1Status',result.provider==='openai'?'OpenAI and x1 completed the response.':result.provider==='ollama'?'Local Ollama completed the response.':'Embedded x1 knowledge completed the response.','success')}catch(error){thinking?.remove();socialX1Message('assistant',error.message||'x1 is temporarily unavailable.');setStatus('fdSocialX1Status','The local x1 interface remains available from Developments.','error')}}
      const x1UsageConservativeDefaults={openai_enabled:true,guest_openai_enabled:true,prefer_local_for_simple:true,reasoning_effort:'none',max_output_tokens:420,response_word_limit:240,signed_prompt_chars:4500,guest_prompt_chars:1800,evidence_items:5,evidence_chars:1000,conversation_turns:3,signed_hourly_limit:8,signed_daily_limit:28,guest_hourly_limit:3,guest_daily_limit:8,signed_cooldown_seconds:15,guest_cooldown_seconds:45,signed_daily_token_budget:90000,guest_daily_token_budget:12000};
      const x1UsageFieldMap={openai_enabled:'fdX1SettingOpenAIEnabled',guest_openai_enabled:'fdX1SettingGuestEnabled',prefer_local_for_simple:'fdX1SettingPreferLocal',reasoning_effort:'fdX1SettingReasoning',max_output_tokens:'fdX1SettingMaxOutput',response_word_limit:'fdX1SettingWordLimit',signed_prompt_chars:'fdX1SettingSignedPrompt',guest_prompt_chars:'fdX1SettingGuestPrompt',evidence_items:'fdX1SettingEvidenceItems',evidence_chars:'fdX1SettingEvidenceChars',conversation_turns:'fdX1SettingConversationTurns',signed_hourly_limit:'fdX1SettingSignedHourly',signed_daily_limit:'fdX1SettingSignedDaily',guest_hourly_limit:'fdX1SettingGuestHourly',guest_daily_limit:'fdX1SettingGuestDaily',signed_cooldown_seconds:'fdX1SettingSignedCooldown',guest_cooldown_seconds:'fdX1SettingGuestCooldown',signed_daily_token_budget:'fdX1SettingSignedTokenBudget',guest_daily_token_budget:'fdX1SettingGuestTokenBudget'};
      function applyX1UsageForm(settings=x1UsageConservativeDefaults){Object.entries(x1UsageFieldMap).forEach(([key,id])=>{const input=$(`#${id}`);if(!input)return;const value=settings[key]??x1UsageConservativeDefaults[key];if(input.type==='checkbox')input.checked=Boolean(value);else input.value=String(value)})}
      function collectX1UsageForm(){const output={};Object.entries(x1UsageFieldMap).forEach(([key,id])=>{const input=$(`#${id}`);if(!input)return;output[key]=input.type==='checkbox'?Boolean(input.checked):key==='reasoning_effort'?String(input.value||'none'):Number(input.value)});output.signed_daily_limit=Math.max(output.signed_hourly_limit,output.signed_daily_limit);output.guest_daily_limit=Math.max(output.guest_hourly_limit,output.guest_daily_limit);return output}
      async function loadX1UsageSettings(){
        if(!isAdmin()||!supabaseClient)return;applyX1UsageForm(x1UsageConservativeDefaults);setStatus('fdX1UsageSettingsStatus','Loading secured settings…','');
        const {data,error}=await supabaseClient.from('findat_x1_runtime_settings').select('*').eq('id',1).maybeSingle();
        if(error){setStatus('fdX1UsageSettingsStatus','Run FINDAT-X1-LOW-TOKEN-SOCIAL-CONTROLS-UPGRADE.sql in Supabase, then reopen this panel.','error');return}
        applyX1UsageForm({...x1UsageConservativeDefaults,...(data||{})});setStatus('fdX1UsageSettingsStatus','These controls are enforced by the Supabase Edge Function.','success')
      }
      async function saveX1UsageSettings(event){
        event?.preventDefault();if(!isAdmin()||!supabaseClient)return;const button=$('#fdX1UsageSettingsForm button[type="submit"]');if(button)button.disabled=true;setStatus('fdX1UsageSettingsStatus','Saving server-side token controls…','');
        const settings={id:1,...collectX1UsageForm(),updated_by:activeAccount.id,updated_at:nowISO()};const {data,error}=await supabaseClient.from('findat_x1_runtime_settings').upsert(settings,{onConflict:'id'}).select().single();if(button)button.disabled=false;
        if(error){setStatus('fdX1UsageSettingsStatus',error.message||'The x1 settings could not be saved.','error');return}
        applyX1UsageForm({...x1UsageConservativeDefaults,...data});window.dispatchEvent(new CustomEvent('findat:x1-settings-updated',{detail:data}));setStatus('fdX1UsageSettingsStatus','x1 usage controls saved. New requests will use the updated limits.','success')
      }
      async function openX1UsageSettings(){if(!isAdmin())return;window.toggleModal?.('fdX1UsageSettingsModal',true);await loadX1UsageSettings()}

      window.FINDAT_SOCIAL_UI={openProfile:openSocialProfile,openAnalytics:openSocialAnalytics,observePosts:observeSocialPosts};
      $('#fdPublicProfileClose')?.addEventListener('click',()=>window.toggleModal?.('fdPublicProfileModal',false));$('#fdPublicProfileEditOwn')?.addEventListener('click',()=>{window.toggleModal?.('fdPublicProfileModal',false);$('#fdUserProfileButton')?.click()});
      $('#fdSocialAnalyticsClose')?.addEventListener('click',()=>window.toggleModal?.('fdSocialAnalyticsModal',false));
      $('#fdX1UsageSettingsButton')?.addEventListener('click',openX1UsageSettings);$('#fdX1UsageSettingsClose')?.addEventListener('click',()=>window.toggleModal?.('fdX1UsageSettingsModal',false));$('#fdX1UsageConservative')?.addEventListener('click',()=>{applyX1UsageForm(x1UsageConservativeDefaults);setStatus('fdX1UsageSettingsStatus','Conservative defaults loaded. Click Save controls to apply them.','')});$('#fdX1UsageSettingsForm')?.addEventListener('submit',saveX1UsageSettings);
      $('#fdSocialAskX1')?.addEventListener('click',()=>window.toggleModal?.('fdSocialX1Modal',true));$('#fdSocialX1Close')?.addEventListener('click',()=>window.toggleModal?.('fdSocialX1Modal',false));
      $('#fdSocialX1Form')?.addEventListener('submit',event=>{event.preventDefault();const input=$('#fdSocialX1Input'),value=input?.value||'';if(input)input.value='';askSocialX1(value)});$('#fdSocialX1Clear')?.addEventListener('click',resetSocialX1Thread);$$('[data-social-x1-prompt]').forEach(button=>button.addEventListener('click',()=>{if($('#fdSocialX1Input'))$('#fdSocialX1Input').value=button.dataset.socialX1Prompt||'';$('#fdSocialX1Input')?.focus()}));
      document.addEventListener('click',event=>{const profile=event.target.closest('[data-social-view-profile]');if(profile){event.preventDefault();openSocialProfile(profile.dataset.socialViewProfile);return}const analytics=event.target.closest('[data-social-analytics]');if(analytics){event.preventDefault();openSocialAnalytics(analytics.dataset.socialAnalytics)}});
      ...
      $('#fdProfileCoverInput')?.addEventListener('change',event=>uploadProfileCover(event.target.files?.[0]));
      $('#fdRemoveProfileCover')?.addEventListener('click',removeProfileCover);
      $('#signupAvatar')?.addEventListener('change',event=>{const file=event.target.files?.[0],preview=$('#signupAvatarPreview');if(!preview)return;if(!file){preview.innerHTML='<i class="fas fa-user"></i>';return}const reader=new FileReader();reader.onload=()=>preview.innerHTML=`<img src="${escapeHTML(String(reader.result||''))}" alt="Selected profile picture">`;reader.readAsDataURL(file)});
      const applyAccountRegistrySearch=()=>{const query=normal($('#fdAccountSearch')?.value),rows=$$('#fdAccountTableBody tr[data-account-search]');let shown=0;rows.forEach(row=>{const visible=!query||String(row.dataset.accountSearch||'').includes(query);row.hidden=!visible;if(visible)shown++});const body=$('#fdAccountTableBody'),empty=body?.querySelector('.fd-account-search-empty');if(empty)empty.remove();if(query&&rows.length&&!shown)body?.insertAdjacentHTML('beforeend','<tr class="fd-account-search-empty"><td colspan="4">No accounts match this search.</td></tr>')};
      $('#fdAccountSearch')?.addEventListener('input',applyAccountRegistrySearch);$('#fdAccountSearchButton')?.addEventListener('click',applyAccountRegistrySearch);
      ...
      $('#fdNewArticle')?.addEventListener('click',()=>startSocialComposition('classic'));
      $('#fdHeroNewArticle')?.addEventListener('click',()=>{startSocialComposition('classic');showView('writing')});
      $('#fdDashboardSearch')?.addEventListener('input',()=>{renderLibrary();renderSocialFeed()});
      ...
      $('#fdSocialComposeButton')?.addEventListener('click',()=>startSocialComposition('classic'));
      $('#fdSocialCloseEditor')?.addEventListener('click',()=>setSocialEditorOpen(false));
      $('#fdSocialRefresh')?.addEventListener('click',async()=>{const button=$('#fdSocialRefresh');button.disabled=true;await Promise.all([refreshDirectory(),refreshArticles(),refreshSocialState()]).catch(error=>{$('#fdSocialSystemStatus').textContent=error.message||'The feed could not refresh.'});button.disabled=false;renderAll()});
      $('#fdSocialOpenMessages')?.addEventListener('click',()=>openSocialChat());
      $('#fdSocialProfileEdit')?.addEventListener('click',()=>window.FINDAT_SOCIAL_UI?.openProfile(activeAccount?.id));
      $('#fdSocialDiscussionClose')?.addEventListener('click',()=>{closeSocialEmojiPickers();window.toggleModal?.('fdSocialDiscussionModal',false)});
      $('#fdSocialDiscussionReplyCancel')?.addEventListener('click',()=>setSocialDiscussionReply());
      $('#fdSocialDiscussionSend')?.addEventListener('click',sendSocialComment);
      $('#fdSocialDiscussionInput')?.addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();sendSocialComment()}});
      $('#fdSocialChatClose')?.addEventListener('click',()=>{closeSocialEmojiPickers();stopSocialTypingChannel();window.toggleModal?.('fdSocialChatModal',false)});
      $('#fdSocialChatSearch')?.addEventListener('input',renderSocialChatPeople);
      $('#fdSocialChatSend')?.addEventListener('click',sendSocialMessage);
      $('#fdSocialChatClear')?.addEventListener('click',clearSocialChat);
      $('#fdSocialChatInput')?.addEventListener('input',queueSocialTyping);
      $('#fdSocialChatInput')?.addEventListener('blur',()=>setTimeout(stopSocialTyping,120));
      $('#fdSocialChatInput')?.addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();sendSocialMessage()}});
      window.addEventListener('focus',async()=>{await refreshPublicArticles();if(activeAccount){try{await refreshCollaborationState();await refreshArticles();await refreshSocialState({quiet:true});renderAll()}catch(error){console.warn(error)}}});document.addEventListener('visibilitychange',async()=>{if(!document.hidden){await refreshPublicArticles();if(activeAccount){try{await refreshCollaborationState();await refreshArticles();await refreshSocialState({quiet:true});renderAll()}catch(error){console.warn(error)}}}});

      document.addEventListener('input',event=>{if(event.target?.id==='fdSocialNetworkSearch'){socialNetworkQuery=event.target.value||'';socialNetworkPage=1;renderSocialNetworkDirectory();setTimeout(()=>{const input=$('#fdSocialNetworkSearch');if(input){input.focus();input.setSelectionRange(input.value.length,input.value.length)}},0)}});
      document.addEventListener('keydown',event=>{if(event.target?.id==='fdSocialNetworkSearch'&&event.key==='Enter'){event.preventDefault();socialNetworkQuery=event.target.value||'';socialNetworkPage=1;renderSocialNetworkDirectory()}});

      document.addEventListener('click',async event=>{
        const emojiTrigger=event.target.closest('[data-social-emoji-trigger]');if(emojiTrigger){event.preventDefault();event.stopPropagation();toggleSocialEmojiPicker(emojiTrigger);return}
        const emojiButton=event.target.closest('[data-social-emoji]');if(emojiButton){event.preventDefault();const control=emojiButton.closest('.fd-social-emoji-control'),targetId=control?.querySelector('[data-social-emoji-trigger]')?.dataset.emojiTarget;if(targetId)insertSocialEmoji(targetId,emojiButton.dataset.socialEmoji||emojiButton.textContent||'');closeSocialEmojiPickers();return}
        if(!event.target.closest('.fd-social-emoji-control'))closeSocialEmojiPickers();
        const tab=event.target.closest('[data-social-tab]');if(tab){event.preventDefault();socialSetTab(tab.dataset.socialTab);return}
        const networkPage=event.target.closest('[data-social-network-page]');if(networkPage){event.preventDefault();socialNetworkPage=Math.max(1,Number(networkPage.dataset.socialNetworkPage)||1);renderSocialNetworkDirectory();return}
        const networkStep=event.target.closest('[data-social-network-step]');if(networkStep){event.preventDefault();socialNetworkPage=Math.max(1,socialNetworkPage+(Number(networkStep.dataset.socialNetworkStep)||0));renderSocialNetworkDirectory();return}
        const networkSearchButton=event.target.closest('#fdSocialNetworkSearchButton');if(networkSearchButton){event.preventDefault();socialNetworkQuery=$('#fdSocialNetworkSearch')?.value||'';socialNetworkPage=1;renderSocialNetworkDirectory();return}
        const compose=event.target.closest('[data-social-compose-kind]');if(compose){event.preventDefault();startSocialComposition(compose.dataset.socialComposeKind);return}
        const like=event.target.closest('[data-social-like]');if(like){event.preventDefault();await toggleSocialReaction(like.dataset.socialLike,'like');return}
        const repost=event.target.closest('[data-social-repost]');if(repost){event.preventDefault();await toggleSocialReaction(repost.dataset.socialRepost,'repost');return}
        const bookmark=event.target.closest('[data-social-bookmark]');if(bookmark){event.preventDefault();await toggleSocialBookmark(bookmark.dataset.socialBookmark);return}
        const follow=event.target.closest('[data-social-follow]');if(follow){event.preventDefault();await toggleSocialFollow(follow.dataset.socialFollow);return}
        const comment=event.target.closest('[data-social-comment]');if(comment){event.preventDefault();openSocialDiscussion(comment.dataset.socialComment);return}
        const share=event.target.closest('[data-social-share]');if(share){event.preventDefault();await shareSocialArticle(share.dataset.socialShare);return}
        const message=event.target.closest('[data-social-message]');if(message){event.preventDefault();openSocialChat(message.dataset.socialMessage);return}
        const chatPerson=event.target.closest('[data-social-chat-person]');if(chatPerson){event.preventDefault();await selectSocialChat(chatPerson.dataset.socialChatPerson);return}
        const reply=event.target.closest('[data-social-comment-reply]');if(reply){event.preventDefault();setSocialDiscussionReply(reply.dataset.socialCommentReply,reply.dataset.socialCommentAuthor||'comment');return}
        const remove=event.target.closest('[data-social-comment-delete]');if(remove){event.preventDefault();await deleteSocialComment(remove.dataset.socialCommentDelete);return}
      });
      ...
      document.addEventListener('click',async event=>{
        ...
        const accountApprovalOpen=event.target.closest('[data-account-approval-open]');if(accountApprovalOpen){event.preventDefault();window.toggleModal?.('fdCollaborationInboxModal',false);showView('accounts');return}
      ...
      $('#fdReaderClose')?.addEventListener('click',()=>closeReader());$('#fdReaderBack')?.addEventListener('click',()=>closeReader());$('#fdReaderPrint')?.addEventListener('click',()=>window.print());$('#fdReaderSpeak')?.addEventListener('click',toggleReaderSpeech);$('#fdReaderStop')?.addEventListener('click',stopReaderSpeech);$('#fdReaderVolume')?.addEventListener('input',()=>{if(fdReaderSpeaking&&!fdReaderPaused)updateReaderSpeechUi()});$('#fdArticleReader')?.addEventListener('click',event=>{if(event.target.id==='fdArticleReader')closeReader()});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('#fdArticleReader')?.classList.contains('is-open'))closeReader()});
      ...
      async function bootstrap(){if($('#fdTodayLabel'))$('#fdTodayLabel').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',day:'numeric',month:'long',year:'numeric'});if(!supabaseClient){syncPublicAuthBar(null);renderHomepagePublications();console.error('FINDAT Supabase Auth is not configured.');return}const googleAuthError=googleAuthErrorFromLocation();if(googleAuthError){openAuthModal('loginPopup');setStatus('loginStatus',`Google authentication was not completed: ${googleAuthError}`,'error');history.replaceState(null,'',location.pathname)}supabaseClient.auth.onAuthStateChange(async(event,session)=>{if(event==='PASSWORD_RECOVERY'){recoverySessionActive=true;openAuthModal('passwordResetPopup');return}if(event==='SIGNED_OUT'){stopSocialRealtime();activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];workspaceCache={articles:[],audit:[]};resetSocialState();clearInterval(collaborationRefreshTimer);syncPublicAuthBar(null);renderHomepagePublications()}if(event==='SIGNED_IN'&&session?.user&&!activeAccount&&!recoverySessionActive&&!authTransitionInProgress){try{const account=await waitForOwnProfile(session.user.id);if(account?.active!==false)await enterDashboard(account)}catch(error){console.warn(error)}}});await refreshPublicArticles();const session=await getCurrentSession();if(session?.user){try{const account=await waitForOwnProfile(session.user.id);if(account?.active!==false&&account){activeAccount=account;accountCache=[account];syncPublicAuthBar(account)}else if(account){await clearSession()}}catch(error){console.warn('FINDAT session profile unavailable',error)}}else syncPublicAuthBar(null);const publicationMatch=location.hash.match(/^#publication-(.+)$/);if(publicationMatch){openReader(publicationMatch[1],{updateHash:false});return}if(location.hash==='#recordings'){window.openRecordingsPanel?.();return}if(location.hash==='#live-training'){window.openLiveTrainingPanel?.();return}if(location.hash==='#publications'){window.openPublicationsPanel?.();return}if(activeAccount&&(location.hash==='#writing'||location.hash==='#workspace'||location.hash==='#dashboard'||location.hash==='#accounts'||location.hash==='#review'))await enterDashboard(activeAccount);clearInterval(publicRefreshTimer);publicRefreshTimer=setInterval(refreshPublicArticles,30000)}
      renderSocialEmojiPickers();
      bootstrap();
    })();
...
(() => {
  ...
  const LESSON_TITLE = 'How Data Analytics Work per Domain';
  ...
  function courseDurationLabel() {
    ...
    if (!video || !Number.isFinite(video.duration) || video.duration <= 0) return 'Recorded lesson';
    const seconds = Math.floor(video.duration % 60);
    const minutes = Math.floor((video.duration / 60) % 60);
    const hours = Math.floor(video.duration / 3600);
    return hours > 0 ? `Recorded lesson · ${hours}h ${String(minutes).padStart(2,'0')}m` : `Recorded lesson · ${minutes}m ${String(seconds).padStart(2,'0')}s`;
  }
...
(()=>{
    ...
    function openX1(){
      ...
      window.dispatchEvent(new CustomEvent('x1:opened'));
    ...
    document.querySelector('[data-x1-app-details]')?.addEventListener('click',event=>{event.stopPropagation();document.querySelector('[data-open-x1-app]')?.focus()});
...
(()=>{
  ...
  const adminKnowledgeManagerButton=document.getElementById('fdKnowledgeManagerButton');
  ...
  const recentStorageKey='findat_x1_recent_analyses_v4';
  const legacyRecentStorageKey='findat_x1_recent_analyses_v3';
  const activeRecentStorageKey='findat_x1_active_analysis_v1';
  ...
  const defaultOpenAIPolicy={openAIEnabled:true,guestOpenAIEnabled:true,preferLocalForSimple:true,reasoningEffort:'none',maxOutputTokens:420,responseWordLimit:240,signedPromptChars:4500,guestPromptChars:1800,evidenceItems:5,evidenceChars:1000,conversationTurns:3,signedHourlyLimit:8,signedDailyLimit:28,guestHourlyLimit:3,guestDailyLimit:8,signedCooldownSeconds:15,guestCooldownSeconds:45,signedDailyTokenBudget:90000,guestDailyTokenBudget:12000};
  const openAI={functionName:String(window.FINDAT_AUTH_CONFIG?.x1OpenAiFunction||'findat-x1-openai'),available:null,checking:false,model:'gpt-5-mini-2025-08-07',lastFailureAt:0,policy:{...defaultOpenAIPolicy}};
  function applyOpenAIPolicy(value={}){const source=value&&typeof value==='object'?value:{};const pick=(camel,snake)=>source[camel]??source[snake]??defaultOpenAIPolicy[camel];openAI.policy={openAIEnabled:Boolean(pick('openAIEnabled','openai_enabled')),guestOpenAIEnabled:Boolean(pick('guestOpenAIEnabled','guest_openai_enabled')),preferLocalForSimple:Boolean(pick('preferLocalForSimple','prefer_local_for_simple')),reasoningEffort:String(pick('reasoningEffort','reasoning_effort')||'none'),maxOutputTokens:Number(pick('maxOutputTokens','max_output_tokens'))||420,responseWordLimit:Number(pick('responseWordLimit','response_word_limit'))||240,signedPromptChars:Number(pick('signedPromptChars','signed_prompt_chars'))||4500,guestPromptChars:Number(pick('guestPromptChars','guest_prompt_chars'))||1800,evidenceItems:Number(pick('evidenceItems','evidence_items'))||5,evidenceChars:Number(pick('evidenceChars','evidence_chars'))||1000,conversationTurns:Number(pick('conversationTurns','conversation_turns'))||3,signedHourlyLimit:Number(pick('signedHourlyLimit','signed_hourly_limit'))||8,signedDailyLimit:Number(pick('signedDailyLimit','signed_daily_limit'))||28,guestHourlyLimit:Number(pick('guestHourlyLimit','guest_hourly_limit'))||3,guestDailyLimit:Number(pick('guestDailyLimit','guest_daily_limit'))||8,signedCooldownSeconds:Number(pick('signedCooldownSeconds','signed_cooldown_seconds'))||15,guestCooldownSeconds:Number(pick('guestCooldownSeconds','guest_cooldown_seconds'))||45,signedDailyTokenBudget:Number(pick('signedDailyTokenBudget','signed_daily_token_budget'))||90000,guestDailyTokenBudget:Number(pick('guestDailyTokenBudget','guest_daily_token_budget'))||12000};return openAI.policy}
  window.addEventListener('findat:x1-settings-updated',event=>applyOpenAIPolicy(event.detail||{}));
  ...
  function publicationKnowledgeUnits(){return (Array.isArray(window.FINDAT_X1_PUBLICATIONS)?window.FINDAT_X1_PUBLICATIONS:[]).slice(-40).map((article,index)=>({id:`publication_${article.id||index}`,title:article.title||'FINDAT publication',tags:tokens(`${article.title||''} ${article.subtitle||''} ${article.category||''}`).slice(0,24),text:String(article.content||article.subtitle||'').slice(0,7000),source:'Authenticated user publication library',chapter:article.category||'Publication',kind:'publication'})).filter(item=>item.text.length>70)}
  function adaptiveWritingProfile(){const articles=Array.isArray(window.FINDAT_X1_PUBLICATIONS)?window.FINDAT_X1_PUBLICATIONS:[],texts=articles.map(item=>String(item.content||'').trim()).filter(Boolean);if(!texts.length)return{available:false,publicationCount:0};const sentences=texts.flatMap(text=>text.match(/[^.!?]+[.!?]+|[^.!?]+$/g)||[]).map(value=>value.trim()).filter(Boolean),paragraphs=texts.flatMap(text=>text.split(/\n{2,}/)).map(value=>value.trim()).filter(Boolean),words=texts.flatMap(text=>text.split(/\s+/)).filter(Boolean),headingCount=texts.reduce((sum,text)=>sum+(text.match(/\b(?:introduction|conclusion|recommendation|analysis|findings|background|summary)\b/gi)||[]).length,0),bulletCount=texts.reduce((sum,text)=>sum+(text.match(/(?:^|\n)\s*(?:[-•*]|\d+[.)])\s+/g)||[]).length,0);return{available:true,publicationCount:texts.length,averageSentenceWords:Number((words.length/Math.max(1,sentences.length)).toFixed(1)),averageParagraphWords:Number((words.length/Math.max(1,paragraphs.length)).toFixed(1)),usesStructuredHeadings:headingCount>0,usesLists:bulletCount>0,preferredTone:'professional and evidence-led',instruction:'Use these aggregate style characteristics only when the user asks for writing assistance. Do not copy past sentences or reproduce a specific author exactly.'}}
  function buildKnowledgeIndex(){
    knowledgeBase=[...BUILT_IN_KB,...importedKnowledge,...publicationKnowledgeUnits()];
  ...
  function retrieveKnowledge(query,limit=8){
    ...
    const scored=indexState.documents.map(doc=>{
      ...
      if(doc.item.kind==='imported')raw+=.08;if(doc.item.kind==='publication')raw+=.06;
    ...
    while(selected.length<limit&&scored.length){
      ...
      scored.forEach((candidate,index)=>{
        ...
        const importedBoost=candidate.item.kind==='imported'?.12:candidate.item.kind==='publication'?.08:0;
  ...
  function recentId(){return `x1_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`}
  function cleanRecentItem(item,index=0){
    if(typeof item==='string')return{id:`legacy_${index}_${hashText(item)}`,title:String(item).trim(),messages:[],updatedAt:0};
    if(!item||typeof item!=='object')return null;
    const title=String(item.title||item.prompt||'').trim();if(!title)return null;
    const messages=Array.isArray(item.messages)?item.messages.filter(turn=>turn&&(turn.role==='user'||turn.role==='assistant')&&String(turn.text||'').trim()).slice(-30).map(turn=>({role:turn.role,text:String(turn.text),meta:String(turn.meta||''),time:Number(turn.time)||0})):[];
    return{id:String(item.id||recentId()),title,messages,updatedAt:Number(item.updatedAt)||0};
  }
  function saveRecentItems(){try{localStorage.setItem(recentStorageKey,JSON.stringify(recentItems.slice(0,20)))}catch(error){}}
  function saveActiveRecentId(){try{if(activeRecentId)localStorage.setItem(activeRecentStorageKey,activeRecentId);else localStorage.removeItem(activeRecentStorageKey)}catch(error){}}
  function renderRecentItems(){
    if(!recent)return;
    recent.innerHTML='';
    recentItems.forEach(item=>{
      const entry=document.createElement('div');entry.className='x1-recent-entry';
      const openButton=document.createElement('button');openButton.type='button';openButton.className='x1-recent-item';openButton.dataset.x1SavedAnalysis=item.id;openButton.textContent=item.title.length>54?`${item.title.slice(0,54)}…`:item.title;openButton.title=item.title;openButton.classList.toggle('is-active',item.id===activeRecentId);
      const deleteButton=document.createElement('button');deleteButton.type='button';deleteButton.className='x1-recent-delete';deleteButton.dataset.x1DeleteAnalysis=item.id;deleteButton.setAttribute('aria-label',`Delete analysis: ${item.title}`);deleteButton.title='Delete analysis';deleteButton.innerHTML='<i class="fas fa-trash-alt" aria-hidden="true"></i>';
      entry.append(openButton,deleteButton);recent.appendChild(entry);
    });
    if(recentEmpty)recentEmpty.hidden=recentItems.length>0;
    if(clearRecentButton)clearRecentButton.disabled=recentItems.length===0;
  }
  function initialiseRecentItems(){
    let stored=null;try{stored=localStorage.getItem(recentStorageKey);if(stored===null)stored=localStorage.getItem(legacyRecentStorageKey)}catch(error){}
    try{const parsed=stored?JSON.parse(stored):[];recentItems=(Array.isArray(parsed)?parsed:[]).map(cleanRecentItem).filter(Boolean)}catch(error){recentItems=[]}
    const seen=new Set();recentItems=recentItems.filter(item=>{const key=item.id||item.title.toLowerCase();if(seen.has(key))return false;seen.add(key);return true}).sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)).slice(0,20);
    try{activeRecentId=localStorage.getItem(activeRecentStorageKey)||null}catch(error){activeRecentId=null}
    if(activeRecentId&&!recentItems.some(item=>item.id===activeRecentId))activeRecentId=null;
    if(conversation.length){
      if(!activeRecentId){const firstUser=conversation.find(turn=>turn.role==='user');if(firstUser){const item={id:recentId(),title:String(firstUser.text).trim(),messages:conversation.slice(-30),updatedAt:Date.now()};recentItems.unshift(item);activeRecentId=item.id}}
      else{const item=recentItems.find(row=>row.id===activeRecentId);if(item&&!item.messages.length)item.messages=conversation.slice(-30)}
    }
    saveActiveRecentId();saveRecentItems();renderRecentItems();
  }
  function ensureActiveRecent(title){
    const clean=String(title||'').trim();if(!clean)return null;
    let item=activeRecentId?recentItems.find(row=>row.id===activeRecentId):null;
    if(!item){item={id:recentId(),title:clean,messages:[],updatedAt:Date.now()};activeRecentId=item.id;recentItems.unshift(item);saveActiveRecentId()}
    return item;
  }
  function addRecent(text){ensureActiveRecent(text);saveRecentItems();renderRecentItems()}
  function syncActiveRecent(){
    if(!activeRecentId||!conversation.length)return;
    const item=recentItems.find(row=>row.id===activeRecentId);if(!item)return;
    const firstUser=conversation.find(turn=>turn.role==='user');if(firstUser&&!item.title)item.title=String(firstUser.text).trim();
    item.messages=conversation.slice(-30);item.updatedAt=Date.now();recentItems=[item,...recentItems.filter(row=>row.id!==item.id)].slice(0,20);saveRecentItems();renderRecentItems();
  }
  function saveConversation(){
    try{if(conversation.length)localStorage.setItem(conversationStorageKey,JSON.stringify(conversation.slice(-30)));else localStorage.removeItem(conversationStorageKey)}catch(error){}
    syncActiveRecent();
  }
  function clearConversationStorage(){try{localStorage.removeItem(conversationStorageKey)}catch(error){}try{sessionStorage.removeItem(conversationStorageKey)}catch(error){}}
  function purgeChatThread(){if(!chatThread)return;chatThread.replaceChildren();chatThread.scrollTop=0;chatThread.removeAttribute('aria-busy');chatThread.dataset.x1Generation=String(conversationGeneration)}
  function showFreshConversationHome(){
    const appWorkspace=document.getElementById('x1AppWorkspace'),storeView=document.getElementById('developmentsStoreView'),homePanel=document.querySelector('[data-x1-workspace-panel="home"]');
    if(storeView)storeView.hidden=true;if(appWorkspace)appWorkspace.hidden=false;document.querySelectorAll('[data-x1-workspace-panel]').forEach(panel=>{panel.hidden=panel!==homePanel});
    document.querySelectorAll('[data-x1-workspace-tab]').forEach(button=>{button.classList.toggle('is-active',button.dataset.x1WorkspaceTab==='home');if(button.dataset.x1WorkspaceTab==='home')button.setAttribute('aria-current','page');else button.removeAttribute('aria-current')});
    document.querySelectorAll('[data-x1-open-workflow]').forEach(button=>button.classList.remove('is-active'));
    const homeCenter=homePanel?.querySelector('.x1-chatgpt-home'),freshElements=[homePanel?.querySelector('.x1-chatgpt-intro'),homePanel?.querySelector('.x1-composer-wrap'),homePanel?.querySelector('.x1-brain-suggestions')];
    freshElements.forEach(element=>{if(!element)return;element.hidden=false;element.removeAttribute('aria-hidden');element.style.removeProperty('display');element.style.removeProperty('visibility');element.style.removeProperty('opacity')});
    ['has-conversation','has-messages','is-chatting','is-conversation','is-busy'].forEach(className=>{homePanel?.classList.remove(className);homeCenter?.classList.remove(className)});
    if(homePanel)homePanel.scrollTop=0;const content=document.querySelector('.x1-fintech-content');if(content)content.scrollTop=0;
  }
  function startNewConversation(){
    conversationGeneration+=1;window.__x1ConversationGeneration=conversationGeneration;conversation=[];activeRecentId=null;saveActiveRecentId();clearConversationStorage();purgeChatThread();showFreshConversationHome();
    ...
    currentEvidence=[];lastIntent='general';lastTopic='x1';renderRecentItems();
    const resetGeneration=conversationGeneration;window.dispatchEvent(new CustomEvent('x1:conversation-reset',{detail:{generation:resetGeneration}}));
    queueMicrotask(()=>{if(resetGeneration!==conversationGeneration)return;purgeChatThread();showFreshConversationHome()});
    requestAnimationFrame(()=>{if(resetGeneration!==conversationGeneration)return;purgeChatThread();showFreshConversationHome();promptInput?.focus({preventScroll:true});document.getElementById('x1AppWorkspace')?.scrollIntoView({behavior:'smooth',block:'start'})});
  }
  function removeRecentAnalysis(id){
    const item=recentItems.find(row=>row.id===id);recentItems=recentItems.filter(row=>row.id!==id);saveRecentItems();if(activeRecentId===id)startNewConversation();else renderRecentItems();showToast(item?'Analysis deleted.':'Analysis history was not found.');
  }
  function clearRecentAnalyses(){
    const hasCurrentConversation=Boolean(conversation.length||chatThread?.children.length||promptInput?.value.trim());if(!recentItems.length&&!hasCurrentConversation){startNewConversation();showToast('No recent analyses yet. A new conversation is ready.');return}
    if(!window.confirm('Clear all recent analyses and the current x1 conversation?'))return;recentItems=[];saveRecentItems();startNewConversation();showToast('Recent analyses and conversation cleared. A new conversation is ready.');
  }
  function addChatMessage(role,text,meta='',persist=true){
    if(!chatThread)return null;const wrapper=document.createElement('div');wrapper.className=`x1-chat-message is-${role}`;wrapper.dataset.x1Generation=String(conversationGeneration);
    ...
    const visibleMeta=/^(Prompt input|Secure OpenAI\b)/i.test(String(meta||''))?'':String(meta||'');if(visibleMeta){const note=document.createElement('span');note.className='x1-chat-message-meta';note.textContent=visibleMeta;body.appendChild(note)}
    wrapper.append(icon,body);chatThread.appendChild(wrapper);chatThread.scrollTop=chatThread.scrollHeight;
    if(persist&&(role==='assistant'||role==='user')){conversation.push({role,text,meta:visibleMeta,time:Date.now()});conversation=conversation.slice(-30);saveConversation()}
    return wrapper;
  }
  function addThinking(label='x1'){
    if(!chatThread)return null;const wrapper=document.createElement('div');wrapper.className='x1-chat-message is-assistant is-thinking';wrapper.dataset.x1Generation=String(conversationGeneration);
    const icon=document.createElement('div');icon.className='x1-chat-message-icon';icon.textContent='x1';const body=document.createElement('div');body.className='x1-chat-message-body';const labelNode=document.createElement('span');labelNode.className='x1-thinking-label';labelNode.textContent=label;body.appendChild(labelNode);
    for(let i=0;i<3;i++){const dot=document.createElement('span');dot.className='x1-thinking-dot';body.appendChild(dot)}wrapper.append(icon,body);chatThread.appendChild(wrapper);chatThread.scrollTop=chatThread.scrollHeight;return wrapper;
  }
  function setThinkingLabel(wrapper,label){const node=wrapper?.querySelector('.x1-thinking-label');if(node)node.textContent=label}
  function initialiseConversation(){purgeChatThread();try{const stored=JSON.parse(localStorage.getItem(conversationStorageKey)||'[]');conversation=Array.isArray(stored)?stored.slice(-20):[]}catch(error){conversation=[]}if(conversation.length)conversation.forEach(item=>addChatMessage(item.role,item.text,item.meta||'',false))}
  function openRecentAnalysis(id){
    const item=recentItems.find(row=>row.id===id);if(!item)return;conversationGeneration+=1;window.__x1ConversationGeneration=conversationGeneration;conversation=[];purgeChatThread();showFreshConversationHome();const generation=conversationGeneration;const thinking=addThinking('x1');setTimeout(()=>{if(generation===conversationGeneration)setThinkingLabel(thinking,'Analyzing')},180);
    setTimeout(()=>{if(generation!==conversationGeneration)return;thinking?.remove();activeRecentId=item.id;saveActiveRecentId();conversation=(item.messages?.length?item.messages:[{role:'user',text:item.title,meta:'',time:item.updatedAt||Date.now()}]).slice(-30);purgeChatThread();conversation.forEach(turn=>addChatMessage(turn.role,turn.text,turn.meta||'',false));try{localStorage.setItem(conversationStorageKey,JSON.stringify(conversation))}catch(error){}renderRecentItems();promptInput?.focus({preventScroll:true})},520);
  }
  ...
  function getAppState(){const activePanel=document.querySelector('[data-x1-workspace-panel]:not([hidden])')?.dataset.x1WorkspacePanel||'home',panelNames={home:'New Chat',transform:'Transform Data',workbench:'Reconciliation workspace',workflows:'Workflows',overview:'Application capabilities'},nodes=document.querySelectorAll('#wfNodeLayer .wf-node').length,connections=document.querySelectorAll('#wfConnectionLayer path').length;return{panel:activePanel,panelName:panelNames[activePanel]||activePanel,reconciliationReady:resultsAvailable(),workflowNodes:nodes,workflowConnections:connections,neuralReady:brain.ready,knowledgeUnits:knowledgeBase.length,importedUnits:importedKnowledge.length,deepModel:ollama.enabled&&ollama.available}}
  ...
  let openAIIntroStarted=false;
  function updateOpenAIUi(text,state='idle'){
    const host=document.getElementById('x1OpenAIStatus'),label=document.getElementById('x1OpenAIStatusText');
    if(host)host.dataset.connectionState=state;if(label&&!openAIIntroStarted)label.textContent=text;
  }
  function runOpenAIIntro(){
    if(openAIIntroStarted)return;const host=document.getElementById('x1OpenAIStatus'),label=document.getElementById('x1OpenAIStatusText'),icon=host?.querySelector('i');if(!host||!label)return;
    openAIIntroStarted=true;host.hidden=false;host.classList.remove('is-ready-step','is-leaving');host.classList.add('is-intro-running');host.dataset.state='busy';label.textContent='Initializing AI';if(icon)icon.className='fas fa-circle-notch';
    setTimeout(()=>{host.dataset.state='ready';host.classList.add('is-ready-step');label.textContent='x1 is Ready';if(icon)icon.className='fas fa-check-circle'},1050);
    setTimeout(()=>host.classList.add('is-leaving'),2100);
    setTimeout(()=>{host.hidden=true;host.classList.remove('is-intro-running','is-ready-step','is-leaving')},2600);
  }
  window.addEventListener('x1:opened',runOpenAIIntro);

  function x1GuestId(){const key='findat_x1_guest_id_v1';let value=localStorage.getItem(key)||'';if(!/^[a-zA-Z0-9_-]{16,100}$/.test(value)){value=(crypto.randomUUID?.()||`${Date.now()}_${Math.random().toString(36).slice(2)}`).replace(/[^a-zA-Z0-9_-]/g,'');localStorage.setItem(key,value)}return value}
  async function invokeOpenAIService(body){
    const config=window.FINDAT_AUTH_CONFIG||{},url=String(config.supabaseUrl||''),key=String(config.publishableKey||config.anonKey||''),client=window.FINDAT_SUPABASE_CLIENT;
    if(!url||!key)throw new Error('The secure x1 service is not configured.');
    let session=null;try{session=(await client?.auth?.getSession?.())?.data?.session||null}catch(error){}
    const headers={'Content-Type':'application/json','apikey':key,'X-X1-Guest-ID':x1GuestId()};if(session?.access_token)headers.Authorization=`Bearer ${session.access_token}`;
    const response=await fetch(`${url}/functions/v1/${encodeURIComponent(openAI.functionName)}`,{method:'POST',headers,body:JSON.stringify(body||{})});
    let payload={};try{payload=await response.json()}catch(error){}
    if(!response.ok)throw new Error(payload.error||payload.message||`x1 OpenAI request failed (${response.status}).`);
    return payload;
  }

  async function testOpenAI({silent=true}={}){
    if(openAI.checking)return openAI.available===true;
    openAI.checking=true;updateOpenAIUi('Checking secure OpenAI connection…','busy');
    try{
      const result=await invokeOpenAIService({action:'health'});openAI.available=Boolean(result?.ready);openAI.model=result?.model||openAI.model;applyOpenAIPolicy(result?.policy||{});openAI.lastFailureAt=0;
      if(!openAI.available){updateOpenAIUi(openAI.policy?.openAIEnabled?'OpenAI restricted · local x1 remains active':'OpenAI paused by Administrator · local x1 remains active','error');if(!silent)showToast('OpenAI is restricted by the current x1 usage policy. The local engine remains active.');return false}
      updateOpenAIUi(`OpenAI connected securely · ${openAI.model}`,'ready');if(!silent)showToast('OpenAI is ready for x1 financial synthesis.');return true;
    }catch(error){
      const message=String(error?.message||'OpenAI is unavailable.');openAI.available=false;openAI.lastFailureAt=Date.now();
      updateOpenAIUi('OpenAI unavailable · local x1 remains active','error');
      if(!silent)showToast(`${message} x1 will continue with its local financial engine.`);return false;
    }finally{openAI.checking=false}
  }

  function reconciliationRows(bodyId,referenceField='id'){
    const body=document.getElementById(bodyId);if(!body)return[];
    return [...body.querySelectorAll('tr')].slice(0,80).map((row,index)=>{const value=name=>row.querySelector(`[data-field="${name}"]`)?.value?.trim()||'';return{row:index+1,date:value('date'),reference:value(referenceField),description:value('description'),debit:value('debit'),credit:value('credit'),balance:value('balance')}}).filter(row=>row.date||row.reference||row.description||row.debit||row.credit||row.balance);
  }
  function collectReconciliationEvidence(prompt){
    const q=normalise(prompt),relevant=/reconcil|ledger|bank|cash|interfund|transaction|classif|label|misplac|debit|credit|table|report|exception|match/.test(q);if(!relevant)return[];
    const interfund=!document.getElementById('x1InterfundModule')?.hidden;
    if(interfund){
      const admin=reconciliationRows('ifAdminRows','reference'),project=reconciliationRows('ifProjectRows','reference');if(!admin.length&&!project.length)return[];
      return[{id:'live-interfund',title:'Live Interfund Reconciliation data and rule',source:'Current x1 Interfund Reconciliation workspace',text:`Rule: Administration debits match Project credits; Administration credits match Project debits. Match each amount once. Labels or columns may be misplaced, so evaluate date, reference, description, debit/credit direction, amount, reciprocal context and closing balances before proposing a classification. Do not silently alter records. Administration closing balance: ${document.getElementById('ifAdminClosing')?.value||'not supplied'}. Project closing balance: ${document.getElementById('ifProjectClosing')?.value||'not supplied'}. Administration rows: ${JSON.stringify(admin)}. Project rows: ${JSON.stringify(project)}.`}];
    }
    const bank=reconciliationRows('x1BankRows'),cash=reconciliationRows('x1CashRows');if(!bank.length&&!cash.length)return[];
    return[{id:'live-bank',title:'Live Bank Reconciliation data and rule',source:'Current x1 Bank Reconciliation workspace',text:`Rule: Cash-ledger debits normally match bank-statement credits; cash-ledger credits normally match bank-statement debits. Match each amount once, then classify unmatched items as timing differences, bank-originated entries, book errors, bank errors or unresolved exceptions. Labels or columns may be misplaced, so evaluate economic substance using date, reference, description, debit/credit direction, amount, running balance and cross-ledger context. Do not silently alter records. Bank closing balance: ${document.getElementById('x1BankClosing')?.value||'not supplied'}. Cash closing balance: ${document.getElementById('x1CashClosing')?.value||'not supplied'}. Bank rows: ${JSON.stringify(bank)}. Cash rows: ${JSON.stringify(cash)}.`}];
  }

  function shouldUseOpenAI(prompt,result){
    const policy=openAI.policy||defaultOpenAIPolicy;if(!policy.openAIEnabled)return false;
    const text=String(prompt||''),q=normalise(text),explicit=/(use openai|ask chatgpt|deep analysis|detailed report|professional rewrite|synthesi[sz]e|complex reconciliation|audit opinion|forensic report)/.test(q),liveData=/reconcil|bank statement|cash book|interfund|uploaded document|attached file|publication draft/.test(q);
    if(explicit||liveData)return true;if(!policy.preferLocalForSimple)return true;
    const confidence=Number(result?.plan?.confidence||0),evidenceCount=Number(result?.matches?.length||0);
    if(text.length<=110)return false;if(text.length<=320&&confidence>=.5&&evidenceCount>0)return false;
    return confidence<.72||text.length>650||evidenceCount===0
  }

  async function callOpenAI(prompt,plan,matches){
    const policy=openAI.policy||defaultOpenAIPolicy;if(!policy.openAIEnabled)return null;
    if(openAI.available===false&&Date.now()-openAI.lastFailureAt<45000)return null;
    const evidenceItems=Math.max(1,Math.min(12,Number(policy.evidenceItems)||5)),evidenceChars=Math.max(400,Math.min(2500,Number(policy.evidenceChars)||1000));
    const segments=[...collectEvidenceSegments(prompt,matches||[],evidenceItems),...collectReconciliationEvidence(prompt)],evidence=segments.slice(0,evidenceItems).map((segment,index)=>({
      id:index+1,fact:String(segment.text||'').slice(0,evidenceChars),source:segment.source,section:segment.title,
      pages:segment.pageStart?segment.pageStart===segment.pageEnd?String(segment.pageStart):`${segment.pageStart}-${segment.pageEnd}`:'',jurisdiction:segment.jurisdiction
    }));
    const conversationTurns=Math.max(0,Math.min(8,Number(policy.conversationTurns)||3)),recentContext=conversationTurns?conversation.slice(0,-1).slice(-conversationTurns).map(turn=>({role:turn.role,text:String(turn.text||'').slice(0,1200)})):[];
    updateOpenAIUi('OpenAI is preparing the financial response…','busy');
    try{
      const result=await invokeOpenAIService({prompt:String(prompt||'').slice(0,Number(policy.signedPromptChars)||4500),plan,evidence,conversation:recentContext,styleProfile:adaptiveWritingProfile()});
      if(!result?.text)return null;openAI.available=true;openAI.model=result.model||openAI.model;applyOpenAIPolicy(result?.policy||policy);openAI.lastFailureAt=0;
      updateOpenAIUi(`OpenAI connected securely · ${openAI.model}`,'ready');
      return{text:String(result.text).trim(),model:openAI.model,usage:result.usage||{},evidenceCount:evidence.length};
    }catch(error){
      const message=String(error?.message||'OpenAI is unavailable.');openAI.available=false;openAI.lastFailureAt=Date.now();
      updateOpenAIUi('OpenAI unavailable · local x1 remains active','error');showToast(`${message} x1 used its local financial engine instead.`);return null;
    }
  }
  ...
  async function callOllama(prompt,plan,matches){
    ...
    const synthesisPrompt=`Adaptive writing profile (aggregate characteristics, never copy past wording):\n${JSON.stringify(adaptiveWritingProfile(),null,2)}\n\nResponse plan:\n${JSON.stringify(plan,null,2)}\n\nRetrieved evidence:\n${JSON.stringify(evidence,null,2)}\n\nRecent conversation:\n${recentContext||'None'}\n\nUser question:\n${prompt}`;
    try{
  ...
  function buildResponse(prompt){
    ...
    if(intent==='identity')return{localText:'I am x1 | ProATR, FINDAT’s financial assistant. I retrieve relevant evidence, classify intent, create a response plan and can use securely configured OpenAI synthesis through a server-side function, with local Ollama and embedded RAG fallback. I am application-aware but not conscious or sentient.',intent,topic:'identity',action:null,meta:'Architecture explanation',plan:{confidence:1},matches:[]};
    if(intent==='app_state')return{localText:`You are in ${state.panelName}. The knowledge base contains ${state.knowledgeUnits} evidence units, including ${state.importedUnits} imported units. Reconciliation results are ${state.reconciliationReady?'available':'not yet generated'}. The workflow canvas contains ${state.workflowNodes} nodes and ${state.workflowConnections} rendered connections. Secure OpenAI synthesis is ${openAI.available===true?'connected':openAI.available===false?'unavailable or not configured':'not yet tested'}, and the optional local deep model is ${state.deepModel?'connected':'off or unavailable'}.`,intent,topic:'state',action:null,meta:'Live application-state inspection',plan:{confidence:1},matches:[]};
  ...
  async function respond(raw){
    ...
    addRecent(prompt);addChatMessage('user',prompt,'');if(promptInput)promptInput.value='';if(sendButton)sendButton.disabled=true;
    const thinking=addThinking('x1');const thinkingTimer=setTimeout(()=>setThinkingLabel(thinking,'Analyzing'),260);
    await sleep(120+Math.min(260,prompt.length*2));
    if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
    if(window.X1AgenticRuntime?.execute){
      try{
        const agentResult=await window.X1AgenticRuntime.execute(prompt);
        if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
        if(agentResult?.handled){
          clearTimeout(thinkingTimer);thinking?.remove();
    ...
    await sleep(60);
    if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
    const result=buildResponse(prompt);currentEvidence=result.matches||[];let finalText=result.localText,meta=result.meta,provider='local';
    const externalEligible=!result.action&&!['calculation','greeting','gratitude','identity','app_state'].includes(result.intent)&&shouldUseOpenAI(prompt,result);
    if(externalEligible){
      const generated=await callOpenAI(prompt,result.plan,result.matches||[]);
      if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
      if(generated?.text){finalText=reviewAnswer(generated.text,prompt,result.plan);meta='';provider='openai'}
    }
    if(provider==='local'&&ollama.enabled&&ollama.available&&result.matches?.length){
      const generated=await callOllama(prompt,result.plan,result.matches);
      if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
      if(generated?.text){finalText=reviewAnswer(generated.text,prompt,result.plan);meta=`Local Ollama ${ollama.model} · ${generated.rewritten?'synthesis + rewrite':'synthesis'} · ${generated.evidenceCount} evidence passages · ${generated.sourceCount} source${generated.sourceCount===1?'':'s'}`;provider='ollama'}
    }
    if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
    clearTimeout(thinkingTimer);thinking?.remove();addChatMessage('assistant',finalText,meta);lastIntent=result.intent||lastIntent;lastTopic=result.topic||lastTopic;
    if(sendButton)sendButton.disabled=false;showToast(result.action?'Instruction understood. Opening workspace…':provider==='openai'?'Answer generated securely with OpenAI.':provider==='ollama'?'Answer synthesised with the local model.':'Response generated from the local knowledge base.');
    if(result.action)setTimeout(result.action,420);promptInput?.focus();
  }


  window.X1PublicAgent={
    async ask(raw,context={}){
      const prompt=String(raw||'').trim();if(!prompt)return{text:'Enter an instruction for x1.',provider:'local'};
      if(window.X1AgenticRuntime?.execute){try{const operation=await window.X1AgenticRuntime.execute(prompt);if(operation?.handled)return{text:operation.text||'The requested FINDAT operation is complete.',provider:'agent',meta:operation.meta||'Agentic execution'}}catch(error){console.warn('x1 public agent operation',error)}}
      const contextText=context?.currentDraft&&(context.currentDraft.title||context.currentDraft.content)?`\n\nCurrent Writing Desk draft (use only when relevant):\nTitle: ${context.currentDraft.title||'Untitled'}\nSubtitle: ${context.currentDraft.subtitle||''}\nTemplate: ${context.currentDraft.template||''}\nContent: ${String(context.currentDraft.content||'').slice(0,Math.min(3500,Number(openAI.policy?.signedPromptChars||4500)))}`:'';
      const combined=`${prompt}${contextText}`,result=buildResponse(combined);let text=result.localText,provider='local';
      if(!result.action&&!['calculation','greeting','gratitude','identity','app_state'].includes(result.intent)&&shouldUseOpenAI(combined,result)){const remote=await callOpenAI(combined,result.plan,result.matches||[]);if(remote?.text){text=reviewAnswer(remote.text,combined,result.plan);provider='openai'}else if(ollama.enabled&&ollama.available&&result.matches?.length){const local=await callOllama(combined,result.plan,result.matches);if(local?.text){text=reviewAnswer(local.text,combined,result.plan);provider='ollama'}}}else if(ollama.enabled&&ollama.available&&result.matches?.length&&String(combined).length>500){const local=await callOllama(combined,result.plan,result.matches);if(local?.text){text=reviewAnswer(local.text,combined,result.plan);provider='ollama'}}
      return{text,provider,meta:result.meta||''}
    },
    getState:()=>({openAI:openAI.available,ollama:ollama.enabled&&ollama.available,knowledgeUnits:knowledgeBase.length,styleProfile:adaptiveWritingProfile(),policy:{...openAI.policy}})
  };
  ...
  function addCapturedDocumentMessage(document){
    const name=document?.sourceName||'Document',format=String(document?.format||'file').toUpperCase(),details=document?.format==='pdf'?`${document.pages?.length||0} page${document.pages?.length===1?'':'s'} captured${document.ocrPages?` · OCR used on ${document.ocrPages} page${document.ocrPages===1?'':'s'}`:''}`:document?.format==='xlsx'?`${document.analysis?.metrics?.find(item=>item.label==='Rows')?.value||'Spreadsheet'} records captured`:'Document text captured';
    addChatMessage('assistant',`${name} has been captured and indexed (${format}; ${details}). Tell x1 what you want done with this document. x1 will wait for your instruction and will not generate an automatic image or analysis report.`,'');
  }
  async function importKnowledgeFiles(files){
    const selected=[...files];if(!selected.length)return;showToast(`Capturing ${selected.length} document${selected.length===1?'':'s'}…`);let added=0,pagesRead=0,ocrPagesRead=0,captured=0;
    for(const file of selected){try{const document=await readKnowledgeFile(file),chunks=chunkKnowledgeDocument(document);if(document.format==='pdf'){pagesRead+=document.pages.length;ocrPagesRead+=document.ocrPages||0}for(const chunk of chunks){if(importedKnowledge.length>=MAX_IMPORTED_CHUNKS)break;if(!importedKnowledge.some(item=>item.id===chunk.id)){importedKnowledge.push(chunk);added++}}addCapturedDocumentMessage(document);captured++;if(importedKnowledge.length>=MAX_IMPORTED_CHUNKS){showToast(`Knowledge capacity reached at ${MAX_IMPORTED_CHUNKS} sections. Clear older imported documents before adding more.`);break}}catch(error){showToast(`${file.name}: ${error.message}`);addChatMessage('assistant',`${file.name} could not be captured: ${error.message}`,'Upload PDF, Excel or Word files only.')}
    }
    saveImportedKnowledge();buildKnowledgeIndex();showToast(`${captured} document${captured===1?'':'s'} captured and ready for your instructions${pagesRead?` · ${pagesRead} PDF page${pagesRead===1?'':'s'}`:''}${ocrPagesRead?` · OCR used on ${ocrPagesRead} page${ocrPagesRead===1?'':'s'}`:''}.`);
  }
  ...
  function closeKnowledgeManager(){if(!knowledgeModal)return;knowledgeModal.hidden=true;knowledgeModal.setAttribute('aria-hidden','true');if(adminKnowledgeManagerButton&&!adminKnowledgeManagerButton.hidden)adminKnowledgeManagerButton.focus();else knowledgeManagerButton?.focus()}
  ...
  loadImportedKnowledge();loadOllamaSettings();buildKnowledgeIndex();window.addEventListener('findat:x1-publications-updated',()=>buildKnowledgeIndex());initialiseConversation();initialiseRecentItems();setTimeout(()=>testOpenAI({silent:true}),900);
  ...
  recent?.addEventListener('click',event=>{
    const deleteButton=event.target.closest('[data-x1-delete-analysis]');
    if(deleteButton){event.preventDefault();event.stopPropagation();removeRecentAnalysis(deleteButton.dataset.x1DeleteAnalysis);return}
    const button=event.target.closest('[data-x1-saved-analysis]');
    if(button)openRecentAnalysis(button.dataset.x1SavedAnalysis);
  });
  ...
  document.querySelector('#x1BankSidebarWorkspace [data-x1-workspace-tab="home"]')?.addEventListener('click',startNewConversation);
  knowledgeManagerButton?.addEventListener('click',openKnowledgeManager);adminKnowledgeManagerButton?.addEventListener('click',openKnowledgeManager);document.getElementById('x1KbClose')?.addEventListener('click',closeKnowledgeManager);knowledgeModal?.addEventListener('click',event=>{if(event.target===knowledgeModal)closeKnowledgeManager()});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&knowledgeModal&&!knowledgeModal.hidden)closeKnowledgeManager()});
...
(()=>{
  ...
  function setModule(name){
    ...
    sideLabel.textContent=isInterfund?'Interfund control workspace':'Financial Intelligence System';
  ...
  document.getElementById('ifAdminRows').innerHTML='';document.getElementById('ifProjectRows').innerHTML='';document.getElementById('ifAdminClosing').value='0';document.getElementById('ifProjectClosing').value='0';addRow('admin');addRow('project');const initialStatus=document.getElementById('ifPrepareStatus');if(initialStatus){initialStatus.textContent='Add Administration and Project transactions or import CSV files.';initialStatus.className='if-inline-status'}
})();
...
(()=>{
  ...
  function saveWorkflow(){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(serialise()));showToast('Workflow saved in this browser.');}catch(error){showToast('The workflow is too large for browser storage. Export the workflow file instead.',true);}}
...
`;
  ...
  const PythonEngine={
    async init(){
      ...
      runtimeState.python.status='loading';runtimeState.python.error='';updateRuntimeUi('Preparing the in-browser analytics engine…','busy');
  ...
  function loadInterfundSample(){return false}
  ...
  const operations=[
    ...
    operation('home','Open New Chat',[/\b(?:open|show|go to|return to)\s+(?:the\s+)?(?:financial\s+)?assistant\b/,/\bopen\s+home\b/],()=>openWorkspace('home')),
...
(()=>{
  ...
  knowledgeUpload?.addEventListener('change',()=>{
    ...
    if(count)setVoiceMessage(`${count} document${count===1?'':'s'} selected for capture. Add your instruction after processing finishes.`, 'success', 3200);
  });
...
(()=>{
  ...
  function courseIsFree(course){return course?.is_free!==false||Number(course?.price||0)<=0}
  function formatCoursePrice(course){return courseIsFree(course)?'Free':`${String(course?.currency||'ZMW')} ${Number(course?.price||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}
  function activeEnrollment(courseId){
    return myEnrollments.find(item=>item.course_id===courseId&&['active','completed'].includes(item.status)&&new Date(item.access_expires_at).getTime()>Date.now())||null
  }
  function latestPayment(courseId){
    return myPayments.filter(item=>item.course_id===courseId).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0]||null
  }
  ...
  function privateCoursePath(value){const text=String(value||'');return text.startsWith('course-media:')?text.slice(13):''}
  async function signedCourseMedia(value,expires=3600){
    const path=privateCoursePath(value);if(!path)return safeUrl(value);
    const{data,error}=await client.storage.from('findat-course-media').createSignedUrl(path,expires);
    if(error)return'';return data?.signedUrl||''
  }
  async function hydrateLessonMedia(lessons){
    return Promise.all((lessons||[]).map(async lesson=>{
      const documents=[];
      for(const doc of Array.isArray(lesson.documents)?lesson.documents:[]){
        documents.push({...doc,url:await signedCourseMedia(doc.url)})
      }
      return {...lesson,_video_url:await signedCourseMedia(lesson.video_url),_thumbnail_url:await signedCourseMedia(lesson.thumbnail_url),_documents:documents}
    }))
  }
  async function loadLearningState(){
    const session=await currentSession();
    if(!session?.user){myEnrollments=[];myPayments=[];return}
    const [enrollmentsResult,paymentsResult]=await Promise.all([
      client.from('findat_course_enrollments').select('*').order('updated_at',{ascending:false}),
      client.from('findat_course_payments').select('*').order('created_at',{ascending:false})
    ]);
    myEnrollments=enrollmentsResult.error?[]:enrollmentsResult.data||[];
    myPayments=paymentsResult.error?[]:paymentsResult.data||[]
  }
  async function requestFreeEnrollment(course){
    if(!course||!courseIsFree(course)||!(await currentSession())?.user)return null;
    const{data,error}=await client.rpc('findat_request_course_access',{p_course_id:course.id,p_payment_method:'',p_payment_reference:''});
    if(error)throw error;await loadLearningState();return data
  }
  async function recordCourseProgress(courseId,progress,completed=false){
    const session=await currentSession();if(!session?.user||!courseId)return null;
    const{data,error}=await client.rpc('findat_record_course_progress',{p_course_id:courseId,p_progress:progress,p_completed:completed});
    if(error){console.warn('Course progress was not recorded',error);return null}
    await loadLearningState();return Array.isArray(data)?data[0]:data
  }
  function renderMyLearning(){
    const courseHost=$('#fdMyCourseList'),paymentHost=$('#fdMyPaymentList');
    if(courseHost){
      courseHost.innerHTML=myEnrollments.length?myEnrollments.map(enrollment=>{
        const course=catalogueCourses.find(item=>item.id===enrollment.course_id);
        const certificate=enrollment.certificate_number?`<span class="fd-learning-chip is-complete"><i class="fas fa-certificate"></i>${escapeHTML(enrollment.certificate_number)}</span>`:'';
        return `<article class="fd-learning-record"><div><h5>${escapeHTML(course?.title||'Course')}</h5><p>${Math.round(Number(enrollment.progress||0))}% complete · Access until ${escapeHTML(formatDateTime(enrollment.access_expires_at))}</p></div><div class="fd-learning-record-meta"><span class="fd-learning-chip ${enrollment.status==='completed'?'is-complete':''}">${escapeHTML(enrollment.status)}</span>${certificate}</div></article>`
      }).join(''):'<div class="fd-course-manager-empty">No course enrolments are recorded yet.</div>'
    }
    if(paymentHost){
      paymentHost.innerHTML=myPayments.length?myPayments.map(payment=>{
        const course=catalogueCourses.find(item=>item.id===payment.course_id);
        return `<article class="fd-learning-record"><div><h5>${escapeHTML(course?.title||'Course payment')}</h5><p>${escapeHTML(payment.currency)} ${Number(payment.amount||0).toFixed(2)} · ${escapeHTML(payment.payment_method||'Payment')} · ${escapeHTML(payment.payment_reference||'No reference')} · ${escapeHTML(formatDateTime(payment.created_at))}</p></div><div class="fd-learning-record-meta"><span class="fd-learning-chip is-${escapeHTML(payment.status)}">${escapeHTML(payment.status)}</span></div></article>`
      }).join(''):'<div class="fd-course-manager-empty">No payment history is available.</div>'
    }
  }
  ...
  function showLegacyCourse(){
    ...
    if(protectedPoster){
      if(video)video.poster=protectedPoster;
      if(poster){poster.classList.add('has-thumbnail');poster.style.backgroundImage=`url("${protectedPoster}")`}
    }
  ...
  function renderCourseCards(){
    ...
    courseGrid.innerHTML=catalogueCourses.map(course=>{
      ...
      const cover=builtIn?protectedPoster:safeUrl(course.cover_url);
      const lessonCount=course.lessons?.length||0;
      const enrollment=activeEnrollment(course.id),payment=latestPayment(course.id),free=courseIsFree(course);
      const accessText=free?'Available to everyone':enrollment?`Access until ${new Date(enrollment.access_expires_at).toLocaleDateString()}`:payment?.status==='pending'?'Payment awaiting verification':'Payment required';
      const accessClass=enrollment?'is-active':payment?.status==='pending'?'is-pending':'';
      const buttonText=free||enrollment?'Open course':payment?.status==='pending'?'Payment pending':'Request access';
      const buttonClass=payment?.status==='pending'?'is-pending':(!free&&!enrollment?'is-signin':'');
      return `<article class="findat-course-card ${!free&&!enrollment?'is-locked':''}"><div class="findat-course-card-cover" ${cover?`style="background-image:url('${escapeHTML(cover)}')"`:''}><span><i class="${free||enrollment?'fas fa-play-circle':'fas fa-lock'}"></i>${lessonCount} lesson${lessonCount===1?'':'s'}</span></div><div class="findat-course-card-body"><h3>${escapeHTML(course.title)}</h3><p>${escapeHTML(course.short_description||course.course_content||'FINDAT Academy course')}</p><div class="findat-course-card-meta"><span class="findat-course-rating">${stars(course.rating)}</span><span>${escapeHTML(course.instructor_name||'FINDAT Academy')}</span></div><div class="findat-course-card-price"><span class="findat-course-price-badge ${free?'is-free':''}">${escapeHTML(formatCoursePrice(course))}</span><span class="findat-course-access-badge ${accessClass}">${escapeHTML(accessText)}</span></div><button class="findat-course-card-button ${buttonClass}" data-course-open="${course.id}" type="button" ${payment?.status==='pending'?'disabled':''}>${escapeHTML(buttonText)}</button></div></article>`
    }).join('')
  }
  async function loadCatalogue(){
    ...
    if(coursesResult.error){
      catalogueCourses=[{id:'legacy-data-analytics',slug:'data-analytics-foundations',title:'Data Analytics Foundations',short_description:'Learn how data analytics supports evidence-based decisions across professional domains.',instructor_name:'FINDAT Academy',instructor_qualifications:'Data Analytics | Research | Professional Practice',rating:5,is_builtin:true,is_free:true,price:0,currency:'ZMW',access_days:30,lessons:[{id:'legacy-lesson',title:'How Data Analytics Work per Domain'}]}];
      renderCourseCards();renderMyLearning();return
    }
    const lessonsResult=await client.from('findat_course_lessons').select('*').eq('is_published',true).order('position',{ascending:true});
    const hydratedLessons=await hydrateLessonMedia(lessonsResult.error?[]:lessonsResult.data||[]);
    catalogueCourses=(coursesResult.data||[]).map(course=>({...course,lessons:hydratedLessons.filter(lesson=>lesson.course_id===course.id)}));
    renderCourseCards();renderMyLearning()
  }
  function renderDynamicLessonList(lessons){
    const host=$('#findatDynamicLessonList');if(!host)return;
    host.innerHTML=lessons.length?lessons.map(lesson=>{const thumb=safeUrl(lesson._thumbnail_url||lesson.thumbnail_url);return `<button class="findat-dynamic-lesson-button" data-dynamic-lesson="${lesson.id}" type="button"><span class="findat-dynamic-lesson-thumb" ${thumb?`style="background-image:url('${escapeHTML(thumb)}')"`:''}>${thumb?'':'<i class="fas fa-play"></i>'}</span><span><strong>${escapeHTML(lesson.title)}</strong><small>Lesson ${Number(lesson.position)||1}${lesson.duration_seconds?` · ${Math.ceil(lesson.duration_seconds/60)} min`:''}</small></span></button>`}).join(''):'<div class="fd-course-manager-empty">This course has no published lessons yet.</div>'
  }
  function openDynamicLesson(lesson){
    ...
    const video=$('#findatDynamicVideo'),empty=$('#findatDynamicVideoEmpty'),videoUrl=safeUrl(lesson?._video_url||lesson?.video_url),thumb=safeUrl(lesson?._thumbnail_url||lesson?.thumbnail_url);
    ...
    const docs=Array.isArray(lesson?._documents)?lesson._documents:(Array.isArray(lesson?.documents)?lesson.documents:[]);
  ...
  function renderDynamicQuiz(quiz){
    ...
    $('#findatDynamicQuizForm')?.addEventListener('submit',async event=>{event.preventDefault();let score=0;quiz.forEach((question,index)=>{const selected=event.currentTarget.querySelector(`input[name="q${index}"]:checked`);if(selected&&Number(selected.value)===Number(question.correct))score++});const percent=Math.round(score/quiz.length*100),passed=percent>=80;$('#findatDynamicQuizResult').textContent=`Score: ${score} / ${quiz.length} (${percent}%).${passed?' Course completion recorded.':''}`;if(selectedCourse?.id)await recordCourseProgress(selectedCourse.id,passed?100:Math.max(10,percent),passed);renderCourseCards();renderMyLearning()})
  }

  courseGrid?.addEventListener('click',async event=>{
    const button=event.target.closest('[data-course-open]');if(!button)return;
    const course=catalogueCourses.find(item=>item.id===button.dataset.courseOpen);if(!course)return;
    const session=await currentSession(),enrollment=activeEnrollment(course.id),payment=latestPayment(course.id);
    if(courseIsFree(course)){
      try{if(session?.user)await requestFreeEnrollment(course)}catch(error){console.warn(error)}
      if(course.slug==='data-analytics-foundations'||course.is_builtin)showLegacyCourse();else showDynamicCourse(course);
      return
    }
    if(!session?.user){window.toggleModal?.('loginPopup',true);return}
    if(enrollment){if(course.slug==='data-analytics-foundations'||course.is_builtin)showLegacyCourse();else showDynamicCourse(course);return}
    if(payment?.status==='pending'){alert('This payment request is waiting for Administrator verification.');return}
    selectedPaymentCourse=course;
    const form=$('#fdCoursePaymentForm');if(form){form.reset();form.elements.courseId.value=course.id}
    $('#fdCoursePaymentTitle').textContent=`Access ${course.title}`;
    $('#fdCoursePaymentSummary').textContent=`Submit your payment reference for ${course.title}.`;
    $('#fdCoursePaymentAmount').textContent=`${formatCoursePrice(course)} · ${Math.max(30,Number(course.access_days)||30)} days access`;
    $('#fdCoursePaymentStatus').textContent='';
    window.toggleModal?.('fdCoursePaymentModal',true)
  });
  ...
  $('#fdCoursePaymentClose')?.addEventListener('click',()=>window.toggleModal?.('fdCoursePaymentModal',false));
  $('#fdMyLearningClose')?.addEventListener('click',()=>window.toggleModal?.('fdMyLearningModal',false));
  $('#fdMyLearningButton')?.addEventListener('click',async()=>{
    const session=await currentSession();
    if(!session?.user){window.toggleModal?.('loginPopup',true);return}
    const status=$('#fdMyLearningStatus');if(status){status.textContent='Loading your learning history…';status.className='auth-status'}
    try{await loadLearningState();renderMyLearning();if(status)status.textContent='';window.toggleModal?.('fdMyLearningModal',true)}
    catch(error){if(status){status.textContent=error.message||'Your learning history could not be loaded.';status.className='auth-status error'}window.toggleModal?.('fdMyLearningModal',true)}
  });
  $('#fdCoursePaymentForm')?.addEventListener('submit',async event=>{
    event.preventDefault();const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),status=$('#fdCoursePaymentStatus');
    if(button)button.disabled=true;if(status){status.textContent='Submitting payment reference…';status.className='auth-status'}
    try{
      const values=Object.fromEntries(new FormData(form).entries()),courseId=String(values.courseId||selectedPaymentCourse?.id||'');
      const{data,error}=await client.rpc('findat_request_course_access',{p_course_id:courseId,p_payment_method:String(values.paymentMethod||'').trim(),p_payment_reference:String(values.paymentReference||'').trim()});
      if(error)throw error;form.reset();if(status){status.textContent='Payment reference submitted. Access will open after Administrator verification.';status.className='auth-status success'}
      await loadCatalogue();setTimeout(()=>window.toggleModal?.('fdCoursePaymentModal',false),500)
    }catch(error){if(status){status.textContent=error.message||'The payment request could not be submitted.';status.className='auth-status error'}}
    finally{if(button)button.disabled=false}
  });
  $('#markRecordingComplete')?.addEventListener('click',async()=>{
    const course=catalogueCourses.find(item=>item.slug==='data-analytics-foundations'||item.is_builtin);
    if(course?.id&&!String(course.id).startsWith('legacy-')){await recordCourseProgress(course.id,100,true);renderCourseCards();renderMyLearning()}
  });
  ...
  function resetCourseForm(){
    const form=$('#fdCourseForm');if(!form)return;form.reset();form.elements.id.value='';form.elements.rating.value='5';form.elements.status.value='draft';form.elements.accessType.value='free';form.elements.price.value='0';form.elements.currency.value='ZMW';form.elements.accessDays.value='30';managerCourseId='';courseCoverFile=null;$('#fdCourseCoverName').textContent='No picture selected';renderManagerCourseList();resetLessonForm();renderManagerLessonList();syncCoursePriceFields()
  }
  ...
  async function loadManagerData(selectCourseId=''){
    ...
    const [coursesResult,lessonsResult,paymentsResult,profilesResult]=await Promise.all([
      client.from('findat_courses').select('*').order('updated_at',{ascending:false}),
      client.from('findat_course_lessons').select('*').order('position',{ascending:true}),
      client.from('findat_course_payments').select('*').order('created_at',{ascending:false}),
      client.from('findat_profiles').select('id,first_name,last_name,username,email').order('first_name',{ascending:true})
    ]);
    if(coursesResult.error)throw coursesResult.error;if(lessonsResult.error)throw lessonsResult.error;
    managerCourses=coursesResult.data||[];managerLessons=lessonsResult.data||[];managerPayments=paymentsResult.error?[]:paymentsResult.data||[];managerProfiles=profilesResult.error?[]:profilesResult.data||[];
    $('#fdCourseNavCount').textContent=String(managerCourses.length);renderManagerCourseList();renderManagerPayments();
  ...
  function renderManagerCourseList(){const host=$('#fdCourseManagerList');if(!host)return;host.innerHTML=managerCourses.length?managerCourses.map(course=>`<button class="fd-course-manager-item ${course.id===managerCourseId?'is-active':''}" data-manager-course="${course.id}" type="button"><strong>${escapeHTML(course.title)}</strong><small>${escapeHTML(course.status)} · ${Number(course.rating||0).toFixed(1)} stars · ${escapeHTML(formatCoursePrice(course))} · ${Math.max(30,Number(course.access_days)||30)} days</small></button>`).join(''):'<div class="fd-course-manager-empty">No courses yet. Click New to create one.</div>'}
  function selectManagerCourse(id){const course=managerCourses.find(item=>item.id===id);if(!course)return;managerCourseId=id;const form=$('#fdCourseForm');form.elements.id.value=course.id;form.elements.title.value=course.title||'';form.elements.slug.value=course.slug||'';form.elements.shortDescription.value=course.short_description||'';form.elements.courseContent.value=course.course_content||'';form.elements.instructorName.value=course.instructor_name||'';form.elements.instructorQualifications.value=course.instructor_qualifications||'';form.elements.rating.value=String(course.rating||5);form.elements.status.value=course.status||'draft';form.elements.accessType.value=course.is_free===false?'paid':'free';form.elements.price.value=String(Number(course.price||0));form.elements.currency.value=course.currency||'ZMW';form.elements.accessDays.value=String(Math.max(30,Number(course.access_days)||30));syncCoursePriceFields();$('#fdCourseCoverName').textContent=course.cover_url?'Existing course picture saved':'No picture selected';courseCoverFile=null;renderManagerCourseList();renderManagerLessonList();const first=managerLessons.filter(item=>item.course_id===id)[0];if(first)selectManagerLesson(first.id);else resetLessonForm()}
  ...
  function renderManagerPayments(){
    const host=$('#fdCoursePaymentAdminList');if(!host)return;
    const rows=managerPayments.filter(item=>item.status==='pending');
    host.innerHTML=rows.length?rows.map(payment=>{
      const course=managerCourses.find(item=>item.id===payment.course_id),person=managerProfiles.find(item=>item.id===payment.user_id);
      const name=[person?.first_name,person?.last_name].filter(Boolean).join(' ')||person?.username||person?.email||'FINDAT user';
      return `<article class="fd-course-payment-request"><div><strong>${escapeHTML(name)} — ${escapeHTML(course?.title||'Course')}</strong><small>${escapeHTML(payment.currency)} ${Number(payment.amount||0).toFixed(2)} · ${escapeHTML(payment.payment_method||'Payment')} · Reference: ${escapeHTML(payment.payment_reference||'Not supplied')} · ${escapeHTML(formatDateTime(payment.created_at))}</small></div><div class="fd-course-payment-request-actions"><button class="fd-mini-btn is-success" data-course-payment-approve="${payment.id}" type="button"><i class="fas fa-check"></i> Approve</button><button class="fd-mini-btn is-danger" data-course-payment-reject="${payment.id}" type="button"><i class="fas fa-times"></i> Reject</button></div></article>`
    }).join(''):'<div class="fd-course-manager-empty">No pending payment requests.</div>'
  }
  function syncCoursePriceFields(){
    const form=$('#fdCourseForm');if(!form)return;const free=form.elements.accessType.value==='free',wrap=$('.fd-course-price-fields',form);
    form.elements.price.disabled=free;form.elements.currency.disabled=free;if(free)form.elements.price.value='0';wrap?.classList.toggle('is-disabled',free)
  }
  async function uploadCourseFile(file,courseId,folder){
    if(!file)return'';
    if(folder==='course-cover'){
      const path=`findat-v1/course-media/${courseId}/${folder}/${crypto.randomUUID()}-${fileNameSafe(file.name)}`;
      const{error}=await client.storage.from('findat-documents').upload(path,file,{upsert:false,contentType:file.type||undefined,cacheControl:'3600'});if(error)throw error;
      const{data}=client.storage.from('findat-documents').getPublicUrl(path);return data?.publicUrl||''
    }
    const path=`${courseId}/${folder}/${crypto.randomUUID()}-${fileNameSafe(file.name)}`;
    const{error}=await client.storage.from('findat-course-media').upload(path,file,{upsert:false,contentType:file.type||undefined,cacheControl:'3600'});if(error)throw error;
  ...
  $('#fdRefreshCoursePayments')?.addEventListener('click',()=>loadManagerData(managerCourseId).catch(error=>showCourseStatus(error.message||'Payment requests could not be refreshed.','error')));
  $('#fdCoursePaymentAdminList')?.addEventListener('click',async event=>{
    const approve=event.target.closest('[data-course-payment-approve]'),reject=event.target.closest('[data-course-payment-reject]');
    const button=approve||reject;if(!button)return;
    const paymentId=approve?approve.dataset.coursePaymentApprove:reject.dataset.coursePaymentReject;
    const note=reject?prompt('Optional reason for rejecting this payment request:','')||'':'';
    if(!confirm(approve?'Approve this payment and grant timed course access?':'Reject this payment request?'))return;
    button.disabled=true;
    try{const{error}=await client.rpc('findat_admin_verify_course_payment',{p_payment_id:paymentId,p_approve:Boolean(approve),p_review_note:note});if(error)throw error;await loadManagerData(managerCourseId);showCourseStatus(approve?'Payment approved and course access granted.':'Payment request rejected.','success')}
    catch(error){showCourseStatus(error.message||'The payment request could not be updated.','error')}
  ...
  $('#fdCourseForm select[name="accessType"]')?.addEventListener('change',syncCoursePriceFields);
  ...
  $('#fdCourseForm')?.addEventListener('submit',async event=>{
    ...
    try{const profile=await requireAdmin(),values=Object.fromEntries(new FormData(form).entries()),id=String(values.id||crypto.randomUUID()),existing=managerCourses.find(item=>item.id===id),coverUrl=courseCoverFile?await uploadCourseFile(courseCoverFile,id,'course-cover'):(existing?.cover_url||'');const row={id,slug:slugify(values.slug||values.title),title:String(values.title||'').trim(),short_description:String(values.shortDescription||'').trim(),course_content:String(values.courseContent||'').trim(),cover_url:coverUrl,instructor_name:String(values.instructorName||'').trim(),instructor_qualifications:String(values.instructorQualifications||'').trim(),rating:Number(values.rating)||5,status:String(values.status||'draft'),is_free:String(values.accessType||'free')==='free',price:String(values.accessType||'free')==='free'?0:Math.max(0,Number(values.price)||0),currency:['ZMW','USD','GBP'].includes(String(values.currency))?String(values.currency):'ZMW',access_days:Math.max(30,Number(values.accessDays)||30),created_by:existing?.created_by||profile.id};if(!row.title)throw new Error('Course title is required.');const{data,error}=await client.from('findat_courses').upsert(row).select().single();if(error)throw error;managerCourseId=data.id;courseCoverFile=null;await loadManagerData(data.id);showCourseStatus('Course saved. Published courses appear as cards in Recordings.','success');await loadCatalogue();setTimeout(()=>window.toggleModal?.('fdCourseManagerModal',false),450)}
  ...
  if(typeof client.channel==='function')client.channel('findat-course-catalogue')
    .on('postgres_changes',{event:'*',schema:'public',table:'findat_courses'},()=>loadCatalogue().catch(console.warn))
    .on('postgres_changes',{event:'*',schema:'public',table:'findat_course_lessons'},()=>loadCatalogue().catch(console.warn))
    .on('postgres_changes',{event:'*',schema:'public',table:'findat_course_payments'},()=>{loadCatalogue().catch(console.warn);if($('#fdCourseManagerModal')?.classList.contains('is-open'))loadManagerData(managerCourseId).catch(console.warn)})
    .on('postgres_changes',{event:'*',schema:'public',table:'findat_course_enrollments'},()=>loadCatalogue().catch(console.warn))
    .subscribe();
