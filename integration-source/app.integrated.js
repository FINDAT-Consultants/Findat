...
const mainNav = document.getElementById('mainNav');
    ...
    function syncRecordingLearningState(){
      ...
      if(recordingPlaybackStatus){
        recordingPlaybackStatus.textContent=complete?'Lesson completed · replay whenever needed':watched>0?`${watched}% watched · continue from ${formatRecordingClock(position)}`:'Not started';
      }
...
(()=>{
      ...
      async function uploadProfilePhoto(file){
        ...
        try{
          ...
          try{
            ...
            if(!publicData?.publicUrl)throw new Error('The profile picture service did not return an address.');
          ...
          setStatus('fdProfilePhotoStatus',savedInStorage?'Profile picture saved and updated across FINDAT.':`Profile picture saved to your secure profile${storageError?.message?`: ${storageError.message}`:''}.`,'success');
        }catch(error){
      ...
      function showView(view){const delegated=activeAccount?.role==='consultant'&&workspaceCache.articles.some(article=>article.status==='Pending approval'&&article.publisherId===activeAccount.id);if(view==='accounts'&&!isAdmin())view='writing';if(view==='review'&&!isAdmin()&&!delegated)view='writing';$$('[data-view-panel]').forEach(panel=>panel.classList.toggle('is-active',panel.dataset.viewPanel===view));$$('[data-dashboard-view]').forEach(button=>button.classList.toggle('is-active',button.dataset.dashboardView===view));const titles={writing:'Writing Desk',review:'Review & Approval',publications:'Published Articles',accounts:'User Accounts',learning:'My Learning','x1-training':'x1 | ProATR Training Lab'};$('#fdTopbarTitle').textContent=titles[view]||'Writing Desk';$('#fdDashboardMain')?.scrollTo({top:0,behavior:'smooth'});if(view==='accounts'&&isAdmin())refreshAdminAccounts();if(view==='learning')window.FINDAT_REFRESH_LEARNING?.();if(view==='x1-training')window.FINDAT_REFRESH_X1?.();closeSidebar()}
      ...
      function displayNotificationRows(){
        ...
        return fallback.map(item=>({...item,kind:item.direction==='incoming'?'collaboration_request':'collaboration_response',is_read:true,payload:{}}));
      }
      function renderCollaborationInbox(){
        ...
        list.innerHTML=rows.length?rows.map(request=>{
          ...
          const isCloud=request.kind==='cloud_access'&&request.payload?.cloud_password;
          const person={displayName:request.other_name||'FINDAT',avatarUrl:request.other_avatar_url||''};
          ...
          const viewLabel=isCloud?'View Cloud password':request.kind==='article_comment'?'Open comments':request.article_id?'Open article':'';
          const openButton=isCloud
            ? `<button class="fd-notification-open" data-cloud-notification="${escapeHTML(request.notification_id||'')}" type="button"><i class="fas fa-key"></i> ${viewLabel}</button>`
            : viewLabel?`<button class="fd-notification-open" data-notification-open="${escapeHTML(request.article_id||'')}" data-notification-open-kind="${escapeHTML(request.kind||'')}" type="button"><i class="fas ${request.kind==='article_comment'?'fa-comments':'fa-arrow-right'}"></i> ${viewLabel}</button>`:'';
          return `<article class="fd-collaboration-request is-${escapeHTML(request.status||'none')} ${request.is_read===false?'is-unread':''} ${(request.article_id||isCloud)?'is-clickable':''}" data-notification-article="${escapeHTML(request.article_id||'')}" data-notification-kind="${escapeHTML(request.kind||'')}">${avatarMarkup(person,'fd-collaboration-avatar')}<div><p>${sentence}</p><div class="fd-collaboration-meta"><span class="fd-collaboration-status is-${escapeHTML(request.status||'none')}">${escapeHTML(collaborationStatusLabel(request.status))}</span><time>${escapeHTML(formatDate(request.updated_at||request.created_at))}</time>${openButton}</div>${actions}</div></article>`
        }).join(''):'<div class="fd-empty-request"><i class="fas fa-bell"></i>No notifications or collaboration history is available yet.</div>'
      }
      function openCloudPasswordNotification(notificationId){
        const row=displayNotificationRows().find(item=>item.notification_id===notificationId&&item.kind==='cloud_access');
        if(!row?.payload?.cloud_password){setStatus('fdCollaborationInboxStatus','This Cloud password is no longer available. Ask an Administrator to generate a new one.','error');return}
        const username=$('#fdCloudPasswordUsername'),password=$('#fdCloudPasswordValue'),expiry=$('#fdCloudPasswordExpiry');
        if(username)username.value=String(row.payload.username||activeAccount?.username||'');
        if(password)password.value=String(row.payload.cloud_password||'');
        if(expiry)expiry.textContent=row.payload.expires_at?`Expires ${formatTime(row.payload.expires_at)}`:'Monthly access';
        setStatus('fdCloudPasswordStatus','Keep this password private. It is intended only for your FINDAT Cloud sign-in.','');
        window.toggleModal?.('fdCollaborationInboxModal',false);window.toggleModal?.('fdCloudPasswordModal',true)
      }
      ...
      async function saveArticle(status='Draft'){
        ...
        Object.assign(article,data);setStatus('fdArticleStatus',status==='Published'?'Publishing securely…':'Saving securely…','');
        ...
        const successMessage=collaborativeEdit?'Collaborative changes saved securely.':status==='Published'?'Article published successfully.':status==='Pending approval'?'Article sent to the administrator for approval.':'Draft saved securely.';clearEditor(false);setStatus('fdArticleStatus',successMessage,'success');await refreshCollaborationState();renderAll()
      }
      ...
      function accountCloudBadge(user){
        if(user.role==='admin')return '<span class="fd-account-status is-confirmed"><i class="fas fa-cloud"></i> Uses FINDAT password</span>';
        if(!user.cloudAccessActive)return '<span class="fd-account-status is-unconfirmed"><i class="fas fa-cloud-slash"></i> No Cloud password</span>';
        const expired=user.cloudAccessExpiresAt&&new Date(user.cloudAccessExpiresAt).getTime()<=Date.now();
        return `<span class="fd-account-status ${expired?'is-suspended':'is-confirmed'}"><i class="fas fa-cloud"></i> ${expired?'Cloud expired':`Cloud until ${escapeHTML(formatDate(user.cloudAccessExpiresAt))}`}</span>`
      }
      function renderAdminRegistry(){
        const body=$('#fdAccountTableBody');if(!body)return;
        const query=normal($('#fdAccountRegistrySearch')?.value),role=$('#fdAccountRegistryRole')?.value||'all',status=$('#fdAccountRegistryStatus')?.value||'all',sort=$('#fdAccountRegistrySort')?.value||'name-asc';
        let users=adminRegistryUsers.filter(user=>{
          const haystack=normal(`${accountName(user)} ${user.username} ${user.email} ${user.organisation} ${user.placeOfWork} ${user.phone}`);
          const roleMatch=role==='all'||user.role===role;
          const statusMatch=status==='all'||(status==='active'&&user.active)||(status==='suspended'&&!user.active)||(status==='cloud'&&user.cloudAccessActive)||(status==='no-cloud'&&!user.cloudAccessActive&&user.role!=='admin');
          return(!query||haystack.includes(query))&&roleMatch&&statusMatch
        });
        users.sort((a,b)=>{
          if(sort==='name-desc')return accountName(b).localeCompare(accountName(a));
          if(sort==='newest')return new Date(b.createdAt||0)-new Date(a.createdAt||0);
          if(sort==='role')return a.role.localeCompare(b.role)||accountName(a).localeCompare(accountName(b));
          return accountName(a).localeCompare(accountName(b))
        });
        body.innerHTML=users.length?users.map(user=>{
          const cloudButtons=user.role==='admin'?'':`<button class="fd-mini-btn is-cloud" data-account-cloud-generate="${user.id}" data-account-name="${escapeHTML(accountName(user))}" type="button"><i class="fas fa-key"></i> ${user.cloudAccessActive?'Renew':'Generate'} Cloud</button>${user.cloudAccessActive?`<button class="fd-mini-btn is-danger" data-account-cloud-suspend="${user.id}" data-account-name="${escapeHTML(accountName(user))}" type="button"><i class="fas fa-cloud-slash"></i> Suspend Cloud</button>`:''}`;
          const promote=user.role==='consultant'?`<button class="fd-mini-btn is-admin-promote" data-account-promote="${user.id}" data-account-name="${escapeHTML(accountName(user))}" type="button"><i class="fas fa-user-shield"></i> Make Administrator</button>`:'';
          const controls=user.role==='admin'
            ? '<button class="fd-mini-btn is-locked" disabled type="button"><i class="fas fa-lock"></i> Protected</button>'
            : `${user.emailConfirmed?'':`<button class="fd-mini-btn is-success" data-account-confirm="${user.id}" type="button"><i class="fas fa-envelope-circle-check"></i> Confirm email</button>`}<button class="fd-mini-btn" data-account-password="${user.id}" type="button"><i class="fas fa-key"></i> Password</button>${promote}${cloudButtons}<button class="fd-mini-btn ${user.active?'is-danger':'is-success'}" data-account-active="${user.id}" data-next-active="${user.active?'false':'true'}" type="button">${user.active?'Suspend':'Activate'}</button>`;
          return `<tr class="${user.role==='admin'?'is-admin-row':''}"><td><strong>${escapeHTML(accountName(user))}</strong><small>@${escapeHTML(user.username)} · ${escapeHTML(user.email)}</small>${user.organisation||user.placeOfWork?`<small>${escapeHTML(user.placeOfWork||user.organisation)}</small>`:''}</td><td>${adminRoleBadge(user.role)}</td><td><div class="fd-account-status-stack">${accountStateBadges(user)}${accountCloudBadge(user)}</div></td><td><div class="fd-account-controls">${controls}</div></td></tr>`
        }).join(''):'<tr><td colspan="4">No accounts match the selected filters.</td></tr>'
      }
      async function refreshAdminAccounts(){
        if(!isAdmin())return;const body=$('#fdAccountTableBody');if(body)body.innerHTML='<tr><td colspan="4">Loading accounts…</td></tr>';
        try{const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'list'},true),users=(response.users||[]).map(profileFromRow);adminRegistryUsers=users;accountCache=[...new Map([activeAccount,...users].filter(Boolean).map(item=>[item.id,item])).values()];if($('#fdAccountNavCount'))$('#fdAccountNavCount').textContent=String(users.length);renderAdminRegistry()}
        catch(error){if(body)body.innerHTML=`<tr><td colspan="4">${escapeHTML(error.message||'Accounts could not be loaded.')}</td></tr>`}
      }
      $('#fdAdminCreateAccountForm')?.addEventListener('submit',async event=>{event.preventDefault();if(!isAdmin())return;const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());form.reset();setStatus('fdAdminAccountStatus','Creating account securely…','');try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'create',email:String(data.email||'').trim(),username:String(data.username||'').trim(),password:String(data.password||''),role:String(data.role||''),firstName:String(data.firstName||'').trim(),lastName:String(data.lastName||'').trim(),phone:String(data.phone||'').trim(),organisation:String(data.organisation||'').trim()},true);setStatus('fdAdminAccountStatus','Account created, activated and email-confirmed.','success');await refreshAdminAccounts();await refreshDirectory();populateCollaborators();form.hidden=true;const reopen=$('#fdCreateAnotherAccount');if(reopen)reopen.hidden=false}catch(error){setStatus('fdAdminAccountStatus',error.message||'The account could not be created.','error')}});
      $('#fdRefreshAccounts')?.addEventListener('click',refreshAdminAccounts);
      $('#fdAccountTableBody')?.addEventListener('click',async event=>{
        const confirmButton=event.target.closest('[data-account-confirm]'),passwordButton=event.target.closest('[data-account-password]'),promoteButton=event.target.closest('[data-account-promote]'),activeButton=event.target.closest('[data-account-active]'),cloudGenerate=event.target.closest('[data-account-cloud-generate]'),cloudSuspend=event.target.closest('[data-account-cloud-suspend]');
        if(confirmButton){if(!confirm('Confirm this Client or Consultant email so the account can log in?'))return;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'confirm_email',userId:confirmButton.dataset.accountConfirm},true);alert('Email confirmed. The user can now log in with the existing password.');await refreshAdminAccounts()}catch(error){alert(error.message||'The email could not be confirmed.')}return}
        if(passwordButton){const password=prompt('Enter the new password (at least 8 characters):');if(!password)return;if(password.length<8){alert('Password must contain at least 8 characters.');return}try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'change_password',userId:passwordButton.dataset.accountPassword,password},true);alert('Password changed and email confirmed. The user can now log in.');await refreshAdminAccounts()}catch(error){alert(error.message||'The password could not be changed.')}return}
        if(promoteButton){const user=adminRegistryUsers.find(item=>item.id===promoteButton.dataset.accountPromote),name=promoteButton.dataset.accountName||'this Consultant';if(user?.role!=='consultant'){alert('Only Consultant accounts can be promoted to Administrator.');return}if(!confirm(`Promote ${name} to Administrator? This grants full FINDAT privileges and the account will become protected.`))return;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'promote_admin',userId:promoteButton.dataset.accountPromote},true);alert(`${name} is now an Administrator.`);await refreshAdminAccounts();await refreshDirectory()}catch(error){alert(error.message||'The account could not be promoted.')}return}
        if(cloudGenerate){const name=cloudGenerate.dataset.accountName||'this user';if(!confirm(`Generate a new monthly FINDAT Cloud password for ${name}? Any previous Cloud password will stop working.`))return;cloudGenerate.disabled=true;try{const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'generate_cloud_password',userId:cloudGenerate.dataset.accountCloudGenerate},true);$('#fdCloudPasswordUsername').value=response.username||'';$('#fdCloudPasswordValue').value=response.password||'';$('#fdCloudPasswordExpiry').textContent=response.expiresAt?`Expires ${formatTime(response.expiresAt)}`:'';setStatus('fdCloudPasswordStatus',`The password was sent to ${name} through the FINDAT notification bell.`,'success');window.toggleModal?.('fdCloudPasswordModal',true);await refreshAdminAccounts();await refreshCollaborationState()}catch(error){alert(error.message||'The Cloud password could not be generated.')}finally{cloudGenerate.disabled=false}return}
        if(cloudSuspend){const name=cloudSuspend.dataset.accountName||'this user';if(!confirm(`Suspend FINDAT Cloud access for ${name}?`))return;cloudSuspend.disabled=true;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'suspend_cloud_password',userId:cloudSuspend.dataset.accountCloudSuspend},true);await refreshAdminAccounts();alert(`${name}'s Cloud password has been suspended.`)}catch(error){alert(error.message||'Cloud access could not be suspended.')}finally{cloudSuspend.disabled=false}return}
      ...
      ['#fdAccountRegistrySearch','#fdAccountRegistryRole','#fdAccountRegistryStatus','#fdAccountRegistrySort'].forEach(selector=>$(selector)?.addEventListener(selector.includes('Search')?'input':'change',renderAdminRegistry));
      $('#fdCreateAnotherAccount')?.addEventListener('click',()=>{const form=$('#fdAdminCreateAccountForm');if(form){form.hidden=false;form.reset();form.querySelector('input')?.focus()}$('#fdCreateAnotherAccount').hidden=true;setStatus('fdAdminAccountStatus','')});
      $('#fdCloudPasswordClose')?.addEventListener('click',()=>window.toggleModal?.('fdCloudPasswordModal',false));
      $('#fdCopyCloudPassword')?.addEventListener('click',async()=>{const value=$('#fdCloudPasswordValue')?.value||'';if(!value)return;try{await navigator.clipboard.writeText(value);setStatus('fdCloudPasswordStatus','Cloud password copied.','success')}catch{const input=$('#fdCloudPasswordValue');input?.select();document.execCommand('copy');setStatus('fdCloudPasswordStatus','Cloud password copied.','success')}});
      ...
      $$('.fd-editor-tool-launchers>details').forEach(panel=>panel.addEventListener('toggle',()=>{const label=panel.querySelector('summary small');if(label)label.textContent=panel.open?'Close':'Open';if(!panel.open)return;$$('.fd-editor-tool-launchers>details').forEach(other=>{if(other!==panel){other.open=false;const otherLabel=other.querySelector('summary small');if(otherLabel)otherLabel.textContent='Open'}})}));
      ...
      document.addEventListener('click',async event=>{
        ...
        const cloudNotification=event.target.closest('[data-cloud-notification]');if(cloudNotification){openCloudPasswordNotification(cloudNotification.dataset.cloudNotification);return}
...
(() => {
  ...
  function courseDurationLabel() {
    ...
    if (!video || !Number.isFinite(video.duration) || video.duration <= 0) return 'Recorded masterclass';
    const seconds = Math.floor(video.duration % 60);
    const minutes = Math.floor((video.duration / 60) % 60);
    const hours = Math.floor(video.duration / 3600);
    return hours > 0 ? `Recorded masterclass · ${hours}h ${String(minutes).padStart(2,'0')}m` : `Recorded masterclass · ${minutes}m ${String(seconds).padStart(2,'0')}s`;
  }
...
(()=>{
  ...
  const sendCloudSession=async()=>{
    try{
      const client=window.FINDAT_SUPABASE_CLIENT;
      const session=client?(await client.auth.getSession()).data?.session:null;
      frame?.contentWindow?.postMessage({type:'findat-cloud:session',account:window.FINDAT_ACTIVE_ACCOUNT||null,accessToken:session?.access_token||''},location.origin);
    }catch(error){console.warn('FINDAT Cloud session could not be shared.',error)}
  };
...
(()=>{
  ...
  let quizDraft=[];
  let learningHistory=[];
  let pendingCoursePurchase=null;
  ...
  function moneyLabel(course){return course.is_free!==false?'Free':`${course.price_currency||'ZMW'} ${Number(course.price_amount||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}
  function validLearningAccess(courseId){const row=learningHistory.find(item=>item.course_id===courseId);if(!row)return false;return ['active','completed'].includes(String(row.enrollment_status||''))&&(!row.access_expires_at||new Date(row.access_expires_at).getTime()>Date.now())}
  async function refreshLearningHistory(){
    const session=await currentSession();if(!session?.user){learningHistory=[];renderLearningHistory();return[]}
    const result=await client.rpc('findat_my_learning_history');if(result.error){console.warn('Learning history unavailable',result.error);learningHistory=[]}else learningHistory=result.data||[];
    renderLearningHistory();return learningHistory
  }
  function renderLearningHistory(){
    const host=$('#fdLearningHistory'),nav=$('#fdLearningNavCount');if(nav)nav.textContent=String(learningHistory.length);if(!host)return;
    host.innerHTML=learningHistory.length?learningHistory.map(item=>{const cover=safeUrl(item.cover_url)||protectedPoster;const access=item.access_expires_at?new Date(item.access_expires_at):null;const active=['active','completed'].includes(item.enrollment_status)&&(!access||access.getTime()>Date.now());const status=item.certificate_number?'Certified':item.enrollment_status==='completed'?'Completed':active?'Active access':item.payment_status==='pending'?'Payment pending':item.payment_status==='rejected'?'Payment not approved':'Expired';return `<article class="fd-learning-history-card"><div class="fd-learning-history-cover" ${cover?`style="background-image:url('${escapeHTML(cover)}')"`:''}></div><div><span class="fd-learning-history-status is-${escapeHTML(String(status).toLowerCase().replace(/\s+/g,'-'))}">${escapeHTML(status)}</span><h3>${escapeHTML(item.course_title||'Course')}</h3>${item.payment_status?`<p><strong>Payment:</strong> ${escapeHTML(item.payment_status)}${item.payment_amount!=null?` · ${escapeHTML(item.payment_currency||'')} ${Number(item.payment_amount).toLocaleString()}`:''}</p>`:''}${access?`<p><strong>Access until:</strong> ${escapeHTML(access.toLocaleDateString())}</p>`:''}<p><strong>Progress:</strong> ${Number(item.progress_percent||0).toFixed(0)}%</p>${item.certificate_number?`<div class="fd-certificate-chip"><i class="fas fa-certificate"></i><span>${escapeHTML(item.certificate_number)}</span></div>`:''}<button class="fd-secondary-btn" data-learning-course="${item.course_id}" type="button"><i class="fas fa-play"></i> ${active?'Open course':'View course'}</button></div></article>`}).join(''):'<div class="fd-empty-large"><i class="fas fa-graduation-cap"></i>No course access, payments or certificates are recorded yet.</div>'
  }
  window.FINDAT_REFRESH_LEARNING=()=>refreshLearningHistory().catch(error=>console.warn(error));
  ...
  function showLegacyCourse(){
    ...
    if(protectedPoster){
      ...
      if(poster){poster.classList.add('has-thumbnail');poster.style.backgroundImage=`linear-gradient(90deg,rgba(8,13,22,.92),rgba(8,13,22,.58) 52%,rgba(8,13,22,.14)),url("${protectedPoster}")`}
    }
  ...
  function renderCourseCards(){
    ...
    courseGrid.innerHTML=catalogueCourses.map(course=>{
      ...
      const cover=safeUrl(course.cover_url)||(builtIn?protectedPoster:'');
      const lessonCount=course.lessons?.length||0;
      const hasAccess=course.is_free!==false||validLearningAccess(course.id);
      const payment=learningHistory.find(item=>item.course_id===course.id)?.payment_status||'';
      const accessLabel=course.is_free!==false?'Free course':hasAccess?'Access active':payment==='pending'?'Payment pending':moneyLabel(course);
      return `<article class="findat-course-card ${hasAccess?'':'is-locked'}"><div class="findat-course-card-cover" ${cover?`style="background-image:url('${escapeHTML(cover)}')"`:''}><span><i class="fas ${hasAccess?'fa-play-circle':'fa-lock'}"></i>${lessonCount} lesson${lessonCount===1?'':'s'}</span><b class="fd-course-price-badge">${escapeHTML(accessLabel)}</b></div><div class="findat-course-card-body"><h3>${escapeHTML(course.title)}</h3><p>${escapeHTML(course.short_description||course.course_content||'FINDAT Academy course')}</p><div class="findat-course-card-meta"><span class="findat-course-rating">${stars(course.rating)}</span><span>${escapeHTML(course.instructor_name||'FINDAT Academy')}</span></div><div class="fd-course-access-meta"><span><i class="far fa-clock"></i> ${Math.max(1,Number(course.access_months)||1)} month${Math.max(1,Number(course.access_months)||1)===1?'':'s'} access</span></div><button class="findat-course-card-button" data-course-open="${course.id}" type="button">${hasAccess?'Open course':payment==='pending'?'View payment':'Get access'}</button></div></article>`
    }).join('')
  }
  async function loadCatalogue(){
    ...
    await refreshLearningHistory().catch(()=>{});
    const coursesResult=await client.from('findat_courses').select('*').eq('status','published').order('updated_at',{ascending:false});
    if(coursesResult.error){
      catalogueCourses=[{id:'legacy-data-analytics',slug:'data-analytics-foundations',title:'Data Analytics Foundations',short_description:'Learn how data analytics supports evidence-based decisions across professional domains.',instructor_name:'FINDAT Academy',instructor_qualifications:'Data Analytics | Research | Professional Practice',rating:5,is_builtin:true,is_free:true,access_months:1,cover_url:protectedPoster,lessons:[{id:'legacy-lesson',title:'How Data Analytics Work per Domain',thumbnail_url:protectedPoster}]}];
    ...
    catalogueCourses=(coursesResult.data||[]).map(course=>({...course,lessons:lessons.filter(lesson=>lesson.course_id===course.id)}));
    const builtIn=catalogueCourses.find(course=>course.slug==='data-analytics-foundations');if(builtIn){builtIn.cover_url=protectedPoster;builtIn.is_builtin=true;builtIn.is_free=true;builtIn.rating=5;(builtIn.lessons||[]).forEach(lesson=>{if(normalizeCourseTitle(lesson.title)==='how data analytics work per domain')lesson.thumbnail_url=protectedPoster})}
    renderCourseCards()
  }
  function normalizeCourseTitle(value){return String(value||'').trim().toLowerCase()}
  ...
  function renderDynamicQuiz(quiz){
    ...
    $('#findatDynamicQuizForm')?.addEventListener('submit',async event=>{event.preventDefault();let score=0;quiz.forEach((question,index)=>{const selected=event.currentTarget.querySelector(`input[name="q${index}"]:checked`);if(selected&&Number(selected.value)===Number(question.correct))score++});const percent=Math.round(score/quiz.length*100),result=$('#findatDynamicQuizResult');if(result)result.textContent=`Score: ${score} / ${quiz.length} (${percent}%).${percent>=70?' Course completion is being recorded.':''}`;if(percent>=70&&selectedCourse?.id&&!selectedCourse.is_builtin){const session=await currentSession();if(session?.user){const completed=await client.rpc('findat_record_course_completion',{p_course_id:selectedCourse.id,p_score:percent});if(completed.error){if(result)result.textContent+=` ${completed.error.message}`}else{if(result)result.textContent+=` Certificate: ${completed.data?.certificate_number||'awarded'}.`;await refreshLearningHistory()}}}})
  }
  ...
  courseGrid?.addEventListener('click',async event=>{
    ...
    if(course.is_free!==false&&session?.user&&!course.is_builtin){await client.rpc('findat_request_course_access',{p_course_id:course.id,p_payment_method:'free',p_payment_reference:''});await refreshLearningHistory().catch(()=>{})}
    if(course.is_free===false&&!validLearningAccess(course.id)){
      if(!session?.user){window.toggleModal?.('loginPopup',true);return}
      pendingCoursePurchase=course;const summary=$('#fdCoursePurchaseSummary');if(summary)summary.innerHTML=`<h3>${escapeHTML(course.title)}</h3><p><strong>${escapeHTML(moneyLabel(course))}</strong> · ${Math.max(1,Number(course.access_months)||1)} month${Math.max(1,Number(course.access_months)||1)===1?'':'s'} access</p><p>Enter a valid payment or receipt reference. An Administrator will verify it before the lessons unlock.</p>`;const form=$('#fdCoursePurchaseForm');form?.reset();const existing=learningHistory.find(item=>item.course_id===course.id);if(existing?.payment_reference&&form?.elements.paymentReference)form.elements.paymentReference.value=existing.payment_reference;const status=$('#fdCoursePurchaseStatus');if(status){status.textContent=existing?.payment_status==='pending'?'Your previous payment reference is waiting for Administrator review.':'';status.className='auth-status'}window.toggleModal?.('fdCoursePurchaseModal',true);return
    }
  ...
  function resetCourseForm(){
    const form=$('#fdCourseForm');if(!form)return;form.reset();form.elements.id.value='';form.elements.rating.value='5';form.elements.status.value='draft';if(form.elements.isFree)form.elements.isFree.value='true';if(form.elements.priceAmount)form.elements.priceAmount.value='0';if(form.elements.priceCurrency)form.elements.priceCurrency.value='ZMW';if(form.elements.accessMonths)form.elements.accessMonths.value='1';managerCourseId='';courseCoverFile=null;$('#fdCourseCoverName').textContent='No picture selected';renderManagerCourseList();resetLessonForm();renderManagerLessonList()
  }
  ...
  async function uploadCourseFile(file,courseId,folder){
    if(!file)return'';const path=`findat-v1/course-media/${courseId}/${folder}/${crypto.randomUUID()}-${fileNameSafe(file.name)}`;const{error}=await client.storage.from('findat-documents').upload(path,file,{upsert:false,contentType:file.type||undefined,cacheControl:'3600'});if(error)throw error;const{data}=client.storage.from('findat-documents').getPublicUrl(path);return data?.publicUrl||''
  }
  $('#fdCourseManagerButton')?.addEventListener('click',async()=>{window.toggleModal?.('fdCourseManagerModal',true);try{await Promise.all([loadManagerData(),loadCoursePayments()])}catch(error){showCourseStatus(error.message||'Course Manager could not be opened.','error')}});
  ...
  $('#fdCourseForm')?.addEventListener('submit',async event=>{
    ...
    try{const profile=await requireAdmin(),values=Object.fromEntries(new FormData(form).entries()),id=String(values.id||crypto.randomUUID()),existing=managerCourses.find(item=>item.id===id),coverUrl=courseCoverFile?await uploadCourseFile(courseCoverFile,id,'course-cover'):(existing?.cover_url||'');const isFree=String(values.isFree||'true')==='true';const row={id,slug:slugify(values.slug||values.title),title:String(values.title||'').trim(),short_description:String(values.shortDescription||'').trim(),course_content:String(values.courseContent||'').trim(),cover_url:coverUrl,instructor_name:String(values.instructorName||'').trim(),instructor_qualifications:String(values.instructorQualifications||'').trim(),rating:Number(values.rating)||5,status:String(values.status||'draft'),is_free:isFree,price_amount:isFree?0:Math.max(0,Number(values.priceAmount)||0),price_currency:String(values.priceCurrency||'ZMW'),access_months:Math.max(1,Number(values.accessMonths)||1),created_by:existing?.created_by||profile.id};if(!row.title)throw new Error('Course title is required.');const{data,error}=await client.from('findat_courses').upsert(row).select().single();if(error)throw error;managerCourseId=data.id;courseCoverFile=null;await loadManagerData(data.id);showCourseStatus('Course saved. Published courses appear as cards in Recordings.','success');await loadCatalogue();setTimeout(()=>window.toggleModal?.('fdCourseManagerModal',false),500)}
  ...
  $('#fdCoursePurchaseClose')?.addEventListener('click',()=>window.toggleModal?.('fdCoursePurchaseModal',false));
  $('#fdCoursePurchaseForm')?.addEventListener('submit',async event=>{event.preventDefault();if(!pendingCoursePurchase)return;const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),values=Object.fromEntries(new FormData(form).entries()),status=$('#fdCoursePurchaseStatus');if(button)button.disabled=true;if(status){status.textContent='Submitting payment reference…';status.className='auth-status'}try{const result=await client.rpc('findat_request_course_access',{p_course_id:pendingCoursePurchase.id,p_payment_method:String(values.paymentMethod||'manual'),p_payment_reference:String(values.paymentReference||'').trim()});if(result.error)throw result.error;form.reset();if(status){status.textContent=result.data?.status==='active'?'Course access activated.':'Payment reference submitted for Administrator review.';status.className='auth-status success'}await refreshLearningHistory();renderCourseCards();setTimeout(()=>window.toggleModal?.('fdCoursePurchaseModal',false),650)}catch(error){if(status){status.textContent=error.message||'The payment reference could not be submitted.';status.className='auth-status error'}}finally{if(button)button.disabled=false}});
  $('#fdRefreshLearning')?.addEventListener('click',()=>refreshLearningHistory().catch(error=>console.warn(error)));
  $('#fdLearningHistory')?.addEventListener('click',event=>{const button=event.target.closest('[data-learning-course]');if(!button)return;const course=catalogueCourses.find(item=>item.id===button.dataset.learningCourse);if(course){window.openRecordingsPanel?.();setTimeout(()=>{if(course.slug==='data-analytics-foundations'||course.is_builtin)showLegacyCourse();else if(validLearningAccess(course.id)||course.is_free!==false)showDynamicCourse(course);else showCatalog()},80)}});
  async function loadCoursePayments(){const host=$('#fdCoursePaymentList');if(!host)return;host.innerHTML='<div class="fd-course-manager-empty"><i class="fas fa-spinner fa-spin"></i> Loading payments…</div>';const result=await client.rpc('findat_admin_course_payments');if(result.error){host.innerHTML=`<div class="fd-course-manager-empty">${escapeHTML(result.error.message||'Payments could not be loaded.')}</div>`;return}const rows=result.data||[];host.innerHTML=rows.length?rows.map(row=>`<article class="fd-course-payment-item is-${escapeHTML(row.status)}"><div><strong>${escapeHTML(row.user_name||'FINDAT learner')}</strong><span>${escapeHTML(row.course_title||'Course')}</span><small>${escapeHTML(row.currency||'')} ${Number(row.amount||0).toLocaleString()} · ${escapeHTML(row.payment_method||'manual')} · ${escapeHTML(row.payment_reference||'No reference')}</small><time>${new Date(row.created_at).toLocaleString()}</time></div><span class="fd-course-payment-state">${escapeHTML(row.status)}</span>${row.status==='pending'?`<div><button class="fd-mini-btn is-success" data-payment-review="${row.payment_id}" data-payment-approve="true" type="button"><i class="fas fa-check"></i> Approve</button><button class="fd-mini-btn is-danger" data-payment-review="${row.payment_id}" data-payment-approve="false" type="button"><i class="fas fa-times"></i> Reject</button></div>`:''}</article>`).join(''):'<div class="fd-course-manager-empty">No payment records are available.</div>'}
  $('#fdRefreshCoursePayments')?.addEventListener('click',loadCoursePayments);
  $('#fdCoursePaymentList')?.addEventListener('click',async event=>{const button=event.target.closest('[data-payment-review]');if(!button)return;button.disabled=true;try{const result=await client.rpc('findat_admin_review_course_payment',{p_payment_id:button.dataset.paymentReview,p_approve:button.dataset.paymentApprove==='true'});if(result.error)throw result.error;await loadCoursePayments();showCourseStatus(button.dataset.paymentApprove==='true'?'Payment approved and course access activated.':'Payment reference rejected.','success')}catch(error){showCourseStatus(error.message||'The payment could not be reviewed.','error')}finally{button.disabled=false}});
...
/* ============================== FINDAT x1 Training Orchestration ============================== */
(() => {
  'use strict';
  const client = window.FINDAT_SUPABASE_CLIENT;
  if (!client) return;
  const $ = (selector, root = document) => root.querySelector(selector);
  const escapeHTML = value => String(value ?? '').replace(/[&<>'"]/g, char => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
  const fileNameSafe = value => String(value || 'file').replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '').slice(-120) || 'file';
  let projects = [];
  let assignments = [];
  let documents = [];
  let jobs = [];
  ...
  function activeAccount() { return window.FINDAT_ACTIVE_ACCOUNT || null; }
  function isAdmin() { return activeAccount()?.role === 'admin'; }
  function setStatus(message = '', state = '') {
    const host = $('#fdX1TrainingStatus');
    if (!host) return;
    host.textContent = message;
    host.className = `auth-status${state ? ` ${state}` : ''}`;
  }
  function projectName(projectId) { return projects.find(item => item.id === projectId)?.name || 'Training project'; }
  function formatDate(value) { const date = new Date(value); return Number.isNaN(date.getTime()) ? '—' : date.toLocaleString(); }

  async function loadConsultants() {
    if (!isAdmin()) { consultants = []; return; }
    const result = await client.from('findat_profiles').select('id,first_name,last_name,username,avatar_url,qualifications,role,active').eq('role','consultant').eq('active',true).order('first_name');
    if (result.error) throw result.error;
    consultants = result.data || [];
  }
  async function loadProjects() {
    const result = await client.from('findat_x1_training_projects').select('*').order('updated_at',{ascending:false});
    if (result.error) throw result.error;
    projects = result.data || [];
    if (!activeProjectId || !projects.some(item => item.id === activeProjectId)) activeProjectId = projects[0]?.id || '';
  }
  async function loadProjectResources() {
    if (!activeProjectId) { assignments=[]; documents=[]; jobs=[]; evaluations=[]; return; }
    const [assignmentResult, documentResult, jobResult, evaluationResult] = await Promise.all([
      client.from('findat_x1_training_assignments').select('*').eq('project_id',activeProjectId).order('assigned_at'),
      client.from('findat_x1_training_documents').select('*').eq('project_id',activeProjectId).order('created_at',{ascending:false}),
      client.from('findat_x1_training_jobs').select('*').eq('project_id',activeProjectId).order('created_at',{ascending:false}),
      client.from('findat_x1_training_evaluations').select('*').eq('project_id',activeProjectId).order('created_at',{ascending:false})
    ]);
    for (const result of [assignmentResult,documentResult,jobResult,evaluationResult]) if (result.error) throw result.error;
    assignments=assignmentResult.data||[];documents=documentResult.data||[];jobs=jobResult.data||[];evaluations=evaluationResult.data||[];
  }
  function renderProjects() {
    const host=$('#fdX1ProjectList'),count=$('#fdX1TrainingNavCount');
    if(count)count.textContent=String(projects.length);
    if(!host)return;
    host.innerHTML=projects.length?projects.map(project=>`<button class="fd-x1-project-item ${project.id===activeProjectId?'is-active':''}" data-x1-project="${project.id}" type="button"><span><strong>${escapeHTML(project.name)}</strong><small>${escapeHTML(project.domain||'General')} · ${escapeHTML(project.technique||'training')}</small></span><em class="is-${escapeHTML(project.status||'active')}">${escapeHTML(project.status||'active')}</em></button>`).join(''):'<div class="fd-course-manager-empty">No training projects are available.</div>';
  }
  function renderProjectForm() {
    const form=$('#fdX1ProjectForm');if(!form)return;
    const project=projects.find(item=>item.id===activeProjectId);
    if(!project){form.reset();form.elements.id.value='';return;}
    form.elements.id.value=project.id;form.elements.name.value=project.name||'';form.elements.domain.value=project.domain||'Finance and accounting';form.elements.objective.value=project.objective||'';form.elements.technique.value=project.technique||'retrieval_augmented_generation';form.elements.status.value=project.status||'active';
  }
  function renderAssignments() {
    const host=$('#fdX1ConsultantPicker');if(!host)return;
    if(!isAdmin()){host.innerHTML='<p class="fd-x1-note">Only an Administrator can change consultant assignments.</p>';return;}
    const selected=new Set(assignments.filter(row=>row.active!==false).map(row=>row.consultant_id));
    host.innerHTML=consultants.length?consultants.map(person=>{const name=`${person.first_name||''} ${person.last_name||''}`.trim()||person.username;return `<label class="fd-x1-consultant-card"><input type="checkbox" value="${person.id}" ${selected.has(person.id)?'checked':''}><span>${person.avatar_url?`<img src="${escapeHTML(person.avatar_url)}" alt="">`:`<b>${escapeHTML(name.split(/\s+/).map(part=>part[0]).slice(0,2).join('').toUpperCase())}</b>`}<strong>${escapeHTML(name)}</strong><small>${escapeHTML(person.qualifications||'Consultant')}</small></span></label>`}).join(''):'<p class="fd-x1-note">No active Consultants are available.</p>';
  }
  function renderDocuments() {
    const host=$('#fdX1DocumentList');if(!host)return;
    host.innerHTML=documents.length?documents.map(item=>`<article class="fd-x1-resource"><i class="fas fa-file-alt"></i><div><strong>${escapeHTML(item.file_name)}</strong><small>${escapeHTML(item.document_kind||'training')} · ${Number(item.size_bytes||0).toLocaleString()} bytes · ${escapeHTML(formatDate(item.created_at))}</small>${item.notes?`<p>${escapeHTML(item.notes)}</p>`:''}</div>${isAdmin()||item.uploaded_by===activeAccount()?.id?`<button data-x1-delete-document="${item.id}" data-x1-storage-path="${escapeHTML(item.storage_path)}" type="button" aria-label="Delete document"><i class="fas fa-trash"></i></button>`:''}</article>`).join(''):'<div class="fd-course-manager-empty">No training documents have been uploaded.</div>';
  }
  function renderJobs() {
    const host=$('#fdX1JobList');if(!host)return;
    host.innerHTML=jobs.length?jobs.map(item=>`<article class="fd-x1-job"><div><strong>${escapeHTML(String(item.job_type||'training').replaceAll('_',' '))}</strong><small>${escapeHTML(formatDate(item.created_at))}</small></div><span class="is-${escapeHTML(item.status||'queued')}">${escapeHTML(item.status||'queued')} · ${Number(item.progress||0).toFixed(0)}%</span>${item.error_message?`<p>${escapeHTML(item.error_message)}</p>`:''}</article>`).join(''):'<div class="fd-course-manager-empty">No training jobs have been queued.</div>';
  }
  function renderEvaluations() {
    const host=$('#fdX1EvaluationList');if(!host)return;
    host.innerHTML=evaluations.length?evaluations.map(item=>`<article class="fd-x1-evaluation"><strong>${escapeHTML(item.prompt)}</strong><p><b>Expected:</b> ${escapeHTML(item.expected_output||'Not specified')}</p>${item.actual_output?`<p><b>Actual:</b> ${escapeHTML(item.actual_output)}</p>`:''}<small>${item.score==null?'Not scored':`Score ${Number(item.score).toFixed(1)}`} · ${escapeHTML(formatDate(item.created_at))}</small></article>`).join(''):'<div class="fd-course-manager-empty">No evaluation cases have been added.</div>';
  }
  function renderAll() { renderProjects();renderProjectForm();renderAssignments();renderDocuments();renderJobs();renderEvaluations(); }

  async function refresh(selectId='') {
    if (!activeAccount()) return;
    setStatus('Loading x1 training workspace…');
    try { if(selectId)activeProjectId=selectId;await Promise.all([loadProjects(),loadConsultants()]);await loadProjectResources();renderAll();setStatus(projects.length?'Training workspace is up to date.':'Create a training project to begin.','success'); }
    catch(error){setStatus(error.message||'The x1 training workspace could not be loaded.','error');}
  }
  window.FINDAT_REFRESH_X1=refresh;

  $('#fdX1Refresh')?.addEventListener('click',()=>refresh());
  $('#fdX1ProjectList')?.addEventListener('click',async event=>{const button=event.target.closest('[data-x1-project]');if(!button)return;activeProjectId=button.dataset.x1Project;await loadProjectResources();renderAll();});
  $('#fdX1ProjectForm')?.addEventListener('submit',async event=>{
    event.preventDefault();if(!isAdmin())return;const form=event.currentTarget,button=form.querySelector('button[type="submit"]'),values=Object.fromEntries(new FormData(form).entries());if(button)button.disabled=true;setStatus('Saving training project…');
    try{const row={id:String(values.id||crypto.randomUUID()),name:String(values.name||'').trim(),domain:String(values.domain||'').trim()||'Finance and accounting',objective:String(values.objective||'').trim(),technique:String(values.technique||'retrieval_augmented_generation'),status:String(values.status||'active'),created_by:activeAccount().id,updated_at:new Date().toISOString()};if(!row.name)throw new Error('Enter a project name.');const existing=projects.find(item=>item.id===row.id);if(existing)row.created_by=existing.created_by;const result=await client.from('findat_x1_training_projects').upsert(row).select().single();if(result.error)throw result.error;activeProjectId=result.data.id;form.reset();form.elements.id.value='';await refresh(activeProjectId);setStatus('Training project saved. Assign Consultants and add governed training data.','success');}
    catch(error){setStatus(error.message||'The training project could not be saved.','error')}finally{if(button)button.disabled=false}
  });
  $('#fdX1SaveAssignments')?.addEventListener('click',async()=>{
    if(!isAdmin()||!activeProjectId){setStatus('Select a project first.','error');return}const selected=[...document.querySelectorAll('#fdX1ConsultantPicker input:checked')].map(input=>input.value);setStatus('Saving Consultant assignments…');
    try{const current=assignments.map(row=>row.consultant_id);const remove=current.filter(id=>!selected.includes(id));if(remove.length){const deleted=await client.from('findat_x1_training_assignments').delete().eq('project_id',activeProjectId).in('consultant_id',remove);if(deleted.error)throw deleted.error}const rows=selected.map(consultantId=>({project_id:activeProjectId,consultant_id:consultantId,assigned_by:activeAccount().id,active:true}));if(rows.length){const upserted=await client.from('findat_x1_training_assignments').upsert(rows,{onConflict:'project_id,consultant_id'});if(upserted.error)throw upserted.error}await loadProjectResources();renderAssignments();setStatus(`${selected.length} Consultant assignment${selected.length===1?'':'s'} saved.`,'success')}
    catch(error){setStatus(error.message||'Assignments could not be saved.','error')}
  });
  $('#fdX1TrainingFiles')?.addEventListener('change',event=>{stagedFiles=[...(event.target.files||[])];const host=$('#fdX1TrainingUploadSummary');if(host)host.textContent=stagedFiles.length?`${stagedFiles.length} file${stagedFiles.length===1?'':'s'} selected.`:'No files selected.'});
  $('#fdX1UploadDocuments')?.addEventListener('click',async()=>{
    if(!activeProjectId){setStatus('Select a training project first.','error');return}if(!stagedFiles.length){setStatus('Choose one or more training documents.','error');return}const notes=$('#fdX1DocumentNotes')?.value.trim()||'';setStatus('Uploading governed training documents…');
    try{for(const file of stagedFiles){const path=`${activeProjectId}/${crypto.randomUUID()}-${fileNameSafe(file.name)}`;const upload=await client.storage.from('findat-x1-training').upload(path,file,{upsert:false,contentType:file.type||undefined,cacheControl:'3600'});if(upload.error)throw upload.error;const inserted=await client.from('findat_x1_training_documents').insert({project_id:activeProjectId,file_name:file.name,storage_path:path,mime_type:file.type||'',size_bytes:file.size||0,document_kind:'training',notes,uploaded_by:activeAccount().id});if(inserted.error)throw inserted.error}stagedFiles=[];$('#fdX1TrainingFiles').value='';$('#fdX1DocumentNotes').value='';$('#fdX1TrainingUploadSummary').textContent='No files selected.';await loadProjectResources();renderDocuments();setStatus('Training documents saved. They are ready for controlled ingestion or retrieval-index jobs.','success')}
    catch(error){setStatus(error.message||'Training documents could not be saved.','error')}
  });
  $('#fdX1DocumentList')?.addEventListener('click',async event=>{const button=event.target.closest('[data-x1-delete-document]');if(!button)return;if(!confirm('Delete this training document?'))return;try{if(button.dataset.x1StoragePath){const removed=await client.storage.from('findat-x1-training').remove([button.dataset.x1StoragePath]);if(removed.error)throw removed.error}const deleted=await client.from('findat_x1_training_documents').delete().eq('id',button.dataset.x1DeleteDocument);if(deleted.error)throw deleted.error;await loadProjectResources();renderDocuments();setStatus('Training document deleted.','success')}catch(error){setStatus(error.message||'The document could not be deleted.','error')}});
  $('#fdX1QueueJob')?.addEventListener('click',async()=>{
    if(!activeProjectId){setStatus('Select a project first.','error');return}let configuration={};const raw=$('#fdX1JobConfiguration')?.value.trim();if(raw){try{configuration=JSON.parse(raw)}catch{setStatus('Training configuration must be valid JSON.','error');return}}const button=$('#fdX1QueueJob');button.disabled=true;setStatus('Queuing governed training job…');
    try{const result=await client.from('findat_x1_training_jobs').insert({project_id:activeProjectId,job_type:$('#fdX1JobType').value,configuration,status:'queued',progress:0,requested_by:activeAccount().id});if(result.error)throw result.error;$('#fdX1JobConfiguration').value='';await loadProjectResources();renderJobs();setStatus('Job queued. Connect an authorised model-training worker or provider to execute it; this browser does not train a foundation model by itself.','success')}
    catch(error){setStatus(error.message||'The training job could not be queued.','error')}finally{button.disabled=false}
  });
  $('#fdX1AddEvaluation')?.addEventListener('click',async()=>{
    if(!activeProjectId){setStatus('Select a project first.','error');return}const prompt=$('#fdX1EvaluationPrompt')?.value.trim()||'',expected=$('#fdX1EvaluationExpected')?.value.trim()||'';if(!prompt){setStatus('Enter an evaluation prompt.','error');return}setStatus('Saving evaluation case…');
    try{const result=await client.from('findat_x1_training_evaluations').insert({project_id:activeProjectId,prompt,expected_output:expected,created_by:activeAccount().id});if(result.error)throw result.error;$('#fdX1EvaluationPrompt').value='';$('#fdX1EvaluationExpected').value='';await loadProjectResources();renderEvaluations();setStatus('Evaluation case saved.','success')}
    catch(error){setStatus(error.message||'The evaluation case could not be saved.','error')}
  });
})();
