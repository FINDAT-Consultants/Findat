...
qs('#systemPanel').addEventListener('click', event => {
  ...
  if (action === 'settings') openApp('settings');
  if (action === 'applications') openApp('launchpad');
  if (action === 'lock') qs('#lockScreen').classList.remove('hidden');
  if (action === 'restart') location.reload();
  if (action === 'exit-cloud') {
    if (window.parent !== window) window.parent.postMessage({ type: 'findat-cloud-exit' }, '*');
    else if (history.length > 1) history.back();
  }
  qs('#systemPanel').classList.add('hidden');
});

qs('#unlockBtn').addEventListener('click', () => qs('#lockScreen').classList.add('hidden'));
