...
function publicEntry(node) {
  if (!node) return node;
  return { name: node.name, path: node.path, type: node.type, size: node.size || 0, storedSize: storedSizeOf(node), compression: node.compression || '', modified: node.modified, mime: node.mime };
}
...
async function localTransfer(source, destination, moving) {
  ...
  const puts = descendants.map(node => {
    ...
    return { ...node, path, parent, name: basename(path), modified: now };
  });
...
async function localRename(path, name) {
  ...
  const puts = descendants.map(item => {
    ...
    return { ...item, path: newPath, parent: newParent, name: basename(newPath), modified: now };
  });
...
function setCloudBusy(busy, failed = false) {
  ...
  indicator.title = failed || !fsState.cloudOnline ? 'FINDAT Cloud storage unavailable' : busy ? 'Syncing FINDAT Cloud' : 'FINDAT Cloud workspace ready';
}
...
function makeEntryButton(entry, surface = 'finder') {
  ...
  button.innerHTML = `${fileGlyph(entry)}<small class="entry-name">${escapeHtml(entry.name)}</small>`;
  button.title = `${entry.name}${entry.type === 'file' ? ` · ${formatBytes(entry.size)}` : ''}`;
...
function viewerMarkup() {
  return `<div class="viewer-app"><div class="viewer-toolbar"><div data-viewer-meta></div><div><button data-viewer-action="save" class="hidden">Save</button><button data-viewer-action="download">Download</button></div></div><div class="viewer-content" data-viewer-content><div class="viewer-loading">Opening file…</div></div></div>`;
}
...
async function renderViewer(win, entry) {
  ...
  try {
    ...
    if (entry.mime?.startsWith('image/')) {
      const url = await objectUrlForEntry(entry);
...
let findatLastDropTarget = '/Desktop';

function findatResolveDropTarget(event) {
  const targetNode = event?.target instanceof Element ? event.target.closest('[data-drop-path]') : null;
  const explicitPath = targetNode?.dataset?.dropPath;
  const path = explicitPath || activeDirectory() || findatLastDropTarget || '/Desktop';
  findatLastDropTarget = normalizeVirtualPath(path);
  return findatLastDropTarget;
}

function findatUpdateDropOverlay(path) {
  const normalized = normalizeVirtualPath(path || activeDirectory() || '/Desktop');
  findatLastDropTarget = normalized;
  const label = qs('[data-drop-destination]');
  if (label) label.textContent = `Save to ${normalized === '/' ? 'FINDAT Cloud' : normalized}`;
  const overlay = qs('#dropOverlay');
  if (overlay) overlay.dataset.dropPath = normalized;
}

function findatHighlightUploadedEntries(paths = []) {
  const wanted = new Set(paths.map(normalizeVirtualPath));
  if (!wanted.size) return;
  qsa('.fs-entry').forEach(node => {
    if (!wanted.has(normalizeVirtualPath(node.dataset.path || '/'))) return;
    node.classList.add('just-uploaded');
    setTimeout(() => node.classList.remove('just-uploaded'), 4200);
  });
}

async function findatRevealUploadDestination(targetPath, uploadedPaths = []) {
  const target = normalizeVirtualPath(targetPath || '/Desktop');
  let finder = qs('.app-window[data-app="finder"]');

  // Desktop uploads are visible immediately as desktop icons. Other folders are
  // opened automatically so a successful drop can never appear to disappear.
  if (target !== '/Desktop' || finder) {
    if (!finder) {
      openFinderAt(target);
      finder = qs('.app-window[data-app="finder"]');
    }
    if (finder) await navigateFinder(finder, target, false);
  }

  requestAnimationFrame(() => findatHighlightUploadedEntries(uploadedPaths));
}

async function uploadFiles(files, targetPath = activeDirectory(), options = {}) {
  const list = [...files].map(uploadRecord).filter(item => item.file instanceof Blob);
  if (!list.length) return { completed: 0, failures: [], paths: [] };

  targetPath = normalizeVirtualPath(targetPath || activeDirectory() || '/Desktop');
  findatLastDropTarget = targetPath;

  try {
    findatGitHubRequire(true);
    await findatLoadGitHubTree(false);
  } catch (error) {
    toast(`Upload unavailable: ${error.message}`);
    return { completed: 0, failures: [error.message], paths: [] };
  }
  ...
  const progress = typeof showSystemProgress === 'function'
    ? showSystemProgress({
        ...
        message: `Saving ${list.length} item${list.length === 1 ? '' : 's'} to ${targetPath === '/' ? 'FINDAT Cloud' : targetPath}`,
        state: 'Uploading…',
        progress: 3
      })
    : null;
  ...
  for (const item of list) {
    progress?.update(progressPercent(transferredBytes, totalBytes), {
      title: 'Uploading to GitHub',
      message: item.name,
      state: `Uploading ${completed + 1} of ${list.length}…`
    });
    try {
      const uploaded = await fsRequest('/upload', {
        method: 'POST',
        headers: {
      ...
      progress?.update(progressPercent(transferredBytes, totalBytes), {
        title: 'Uploading to GitHub',
        message: item.name,
        state: `Uploaded ${completed} of ${list.length}…`
      });
    } catch (error) {
  ...
  if (!completed) {
    progress?.fail({
      title: 'Upload failed',
      message: failures[0] || 'No items were uploaded',
      state: 'Failed',
      progress: 100
    });
    if (failures[0]) toast(failures[0]);
    return { completed, failures, paths: uploadedPaths };
  }

  // Confirm the repository state, then redraw every cloud view before showing
  // completion. This removes the old cache delay that could hide a new file.
  findatGitHubDrive.loadedAt = 0;
  try { await findatLoadGitHubTree(true); }
  catch (_) { /* the confirmed upload remains in the in-memory tree */ }
  await refreshAllFileViews();
  if (options.reveal !== false) await findatRevealUploadDestination(targetPath, uploadedPaths);

  const location = targetPath === '/' ? 'FINDAT Cloud' : targetPath;
  if (failures.length) toast(`${completed} uploaded to ${location}; ${failures.length} failed`);
  else toast(`${completed} item${completed === 1 ? '' : 's'} uploaded to ${location}`);

  progress?.complete({
    title: 'Upload complete',
    message: `${completed} item${completed === 1 ? '' : 's'} saved in ${location}`,
    state: 'Visible in FINDAT Cloud'
  });
  return { completed, failures, paths: uploadedPaths };
}
...
async function refreshAllFileViews() {
  const jobs = [refreshDesktop(), refreshTrash(), refreshUsage()];
  const finder = qs('.app-window[data-app="finder"]');
  if (finder) jobs.push(navigateFinder(finder, finder.dataset.currentPath || '/Desktop', false));
...
function bindFilesystemUI() {
  ...
  window.addEventListener('dragenter', event => {
    ...
    fsState.dragDepth += 1;
    findatUpdateDropOverlay(findatResolveDropTarget(event));
  ...
  window.addEventListener('drop', event => {
    const target = findatResolveDropTarget(event);
    fsState.dragDepth = 0;
    qs('#dropOverlay').classList.add('hidden');
    const files = [...(event.dataTransfer?.files || [])];
    if (!files.length) return;
    event.preventDefault();
    uploadFiles(files, target, { reveal: true }).catch(error => toast(error.message));
  });
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    <div class="settings-heading"><div><h2>FINDAT Cloud</h2><p>Manage the 4 TB virtual workspace, compression, imports and backups.</p></div><span class="settings-heading-icon cloud-heading-logo"></span></div>
  ...
  if (section === 'cloud') {
    qsa('[data-cloud-action]', win).forEach(button => button.addEventListener('click', async () => {
      ...
      if (action === 'persist') {
        ...
        } catch (_) { toast('Could not request persistent storage'); }
      }
      if (action === 'refresh' || action === 'persist' || action === 'empty-trash') refreshCloudSettingsPanel(win);
    }));
...
async function refreshCloudSettingsPanel(win) {
  ...
  try {
    ...
    const detail = qs('[data-cloud-status-detail]', panel); if (detail) detail.textContent = `4 TB virtual workspace · ${files + folders} items · ${formatBytes(usage.saved)} saved by compression`;
    let persisted = false;
...
/* FINDAT Cloud GitHub repository backend ---------------------------------- */
const FINDAT_GITHUB_CONFIG_KEY = 'findat.github.repository.v1';
const FINDAT_GITHUB_SESSION_TOKEN_KEY = 'findat.github.token.session.v1';
const FINDAT_GITHUB_MARKER = '.findat-folder';
const FINDAT_GITHUB_API_VERSION = '2026-03-10';
const FINDAT_GITHUB_BROWSER_UPLOAD_LIMIT = 50 * 1024 * 1024;
const FINDAT_GITHUB_REFRESH_MS = 60 * 1000;

const findatGitHubDrive = {
  config: null,
  token: '',
  nodes: new Map(),
  loadedAt: 0,
...
function findatSanitizeGitHubPath(value) {
  return String(value || '').replace(/\\/g, '/').split('/').map(part => part.trim()).filter(Boolean).join('/');
}

function findatParseRepository(value) {
  const source = String(value || '').trim();
  if (!source) return { owner: '', repo: '' };
  try {
    const url = new URL(source.includes('://') ? source : `https://github.com/${source}`);
    const parts = url.pathname.replace(/^\/+|\/+$/g, '').split('/').filter(Boolean);
    return { owner: parts[0] || '', repo: String(parts[1] || '').replace(/\.git$/i, '') };
  } catch (_) {
...
function findatLoadGitHubConfig() {
  const defaults = window.FINDAT_GITHUB_DEFAULTS || {};
  let saved = {};
  try { saved = JSON.parse(localStorage.getItem(FINDAT_GITHUB_CONFIG_KEY) || '{}') || {}; } catch (_) { saved = {}; }
  const params = new URLSearchParams(location.search);
  const queryRepo = findatParseRepository(params.get('ghrepo') || '');
  const config = {
    owner: String(queryRepo.owner || saved.owner || defaults.owner || '').trim(),
    repo: String(queryRepo.repo || saved.repo || defaults.repo || '').trim().replace(/\.git$/i, ''),
    branch: String(params.get('ghbranch') || saved.branch || defaults.branch || 'main').trim() || 'main',
    root: findatSanitizeGitHubPath(params.get('ghroot') || saved.root || defaults.root || 'findat-cloud')
  };
  findatGitHubDrive.config = config;
  if (!findatGitHubDrive.token) {
    try { findatGitHubDrive.token = String(sessionStorage.getItem(FINDAT_GITHUB_SESSION_TOKEN_KEY) || '').trim(); }
    catch (_) { findatGitHubDrive.token = ''; }
  }
  return config;
}

function findatSaveGitHubConfig(config) {
  const clean = {
    owner: String(config.owner || '').trim(),
    repo: String(config.repo || '').trim().replace(/\.git$/i, ''),
    branch: String(config.branch || 'main').trim() || 'main',
    root: findatSanitizeGitHubPath(config.root || 'findat-cloud')
  };
  findatGitHubDrive.config = clean;
  try { localStorage.setItem(FINDAT_GITHUB_CONFIG_KEY, JSON.stringify(clean)); } catch (_) { /* repository settings remain usable for this page */ }
  return clean;
}

function findatGitHubConfigured() {
  const config = findatGitHubDrive.config || findatLoadGitHubConfig();
  return Boolean(config.owner && config.repo);
}

function findatGitHubRequire(write = false) {
  if (!findatGitHubConfigured()) throw new Error('Connect a GitHub repository in Settings → FINDAT Cloud');
  if (write && !findatGitHubDrive.token) throw new Error('Enter a fine-grained GitHub token with Contents read and write permission');
}

function findatGitHubRepoBase() {
  const { owner, repo } = findatGitHubDrive.config;
  return `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`;
}

function findatEncodeRepoPath(path) {
  return String(path || '').split('/').filter(Boolean).map(encodeURIComponent).join('/');
}

function findatVirtualToRepoPath(path) {
  const virtual = normalizeVirtualPath(path);
  const relative = virtual === '/' ? '' : virtual.slice(1);
  return [findatGitHubDrive.config?.root || '', relative].filter(Boolean).join('/');
}

function findatFolderMarkerPath(path) {
  return [findatVirtualToRepoPath(path), FINDAT_GITHUB_MARKER].filter(Boolean).join('/');
}

function findatGitHubHeaders(raw = false) {
  const headers = {
    Accept: raw ? 'application/vnd.github.raw+json' : 'application/vnd.github+json',
    'X-GitHub-Api-Version': FINDAT_GITHUB_API_VERSION
  };
  if (findatGitHubDrive.token) headers.Authorization = `Bearer ${findatGitHubDrive.token}`;
  return headers;
}

async function findatGitHubRequest(endpoint, options = {}) {
  const { method = 'GET', body, raw = false, allow404 = false } = options;
  const headers = findatGitHubHeaders(raw);
  ...
  let response;
  try {
    response = await fetch(`https://api.github.com${endpoint}`, request);
  } catch (_) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) throw new Error(detail || 'GitHub denied access or the API rate limit was reached');
    if (response.status === 404) throw new Error('GitHub repository, branch, or file was not found');
    if (response.status === 409) throw new Error('The GitHub file changed on another device. Refresh and try again');
    if (response.status === 422) throw new Error(detail || 'GitHub could not complete the file operation');
...
function findatSeedSystemNodes(target = new Map()) {
  const now = new Date().toISOString();
  if (!target.has('/')) target.set('/', { path: '/', parent: null, name: 'FINDAT Cloud', type: 'folder', size: 0, storedSize: 0, mime: 'inode/directory', modified: now, placeholder: true });
  for (const name of SYSTEM_FOLDERS) {
    const path = `/${name}`;
    if (!target.has(path)) target.set(path, { path, parent: '/', name, type: 'folder', size: 0, storedSize: 0, mime: 'inode/directory', modified: now, placeholder: true });
  }
  return target;
}

function findatFolderNode(path, extra = {}) {
  return {
    path,
    parent: path === '/' ? null : parentPath(path),
    name: path === '/' ? 'FINDAT Cloud' : basename(path),
    type: 'folder',
    size: 0,
    storedSize: 0,
    mime: 'inode/directory',
    modified: extra.modified || new Date().toISOString(),
    ...extra
  };
}

async function findatWalkGitHubDirectory(repoPath, virtualPath, target, prior) {
  const branch = encodeURIComponent(findatGitHubDrive.config.branch);
  const encoded = findatEncodeRepoPath(repoPath);
  const endpoint = `${findatGitHubRepoBase()}/contents${encoded ? `/${encoded}` : ''}?ref=${branch}`;
  const items = await findatGitHubRequest(endpoint, { allow404: true });
  if (items === null) return false;
  if (!Array.isArray(items)) throw new Error('The configured GitHub storage path is not a folder');

  const folder = target.get(virtualPath) || findatFolderNode(virtualPath, { repoPath });
  folder.repoPath = repoPath;
  folder.placeholder = false;
  target.set(virtualPath, folder);

  for (const item of items) {
    if (item.name === FINDAT_GITHUB_MARKER) {
      folder.markerSha = item.sha;
      continue;
    }
    const childVirtual = virtualPath === '/' ? `/${item.name}` : `${virtualPath}/${item.name}`;
    const previous = prior.get(childVirtual);
    if (item.type === 'dir') {
      target.set(childVirtual, findatFolderNode(childVirtual, {
        repoPath: item.path,
        sha: item.sha,
        placeholder: false,
        originalPath: previous?.originalPath
      }));
      await findatWalkGitHubDirectory(item.path, childVirtual, target, prior);
      continue;
    }
    if (item.type !== 'file' && item.type !== 'symlink') continue;
    target.set(childVirtual, {
      path: childVirtual,
      parent: virtualPath,
      name: item.name,
      type: 'file',
      size: Number(item.size) || 0,
      storedSize: Number(item.size) || 0,
      compression: '',
      compressionChecked: 1,
      mime: driveMime(item.name),
      modified: previous?.modified || new Date().toISOString(),
      sha: item.sha,
      repoPath: item.path,
...
async function findatBlobToBase64(blob) {
...
async function findatGitHubContentMetadata(repoPath) {
  const encoded = findatEncodeRepoPath(repoPath);
  const branch = encodeURIComponent(findatGitHubDrive.config.branch);
  return findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}?ref=${branch}`, { allow404: true });
}

async function findatGitHubPutFile(repoPath, blob, message, sha = '') {
  findatGitHubRequire(true);
  if (!(blob instanceof Blob)) blob = new Blob([blob || ''], { type: 'application/octet-stream' });
  if (blob.size > FINDAT_GITHUB_BROWSER_UPLOAD_LIMIT) {
    throw new Error(`This browser build accepts files up to ${formatBytes(FINDAT_GITHUB_BROWSER_UPLOAD_LIMIT)} per upload`);
  }
  const encoded = findatEncodeRepoPath(repoPath);
  const payload = {
    message: message || `Update ${repoPath}`,
    content: await findatBlobToBase64(blob),
    branch: findatGitHubDrive.config.branch
  };
  if (sha) payload.sha = sha;
  try {
    return await findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}`, { method: 'PUT', body: payload });
  } catch (error) {
    if (findatGitHubDrive.repoInfo?.size === 0 && !sha) {
      const initialPayload = { ...payload };
      delete initialPayload.branch;
      try {
        return await findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}`, { method: 'PUT', body: initialPayload });
      } catch (_) {
        throw new Error('The GitHub repository is empty. Add a README on GitHub, then connect again');
      }
    }
    if (!/changed on another device|could not complete/i.test(error.message)) throw error;
    const latest = await findatGitHubContentMetadata(repoPath);
    if (!latest?.sha) throw error;
    payload.sha = latest.sha;
    return findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}`, { method: 'PUT', body: payload });
  }
}

async function findatGitHubDeleteFile(repoPath, sha = '', message = '') {
  findatGitHubRequire(true);
  const metadata = sha ? { sha } : await findatGitHubContentMetadata(repoPath);
  if (!metadata?.sha) return;
  const encoded = findatEncodeRepoPath(repoPath);
  await findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}`, {
    method: 'DELETE',
    body: {
...
async function findatGitHubFetchBlob(repoPath) {
  findatGitHubRequire(false);
  const encoded = findatEncodeRepoPath(repoPath);
  const branch = encodeURIComponent(findatGitHubDrive.config.branch);
  return findatGitHubRequest(`${findatGitHubRepoBase()}/contents/${encoded}?ref=${branch}`, { raw: true });
}

async function findatWriteFolderMarker(path, existing = null) {
  if (path === '/' && !findatGitHubDrive.config.root) return findatFolderNode('/');
  const markerPath = findatFolderMarkerPath(path);
  const content = new Blob([JSON.stringify({ format: 'findat-folder-v1', path, updatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
  const result = await findatGitHubPutFile(markerPath, content, `FINDAT Cloud: create ${path}`, existing?.markerSha || '');
  return findatFolderNode(path, {
    repoPath: findatVirtualToRepoPath(path),
    markerSha: result?.content?.sha || existing?.markerSha || '',
    placeholder: false,
    originalPath: existing?.originalPath
  });
}

async function findatWriteGitHubNode(node) {
  if (!node || node.path === '/') return findatFolderNode('/');
  if (node.type === 'folder') {
    const existing = findatGitHubDrive.nodes.get(node.path);
    const saved = await findatWriteFolderMarker(node.path, existing);
    findatGitHubDrive.nodes.set(node.path, { ...saved, ...node, markerSha: saved.markerSha, repoPath: saved.repoPath, placeholder: false });
    return findatGitHubDrive.nodes.get(node.path);
  }

  let blob = node.blob instanceof Blob ? await restoreStoredBlob(node) : null;
  if (!blob && node.repoPath) blob = await findatGitHubFetchBlob(node.repoPath);
  if (!blob) {
    const source = findatGitHubDrive.nodes.get(node.path);
    if (source?.repoPath) blob = await findatGitHubFetchBlob(source.repoPath);
  }
  if (!blob) throw new Error(`Could not read ${node.name || basename(node.path)} for upload`);

  const destinationRepoPath = findatVirtualToRepoPath(node.path);
  const destination = findatGitHubDrive.nodes.get(node.path);
  const sha = destination?.repoPath === destinationRepoPath ? destination.sha || '' : '';
  const result = await findatGitHubPutFile(destinationRepoPath, blob, `FINDAT Cloud: save ${node.path}`, sha);
  const saved = {
    ...node,
    path: normalizeVirtualPath(node.path),
    ...
    compression: '',
    compressionChecked: 1,
    mime: node.mime || blob.type || driveMime(node.name || node.path),
    modified: new Date().toISOString(),
    sha: result?.content?.sha || sha,
    repoPath: result?.content?.path || destinationRepoPath,
    downloadUrl: result?.content?.download_url || '',
    blob: undefined
  };
  findatGitHubDrive.nodes.set(saved.path, saved);
  return saved;
}

async function findatDeleteGitHubNode(node) {
  if (!node || node.path === '/') return;
  if (node.type === 'file') {
    await findatGitHubDeleteFile(node.repoPath || findatVirtualToRepoPath(node.path), node.sha || '', `FINDAT Cloud: delete ${node.path}`);
    return;
  }
  if (node.markerSha) await findatGitHubDeleteFile(findatFolderMarkerPath(node.path), node.markerSha, `FINDAT Cloud: remove ${node.path}`);
}

async function findatLoadGitHubTree(force = false) {
  if (!findatGitHubConfigured()) {
    findatGitHubDrive.nodes = findatSeedSystemNodes(new Map());
    findatGitHubDrive.connected = false;
    findatGitHubDrive.writable = false;
    findatGitHubDrive.loadedAt = Date.now();
    fsState.cloudOnline = false;
    return [...findatGitHubDrive.nodes.values()];
  }
  if (!force && findatGitHubDrive.loadedAt && Date.now() - findatGitHubDrive.loadedAt < 15000) return [...findatGitHubDrive.nodes.values()];
  if (findatGitHubDrive.loading) return findatGitHubDrive.loading;

  findatGitHubDrive.loading = (async () => {
    findatGitHubRequire(false);
    const prior = findatGitHubDrive.nodes;
    const repoInfo = await findatGitHubRequest(findatGitHubRepoBase());
    findatGitHubDrive.repoInfo = repoInfo;
    if (!findatGitHubDrive.config.branch) findatGitHubDrive.config.branch = repoInfo.default_branch || 'main';

    let target = new Map();
    target.set('/', findatFolderNode('/', { repoPath: findatGitHubDrive.config.root, placeholder: false }));
    const rootRepoPath = findatGitHubDrive.config.root;
    let exists = await findatWalkGitHubDirectory(rootRepoPath, '/', target, prior);

    if (!exists && findatGitHubDrive.token) {
      await findatWriteFolderMarker('/', target.get('/'));
      target = new Map();
      target.set('/', findatFolderNode('/', { repoPath: rootRepoPath, placeholder: false }));
      exists = await findatWalkGitHubDirectory(rootRepoPath, '/', target, prior);
    }
    if (!exists && !findatGitHubDrive.token) {
      throw new Error('The GitHub storage folder does not exist. Connect with a write token to create it');
    }

    findatGitHubDrive.nodes = target;
    let created = false;
    for (const name of SYSTEM_FOLDERS) {
      const path = `/${name}`;
      if (findatGitHubDrive.nodes.has(path)) continue;
      if (findatGitHubDrive.token) {
        const saved = await findatWriteFolderMarker(path);
        findatGitHubDrive.nodes.set(path, saved);
        created = true;
      } else {
        findatGitHubDrive.nodes.set(path, findatFolderNode(path, { placeholder: true }));
      }
    }
    if (created) {
      target = new Map();
      target.set('/', findatFolderNode('/', { repoPath: rootRepoPath, placeholder: false }));
      await findatWalkGitHubDirectory(rootRepoPath, '/', target, prior);
      findatGitHubDrive.nodes = findatSeedSystemNodes(target);
    }

    findatGitHubDrive.connected = true;
    findatGitHubDrive.writable = Boolean(findatGitHubDrive.token);
    findatGitHubDrive.lastError = '';
    findatGitHubDrive.loadedAt = Date.now();
    fsState.cloudOnline = true;
    return [...findatGitHubDrive.nodes.values()];
  })().catch(error => {
    findatGitHubDrive.connected = false;
    findatGitHubDrive.lastError = error.message;
    fsState.cloudOnline = false;
    throw error;
  }).finally(() => { findatGitHubDrive.loading = null; });

  return findatGitHubDrive.loading;
}

openDriveDatabase = function() {
  return Promise.reject(new Error('Local browser document storage is disabled'));
};

databaseAll = async function() {
  await findatLoadGitHubTree(false);
  return [...findatGitHubDrive.nodes.values()];
};

databaseGet = async function(path) {
  await findatLoadGitHubTree(false);
  return findatGitHubDrive.nodes.get(normalizeVirtualPath(path));
};

databasePut = async function(node) {
  findatGitHubRequire(true);
  await findatLoadGitHubTree(false);
  return findatWriteGitHubNode(node);
};

databaseCommit = async function(puts = [], deletes = []) {
  findatGitHubRequire(true);
  await findatLoadGitHubTree(false);
  const before = new Map(findatGitHubDrive.nodes);
  const validPuts = puts.filter(node => node?.path && node.path !== '/').sort((a, b) => {
    if (a.type !== b.type) return a.type === 'folder' ? -1 : 1;
    return a.path.split('/').length - b.path.split('/').length;
  });
  for (const node of validPuts) await findatWriteGitHubNode(node);

  const protectedPaths = new Set(validPuts.map(node => normalizeVirtualPath(node.path)));
  const paths = [...new Set(deletes.map(normalizeVirtualPath))]
    .filter(path => path !== '/' && !protectedPaths.has(path))
    .sort((a, b) => b.split('/').length - a.split('/').length);
  for (const path of paths) {
    const node = before.get(path);
    if (node) await findatDeleteGitHubNode(node);
  }
  await findatLoadGitHubTree(true);
};

compressionSupported = function() { return false; };
compressBlobForStorage = async function(blobValue, name = '', mime = '') {
  const blob = blobValue instanceof Blob ? blobValue : new Blob([blobValue || ''], { type: mime || 'application/octet-stream' });
  return { blob, compression: '', storedSize: blob.size, originalSize: blob.size };
};
restoreStoredBlob = async function(node) {
  if (node?.blob instanceof Blob) return node.blob;
  if (node?.repoPath) return findatGitHubFetchBlob(node.repoPath);
  return new Blob([], { type: node?.mime || 'application/octet-stream' });
};
ensureBrowserCapacity = async function() {};
requestPersistentDriveStorage = async function() {};
browserStorageEstimate = async function() { return { usage: 0, quota: 0 }; };
migrateExistingDriveCompression = async function() { return { files: 0, saved: 0 }; };

initializeDrive = async function(force = false) {
  findatLoadGitHubConfig();
  if (!findatGitHubConfigured()) {
    findatGitHubDrive.nodes = findatSeedSystemNodes(new Map());
    findatGitHubDrive.connected = false;
    findatGitHubDrive.writable = false;
    findatGitHubDrive.loadedAt = Date.now();
    fsState.cloudOnline = false;
    setTimeout(() => toast('Connect GitHub in Settings → FINDAT Cloud before uploading documents'), 900);
    return true;
  }
  await findatLoadGitHubTree(Boolean(force));
  return true;
};
...
readLocalFile = async function(path) {
  const node = await databaseGet(normalizeVirtualPath(path));
  if (!node || node.type !== 'file') throw new Error('File not found');
  const blob = await findatGitHubFetchBlob(node.repoPath || findatVirtualToRepoPath(node.path));
  if (blob.type || !node.mime) return blob;
  return new Blob([await blob.arrayBuffer()], { type: node.mime });
};

localUsage = async function() {
  const all = await databaseAll();
  const used = all.reduce((sum, node) => sum + (node.type === 'file' ? Number(node.size) || 0 : 0), 0);
  return {
    used,
    logicalUsed: used,
    stored: used,
    ...
    browserQuota: 0,
    browserFree: 0,
    fileLimit: FINDAT_GITHUB_BROWSER_UPLOAD_LIMIT,
    repository: findatGitHubConfigured() ? `${findatGitHubDrive.config.owner}/${findatGitHubDrive.config.repo}` : ''
  };
};

refreshUsage = async function(root = document) {
  try {
    const usage = await localUsage();
    qsa('[data-storage-bar], [data-sidebar-usage], [data-cloud-used-bar]', root).forEach(bar => { bar.style.width = usage.logicalUsed ? '100%' : '0%'; });
    qsa('[data-storage-free-bar], [data-sidebar-free], [data-cloud-free-bar]', root).forEach(bar => { bar.style.width = '0%'; });
    qsa('[data-storage-label]', root).forEach(label => { label.textContent = `${formatBytes(usage.logicalUsed)} stored in GitHub`; });
    qsa('[data-sidebar-usage-label]', root).forEach(label => { label.textContent = `${formatBytes(usage.logicalUsed)} stored on GitHub`; });
    qsa('[data-used-space]', root).forEach(label => { label.textContent = formatBytes(usage.logicalUsed); });
    qsa('[data-free-space]', root).forEach(label => { label.textContent = 'GitHub'; });
  } catch (_) { /* connection status is shown separately */ }
};

exportDrive = async function() {
  try {
    findatGitHubRequire(false);
    toast('Preparing GitHub cloud export…');
    const nodes = await databaseAll();
    const exported = [];
    for (const node of nodes) {
      const item = { ...node };
      delete item.blob;
      delete item.sha;
      delete item.markerSha;
      delete item.repoPath;
      delete item.downloadUrl;
      delete item.htmlUrl;
      delete item.placeholder;
      if (item.type === 'file') item.data = await blobToDataUrl(await readLocalFile(item.path));
      exported.push(item);
    }
    const payload = new Blob([JSON.stringify({ format: 'aurelia-drive-v1', backend: 'github', exportedAt: new Date().toISOString(), nodes: exported })], { type: 'application/json' });
    const url = URL.createObjectURL(payload);
    const link = document.createElement('a');
    link.href = url;
    link.download = `FINDAT-GitHub-Cloud-${new Date().toISOString().slice(0, 10)}.findat.json`;
    link.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    toast('GitHub cloud export downloaded');
  } catch (error) { toast(error.message); }
};

setCloudBusy = function(busy, failed = false) {
  const indicator = qs('#cloudStatus');
  if (!indicator) return;
  const unavailable = failed || !findatGitHubDrive.connected;
  indicator.classList.toggle('syncing', busy);
  indicator.classList.toggle('offline', unavailable);
  indicator.textContent = unavailable ? '◌' : busy ? '↻' : '☁';
  if (!findatGitHubConfigured()) indicator.title = 'GitHub storage is not connected';
  else if (unavailable) indicator.title = findatGitHubDrive.lastError || 'GitHub storage is unavailable';
  else if (busy) indicator.title = 'Syncing files with GitHub';
  else indicator.title = `Connected to ${findatGitHubDrive.config.owner}/${findatGitHubDrive.config.repo}`;
};

function findatGitHubSettingsMarkup() {
  const config = findatGitHubDrive.config || findatLoadGitHubConfig();
  const access = findatGitHubDrive.token ? 'Write token active for this tab session' : 'No token — public repositories are read-only';
  return `<div class="settings-section" data-settings-panel="cloud">
    <div class="settings-heading"><div><h2>FINDAT Cloud</h2><p>Documents are stored in a GitHub repository, not in the browser.</p></div><span class="settings-heading-icon cloud-heading-logo"></span></div>
    <section class="settings-card cloud-overview-card">
      <div class="cloud-status-line"><span class="cloud-status-dot"></span><div><b data-cloud-status-title>Checking GitHub connection…</b><small data-cloud-status-detail>Repository files can be shared across devices.</small></div></div>
      <div class="cloud-stat-grid"><div><b data-cloud-used-space>—</b><small>Stored</small></div><div><b data-cloud-files>—</b><small>Files</small></div><div><b data-cloud-folders>—</b><small>Folders</small></div><div><b data-cloud-protection>—</b><small>Access</small></div></div>
    </section>
    <section class="settings-card github-storage-card">
      <b>GitHub repository connection</b>
      <p>Use one repository and branch on every device. Repository settings may be remembered locally; the token is kept only in this browser tab session and is restored after a page reload.</p>
      <div class="github-settings-grid">
        <label><span>Owner</span><input data-github-owner autocomplete="off" value="${escapeHtml(config.owner)}" placeholder="github-user-or-organisation"></label>
        <label><span>Repository</span><input data-github-repo autocomplete="off" value="${escapeHtml(config.repo)}" placeholder="findat-storage"></label>
        <label><span>Branch</span><input data-github-branch autocomplete="off" value="${escapeHtml(config.branch)}" placeholder="main"></label>
        <label><span>Storage folder</span><input data-github-root autocomplete="off" value="${escapeHtml(config.root)}" placeholder="findat-cloud"></label>
        <label class="github-token-field"><span>Fine-grained token</span><input data-github-token type="password" autocomplete="off" placeholder="${findatGitHubDrive.token ? 'Token active for this tab — enter to replace' : 'Contents: Read and write'}"></label>
      </div>
      <small class="github-security-note">Use a fine-grained token restricted to this repository. Never place a token in github-config.js or commit it to GitHub. Public repositories expose uploaded documents to everyone.</small>
      <div class="settings-button-row"><button data-github-action="connect">Connect and sync</button><button class="settings-secondary-button" data-github-action="disconnect">Disconnect</button></div>
      <small data-github-access>${access}</small>
    </section>
    <section class="settings-card"><b>Workspace</b><p>Drop files on the desktop, Finder window, FINDAT Cloud icon, or a folder. The destination is shown before upload and revealed after GitHub confirms it.</p><div class="settings-button-row"><button data-cloud-action="open">Open FINDAT Cloud</button><button data-cloud-action="upload">Upload Files</button><button class="settings-secondary-button" data-cloud-action="refresh">Refresh from GitHub</button></div></section>
    <section class="settings-card"><b>Copy files to this computer</b><p>Choose Download from a file menu, use the viewer Download button, or drag a prepared file from FINDAT Cloud onto the desktop when the browser supports drag-out downloads.</p></section>
    <section class="settings-card"><b>Backup and restore</b><p>Export a complete JSON backup or restore one into the connected GitHub folder.</p><div class="settings-button-row"><button data-cloud-action="export">Export Cloud Backup</button><button data-cloud-action="import">Import Cloud Backup</button></div></section>
    <section class="settings-card danger-settings-card"><b>Trash</b><p>Permanently remove all items currently in Trash from GitHub.</p><button data-cloud-action="empty-trash">Empty Trash</button></section>
  </div>`;
}

cloudSettingsMarkup = findatGitHubSettingsMarkup;

async function findatConnectFromSettings(win) {
  const panel = qs('[data-settings-panel="cloud"]', win);
  if (!panel) return;
  const config = findatSaveGitHubConfig({
    owner: qs('[data-github-owner]', panel)?.value,
    repo: qs('[data-github-repo]', panel)?.value,
    branch: qs('[data-github-branch]', panel)?.value,
    root: qs('[data-github-root]', panel)?.value
  });
  const enteredToken = String(qs('[data-github-token]', panel)?.value || '').trim();
  if (enteredToken) findatGitHubDrive.token = enteredToken;
  try {
    if (findatGitHubDrive.token) sessionStorage.setItem(FINDAT_GITHUB_SESSION_TOKEN_KEY, findatGitHubDrive.token);
    else sessionStorage.removeItem(FINDAT_GITHUB_SESSION_TOKEN_KEY);
  } catch (_) { /* token remains available in memory for this page */ }
  findatGitHubDrive.loadedAt = 0;
  findatGitHubDrive.connected = false;
  driveReadyPromise = initializeDrive(true);
  try {
    await driveReadyPromise;
    clearSelection();
    await refreshAllFileViews();
    await refreshCloudSettingsPanel(win);
    toast(`Connected to ${config.owner}/${config.repo}`);
  } catch (error) {
    toast(error.message);
    await refreshCloudSettingsPanel(win);
  }
...
function findatDisconnectGitHub(win) {
  findatGitHubDrive.token = '';
  try { sessionStorage.removeItem(FINDAT_GITHUB_SESSION_TOKEN_KEY); } catch (_) { /* already cleared in memory */ }
  findatSaveGitHubConfig({ owner: '', repo: '', branch: 'main', root: 'findat-cloud' });
  findatGitHubDrive.nodes = findatSeedSystemNodes(new Map());
  findatGitHubDrive.loadedAt = Date.now();
  findatGitHubDrive.connected = false;
  findatGitHubDrive.writable = false;
  fsState.cloudOnline = false;
  driveReadyPromise = Promise.resolve(true);
  refreshAllFileViews();
  renderSettingsSection(win, 'cloud');
  toast('GitHub storage disconnected');
}

refreshCloudSettingsPanel = async function(win) {
  const panel = qs('[data-settings-panel="cloud"]', win);
  if (!panel) return;
  const title = qs('[data-cloud-status-title]', panel);
  const detail = qs('[data-cloud-status-detail]', panel);
  try {
    if (!findatGitHubConfigured()) throw new Error('Enter a GitHub repository to connect FINDAT Cloud');
    await findatLoadGitHubTree(true);
    const nodes = [...findatGitHubDrive.nodes.values()];
    const files = nodes.filter(node => node.type === 'file').length;
    const folders = nodes.filter(node => node.type === 'folder').length;
    const used = nodes.reduce((sum, node) => sum + (node.type === 'file' ? Number(node.size) || 0 : 0), 0);
    qs('.cloud-status-dot', panel)?.classList.remove('error');
    if (title) title.textContent = 'GitHub cloud is connected';
    if (detail) detail.textContent = `${findatGitHubDrive.config.owner}/${findatGitHubDrive.config.repo} · ${findatGitHubDrive.config.branch} · /${findatGitHubDrive.config.root || ''}`;
    const usedSpace = qs('[data-cloud-used-space]', panel); if (usedSpace) usedSpace.textContent = formatBytes(used);
    const fileLabel = qs('[data-cloud-files]', panel); if (fileLabel) fileLabel.textContent = String(files);
    const folderLabel = qs('[data-cloud-folders]', panel); if (folderLabel) folderLabel.textContent = String(folders);
    const protection = qs('[data-cloud-protection]', panel); if (protection) protection.textContent = findatGitHubDrive.repoInfo?.private ? 'Private' : 'Public';
    const access = qs('[data-github-access]', panel); if (access) access.textContent = findatGitHubDrive.token ? 'Read and write access is active for this tab session' : 'Read-only access; enter a token to upload or change files';
  } catch (error) {
    qs('.cloud-status-dot', panel)?.classList.add('error');
    if (title) title.textContent = 'GitHub cloud needs attention';
    if (detail) detail.textContent = error.message;
    const access = qs('[data-github-access]', panel); if (access) access.textContent = findatGitHubDrive.token ? 'Token entered, but connection failed' : 'No write token is active';
  }
};

const findatRenderSettingsSectionBeforeGitHub = renderSettingsSection;
renderSettingsSection = function(win, section) {
  findatRenderSettingsSectionBeforeGitHub(win, section);
  if (section !== 'cloud') return;
  qsa('[data-github-action]', win).forEach(button => button.addEventListener('click', () => {
    if (button.dataset.githubAction === 'connect') findatConnectFromSettings(win);
    if (button.dataset.githubAction === 'disconnect') findatDisconnectGitHub(win);
  }));
  const refresh = qs('[data-cloud-action="refresh"]', win);
  refresh?.addEventListener('click', async () => {
    findatGitHubDrive.loadedAt = 0;
    try {
      await findatLoadGitHubTree(true);
      refreshAllFileViews();
      toast('FINDAT Cloud refreshed from GitHub');
    } catch (error) { toast(error.message); }
  });
};

const findatMakeEntryButtonBeforeDragOut = makeEntryButton;
makeEntryButton = function(entry, surface = 'finder') {
  const button = findatMakeEntryButtonBeforeDragOut(entry, surface);
  if (entry.type !== 'file') return button;
  button.draggable = true;
  button.addEventListener('pointerdown', () => {
    if (button._findatDragPreparing || button._findatDragUrl) return;
    button._findatDragPreparing = true;
    readLocalFile(entry.path).then(blob => {
      button._findatDragBlob = blob;
      button._findatDragUrl = URL.createObjectURL(blob);
      button.title = `${entry.name} · ${formatBytes(entry.size)} · Ready to drag to this computer`;
    }).catch(() => {}).finally(() => { button._findatDragPreparing = false; });
  });
  button.addEventListener('dragstart', event => {
    const url = button._findatDragUrl || (!findatGitHubDrive.repoInfo?.private ? entry.downloadUrl : '');
    if (url) {
      const mime = entry.mime || 'application/octet-stream';
      event.dataTransfer.setData('DownloadURL', `${mime}:${entry.name}:${url}`);
      event.dataTransfer.setData('text/uri-list', url);
    } else {
      setTimeout(() => toast('Preparing the file for drag-out. Drag it again, or choose Download'), 0);
    }
  });
  button.addEventListener('dragend', () => {
    if (!button._findatDragUrl) return;
    const url = button._findatDragUrl;
    button._findatDragUrl = '';
    button._findatDragBlob = null;
    setTimeout(() => URL.revokeObjectURL(url), 30000);
  });
  return button;
};

apps.about.html = () => `<div class="about"><div class="about-logo"><span class="aurelia-symbol"></span></div><h1>FINDAT Cloud</h1><p>Version 5.1 GitHub Cloud Visibility Edition</p><p>Documents are stored in a GitHub repository so multiple devices can access the same workspace.</p><small>Uploaded files are refreshed and revealed immediately. Browser storage is used only for interface preferences, repository settings, and the current tab token session; document contents remain on GitHub.</small></div>`;

findatLoadGitHubConfig();
setInterval(() => {
  if (document.visibilityState !== 'visible' || !findatGitHubConfigured() || Date.now() - findatGitHubDrive.loadedAt < FINDAT_GITHUB_REFRESH_MS) return;
  findatLoadGitHubTree(true).then(() => refreshAllFileViews()).catch(() => {});
}, FINDAT_GITHUB_REFRESH_MS);

/* End FINDAT Cloud GitHub repository backend ------------------------------ */
