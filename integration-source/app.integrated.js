...
(()=>{
      ...
      let accountCache=[];
      let adminRegistryUsers=[];
      let signupProfilePhotoFile=null;
      let signupProfilePhotoPreviewUrl='';
      let activeProfileViewId='';
      let socialAnalyticsArticleId='';
      const socialImpressionKeys=new Set();
      ...
      let socialCache={reactions:[],bookmarks:[],follows:[],comments:[],messages:[],events:[],trending:[]};
      ...
      function profileFromRow(row){
        ...
        return{id:row.id,displayName,firstName:row.first_name||'',lastName:row.last_name||'',email:row.email||'',username:row.username||'',phone:row.phone||'',organisation:row.organisation||'',role:normaliseRole(row.role),country:row.country||'',region:row.region||'',avatarUrl:row.avatar_url||'',coverUrl:row.cover_url||'',bio:row.bio||'',industry:row.industry||'',qualifications:row.qualifications||'',jobTitle:row.job_title||'',placeOfWork:row.place_of_work||'',active:row.active!==false,emailConfirmed:row.emailConfirmed===true||Boolean(row.email_confirmed_at),emailConfirmedAt:row.email_confirmed_at||'',lastSignInAt:row.last_sign_in_at||'',createdAt:row.created_at||'',updatedAt:row.updated_at||''}
      }
      ...
      function resizeCoverFile(file){
        return new Promise((resolve,reject)=>{
          const reader=new FileReader();reader.onerror=()=>reject(new Error('The cover picture could not be read.'));
          reader.onload=()=>{const image=new Image();image.onerror=()=>reject(new Error('Choose a valid cover image.'));image.onload=()=>{const width=1280,height=420,canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const context=canvas.getContext('2d'),scale=Math.max(width/image.naturalWidth,height/image.naturalHeight),sourceWidth=width/scale,sourceHeight=height/scale,sx=(image.naturalWidth-sourceWidth)/2,sy=(image.naturalHeight-sourceHeight)/2;context.drawImage(image,sx,sy,sourceWidth,sourceHeight,0,0,width,height);canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('The cover picture could not be prepared.')),'image/jpeg',.84)};image.src=String(reader.result)};reader.readAsDataURL(file)
        })
      }
      ...
      function updateCachedCover(url){activeAccount.coverUrl=url;accountCache=accountCache.map(item=>item.id===activeAccount.id?{...item,coverUrl:url}:item);directoryCache=directoryCache.map(item=>item.id===activeAccount.id?{...item,coverUrl:url}:item);renderProfilePhotoModal();renderAll()}
      async function uploadProfileCover(file){
        if(!file||!activeAccount)return;if(file.size>16*1024*1024){setStatus('fdProfilePhotoStatus','Choose a cover image smaller than 16 MB.','error');return}
        setStatus('fdProfilePhotoStatus','Preparing and uploading your cover picture…','');
        try{const blob=await resizeCoverFile(file),path=profileCoverPath();let url='',savedInStorage=false;try{const upload=await supabaseClient.storage.from('findat-documents').upload(path,blob,{upsert:true,contentType:'image/jpeg',cacheControl:'3600'});if(upload.error)throw upload.error;const publicData=supabaseClient.storage.from('findat-documents').getPublicUrl(path).data;if(!publicData?.publicUrl)throw new Error('The cover picture address was not returned.');url=`${publicData.publicUrl}?v=${Date.now()}`;savedInStorage=true}catch(error){console.warn('Profile cover storage fallback used.',error);url=await blobAsDataURL(blob)}const savedUrl=await persistProfileCover(url);updateCachedCover(savedUrl);setStatus('fdProfilePhotoStatus',savedInStorage?'Cover picture saved across FINDAT.':'Cover picture saved using the available profile storage.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The cover picture could not be uploaded.','error')}finally{if($('#fdProfileCoverInput'))$('#fdProfileCoverInput').value=''}
      }
      async function removeProfileCover(){if(!activeAccount)return;setStatus('fdProfilePhotoStatus','Removing cover picture…','');try{try{await supabaseClient.storage.from('findat-documents').remove([profileCoverPath()])}catch(error){console.warn(error)}const result=await supabaseClient.rpc('findat_clear_profile_cover');if(result.error)throw result.error;updateCachedCover('');setStatus('fdProfilePhotoStatus','Cover picture removed.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The cover picture could not be removed.','error')}}
      function renderProfilePhotoModal(){const preview=$('#fdProfilePhotoPreview'),cover=$('#fdProfileCoverPreview');if(!preview||!activeAccount)return;preview.innerHTML=activeAccount.avatarUrl?`<img src="${escapeHTML(activeAccount.avatarUrl)}" alt="${escapeHTML(accountName(activeAccount))} profile picture">`:escapeHTML(initials(activeAccount));if(cover){cover.style.backgroundImage=activeAccount.coverUrl?`linear-gradient(90deg,rgba(8,24,45,.12),rgba(8,24,45,.12)),url("${escapeHTML(activeAccount.coverUrl)}")`:'';cover.classList.toggle('has-image',Boolean(activeAccount.coverUrl))}if($('#fdRemoveProfilePhoto'))$('#fdRemoveProfilePhoto').disabled=!activeAccount.avatarUrl;if($('#fdRemoveProfileCover'))$('#fdRemoveProfileCover').disabled=!activeAccount.coverUrl;setStatus('fdProfilePhotoStatus','')}
      ...
      const WRITING_MEMORY_KEY='findat_x1_writing_memory_v1';
      function loadWritingMemory(){return safeParse(localStorage.getItem(WRITING_MEMORY_KEY),{samples:0,averageWords:0,averageSentenceWords:0,averageParagraphWords:0,categories:{},headings:{},keywords:{},updatedAt:''})}
      function writeStyleTokens(text){return normal(stripHTML(text)).split(/[^a-z0-9]+/).filter(word=>word.length>4&&!['about','which','their','there','these','would','could','should','because','where','while','through','financial','findat'].includes(word))}
      function learnWritingPattern(article,account=activeAccount){
        if(!article||!account||!stripHTML(article.content))return;const current=loadWritingMemory(),plain=stripHTML(article.content),words=plain.split(/\s+/).filter(Boolean),sentences=plain.split(/[.!?]+/).map(x=>x.trim()).filter(Boolean),paragraphs=String(article.content||'').split(/<\/?p[^>]*>|<br\s*\/?>/i).map(stripHTML).filter(Boolean),headings=[...String(article.content||'').matchAll(/<h[1-4][^>]*>(.*?)<\/h[1-4]>/gi)].map(match=>stripHTML(match[1])).filter(Boolean),counts={};writeStyleTokens(plain).forEach(token=>counts[token]=(counts[token]||0)+1);const n=Math.min(200,(current.samples||0)+1),blend=(oldValue,newValue)=>Number((((Number(oldValue)||0)*(n-1)+newValue)/n).toFixed(2));current.samples=n;current.averageWords=blend(current.averageWords,words.length);current.averageSentenceWords=blend(current.averageSentenceWords,words.length/Math.max(1,sentences.length));current.averageParagraphWords=blend(current.averageParagraphWords,words.length/Math.max(1,paragraphs.length));current.categories=current.categories||{};current.categories[article.category||'Article']=(current.categories[article.category||'Article']||0)+1;current.headings=current.headings||{};headings.slice(0,12).forEach(heading=>{const key=heading.slice(0,80);current.headings[key]=(current.headings[key]||0)+1});current.keywords=current.keywords||{};Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,16).forEach(([word,count])=>current.keywords[word]=(current.keywords[word]||0)+count);current.updatedAt=nowISO();localStorage.setItem(WRITING_MEMORY_KEY,JSON.stringify(current));window.dispatchEvent(new CustomEvent('findat-writing-memory-updated'))
      }
      function writingMemorySummary(){const memory=loadWritingMemory(),top=obj=>Object.entries(obj||{}).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([key])=>key);if(!memory.samples)return'No user-approved publication patterns have been learned yet.';return`Learned from ${memory.samples} saved publication${memory.samples===1?'':'s'}: average ${Math.round(memory.averageWords||0)} words, ${Math.round(memory.averageSentenceWords||0)} words per sentence and ${Math.round(memory.averageParagraphWords||0)} words per paragraph. Common categories: ${top(memory.categories).join(', ')||'not established'}. Recurring vocabulary: ${top(memory.keywords).join(', ')||'not established'}.`}
      function writingKnowledgeUnits(){const memory=loadWritingMemory();if(!memory.samples)return[];return[{id:'adaptive-writing-memory',title:'User-approved writing patterns',tags:['writing style','publications','adaptive memory','author voice'],text:writingMemorySummary(),points:['This memory contains aggregate style signals, not passwords or hidden prompts.','It updates after the signed-in user saves or publishes work.','Use it as guidance while preserving factual accuracy and professional standards.'],source:'FINDAT adaptive writing memory',kind:'adaptive'}]}
      window.FindatWritingMemory={learn:learnWritingPattern,summary:writingMemorySummary,knowledgeUnits:writingKnowledgeUnits,clear:()=>{localStorage.removeItem(WRITING_MEMORY_KEY);window.dispatchEvent(new CustomEvent('findat-writing-memory-updated'))}};
      ...
      async function refreshSocialState({quiet=false}={}){
        ...
        if(firstError){if(socialSetupError(firstError)){socialAvailable=false;socialError='Run FINDAT-WRITING-DESK-SOCIAL-WORKSPACE-UPGRADE.sql in Supabase to activate reactions, discussions, follows and private messages.';if(!quiet)console.warn('FINDAT social workspace is awaiting its database upgrade.',firstError)}else{socialError=firstError.message||'The social workspace could not refresh.';if(!quiet)console.warn('FINDAT social refresh failed',firstError)}renderSocialWorkspace();return}
        let events=[],trending=[];
        try{const ownIds=workspaceCache.articles.filter(article=>(article.ownerId||article.createdBy)===activeAccount.id||isAdmin()).map(article=>article.id);if(ownIds.length){const result=await supabaseClient.from('findat_article_events').select('article_id,actor_id,event_type,event_day,created_at').in('article_id',ownIds).limit(5000);if(!result.error)events=result.data||[]}}catch(error){if(!quiet)console.warn('Publication analytics events are not deployed yet.',error)}
        try{const result=await supabaseClient.rpc('findat_trending_articles',{p_limit:100});if(!result.error)trending=result.data||[]}catch(error){if(!quiet)console.warn('Trending RPC is not deployed yet.',error)}
        socialAvailable=true;socialError='';socialCache={reactions:results[0].data||[],bookmarks:results[1].data||[],follows:results[2].data||[],comments:results[3].data||[],messages:results[4].data||[],events,trending};renderSocialWorkspace()
      }
      ...
      function socialEventCount(articleId,type){return socialCache.events.filter(row=>row.article_id===articleId&&(!type||row.event_type===type)).length}
      function socialTrendRow(articleId){return socialCache.trending.find(row=>row.article_id===articleId)||null}
      function socialTrendScore(article){const remote=socialTrendRow(article.id);if(remote)return Number(remote.score||0);const ageHours=Math.max(0,(Date.now()-new Date(article.publishedAt||article.updatedAt||article.createdAt||Date.now()).getTime())/36e5),engagement=socialReactionCount(article.id,'like')*2.4+socialReactionCount(article.id,'repost')*4.5+socialCommentCount(article.id)*3.8+Number(socialIsBookmarked(article.id))*1.2;return Number((engagement/Math.pow(ageHours+2,0.58)).toFixed(4))}
      async function recordSocialEvent(articleId,eventType,metadata={}){if(!supabaseClient||!activeAccount||!articleId)return;const key=`${articleId}:${eventType}:${new Date().toISOString().slice(0,10)}`;if(['impression','view'].includes(eventType)&&socialImpressionKeys.has(key))return;socialImpressionKeys.add(key);try{await supabaseClient.rpc('findat_record_article_event',{p_article_id:articleId,p_event_type:eventType,p_session_id:sessionStorage.getItem('findat-social-session')||(()=>{const id=uid();sessionStorage.setItem('findat-social-session',id);return id})(),p_metadata:metadata})}catch(error){console.warn('Social event analytics are awaiting deployment.',error)}}
      function queueFeedImpressions(articles){articles.filter(article=>article.status==='Published').slice(0,30).forEach(article=>setTimeout(()=>recordSocialEvent(article.id,'impression',{surface:'feed'}),180))}
      function profileHeadline(profile){return [profile?.jobTitle,profile?.organisation||profile?.placeOfWork,profile?.country].filter(Boolean).join(' · ')||roleLabel(profile?.role)}
      function openMemberProfile(userId){const profile=socialProfileById(userId);if(!profile)return;activeProfileViewId=userId;const followers=socialCache.follows.filter(row=>row.following_id===userId).length,following=socialCache.follows.filter(row=>row.follower_id===userId).length,posts=workspaceCache.articles.filter(article=>article.status==='Published'&&(article.ownerId||article.createdBy)===userId).sort((a,b)=>new Date(b.publishedAt||b.updatedAt)-new Date(a.publishedAt||a.updatedAt));const avatar=$('#fdMemberProfileAvatar'),cover=$('#fdMemberProfileCover');if(avatar)avatar.innerHTML=profile.avatarUrl?`<img src="${escapeHTML(profile.avatarUrl)}" alt="">`:escapeHTML(initials(profile));if(cover){cover.style.backgroundImage=profile.coverUrl?`linear-gradient(90deg,rgba(6,23,43,.15),rgba(6,23,43,.15)),url("${escapeHTML(profile.coverUrl)}")`:'';cover.classList.toggle('has-image',Boolean(profile.coverUrl))}$('#fdMemberProfileName').textContent=accountName(profile);$('#fdMemberProfileHeadline').textContent=profileHeadline(profile);$('#fdMemberProfileBio').textContent=profile.bio||'This member has not added a professional summary.';$('#fdMemberProfileMetrics').innerHTML=`<span><strong>${followers}</strong> Followers</span><span><strong>${following}</strong> Following</span><span><strong>${posts.length}</strong> Publications</span>`;$('#fdMemberProfileDetails').innerHTML=[['Industry',profile.industry],['Organisation',profile.organisation||profile.placeOfWork],['Region',profile.region],['Country',profile.country],['Qualifications',profile.qualifications]].filter(([,value])=>value).map(([label,value])=>`<div><small>${escapeHTML(label)}</small><strong>${escapeHTML(value)}</strong></div>`).join('')||'<p>No additional professional details have been supplied.</p>';$('#fdMemberProfileActions').innerHTML=userId===activeAccount.id?'<button class="fd-secondary-btn" data-profile-edit-self type="button"><i class="fas fa-user-edit"></i> Edit profile</button>':`<button class="fd-secondary-btn ${socialIsFollowing(userId)?'is-following':''}" data-social-follow="${userId}" type="button"><i class="fas ${socialIsFollowing(userId)?'fa-user-check':'fa-user-plus'}"></i> ${socialIsFollowing(userId)?'Following':'Follow'}</button><button class="fd-secondary-btn" data-social-message="${userId}" type="button"><i class="fas fa-comment-dots"></i> Message</button>`;$('#fdMemberProfilePosts').innerHTML=posts.length?posts.slice(0,8).map(article=>`<button data-article-preview="${article.id}" type="button"><strong>${escapeHTML(article.title)}</strong><small>${escapeHTML(formatDate(article.publishedAt))} · ${escapeHTML(article.category||'Article')}</small></button>`).join(''):'<p>No published work yet.</p>';window.toggleModal?.('fdMemberProfileModal',true);recordSocialEvent(posts[0]?.id||'', 'profile_view',{profile_id:userId})}
      const countryMap={zambia:[53,63],southafrica:[55,78],nigeria:[47,55],kenya:[58,59],ghana:[45,56],egypt:[55,44],unitedstates:[21,43],canada:[22,29],brazil:[33,69],unitedkingdom:[48,32],france:[49,36],germany:[52,34],india:[69,51],china:[78,43],japan:[88,44],australia:[85,76],unitedarabemirates:[63,50]};
      function analyticsBreakdown(rows,keyGetter){const counts={};rows.forEach(row=>{const key=String(keyGetter(row)||'Not supplied').trim()||'Not supplied';counts[key]=(counts[key]||0)+1});const total=Math.max(1,rows.length);return Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([key,count])=>`<div><span>${escapeHTML(key)}</span><i><b style="width:${(count/total*100).toFixed(1)}%"></b></i><strong>${count}</strong></div>`).join('')||'<p>No audience profile data yet.</p>'}
      function renderAudienceMap(followers){const points=followers.map(profile=>{const key=normal(profile.country).replace(/[^a-z]/g,''),point=countryMap[key];return point?`<circle cx="${point[0]}" cy="${point[1]}" r="2.2"><title>${escapeHTML(profile.country)}</title></circle>`:''}).join('');return`<svg viewBox="0 0 100 100" role="img" aria-label="Follower location map"><path d="M8 31L24 23 36 31 31 45 18 50 10 42ZM31 54L42 50 46 65 39 86 30 72ZM44 25L57 21 66 31 59 41 47 38ZM48 44L63 43 66 64 57 82 49 67ZM65 28L88 26 95 43 83 55 70 50ZM75 65L94 67 91 84 78 82Z"></path>${points}</svg>`}
      function openSocialAnalytics(articleId=''){socialAnalyticsArticleId=articleId||'';const owned=workspaceCache.articles.filter(article=>(isAdmin()||(article.ownerId||article.createdBy)===activeAccount.id)&&(!articleId||article.id===articleId)),followers=socialCache.follows.filter(row=>row.following_id===activeAccount.id).map(row=>socialProfileById(row.follower_id)).filter(Boolean),totals=owned.reduce((sum,article)=>{const remote=socialTrendRow(article.id)||{};sum.views+=Number(remote.views||socialEventCount(article.id,'view'));sum.impressions+=Number(remote.impressions||socialEventCount(article.id,'impression'));sum.likes+=socialReactionCount(article.id,'like');sum.comments+=socialCommentCount(article.id);sum.saves+=Number(remote.bookmarks||socialCache.bookmarks.filter(row=>row.article_id===article.id).length);sum.reposts+=socialReactionCount(article.id,'repost');return sum},{views:0,impressions:0,likes:0,comments:0,saves:0,reposts:0});$('#fdAnalyticsSummary').innerHTML=[['Publications',owned.length],['Views',totals.views],['Impressions',totals.impressions],['Likes',totals.likes],['Comments',totals.comments],['Followers',followers.length]].map(([label,value])=>`<div><small>${label}</small><strong>${Number(value).toLocaleString()}</strong></div>`).join('');$('#fdAnalyticsPostTable').innerHTML=owned.length?`<table><thead><tr><th>Publication</th><th>Views</th><th>Likes</th><th>Comments</th><th>Saves</th><th>Trend</th></tr></thead><tbody>${owned.sort((a,b)=>socialTrendScore(b)-socialTrendScore(a)).map(article=>{const remote=socialTrendRow(article.id)||{};return`<tr><td><strong>${escapeHTML(article.title)}</strong><small>${escapeHTML(formatDate(article.publishedAt||article.updatedAt))}</small></td><td>${Number(remote.views||socialEventCount(article.id,'view')).toLocaleString()}</td><td>${socialReactionCount(article.id,'like')}</td><td>${socialCommentCount(article.id)}</td><td>${Number(remote.bookmarks||socialCache.bookmarks.filter(row=>row.article_id===article.id).length)}</td><td>${socialTrendScore(article).toFixed(2)}</td></tr>`}).join('')}</tbody></table>`:'<p>No owned publications are available for analytics.</p>';$('#fdAudienceMap').innerHTML=renderAudienceMap(followers);$('#fdFollowerLocationBreakdown').innerHTML=analyticsBreakdown(followers,profile=>[profile.country,profile.region].filter(Boolean).join(' · '));$('#fdFollowerOrganisationBreakdown').innerHTML=analyticsBreakdown(followers,profile=>profile.organisation||profile.placeOfWork||profile.industry);setStatus('fdSocialAnalyticsStatus',followers.length?'Audience distribution uses profile information voluntarily supplied by followers.':'Follower analytics will appear as your audience grows.','');window.toggleModal?.('fdSocialAnalyticsModal',true)}
      function openX1AgentWithContext(text){
        const context=String(text||'').trim();
        const open=window.FINDAT_OPEN_X1;
        if(typeof open==='function')open();else document.querySelector('[data-open-x1-app]')?.click();
        setTimeout(()=>{
          const input=document.getElementById('x1PromptInput'),form=document.getElementById('x1FintechComposer');
          if(!input)return;
          input.value=context;input.dispatchEvent(new Event('input',{bubbles:true}));input.focus();
          if(context&&form&&!form.dataset.x1Submitting){form.dataset.x1Submitting='true';form.requestSubmit();setTimeout(()=>delete form.dataset.x1Submitting,500)}
        },220);
      }
      ...
      function renderSocialPeople(limit=5){
        ...
        host.innerHTML=people.length?people.map(person=>`<div class="fd-social-person-row" data-social-profile="${person.id}">${socialAvatarMarkup(person,'fd-avatar')}<div><strong>${escapeHTML(accountName(person))}</strong><span>${escapeHTML(person.jobTitle||person.organisation||roleLabel(person.role))}</span>${socialPresenceBadge(person.id)}</div><div class="fd-social-person-actions"><button class="${socialIsFollowing(person.id)?'is-following':''}" data-social-follow="${person.id}" title="${socialIsFollowing(person.id)?'Unfollow':'Follow'}" type="button"><i class="fas ${socialIsFollowing(person.id)?'fa-user-check':'fa-user-plus'}"></i></button><button data-social-message="${person.id}" title="Message" type="button"><i class="fas fa-comment-dots"></i></button></div></div>`).join(''):'<div class="fd-empty-compact"><i class="fas fa-users"></i>No other active members are available.</div>'
      }
      function socialPostMarkup(article){
        const owner=socialProfileById(article.ownerId||article.createdBy)||articleOwner(article),following=owner?.id&&owner.id!==activeAccount?.id&&socialIsFollowing(owner.id),published=article.status==='Published',excerpt=(article.subtitle||stripHTML(article.content)||'').slice(0,360),likes=socialReactionCount(article.id,'like'),reposts=socialReactionCount(article.id,'repost'),comments=socialCommentCount(article.id),canEdit=socialCanEditArticle(article),trend=socialTrendScore(article),rank=[...workspaceCache.articles.filter(row=>row.status==='Published')].sort((a,b)=>socialTrendScore(b)-socialTrendScore(a)).findIndex(row=>row.id===article.id),trending=published&&rank>=0&&rank<5&&trend>0;
        return `<article class="fd-social-post ${published?'':'is-private'}" data-social-post="${article.id}"><header class="fd-social-post-head"><button class="fd-social-profile-trigger" data-social-profile="${owner?.id||''}" type="button">${socialAvatarMarkup(owner,'fd-avatar fd-social-post-avatar')}</button><div class="fd-social-post-author"><button class="fd-social-author-button" data-social-profile="${owner?.id||''}" type="button"><div class="fd-social-post-author-line"><strong>${escapeHTML(accountName(owner))}</strong><small>${owner?.username?`@${escapeHTML(owner.username)}`:escapeHTML(roleLabel(owner?.role))}</small></div><p>${escapeHTML(profileHeadline(owner))} · ${escapeHTML(formatTime(article.publishedAt||article.updatedAt||article.createdAt))}</p></button></div>${owner?.id&&owner.id!==activeAccount?.id?`<button class="fd-social-post-follow ${following?'is-following':''}" data-social-follow="${owner.id}" type="button">${following?'Following':'Follow'}</button>`:''}</header><div class="fd-social-post-body"><div class="fd-social-post-labels"><span class="fd-social-post-category"><i class="fas ${published?'fa-globe-africa':'fa-lock'}"></i>${escapeHTML(article.category||'Article')} · ${escapeHTML(published?'Published':article.status||'Draft')}</span>${trending?`<span class="fd-trending-badge"><i class="fas fa-arrow-trend-up"></i> Trending #${rank+1}</span>`:''}</div><h4>${escapeHTML(article.title||'Untitled article')}</h4><p class="fd-social-post-copy">${escapeHTML(excerpt)}${(article.subtitle||stripHTML(article.content)||'').length>360?'…':''}</p>${article.image?.data?`<div class="fd-social-post-media" data-article-preview="${article.id}"><img src="${article.image.data}" alt="${escapeHTML(article.title||'Article cover')}"></div>`:''}<div class="fd-social-post-meta"><span>${likes} like${likes===1?'':'s'} · ${comments} comment${comments===1?'':'s'} · ${reposts} repost${reposts===1?'':'s'}</span><span>${escapeHTML(contributorSummary(article))}</span></div><div class="fd-social-post-actions"><button class="${socialHasReaction(article.id,'like')?'is-active':''}" data-social-like="${article.id}" type="button"><i class="fas fa-thumbs-up"></i><span>Like</span><b>${likes||''}</b></button><button data-social-comment="${article.id}" type="button"><i class="fas fa-comment"></i><span>Comment</span><b>${comments||''}</b></button><button class="${socialHasReaction(article.id,'repost')?'is-active':''}" data-social-repost="${article.id}" type="button"><i class="fas fa-retweet"></i><span>Repost</span><b>${reposts||''}</b></button><button class="${socialIsBookmarked(article.id)?'is-active':''}" data-social-bookmark="${article.id}" type="button"><i class="fas fa-bookmark"></i><span>Save</span></button><button data-social-share="${article.id}" type="button"><i class="fas fa-share-alt"></i><span>Share</span></button></div>${published?`<div class="fd-library-item-actions"><button class="fd-mini-btn" data-article-preview="${article.id}" type="button">Read article</button>${(isAdmin()||(article.ownerId||article.createdBy)===activeAccount.id)?`<button class="fd-mini-btn" data-social-analytics="${article.id}" type="button"><i class="fas fa-chart-line"></i> Analytics</button>`:''}${canEdit?`<button class="fd-mini-btn" data-article-open-editor="${article.id}" type="button">Edit</button>`:''}</div>`:`<div class="fd-social-post-private-note"><i class="fas fa-shield-alt"></i>This work is visible only to authorised participants.${canEdit?` <button class="fd-mini-btn" data-article-open-editor="${article.id}" type="button">Continue writing</button>`:''}</div>`}</div></article>`
      }
      function renderSocialNetworkDirectory(){
        const host=$('#fdSocialFeed');if(!host)return;const query=normal($('#fdDashboardSearch')?.value);const people=accountCache.filter(person=>person.id!==activeAccount?.id&&person.active!==false).filter(person=>!query||normal(`${accountName(person)} ${person.username} ${person.organisation} ${person.jobTitle}`).includes(query)).sort((a,b)=>Number(socialPresenceState(b.id)==='online')-Number(socialPresenceState(a.id)==='online')||Number(socialIsFollowing(b.id))-Number(socialIsFollowing(a.id))||accountName(a).localeCompare(accountName(b)));
        host.innerHTML=people.length?`<div class="fd-social-network-directory">${people.map(person=>`<article class="fd-social-network-profile" data-social-profile="${person.id}">${socialAvatarMarkup(person,'fd-avatar')}<div><h4>${escapeHTML(accountName(person))}</h4><p>${escapeHTML(person.jobTitle||roleLabel(person.role))}${person.organisation?` · ${escapeHTML(person.organisation)}`:''}</p>${socialPresenceBadge(person.id)}</div><div class="fd-social-network-profile-actions"><button class="${socialIsFollowing(person.id)?'is-following':''}" data-social-follow="${person.id}" type="button"><i class="fas ${socialIsFollowing(person.id)?'fa-user-check':'fa-user-plus'}"></i> ${socialIsFollowing(person.id)?'Following':'Follow'}</button><button data-social-message="${person.id}" type="button"><i class="fas fa-comment-dots"></i> Message</button></div></article>`).join('')}</div>`:'<div class="fd-social-feed-empty"><i class="fas fa-user-friends"></i>No members match this search.</div>'
      }
      function renderSocialFeed(){
        ...
        host.innerHTML=articles.length?articles.map(socialPostMarkup).join(''):`<div class="fd-social-feed-empty"><i class="fas ${socialActiveTab==='saved'?'fa-bookmark':'fa-newspaper'}"></i>${socialActiveTab==='saved'?'No saved publications yet.':'No posts are available in this view.'}</div>`;if(socialActiveTab==='feed')queueFeedImpressions(articles)
      }
      ...
      function renderSocialChatThread(){
        const host=$('#fdSocialChatMessages'),person=socialProfileById(socialChatUserId),input=$('#fdSocialChatInput'),send=$('#fdSocialChatSend');if(!host)return;if(!person){$('#fdSocialChatTitle').textContent='Select a conversation';$('#fdSocialChatPresence').textContent='Choose a FINDAT member to begin.';$('#fdSocialChatPresence').className='';host.innerHTML='<div class="fd-social-chat-empty"><i class="fas fa-comments"></i><strong>Your FINDAT messages</strong><span>Private conversations are visible only to the participants.</span></div>';input.disabled=true;send.disabled=true;return}const header=$('#fdSocialChatHeader');const avatar=header.querySelector('.fd-avatar');if(avatar)avatar.outerHTML=socialAvatarMarkup(person,'fd-avatar');const typing=Boolean(socialTypingByUser.get(person.id)),presence=socialPresenceState(person.id),detail=person.jobTitle||person.organisation||roleLabel(person.role);$('#fdSocialChatTitle').textContent=accountName(person);const presenceEl=$('#fdSocialChatPresence');presenceEl.textContent=typing?`${accountName(person)} is typing…`:`${socialPresenceLabel(person.id)}${detail?` · ${detail}`:''}`;presenceEl.className=`is-${typing?'typing':presence}`;input.disabled=false;send.disabled=false;const messages=socialCache.messages.filter(row=>(row.sender_id===activeAccount.id&&row.recipient_id===person.id)||(row.recipient_id===activeAccount.id&&row.sender_id===person.id)).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));const messageHTML=messages.length?messages.map(row=>`<div class="fd-social-chat-message ${row.sender_id===activeAccount.id?'is-mine':''}"><div class="fd-social-chat-bubble">${escapeHTML(row.body||'')}<time>${escapeHTML(formatTime(row.created_at))}${row.sender_id===activeAccount.id&&row.read_at?' · Read':''}</time></div></div>`).join(''):'<div class="fd-social-chat-empty"><i class="fas fa-comment-medical"></i><strong>Start the conversation</strong><span>Send a private message to this FINDAT member.</span></div>';const typingHTML=typing?`<div class="fd-social-chat-message is-typing" aria-live="polite" aria-label="${escapeHTML(accountName(person))} is typing"><div class="fd-social-chat-bubble fd-social-typing-bubble"><span></span><span></span><span></span></div></div>`:'';host.innerHTML=messageHTML+typingHTML;host.scrollTop=host.scrollHeight
      }
      ...
      async function saveArticle(status='Draft'){
        ...
        const auditAction=collaborativeEdit?'Collaborative editorial changes saved':status==='Published'?'Article published by administrator':status==='Pending approval'?'Article submitted for approval':'Article draft saved';await addAudit(auditAction,converted.title,converted.id);if((converted.ownerId||converted.createdBy)===activeAccount.id)learnWritingPattern(converted,activeAccount);
      ...
      function openReader(id,{updateHash=true}={}){
        ...
        currentReaderArticleId=id;if(article.status==='Published')recordSocialEvent(article.id,'view',{surface:document.body.classList.contains('dashboard-mode')?'dashboard-reader':'public-reader'});
      ...
      $('#signupProfilePhoto')?.addEventListener('change',async event=>{
        const file=event.target.files?.[0]||null,preview=$('#signupProfilePhotoPreview');signupProfilePhotoFile=null;
        if(signupProfilePhotoPreviewUrl){URL.revokeObjectURL(signupProfilePhotoPreviewUrl);signupProfilePhotoPreviewUrl=''}
        if(!file){if(preview)preview.innerHTML='<span><i class="fas fa-user"></i></span>';return}
        if(!String(file.type||'').startsWith('image/')){event.target.value='';setStatus('signupStatus','Choose a valid image file for your profile picture.','error');return}
        if(file.size>12*1024*1024){event.target.value='';setStatus('signupStatus','Choose a profile picture smaller than 12 MB.','error');return}
        signupProfilePhotoFile=file;signupProfilePhotoPreviewUrl=URL.createObjectURL(file);if(preview)preview.innerHTML=`<img src="${signupProfilePhotoPreviewUrl}" alt="Selected profile picture">`;setStatus('signupStatus','Profile picture selected.','success')
      });
      async function saveRegistrationAvatar(userId,blob){
        if(!userId||!blob)return'';let url='';try{const path=profileAvatarPath(userId),upload=await supabaseClient.storage.from('findat-documents').upload(path,blob,{upsert:true,contentType:'image/jpeg',cacheControl:'3600'});if(upload.error)throw upload.error;const publicData=supabaseClient.storage.from('findat-documents').getPublicUrl(path).data;url=publicData?.publicUrl?`${publicData.publicUrl}?v=${Date.now()}`:''}catch(error){console.warn('Registration avatar storage fallback used.',error);url=await blobAsDataURL(blob)}if(url){const result=await supabaseClient.rpc('findat_set_profile_avatar',{p_avatar_url:url});if(result.error)console.warn('Registration avatar profile update delayed.',result.error)}return url
      }
      ...
      $('#signupForm')?.addEventListener('submit',async event=>{
        ...
        if(!data.firstName||!data.lastName||!data.email||!data.username||!data.password||!data.confirmPassword||!signupProfilePhotoFile){setStatus('signupStatus','Complete all required fields and upload a profile picture.','error');return}
        ...
        try{
          ...
          const registrationAvatarBlob=await resizeAvatarFile(signupProfilePhotoFile),registrationAvatarData=await blobAsDataURL(registrationAvatarBlob);
          const{data:authData,error}=await supabaseClient.auth.signUp({email:String(data.email).trim(),password:String(data.password),options:{emailRedirectTo:`${location.origin}${location.pathname}#login`,data:{username,first_name:String(data.firstName).trim(),last_name:String(data.lastName).trim(),phone:String(data.phone||'').trim(),organisation:String(data.organisation||'').trim(),country:'',avatar_url:registrationAvatarData,role_request:'client',registration_source:'public'}}});
          if(error)throw error;
          if(authData.session?.user?.id)await saveRegistrationAvatar(authData.session.user.id,registrationAvatarBlob);
          form.reset();signupProfilePhotoFile=null;if(signupProfilePhotoPreviewUrl){URL.revokeObjectURL(signupProfilePhotoPreviewUrl);signupProfilePhotoPreviewUrl=''};if($('#signupProfilePhotoPreview'))$('#signupProfilePhotoPreview').innerHTML='<span><i class="fas fa-user"></i></span>';
...
For security, the password itself is never sent; the user keeps the registration password and receives a secure password setup/reset link.`
      ...
      function renderAdminAccountsTable(){const body=$('#fdAccountTableBody');if(!body)return;const query=normal($('#fdAccountSearchInput')?.value),users=adminRegistryUsers.filter(user=>!query||normal(`${accountName(user)} ${user.username} ${user.email} ${user.organisation} ${user.phone} ${user.role} ${user.country} ${user.region}`).includes(query));body.innerHTML=users.length?users.map(user=>`<tr class="${user.role==='admin'?'is-admin-row':''}"><td><button class="fd-account-profile-link" data-social-profile="${user.id}" type="button"><strong>${escapeHTML(accountName(user))}</strong><small>@${escapeHTML(user.username)} · ${escapeHTML(user.email)}</small>${user.organisation||user.phone?`<small>${escapeHTML([user.organisation,user.phone].filter(Boolean).join(' · '))}</small>`:''}</button></td><td>${adminRoleBadge(user.role)}</td><td><div class="fd-account-status-stack">${accountStateBadges(user)}</div></td><td><div class="fd-account-controls">${user.role==='admin'?'<button class="fd-mini-btn is-locked" disabled type="button"><i class="fas fa-lock"></i> Protected</button>':`${user.emailConfirmed?'':`<button class="fd-mini-btn is-success" data-account-confirm="${user.id}" type="button"><i class="fas fa-envelope-circle-check"></i> Confirm email</button>`}<button class="fd-mini-btn" data-account-password="${user.id}" type="button"><i class="fas fa-key"></i> Password</button>${user.role==='consultant'?`<button class="fd-mini-btn is-admin-promote" data-account-promote="${user.id}" data-account-name="${escapeHTML(accountName(user))}" type="button"><i class="fas fa-user-shield"></i> Make Administrator</button>`:''}<button class="fd-mini-btn ${user.active?'is-danger':'is-success'}" data-account-active="${user.id}" data-next-active="${user.active?'false':'true'}" type="button">${user.active?'Suspend':'Activate'}</button>`}</div></td></tr>`).join(''):`<tr><td colspan="4">${adminRegistryUsers.length?'No accounts match this search.':'No accounts found.'}</td></tr>`}
      async function refreshAdminAccounts(){if(!isAdmin())return;const body=$('#fdAccountTableBody');if(body)body.innerHTML='<tr><td colspan="4">Loading accounts…</td></tr>';try{const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'list'},true),users=(response.users||[]).map(profileFromRow);adminRegistryUsers=users;accountCache=[...new Map([activeAccount,...users].filter(Boolean).map(item=>[item.id,item])).values()];if($('#fdAccountNavCount'))$('#fdAccountNavCount').textContent=String(users.length);renderAdminAccountsTable()}catch(error){if(body)body.innerHTML=`<tr><td colspan="4">${escapeHTML(error.message||'Accounts could not be loaded.')}</td></tr>`}}
      ...
      $('#fdAccountSearchInput')?.addEventListener('input',renderAdminAccountsTable);
      $('#fdAccountSearchInput')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();renderAdminAccountsTable()}});
      $('#fdAccountSearchButton')?.addEventListener('click',renderAdminAccountsTable);
      $('#fdAccountSearchClear')?.addEventListener('click',()=>{if($('#fdAccountSearchInput'))$('#fdAccountSearchInput').value='';renderAdminAccountsTable();$('#fdAccountSearchInput')?.focus()});
      ...
      $('#fdSocialProfileEdit')?.addEventListener('click',()=>$('#fdUserProfileButton')?.click());
      $('#fdSocialProfileView')?.addEventListener('click',()=>openMemberProfile(activeAccount?.id));
      $('#fdMemberProfileClose')?.addEventListener('click',()=>window.toggleModal?.('fdMemberProfileModal',false));
      $('#fdSocialAnalyticsButton')?.addEventListener('click',()=>openSocialAnalytics());
      $('#fdSocialAnalyticsClose')?.addEventListener('click',()=>window.toggleModal?.('fdSocialAnalyticsModal',false));
      $('#fdSocialX1Agent')?.addEventListener('click',()=>openX1AgentWithContext('Act as my FINDAT social and publication agent. Review my Writing Desk, current publications, engagement and professional profile. Ask for the exact in-system task, then complete verified steps from start to finish without publishing or deleting anything unless I explicitly instruct you.'));
      $('#fdEditorX1Agent')?.addEventListener('click',()=>{const title=$('#fdArticleTitle')?.value.trim()||'Untitled draft',content=stripHTML($('#fdArticleEditor')?.innerHTML||'').slice(0,6000);openX1AgentWithContext(`Assist with this FINDAT publication. Title: ${title}. Current draft: ${content||'No draft text yet.'}\n\nWait for my instruction before changing, summarising, analysing or restructuring the publication.`)});
      ...
      document.addEventListener('click',async event=>{
        ...
        const profile=event.target.closest('[data-social-profile]');if(profile){event.preventDefault();event.stopPropagation();openMemberProfile(profile.dataset.socialProfile);return}
        const analytics=event.target.closest('[data-social-analytics]');if(analytics){event.preventDefault();openSocialAnalytics(analytics.dataset.socialAnalytics);return}
        const editSelf=event.target.closest('[data-profile-edit-self]');if(editSelf){event.preventDefault();window.toggleModal?.('fdMemberProfileModal',false);$('#fdUserProfileButton')?.click();return}
...
(()=>{
    ...
    window.FINDAT_OPEN_X1=openX1;
    window.FINDAT_CLOSE_X1=closeX1;
...
(()=>{
  ...
  let lastTopic='x1';
  let currentEvidence=[];
  const openAI={functionName:String(window.FINDAT_AUTH_CONFIG?.x1OpenAiFunction||'findat-x1-openai'),available:null,checking:false,model:'gpt-5-mini',lastFailureAt:0};
  ...
  function buildKnowledgeIndex(){
    const adaptiveKnowledge=window.FindatWritingMemory?.knowledgeUnits?.()||[];knowledgeBase=[...BUILT_IN_KB,...adaptiveKnowledge,...importedKnowledge];
  ...
  async function invokeOpenAIService(body){
    ...
    if(!url||!key||!client)throw new Error('The secure x1 service is not configured.');
    const{data}=await client.auth.getSession(),session=data?.session;
    if(!session?.access_token)throw new Error('Log in to use OpenAI-powered x1 assistance.');
    const response=await fetch(`${url}/functions/v1/${encodeURIComponent(openAI.functionName)}`,{method:'POST',headers:{'Content-Type':'application/json','apikey':key,'Authorization':`Bearer ${session.access_token}`},body:JSON.stringify(body||{})});
  ...
  async function testOpenAI({silent=true}={}){
    ...
    try{
      ...
      const message=String(error?.message||'OpenAI is unavailable.'),needsLogin=/log in/i.test(message);openAI.available=needsLogin?null:false;openAI.lastFailureAt=needsLogin?0:Date.now();
      updateOpenAIUi(needsLogin?'Sign in to activate secure OpenAI':'OpenAI connection required',needsLogin?'idle':'error');
      if(!silent)showToast(message);return false;
  ...
  async function callOpenAI(prompt,plan,matches,verifiedContext=[]){
    const segments=[...collectEvidenceSegments(prompt,matches||[],12),...collectReconciliationEvidence(prompt),...(Array.isArray(verifiedContext)?verifiedContext.slice(0,8):[])],evidence=segments.slice(0,20).map((segment,index)=>({
      id:index+1,fact:String(segment.text||segment.fact||'').slice(0,3500),source:String(segment.source||'FINDAT verified runtime').slice(0,300),section:String(segment.title||segment.section||'Verified context').slice(0,300),
      pages:segment.pageStart?segment.pageStart===segment.pageEnd?String(segment.pageStart):`${segment.pageStart}-${segment.pageEnd}`:String(segment.pages||''),jurisdiction:String(segment.jurisdiction||'')
    })).filter(item=>item.fact);
    const recentContext=conversation.slice(0,-1).slice(-10).map(turn=>({role:turn.role,text:turn.text}));
    updateOpenAIUi('OpenAI is preparing the x1 response…','busy');
    try{
      const result=await invokeOpenAIService({prompt,plan,evidence,conversation:recentContext});
      if(!result?.text)throw new Error('OpenAI returned an empty response.');openAI.available=true;openAI.model=result.model||openAI.model;openAI.lastFailureAt=0;
  ...
  function buildResponse(prompt){
    ...
    if(intent==='greeting')return{localText:'The user greeted x1. Respond briefly as x1 | ProATR and invite a financial, document, writing or verified FINDAT task.',intent,topic:'x1',action:null,meta:'Conversation intent',plan:{confidence:1},matches:[]};
    if(intent==='gratitude')return{localText:'The user expressed thanks. Respond briefly and retain the current topic for the next request.',intent,topic:lastTopic,action:null,meta:'Conversation context retained',plan:{confidence:1},matches:[]};
    if(intent==='identity')return{localText:'x1 | ProATR is FINDAT’s OpenAI-powered financial assistant. FINDAT retrieves relevant Knowledge and verified application state, then the secure Supabase Edge Function sends the request to OpenAI. OpenAI is the mandatory provider for conversational answers. x1 is application-aware but not conscious or sentient.',intent,topic:'identity',action:null,meta:'Architecture explanation',plan:{confidence:1},matches:[]};
    if(intent==='app_state')return{localText:`You are in ${state.panelName}. The knowledge base contains ${state.knowledgeUnits} evidence units, including ${state.importedUnits} imported units. Reconciliation results are ${state.reconciliationReady?'available':'not yet generated'}. The workflow canvas contains ${state.workflowNodes} nodes and ${state.workflowConnections} rendered connections. Secure OpenAI synthesis is ${openAI.available===true?'connected':openAI.available===false?'unavailable or not configured':'not yet tested'}.`,intent,topic:'state',action:null,meta:'Live application-state inspection',plan:{confidence:1},matches:[]};
  ...
  async function respond(raw){
    ...
    const result=buildResponse(prompt);currentEvidence=result.matches||[];
    let agentResult=null;
    if(window.X1AgenticRuntime?.execute){
      try{
        agentResult=await window.X1AgenticRuntime.execute(prompt);
        if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
      }catch(error){
        console.error('x1 agentic runtime error',error);
        agentResult={handled:true,text:`The verified FINDAT action failed without taking any hidden action: ${error.message||'unknown execution error'}.`,meta:'Verified FINDAT runtime error',intent:'agentic_command',topic:'x1 operations',toast:'The requested FINDAT action could not be completed.'};
      }
    }

    const verifiedContext=[{
      title:'Verified FINDAT response context',source:'FINDAT verified runtime',text:result.localText
    }];
    if(agentResult?.handled)verifiedContext.push({
      title:'Verified FINDAT action outcome',source:'FINDAT agent runtime',text:`Operational request outcome: ${agentResult.text||'The requested FINDAT operation completed.'} Metadata: ${agentResult.meta||'Verified in-system action.'} Do not claim any action beyond this outcome.`
    });

    try{
      const generated=await callOpenAI(prompt,result.plan,result.matches||[],verifiedContext);
      if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
      clearTimeout(thinkingTimer);thinking?.remove();
      addChatMessage('assistant',reviewAnswer(generated.text,prompt,result.plan),'');
      lastIntent=agentResult?.intent||result.intent||lastIntent;lastTopic=agentResult?.topic||result.topic||lastTopic;
      showToast(agentResult?.handled?(agentResult.toast||'x1 completed the verified FINDAT action with OpenAI guidance.'):'Answer generated securely with OpenAI.');
      if(!agentResult?.handled&&result.action)setTimeout(result.action,420);
    }catch(error){
      if(generation!==conversationGeneration){clearTimeout(thinkingTimer);thinking?.remove();return}
      clearTimeout(thinkingTimer);thinking?.remove();
      const message=String(error?.message||'OpenAI is unavailable.');
      addChatMessage('assistant',`OpenAI could not complete this x1 request: ${message} No local chatbot answer was generated.`,'OpenAI required');
      showToast('OpenAI is required for x1 responses. Check the Edge Function, API key, billing and logs.');
    }finally{
      if(sendButton)sendButton.disabled=false;promptInput?.focus();
    }
  }
  ...
  loadImportedKnowledge();buildKnowledgeIndex();initialiseConversation();initialiseRecentItems();setTimeout(()=>testOpenAI({silent:true}),900);
  window.addEventListener('findat-writing-memory-updated',()=>buildKnowledgeIndex());
...
`;
  ...
  async function openWritingDeskFromX1(){const dashboard=document.getElementById('findatDashboard');if(!document.body.classList.contains('dashboard-mode')){document.getElementById('utilityRegister')?.click();await sleep(220)}const button=document.querySelector('[data-dashboard-view="writing"]');if(button){button.click();await sleep(140);return true}return Boolean(dashboard&&!dashboard.hidden)}
  ...
  const operations=[
    ...
    operation('writing_desk','Open Writing Desk',[/\b(?:open|show|go to)\s+(?:the\s+)?(?:writing desk|professional network|social workspace)\b/],()=>{return openWritingDeskFromX1()}),
    operation('social_new_article','Create New Publication Draft',[/\b(?:create|start|write|open)\s+(?:a\s+)?new\s+(?:article|publication|post|draft)\b/],async()=>{await openWritingDeskFromX1();await sleep(180);return clickSelector('[data-social-compose-kind="classic"]')}),
    operation('social_network','Open Professional Network',[/\b(?:open|show|go to)\s+(?:my\s+|the\s+)?network\b/],async()=>{await openWritingDeskFromX1();await sleep(160);return clickSelector('[data-social-tab="network"]')}),
    operation('social_messages','Open Social Messages',[/\b(?:open|show|go to)\s+(?:my\s+)?(?:messages|inbox|social chat)\b/],async()=>{await openWritingDeskFromX1();await sleep(160);return clickSelector('#fdSocialOpenMessages')}),
    operation('social_analytics','Open Publication Analytics',[/\b(?:open|show|review|analyse|analyze)\s+(?:my\s+)?(?:publication|post|audience|follower)?\s*analytics\b/],async()=>{await openWritingDeskFromX1();await sleep(160);return clickSelector('#fdSocialAnalyticsButton')}),
    operation('social_profile','Open Professional Profile',[/\b(?:open|show|view|edit)\s+(?:my\s+)?(?:professional\s+)?profile\b/],async()=>{await openWritingDeskFromX1();await sleep(160);return clickSelector('#fdSocialProfileView')})
  ];
...
(()=>{
  ...
  $('#fdProfessionalProfileForm')?.addEventListener('submit',async event=>{
    ...
    try{const values=Object.fromEntries(new FormData(form).entries());const{data,error}=await client.rpc('findat_update_own_profile',{p_first_name:String(values.firstName||'').trim(),p_last_name:String(values.lastName||'').trim(),p_phone:String(values.phone||'').trim(),p_organisation:String(values.organisation||'').trim(),p_country:String(values.country||'').trim(),p_qualifications:String(values.qualifications||'').trim(),p_job_title:String(values.jobTitle||'').trim(),p_place_of_work:String(values.placeOfWork||'').trim(),p_industry:String(values.industry||'').trim(),p_region:String(values.region||'').trim(),p_bio:String(values.bio||'').trim()});if(error)throw error;const row=Array.isArray(data)?data[0]:data;window.dispatchEvent(new CustomEvent('findat-profile-updated',{detail:{displayName:`${row?.first_name||values.firstName} ${row?.last_name||values.lastName}`.trim(),firstName:row?.first_name||values.firstName,lastName:row?.last_name||values.lastName,phone:row?.phone||values.phone,organisation:row?.organisation||values.organisation,country:row?.country||values.country,qualifications:row?.qualifications||values.qualifications,jobTitle:row?.job_title||values.jobTitle,placeOfWork:row?.place_of_work||values.placeOfWork,industry:row?.industry||values.industry,region:row?.region||values.region,bio:row?.bio||values.bio,coverUrl:row?.cover_url||''}}));if(status){status.textContent='Profile saved. Updated names and qualifications will appear in collaboration and article bylines.';status.className='auth-status success'}setTimeout(()=>window.toggleModal?.('fdProfilePhotoModal',false),350)}catch(error){if(status){status.textContent=error.message||'The profile could not be saved.';status.className='auth-status error'}}finally{if(button)button.disabled=false}
  });
