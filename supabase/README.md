...
- `migrations/20260801150000_findat_x1_openai_api.sql` adds metadata-only
  OpenAI usage auditing and quota indexes.
- `functions/findat-x1-openai` securely proxies authenticated x1 financial
  synthesis to the OpenAI Responses API. Store `OPENAI_API_KEY` only as a
  Supabase secret.
