...
(()=>{
      ...
      $('#googleSignupButton')?.addEventListener('click',()=>{try{sessionStorage.setItem('findat_google_registration_pending','1')}catch(error){}beginGoogleAuthentication('signupStatus',true)});
      ...
      $('#signupForm')?.addEventListener('submit',async event=>{
        ...
        setStatus('signupStatus','Creating your secure Client account…','');
        try{
          ...
          if(error)throw error;
          if(authData.session)await supabaseClient.auth.signOut();
          form.reset();
          $('#signupRole').value='client';
          setStatus('signupStatus','Account created successfully. Administrator review is required.','success');
...
For security, the password itself is never sent; the user keeps the registration password and receives a secure password setup/reset link.`
      ...
      async function bootstrap(){if($('#fdTodayLabel'))$('#fdTodayLabel').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',day:'numeric',month:'long',year:'numeric'});if(!supabaseClient){syncPublicAuthBar(null);renderHomepagePublications();console.error('FINDAT Supabase Auth is not configured.');return}const googleAuthError=googleAuthErrorFromLocation();if(googleAuthError){try{sessionStorage.removeItem('findat_google_registration_pending')}catch(error){}openAuthModal('loginPopup');setStatus('loginStatus',`Google authentication was not completed: ${googleAuthError}`,'error');history.replaceState(null,'',location.pathname)}supabaseClient.auth.onAuthStateChange(async(event,session)=>{if(event==='PASSWORD_RECOVERY'){recoverySessionActive=true;openAuthModal('passwordResetPopup');return}if(event==='SIGNED_OUT'){stopSocialRealtime();activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];workspaceCache={articles:[],audit:[]};resetSocialState();clearInterval(collaborationRefreshTimer);syncPublicAuthBar(null);renderHomepagePublications()}if(event==='SIGNED_IN'&&session?.user&&!activeAccount&&!recoverySessionActive&&!authTransitionInProgress){try{const account=await fetchOwnProfile(session.user.id);let pendingGoogleSignup=false;try{pendingGoogleSignup=sessionStorage.getItem('findat_google_registration_pending')==='1'}catch(error){}if(pendingGoogleSignup&&account?.active===false){try{sessionStorage.removeItem('findat_google_registration_pending')}catch(error){}await supabaseClient.auth.signOut();window.toggleModal?.('signupApprovalPopup',true);return}if(pendingGoogleSignup){try{sessionStorage.removeItem('findat_google_registration_pending')}catch(error){}}if(account?.active)await enterDashboard(account)}catch(error){console.warn(error)}}});await refreshPublicArticles();const session=await getCurrentSession();if(session?.user){try{const account=await fetchOwnProfile(session.user.id);let pendingGoogleSignup=false;try{pendingGoogleSignup=sessionStorage.getItem('findat_google_registration_pending')==='1'}catch(error){}if(pendingGoogleSignup&&account?.active===false){try{sessionStorage.removeItem('findat_google_registration_pending')}catch(error){}await clearSession();syncPublicAuthBar(null);window.toggleModal?.('signupApprovalPopup',true)}else if(account?.active){if(pendingGoogleSignup){try{sessionStorage.removeItem('findat_google_registration_pending')}catch(error){}}activeAccount=account;accountCache=[account];syncPublicAuthBar(account)}else if(account){await clearSession()}}catch(error){console.warn('FINDAT session profile unavailable',error)}}else syncPublicAuthBar(null);const publicationMatch=location.hash.match(/^#publication-(.+)$/);if(publicationMatch){openReader(publicationMatch[1],{updateHash:false});return}if(location.hash==='#recordings'){window.openRecordingsPanel?.();return}if(location.hash==='#live-training'){window.openLiveTrainingPanel?.();return}if(location.hash==='#publications'){window.openPublicationsPanel?.();return}if(activeAccount&&(location.hash==='#writing'||location.hash==='#workspace'||location.hash==='#dashboard'||location.hash==='#accounts'||location.hash==='#review'))await enterDashboard(activeAccount);clearInterval(publicRefreshTimer);publicRefreshTimer=setInterval(refreshPublicArticles,30000)}
