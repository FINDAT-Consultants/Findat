...
(()=>{
      ...
      function openX1AgentWithContext(text){document.getElementById('developmentsTrigger')?.click();setTimeout(()=>{document.querySelector('[data-open-x1-app]')?.click();setTimeout(()=>{const input=document.getElementById('x1PromptInput');if(input){input.value=String(text||'').trim();input.dispatchEvent(new Event('input',{bubbles:true}));input.focus()}},180)},80)}
...
(()=>{
  ...
  async function testOpenAI({silent=true}={}){
    ...
    try{
      ...
      updateOpenAIUi(needsLogin?'Sign in to activate secure OpenAI synthesis':'OpenAI unavailable · local x1 remains active',needsLogin?'idle':'error');
  ...
  async function callOpenAI(prompt,plan,matches){
    ...
    try{
      ...
      const message=String(error?.message||'OpenAI is unavailable.'),needsLogin=/log in/i.test(message);openAI.available=needsLogin?null:false;openAI.lastFailureAt=needsLogin?0:Date.now();
      updateOpenAIUi(needsLogin?'Sign in to activate secure OpenAI synthesis':'OpenAI unavailable · local x1 remains active',needsLogin?'idle':'error');showToast(`${message} x1 used its local financial engine instead.`);return null;
    }
  }
  ...
  loadImportedKnowledge();loadOllamaSettings();buildKnowledgeIndex();initialiseConversation();initialiseRecentItems();setTimeout(()=>testOpenAI({silent:true}),900);
