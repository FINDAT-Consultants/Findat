...
## 2026-08-03 social, audio and low-token controls

The package includes left-aligned recording controls, a glass-blurred Writing Desk tab junction, original x1 branding and clear controls, private-chat clearing, searchable paginated Network profiles, two-profile People in FINDAT previews, publication audio reading controls, and Administrator-enforced x1/OpenAI usage limits. Run `FINDAT-X1-LOW-TOKEN-SOCIAL-CONTROLS-UPGRADE.sql`, redeploy `supabase/functions/findat-x1-openai/index.ts`, and follow `X1-LOW-TOKEN-SOCIAL-CONTROLS-DEPLOYMENT.md`.
