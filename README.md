...
## X1 full-document PDF transaction learning

The protected build reads transaction PDFs sequentially from **page 1 through the final page**. It uses native PDF text, automatic OCR for scanned pages, and verification OCR when a transaction page has weak column or amount recognition.

An encrypted ADRA Together/TOG ledger profile is built into X1. It recognises Transaction Date, Filling Reference, Journal Number, Description, Debit, Credit and Running Total; joins wrapped descriptions; excludes Period Total, Total for Account, End of Report, page counters and report footers; and validates the rule `previous balance + debit - credit`. Uncertain corrections are marked **Review**.

X1 stores learned layout signatures and column-position patterns in the current browser's local storage so related transaction PDFs can be transformed more effectively later. The confidential reference PDF is not included in this public package.
