...
if (aureliaAccount.username === 'Aurelia User') {
  aureliaAccount.username = 'FINDAT User';
...
function accountInitials(name = aureliaAccount.username) {
  const parts = String(name || 'FINDAT Cloud User').trim().split(/\s+/).filter(Boolean);
...
function profileAvatarMarkup(className = '') {
  ...
  return `<span class="account-avatar ${className}">${image ? `<img src="${image}" alt="">` : `<span>${accountEscape(accountInitials())}</span>`}</span>`;
}
...
function updateAuthScreen(mode = 'lock') {
  ...
  if (description) {
    description.textContent = hasPassword
      ...
      : 'Use the arrow to access FINDAT Cloud';
  }
...
function profileSettingsMarkup() {
  const storageNote = window.FINDATSharedStorage?.enabled
    ? 'This profile is synchronized through the shared FINDAT workspace. Passwords are stored as a salted hash, not as readable text.'
    : 'This browser-only profile is stored on this device. Passwords are stored as a salted hash, not as readable text.';
  return `<div class="settings-section" data-settings-panel="profile">
    <div class="settings-heading"><div><h2>Profile Account</h2><p>Manage your FINDAT Cloud name, profile picture, password and session.</p></div><span class="settings-heading-icon">●</span></div>
    ...
    <section class="settings-card">
      <b>User name</b><p>Change the name displayed on the lock screen and account menu.</p>
      <form class="account-settings-form" data-account-name-form>
        <label><span>User name</span><input name="username" type="text" minlength="2" maxlength="40" value="${accountEscape(aureliaAccount.username)}" autocomplete="username" required></label>
    ...
    <p class="account-storage-note">${storageNote}</p>
  </div>`;
}
...
fullSettingsMarkup = function() {
  ...
  return `<div class="settings-app full-settings-app">
    <aside class="sidebar settings-navigation"><div class="settings-nav-title"><span class="aurelia-symbol"></span><div><h3>Settings</h3><small>FINDAT Cloud</small></div></div>
      <button data-settings-section="profile" class="${active === 'profile' ? 'active' : ''}"><span class="settings-profile-avatar">${aureliaAccount.profilePicture ? `<img src="${aureliaAccount.profilePicture}" alt="">` : accountEscape(accountInitials())}</span>Profile</button>
...
bindSettingsSection = function(win, section) {
  ...
  qs('[data-account-name-form]', win)?.addEventListener('submit', async event => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const username = String(data.get('username') || '').trim().replace(/\s+/g, ' ');
    if (username.length < 2) return toast('Enter a user name with at least 2 characters');
    ...
    aureliaAccount.username = username.slice(0, 40);
    saveAureliaAccount();
