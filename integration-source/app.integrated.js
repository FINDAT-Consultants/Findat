...
(()=>{
      ...
      function profileFromRow(row){
        ...
        return{id:row.id,displayName,firstName:row.first_name||'',lastName:row.last_name||'',email:row.email||'',username:row.username||'',phone:row.phone||'',organisation:row.organisation||'',role:normaliseRole(row.role),country:row.country||'',avatarUrl:row.avatar_url||'',active:row.active!==false,emailConfirmed:row.emailConfirmed===true||Boolean(row.email_confirmed_at),emailConfirmedAt:row.email_confirmed_at||'',lastSignInAt:row.last_sign_in_at||'',createdAt:row.created_at||'',updatedAt:row.updated_at||''}
      }
      ...
      function articleToRow(article){return{id:article.id,owner_id:article.ownerId||article.createdBy,created_by:article.createdBy||article.ownerId,collaborator_id:article.collaboratorId||null,publisher_id:article.publisherId||null,publisher_assigned_by:article.publisherAssignedBy||null,publisher_assigned_at:article.publisherAssignedAt||null,contributor_layout:Array.isArray(article.contributorLayout)?article.contributorLayout:[],title:article.title||'',subtitle:article.subtitle||'',content:article.content||'',template:article.template||'classic',category:article.category||'Research',image:article.image||null,attachments:Array.isArray(article.attachments)?article.attachments:[],status:article.status||'Draft',review_note:article.reviewNote||null,submitted_at:article.submittedAt||null,reviewed_at:article.reviewedAt||null,reviewed_by:article.reviewedBy||null,published_at:article.publishedAt||null}}
      ...
      async function refreshCollaborationState(){
        if(!supabaseClient||!activeAccount){collaborationCache=[];collaborationInbox=[];return}
        const [rowsResult,inboxResult]=await Promise.all([
          supabaseClient.from('findat_article_collaborators').select('article_id,user_id,invited_by,status,created_at,responded_at'),
        ...
        collaborationInbox=inboxResult.data||[];
      ...
      async function clearSession(){if(supabaseClient)await supabaseClient.auth.signOut();activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];contributorLayoutDraft=[];contributorLayoutArticleId='';contributorLayoutDirty=false;reviewSelection.clear();workspaceCache={articles:[],audit:[]}}
      ...
      async function uploadProfilePhoto(file){if(!file||!activeAccount)return;if(file.size>12*1024*1024){setStatus('fdProfilePhotoStatus','Choose an image smaller than 12 MB.','error');return}setStatus('fdProfilePhotoStatus','Preparing and uploading your profile picture…','');try{const blob=await resizeAvatarFile(file),path=profileAvatarPath();const upload=await supabaseClient.storage.from('findat-documents').upload(path,blob,{upsert:true,contentType:'image/jpeg',cacheControl:'3600'});if(upload.error)throw upload.error;const publicData=supabaseClient.storage.from('findat-documents').getPublicUrl(path).data;const url=`${publicData.publicUrl}?v=${Date.now()}`;const updated=await supabaseClient.from('findat_profiles').update({avatar_url:url}).eq('id',activeAccount.id);if(updated.error)throw updated.error;updateCachedAvatar(url);setStatus('fdProfilePhotoStatus','Profile picture saved to Supabase and updated across FINDAT.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The profile picture could not be uploaded.','error')}finally{if($('#fdProfilePhotoInput'))$('#fdProfilePhotoInput').value=''}}
      async function removeProfilePhoto(){if(!activeAccount)return;setStatus('fdProfilePhotoStatus','Removing profile picture…','');try{const path=profileAvatarPath();await supabaseClient.storage.from('findat-documents').remove([path]);const updated=await supabaseClient.from('findat_profiles').update({avatar_url:''}).eq('id',activeAccount.id);if(updated.error)throw updated.error;updateCachedAvatar('');setStatus('fdProfilePhotoStatus','Profile picture removed. Initials will be shown instead.','success')}catch(error){setStatus('fdProfilePhotoStatus',error.message||'The profile picture could not be removed.','error')}}
      ...
      function canManageCollaborators(article){return Boolean(article&&activeAccount&&(isAdmin()||(article.ownerId||article.createdBy)===activeAccount.id))}
      function visibleWritingArticles(){const state=loadWorkspace();if(!activeAccount)return[];if(isAdmin())return state.articles;return state.articles.filter(article=>(article.ownerId||article.createdBy)===activeAccount.id||article.collaboratorId===activeAccount.id||article.publisherId===activeAccount.id||isAcceptedCollaborator(article.id))}
      ...
      function contributorProfiles(article){
        ...
        if(article.collaboratorId&&!accepted.some(item=>item.id===article.collaboratorId)){const legacy=accountById(article.collaboratorId);if(legacy)accepted.push(legacy)}
      ...
      function normaliseContributorLayout(article){
        ...
        return people.map((person,index)=>{const old=saved.find(item=>item.userId===person.id)||{};return{userId:person.id,name:accountName(person),avatarUrl:person.avatarUrl||'',x:Number.isFinite(Number(old.x))?Math.max(0,Math.min(88,Number(old.x))):Math.min(82,(index%3)*31+3),y:Number.isFinite(Number(old.y))?Math.max(0,Math.min(78,Number(old.y))):Math.floor(index/3)*43+6,size:Number.isFinite(Number(old.size))?Math.max(28,Math.min(64,Number(old.size))):36}})
      }
      ...
      function renderCollaborationInbox(){
        const count=$('#fdCollaborationInboxCount'),list=$('#fdCollaborationInboxList');
        if(count){count.textContent=String(collaborationInbox.length);count.dataset.count=String(collaborationInbox.length)}
        if(!list)return;
        list.innerHTML=collaborationInbox.length?collaborationInbox.map(request=>{
          const inviter={displayName:request.inviter_name,avatarUrl:request.inviter_avatar_url};
          return `<article class="fd-collaboration-request">${avatarMarkup(inviter,'fd-collaboration-avatar')}<div><p><strong>${escapeHTML(request.inviter_name||'A FINDAT member')}</strong> sent you a request to collaborate on <strong>“${escapeHTML(request.article_title||'Untitled paper')}”</strong>.</p></div><div class="fd-collaboration-request-actions"><button class="fd-mini-btn is-success" data-collaboration-response="accept" data-collaboration-article="${request.article_id}" type="button"><i class="fas fa-check"></i> Accept</button><button class="fd-mini-btn is-danger" data-collaboration-response="reject" data-collaboration-article="${request.article_id}" type="button"><i class="fas fa-times"></i> Reject</button></div></article>`
        }).join(''):'<div class="fd-empty-request"><i class="fas fa-user-group"></i>No collaboration requests are waiting.</div>'
      }
      function renderActiveCollaborators(article){
        ...
        if(count)count.textContent=`${rows.filter(row=>row.status!=='rejected').length} / 5`;
      ...
      function renderReaderContributors(article){
        ...
        const layout=(Array.isArray(article.contributorLayout)&&article.contributorLayout.length)?article.contributorLayout:[{userId:article.ownerId,name:article.authorName||accountName(articleOwner(article)),avatarUrl:'',x:3,y:12,size:38}];
        host.innerHTML=layout.map(item=>`<div class="fd-contributor-card" style="left:${Math.max(0,Math.min(88,Number(item.x)||0))}%;top:${Math.max(0,Math.min(78,Number(item.y)||0))}%;--contributor-size:${Math.max(28,Math.min(64,Number(item.size)||36))}px">${avatarMarkup({displayName:item.name,avatarUrl:item.avatarUrl,username:item.name})}<strong>${escapeHTML(item.name||'FINDAT Member')}</strong></div>`).join('')
      }
      ...
      async function openCollaboratorPicker(){
        setStatus('fdCollaboratorStatus','');
        try{
          const article=await ensureArticleForCollaboration();if(!article)return;
          if(!canManageCollaborators(article)){setStatus('fdArticleStatus','Only the article author or an Administrator can manage collaborators.','error');return}
          await refreshCollaborationState();
          const selected=new Set(collaborationRows(article.id).filter(row=>row.status!=='rejected').map(row=>row.user_id));
          const list=$('#fdCollaboratorProfileList');
          list.innerHTML=accountCache.filter(person=>person.id!==activeAccount.id&&person.active!==false).map(person=>`<label class="fd-profile-option ${selected.has(person.id)?'is-selected':''}" data-profile-id="${person.id}"><input type="checkbox" ${selected.has(person.id)?'checked':''}>${avatarMarkup(person)}<strong>${escapeHTML(accountName(person))}</strong><span class="fd-profile-check"><i class="fas fa-check"></i></span></label>`).join('');
          updateCollaboratorSelectionCount();window.toggleModal?.('fdCollaboratorModal',true)
        }catch(error){setStatus('fdArticleStatus',error.message||'Collaborators could not be loaded.','error')}
      }
      function updateCollaboratorSelectionCount(){
        const checked=$$('#fdCollaboratorProfileList input:checked');if($('#fdCollaboratorSelectionCount'))$('#fdCollaboratorSelectionCount').textContent=String(checked.length);
      ...
      async function saveCollaborators(){
        const article=workspaceCache.articles.find(row=>row.id===editingArticleId);if(!article)return;
        ...
        if(result.error){setStatus('fdCollaboratorStatus',result.error.message||'Requests could not be sent.','error');return}
        await refreshCollaborationState();article.contributorLayout=normaliseContributorLayout(article);contributorLayoutDraft=article.contributorLayout.map(item=>({...item}));contributorLayoutArticleId=article.id;contributorLayoutDirty=true;renderContributorBoard(article);window.toggleModal?.('fdCollaboratorModal',false);setStatus('fdArticleStatus','Collaboration requests updated. Selected people can accept or reject them from their workspace.','success')
      }
      async function respondCollaboration(articleId,accept){
        const result=await supabaseClient.rpc('findat_respond_collaboration',{p_article_id:articleId,p_accept:Boolean(accept)});if(result.error){alert(result.error.message||'The request could not be updated.');return}
        await Promise.all([refreshCollaborationState(),refreshArticles()]);renderAll();if(accept){window.toggleModal?.('fdCollaborationInboxModal',false);const article=workspaceCache.articles.find(row=>row.id===articleId);if(article)openEditor(article.id)}
      }
      ...
      function renderAll(){renderHomepagePublications();if(!activeAccount)return;renderCounters();renderLibrary();renderReview();renderPublications();renderCover();renderCollaborationInbox();const current=loadWorkspace().articles.find(a=>a.id===editingArticleId);renderStatusBanner(current);renderContributorBoard(current)}
      ...
      async function saveArticle(status='Draft'){
        ...
        const{data:saved,error}=await supabaseClient.from('findat_articles').upsert(articleToRow(article)).select().single();if(error){setStatus('fdArticleStatus',error.message||'The article could not be saved.','error');return}
      ...
      function openReader(id,{updateHash=true}={}){
        ...
        const canRead=article.status==='Published'||isAdmin()||(article.ownerId||article.createdBy)===activeAccount?.id||article.collaboratorId===activeAccount?.id||article.publisherId===activeAccount?.id||isAcceptedCollaborator(article.id);if(!canRead)return;
      ...
      $('#fdCollaboratorModalClose')?.addEventListener('click',()=>window.toggleModal?.('fdCollaboratorModal',false));
      $('#fdCollaborationInboxButton')?.addEventListener('click',()=>{renderCollaborationInbox();window.toggleModal?.('fdCollaborationInboxModal',true)});
      ...
      $('#fdCollaboratorProfileList')?.addEventListener('change',event=>{const count=updateCollaboratorSelectionCount();if(count>5){event.target.checked=false;updateCollaboratorSelectionCount();setStatus('fdCollaboratorStatus','A group may contain no more than five collaborators.','error')}else setStatus('fdCollaboratorStatus','')});
      ...
      document.addEventListener('click',async event=>{
        ...
        const response=event.target.closest('[data-collaboration-response]');if(response)respondCollaboration(response.dataset.collaborationArticle,response.dataset.collaborationResponse==='accept');
