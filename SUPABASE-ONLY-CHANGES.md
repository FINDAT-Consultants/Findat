...
- Upload progress language no longer says “saving locally” or refers to SQL
  metadata.
- Desktop documents now fill neatly spaced horizontal rows from left to right; newly dropped items use the next free cell, while manually placed icons keep their exact saved positions and cannot cover existing files, applications, the drive, or Trash.
