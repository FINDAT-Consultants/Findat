...
async function createMissingProfile(admin: any, user: any) {
  ...
  const { data, error } = await admin
    ...
    .upsert({
      ...
      active: ['admin', 'consultant', 'client'].includes(String(user?.app_metadata?.findat_role ?? '')),
    }, { onConflict: 'id' })
  ...
  fetch: withSupabase({ auth: 'none' }, async (request, ctx) => {
    ...
    try {
      ...
      if (profile.active === false) {
        return jsonResponse({ error: 'This account is awaiting Administrator review or is currently inactive. Contact a FINDAT Administrator.' }, 403)
      }
      ...
      if (error) {
        ...
        if (code === 'email_not_confirmed') {
          return jsonResponse({
            error: 'This Client account exists, but its email is not confirmed. Ask an Administrator to confirm the account, then log in again.',
            code,
          }, 403)
        }
        if (code === 'user_banned') {
          return jsonResponse({ error: 'This account is awaiting Administrator review or is currently inactive. Contact a FINDAT Administrator.', code }, 403)
        }
