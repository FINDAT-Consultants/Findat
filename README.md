...
## Administrator Knowledge and public registration approval

The x1 Knowledge manager is now opened from the Administrator sidebar and remains connected to the same built-in and imported x1 evidence index. The composer analytics shortcut and composer Knowledge shortcut are no longer displayed.

Run `FINDAT-PUBLIC-REGISTRATION-APPROVAL-UPGRADE.sql` in the Supabase SQL Editor, then redeploy `findat-username-login`. New public Client and Google registrations will show the successful-registration popup and remain inactive until an Administrator reviews and activates the account from **User Accounts**. Existing active accounts are not changed.


## Account approval notifications and activation delivery

The latest package notifies every active Administrator when a new public Client registers. The notification includes **Review account**, which opens **User Accounts** for approval. Approval confirms the Supabase Auth email, activates the profile, and attempts delivery through Resend email and Meta WhatsApp Cloud API.

Run `FINDAT-ACCOUNT-ACTIVATION-NOTIFICATIONS-UPGRADE.sql`, redeploy `supabase/functions/findat-admin-users/index.ts`, and follow `ACCOUNT-APPROVAL-EMAIL-WHATSAPP-DEPLOYMENT.md` to configure the required Supabase Secrets.

For security, FINDAT never sends readable passwords. Users receive their username, continue using the password created during registration, and receive a secure password setup/reset link.
