...
function setCloudBusy(busy, failed = false) {
  ...
  indicator.title = failed || !fsState.cloudOnline
    ? shared ? 'Supabase Storage unavailable; document uploads are paused' : 'FINDAT Cloud storage unavailable'
    : busy ? shared ? 'Syncing directly with Supabase Storage' : 'Saving to FINDAT Cloud'
      : shared ? 'Supabase Storage is connected' : 'FINDAT Cloud workspace ready';
}
...
function makeEntryButton(entry, surface = 'finder') {
  ...
  const cloudBadge = entry.pending
    ...
    : entry.shared ? `<span class="shared-cloud-badge" title="${storedTitle}" aria-label="Saved in Supabase">✓</span>` : '';
...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const progress = typeof showSystemProgress === 'function'
    ? showSystemProgress({
        ...
        state: 'Preparing direct Supabase upload…',
        progress: 3
      })
    : null;
  ...
  const cloudProgressHandler = event => {
    ...
    progress?.update(value, {
      ...
      state: supabase
        ? (detail.phase === 'uploaded' ? 'Supabase confirmed the upload; applying green tick…' : 'Uploading document bytes directly…')
  ...
  try {
    for (const item of list) {
      ...
      progress?.update(progressPercent(transferredBytes, totalBytes, 3, 9), {
        title: 'Uploading documents',
        message: item.name,
        state: `Uploading ${completed + 1} of ${list.length} directly to Supabase…`
      });
      try {
  ...
  if (!completed) {
    progress?.fail({
      ...
      state: 'Not saved: Supabase Storage rejected the upload',
      progress: 100
    });
  ...
  progress?.complete({
    ...
    message: githubActive
      ...
      : (supabaseActive
        ? `${completed} item${completed === 1 ? '' : 's'} saved directly in Supabase Storage`
...
async function importDriveFile(file) {
  ...
  try {
    ...
    clearSelection();
    refreshAllFileViews();
    toast(remoteDocumentsOnly() ? 'Drive image restored to Supabase' : 'Drive image imported');
  } catch (error) {
...
async function saveWallpaperPreferenceToCloud(options = {}) {
  if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) || typeof sharedCloudWritePersonalization !== 'function') {
    if (!options.quiet) toast('Wallpaper saved on this device; Supabase sync is unavailable');
    return false;
  }
  try {
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    <div class="settings-heading"><div><h2>FINDAT Cloud</h2><p>Manage Supabase documents, imports and backups. File bytes are never cached in this browser.</p></div><span class="settings-heading-icon cloud-heading-logo"></span></div>
    ...
    <section class="settings-card"><b>Supabase Storage</b><p data-shared-cloud-status>The Supabase Storage bucket is being checked.</p><div class="settings-button-row"><button data-cloud-action="sync">Sync Now</button><button data-cloud-action="publish-local">Send Pending Files</button></div><small>Documents are stored directly in the findat-documents bucket. FINDAT paths are encoded in Storage object names; no custom SQL document table is used.</small></section>
    ...
    <section class="settings-card"><b>Browser storage</b><p>Only lightweight desktop metadata and preferences are stored in this browser. Document bytes remain in Supabase Storage and are downloaded into memory only when opened.</p></section>
    ...
    qsa('[data-cloud-action]', win).forEach(button => button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (action === 'sync') {
          ...
          toast('Supabase Storage is up to date');
        }
        ...
        if (action === 'refresh' && typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) {
          ...
          await sharedCloudPull();
          toast('Supabase Storage status refreshed');
        }
        if (action === 'persist') {
