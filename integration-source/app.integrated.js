...
(()=>{
      ...
      let socialCache={reactions:[],bookmarks:[],follows:[],comments:[],messages:[]};
      ...
      function socialSetupError(error){const message=String(error?.message||error?.details||'').toLowerCase();return error?.code==='42P01'||error?.code==='PGRST205'||message.includes('findat_article_reactions')||message.includes('findat_direct_messages')||message.includes('does not exist')}
      function socialActionError(error,fallback='The action could not be completed.'){
        const message=String(error?.message||error?.details||'').trim(),lower=message.toLowerCase();
        if(error?.code==='42501'||lower.includes('row-level security')||lower.includes('permission denied'))return 'Supabase permissions need the FINDAT social repair migration. Run FINDAT-SOCIAL-PUBLISHING-RESPONSIVE-FIX.sql, then sign out and sign in again.';
        if(lower.includes('jwt')||lower.includes('not authenticated')||lower.includes('authentication required'))return 'Your sign-in session has expired. Sign in again and retry.';
        return message||fallback
      }
      function resetSocialState(){socialCache={reactions:[],bookmarks:[],follows:[],comments:[],messages:[]};socialAvailable=true;socialError='';socialActiveTab='feed';socialCommentArticleId='';socialCommentReplyId='';socialChatUserId='';socialPresenceByUser=new Map();clearSocialTypingUsers();socialPresenceReady=false;socialPresenceStatus='offline'}
      async function refreshSocialState({quiet=false}={}){
        if(!supabaseClient||!activeAccount){resetSocialState();return}
        const articleIds=workspaceCache.articles.map(article=>article.id).filter(Boolean),empty=Promise.resolve({data:[],error:null});
        ...
        ]),keys=['reactions','bookmarks','follows','comments','messages'];
        const setupFailure=results.map(result=>result.error).find(error=>error&&socialSetupError(error));
        socialAvailable=!setupFailure;
        if(setupFailure){socialError='Run FINDAT-SOCIAL-PUBLISHING-RESPONSIVE-FIX.sql in Supabase to activate and repair reactions, discussions, follows and private messages.';if(!quiet)console.warn('FINDAT social workspace requires its repair migration.',setupFailure)}
        else{
          const failed=results.map(result=>result.error).filter(Boolean);
          socialError=failed.length?'Some social information could not refresh. Available actions remain enabled.':'';
          if(failed.length&&!quiet)console.warn('FINDAT social refresh partially failed',failed)
        }
        const next={...socialCache};
        results.forEach((result,index)=>{if(!result.error)next[keys[index]]=result.data||[]});
        socialCache=next;renderSocialWorkspace()
      }
      function clearSocialTypingUsers(){socialTypingTimers.forEach(timer=>clearTimeout(timer));socialTypingTimers.clear();socialTypingByUser.clear();clearTimeout(socialTypingStopTimer);socialTypingStopTimer=0;socialTypingSent=false;socialTypingLastSentAt=0}
      ...
      function socialUnreadCount(userId=''){return socialCache.messages.filter(row=>row.recipient_id===activeAccount?.id&&!row.read_at&&(!userId||row.sender_id===userId)).length}
      const SOCIAL_NETWORK_PAGE_SIZE=4;
      let socialNetworkPage=1;
      function socialNetworkQuery(){return normal($('#fdNetworkSearchInput')?.value||'')}
      function socialNetworkCandidates(){const query=socialNetworkQuery();return accountCache.filter(person=>person.id!==activeAccount?.id&&person.active!==false).filter(person=>!query||normal(`${accountName(person)} ${person.username||''} ${person.organisation||''} ${person.jobTitle||''} ${person.country||''}`).includes(query)).sort((a,b)=>Number(socialPresenceState(b.id)==='online')-Number(socialPresenceState(a.id)==='online')||Number(socialIsFollowing(b.id))-Number(socialIsFollowing(a.id))||accountName(a).localeCompare(accountName(b)))}
      function socialLatestMessage(userId){return socialCache.messages.filter(row=>(row.sender_id===activeAccount?.id&&row.recipient_id===userId)||(row.recipient_id===activeAccount?.id&&row.sender_id===userId)).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0]||null}
      function socialSetTab(tab='feed'){
        const next=['feed','work','saved','network'].includes(tab)?tab:'feed';
        if(next!==socialActiveTab&&next==='network')socialNetworkPage=1;
        socialActiveTab=next;
        $$('[data-social-tab]').forEach(button=>button.classList.toggle('is-active',button.dataset.socialTab===socialActiveTab));
        const searchBox=$('#fdNetworkSearchBox');if(searchBox)searchBox.hidden=socialActiveTab!=='network';
        renderSocialFeed();
      ...
      function renderSocialNetworkDirectory(){
        const host=$('#fdSocialFeed');if(!host)return;const people=socialNetworkCandidates(),pageCount=Math.max(1,Math.ceil(people.length/SOCIAL_NETWORK_PAGE_SIZE));socialNetworkPage=Math.min(Math.max(1,socialNetworkPage),pageCount);const pageStart=(socialNetworkPage-1)*SOCIAL_NETWORK_PAGE_SIZE,pagePeople=people.slice(pageStart,pageStart+SOCIAL_NETWORK_PAGE_SIZE);
        const cards=pagePeople.length?`<div class="fd-social-network-directory">${pagePeople.map(person=>`<article class="fd-social-network-profile">${socialAvatarMarkup(person,'fd-avatar')}<div><h4>${escapeHTML(accountName(person))}</h4><p>${escapeHTML(person.jobTitle||roleLabel(person.role))}${person.organisation?` · ${escapeHTML(person.organisation)}`:''}</p>${socialPresenceBadge(person.id)}</div><div class="fd-social-network-profile-actions"><button class="${socialIsFollowing(person.id)?'is-following':''}" data-social-follow="${person.id}" type="button"><i class="fas ${socialIsFollowing(person.id)?'fa-user-check':'fa-user-plus'}"></i> ${socialIsFollowing(person.id)?'Following':'Follow'}</button><button data-social-view-profile="${person.id}" type="button"><i class="fas fa-user-circle"></i> Profile</button><button data-social-message="${person.id}" type="button"><i class="fas fa-comment-dots"></i> Message</button></div></article>`).join('')}</div>`:'<div class="fd-social-feed-empty"><i class="fas fa-user-friends"></i>No members match this search.</div>';
        const pagination=people.length>SOCIAL_NETWORK_PAGE_SIZE?`<nav aria-label="Network pages" class="fd-network-pagination"><button aria-label="Previous network page" data-network-page="${Math.max(1,socialNetworkPage-1)}" ${socialNetworkPage===1?'disabled':''} type="button"><i class="fas fa-chevron-left"></i></button>${Array.from({length:pageCount},(_,index)=>index+1).map(page=>`<button aria-label="Network page ${page}" class="${page===socialNetworkPage?'is-active':''}" data-network-page="${page}" type="button">${page}</button>`).join('')}<button aria-label="Next network page" data-network-page="${Math.min(pageCount,socialNetworkPage+1)}" ${socialNetworkPage===pageCount?'disabled':''} type="button"><i class="fas fa-chevron-right"></i></button></nav>`:'';
        host.innerHTML=`${cards}${pagination}`
      }
      ...
      async function toggleSocialReaction(articleId,type){
        ...
        if(existing)result=await supabaseClient.from('findat_article_reactions').delete().eq('article_id',articleId).eq('user_id',activeAccount.id).eq('reaction_type',type);else result=await supabaseClient.from('findat_article_reactions').insert({article_id:articleId,reaction_type:type});if(result.error){$('#fdSocialSystemStatus').textContent=socialActionError(result.error,'The reaction could not be saved.');return}await refreshSocialState({quiet:true})
      }
      async function toggleSocialBookmark(articleId){
        if(!socialAvailable){$('#fdSocialSystemStatus').textContent=socialError;return}const existing=socialIsBookmarked(articleId);const result=existing?await supabaseClient.from('findat_article_bookmarks').delete().eq('article_id',articleId).eq('user_id',activeAccount.id):await supabaseClient.from('findat_article_bookmarks').insert({article_id:articleId});if(result.error){$('#fdSocialSystemStatus').textContent=socialActionError(result.error,'The bookmark could not be saved.');return}await refreshSocialState({quiet:true})
      }
      async function toggleSocialFollow(userId){
        if(!socialAvailable||!userId||userId===activeAccount?.id){if(!socialAvailable)$('#fdSocialSystemStatus').textContent=socialError;return}const following=socialIsFollowing(userId);const result=following?await supabaseClient.from('findat_user_follows').delete().eq('follower_id',activeAccount.id).eq('following_id',userId):await supabaseClient.from('findat_user_follows').insert({following_id:userId});if(result.error){$('#fdSocialSystemStatus').textContent=socialActionError(result.error,'The connection could not be updated.');return}await refreshSocialState({quiet:true})
      }
      async function copySocialText(text){
        if(navigator.clipboard?.writeText&&window.isSecureContext){await navigator.clipboard.writeText(text);return true}
        const area=document.createElement('textarea');area.value=text;area.setAttribute('readonly','');area.style.cssText='position:fixed;left:-9999px;top:0;opacity:0';document.body.appendChild(area);area.select();area.setSelectionRange(0,area.value.length);let copied=false;try{copied=document.execCommand('copy')}finally{area.remove()}return copied
      }
      async function shareSocialArticle(articleId){
        const article=workspaceCache.articles.find(row=>row.id===articleId);if(!article)return;const url=article.status==='Published'?`${location.origin}${location.pathname}#publication-${article.id}`:location.href,copyText=article.status==='Published'?url:`${article.title}\n\n${article.subtitle||stripHTML(article.content).slice(0,160)}`,shareData={title:article.title||'FINDAT article',text:article.subtitle||stripHTML(article.content).slice(0,160),url};
        try{if(navigator.share&&article.status==='Published')await navigator.share(shareData);else{const copied=await copySocialText(copyText);if(!copied)throw new Error('Clipboard unavailable');$('#fdSocialSystemStatus').textContent=article.status==='Published'?'Publication link copied.':'Article summary copied.';setTimeout(()=>{if($('#fdSocialSystemStatus')?.textContent.includes('copied'))$('#fdSocialSystemStatus').textContent=''},2500)}}catch(error){if(error?.name!=='AbortError')$('#fdSocialSystemStatus').textContent='The share action could not be completed. Open this site through HTTPS and retry.'}
      }
      ...
      function socialChatClearStorageKey(){return `findat_social_chat_clears_${activeAccount?.id||'guest'}`}
      function socialChatClearMap(){try{return JSON.parse(localStorage.getItem(socialChatClearStorageKey())||'{}')||{}}catch{return{}}}
      function socialChatClearedAt(userId){return socialChatClearMap()[userId]||''}
      function clearCurrentSocialChat(){if(!socialChatUserId)return;if(!confirm('Clear this conversation from your current FINDAT view? New messages will continue to appear.'))return;const map=socialChatClearMap();map[socialChatUserId]=nowISO();localStorage.setItem(socialChatClearStorageKey(),JSON.stringify(map));renderSocialChatThread();setStatus('fdSocialChatStatus','Chat cleared from this device.','success')}
      ...
      function renderSocialChatThread(){
        const host=$('#fdSocialChatMessages'),person=socialProfileById(socialChatUserId),input=$('#fdSocialChatInput'),send=$('#fdSocialChatSend'),clear=$('#fdSocialChatClear');if(!host)return;if(!person){$('#fdSocialChatTitle').textContent='Select a conversation';$('#fdSocialChatPresence').textContent='Choose a FINDAT member to begin.';$('#fdSocialChatPresence').className='';host.innerHTML='<div class="fd-social-chat-empty"><i class="fas fa-comments"></i><strong>Your FINDAT messages</strong><span>Private conversations are visible only to the participants.</span></div>';input.disabled=true;send.disabled=true;if(clear)clear.disabled=true;return}const header=$('#fdSocialChatHeader');const avatar=header.querySelector('.fd-avatar');if(avatar)avatar.outerHTML=socialAvatarMarkup(person,'fd-avatar');const typing=Boolean(socialTypingByUser.get(person.id)),presence=socialPresenceState(person.id),detail=person.jobTitle||person.organisation||roleLabel(person.role);$('#fdSocialChatTitle').textContent=accountName(person);const presenceEl=$('#fdSocialChatPresence');presenceEl.textContent=typing?`${accountName(person)} is typing…`:`${socialPresenceLabel(person.id)}${detail?` · ${detail}`:''}`;presenceEl.className=`is-${typing?'typing':presence}`;input.disabled=false;send.disabled=false;if(clear)clear.disabled=false;const clearedAt=socialChatClearedAt(person.id);const messages=socialCache.messages.filter(row=>((row.sender_id===activeAccount.id&&row.recipient_id===person.id)||(row.recipient_id===activeAccount.id&&row.sender_id===person.id))&&(!clearedAt||new Date(row.created_at)>new Date(clearedAt))).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));const messageHTML=messages.length?messages.map(row=>`<div class="fd-social-chat-message ${row.sender_id===activeAccount.id?'is-mine':''}"><div class="fd-social-chat-bubble">${escapeHTML(row.body||'')}<time>${escapeHTML(formatTime(row.created_at))}${row.sender_id===activeAccount.id&&row.read_at?' · Read':''}</time></div></div>`).join(''):'<div class="fd-social-chat-empty"><i class="fas fa-comment-medical"></i><strong>Start the conversation</strong><span>Send a private message to this FINDAT member.</span></div>';const typingHTML=typing?`<div class="fd-social-chat-message is-typing" aria-live="polite" aria-label="${escapeHTML(accountName(person))} is typing"><div class="fd-social-chat-bubble fd-social-typing-bubble"><span></span><span></span><span></span></div></div>`:'';host.innerHTML=messageHTML+typingHTML;host.scrollTop=host.scrollHeight
      }
      ...
      async function sendSocialMessage(){
        const input=$('#fdSocialChatInput'),body=String(input?.value||'').trim();if(!socialAvailable){setStatus('fdSocialChatStatus',socialError,'error');return}if(!socialChatUserId||!body){setStatus('fdSocialChatStatus','Choose a member and write a message.','error');return}stopSocialTyping();const button=$('#fdSocialChatSend');button.disabled=true;setStatus('fdSocialChatStatus','Sending…','');const result=await supabaseClient.from('findat_direct_messages').insert({recipient_id:socialChatUserId,body});button.disabled=false;if(result.error){setStatus('fdSocialChatStatus',socialActionError(result.error,'The message could not be sent.'),'error');return}input.value='';await refreshSocialState({quiet:true});renderSocialChatThread();setStatus('fdSocialChatStatus','','')
      }
      ...
      async function saveArticle(status='Draft'){
        ...
        const articleRow=articleToRow(article);if(articleAlreadyExists)delete articleRow.id;
        ...
        const{data:saved,error}=saveResult;if(error){setStatus('fdArticleStatus',socialActionError(error,'The article could not be saved.'),'error');return}
      ...
      const readerSpeech={chunks:[],index:0,volume:.9,playing:false,paused:false,voice:null,utterance:null,run:0};
      function readerSpeechVoices(){return typeof speechSynthesis!=='undefined'?speechSynthesis.getVoices():[]}
      function selectReaderVoice(){const voices=readerSpeechVoices().filter(voice=>/^en([_-]|$)/i.test(voice.lang||''));const preferred=['ryan','guy','daniel','arthur','george','christopher','alex','david','male','natural','neural','premium','enhanced'];readerSpeech.voice=[...voices].sort((a,b)=>{const score=voice=>preferred.reduce((total,key,index)=>total+(String(voice.name).toLowerCase().includes(key)?preferred.length-index:0),0)+(voice.localService?2:0);return score(b)-score(a)})[0]||readerSpeechVoices()[0]||null;return readerSpeech.voice}
      function readerSpeechText(){return [$('#fdReaderTitle')?.textContent,$('#fdReaderSubtitle')?.textContent,$('#fdReaderContent')?.innerText].map(value=>String(value||'').trim()).filter(Boolean).join('. ')}
      function readerSpeechChunks(text){const sentences=String(text||'').replace(/\s+/g,' ').match(/[^.!?]+[.!?]+|[^.!?]+$/g)||[];const chunks=[];let current='';sentences.forEach(sentence=>{const clean=sentence.trim();if(!clean)return;if((current+' '+clean).trim().length>240&&current){chunks.push(current);current=clean}else current=(current+' '+clean).trim()});if(current)chunks.push(current);return chunks}
      function syncReaderAudioUI(){const toggle=$('#fdReaderAudioToggle'),stop=$('#fdReaderAudioStop'),wave=$('#fdReaderAudioWave');if(toggle){toggle.classList.toggle('is-active',readerSpeech.playing||readerSpeech.paused);toggle.setAttribute('aria-label',readerSpeech.paused?'Resume reading':readerSpeech.playing?'Pause reading':'Read publication aloud');toggle.title=readerSpeech.paused?'Resume reading':readerSpeech.playing?'Pause reading':'Read aloud';toggle.innerHTML=`<i class="fas ${readerSpeech.paused?'fa-play':readerSpeech.playing?'fa-pause':'fa-volume-up'}"></i>`}if(stop)stop.disabled=!(readerSpeech.playing||readerSpeech.paused);wave?.classList.toggle('is-speaking',readerSpeech.playing&&!readerSpeech.paused)}
      function stopReaderSpeech(){readerSpeech.run+=1;if(typeof speechSynthesis!=='undefined')speechSynthesis.cancel();readerSpeech.chunks=[];readerSpeech.index=0;readerSpeech.playing=false;readerSpeech.paused=false;readerSpeech.utterance=null;syncReaderAudioUI()}
      function speakReaderChunk(run=readerSpeech.run){if(run!==readerSpeech.run)return;if(typeof speechSynthesis==='undefined'||!readerSpeech.chunks.length||readerSpeech.index>=readerSpeech.chunks.length){stopReaderSpeech();return}const utterance=new SpeechSynthesisUtterance(readerSpeech.chunks[readerSpeech.index]);readerSpeech.utterance=utterance;utterance.voice=readerSpeech.voice||selectReaderVoice();utterance.rate=.88;utterance.pitch=.76;utterance.volume=readerSpeech.volume;utterance.onend=()=>{if(run!==readerSpeech.run||!readerSpeech.playing)return;readerSpeech.index+=1;speakReaderChunk(run)};utterance.onerror=event=>{if(run!==readerSpeech.run)return;if(event.error!=='canceled'&&event.error!=='interrupted')setStatus('fdSocialSystemStatus','The browser audio reader could not continue.','error');stopReaderSpeech()};speechSynthesis.speak(utterance)}
      function toggleReaderSpeech(){if(typeof speechSynthesis==='undefined'){alert('Reading aloud is not supported by this browser.');return}if(readerSpeech.playing&&!readerSpeech.paused){speechSynthesis.pause();readerSpeech.paused=true;readerSpeech.playing=false;syncReaderAudioUI();return}if(readerSpeech.paused){speechSynthesis.resume();readerSpeech.paused=false;readerSpeech.playing=true;syncReaderAudioUI();return}readerSpeech.chunks=readerSpeechChunks(readerSpeechText());if(!readerSpeech.chunks.length)return;readerSpeech.index=0;readerSpeech.voice=selectReaderVoice();readerSpeech.playing=true;readerSpeech.paused=false;readerSpeech.run+=1;syncReaderAudioUI();speakReaderChunk(readerSpeech.run)}
      if(typeof speechSynthesis!=='undefined')speechSynthesis.onvoiceschanged=()=>selectReaderVoice();
      ...
      function openReader(id,{updateHash=true}={}){
        ...
        stopReaderSpeech();currentReaderArticleId=id;if(article.status==='Published')recordSocialImpression(article.id,'reader');
...
For security, the password itself is never sent; the user keeps the registration password and receives a secure password setup/reset link.`
      ...
      function clearSocialX1Conversation(){socialX1Generation+=1;const thread=$('#fdSocialX1Thread');if(thread)thread.innerHTML='<div class="fd-social-x1-welcome"><span aria-hidden="true" class="fd-social-x1-welcome-logo">x1<small>|</small></span><strong>x1 is ready</strong><span>OpenAI is used when available; Ollama and embedded FINDAT knowledge remain as fallbacks.</span></div>';const input=$('#fdSocialX1Input');if(input)input.value='';setStatus('fdSocialX1Status','');input?.focus()}
      async function askSocialX1(prompt){const text=String(prompt||'').trim();if(!text)return;const generation=++socialX1Generation;socialX1Message('user',text);const thinking=socialX1Message('assistant','x1 is analyzing…',true);setStatus('fdSocialX1Status','');try{const current={title:$('#fdArticleTitle')?.value||'',subtitle:$('#fdArticleSubtitle')?.value||'',content:stripHTML($('#fdArticleEditor')?.innerHTML||'').slice(0,9000),template:$('#fdArticleTemplate')?.value||''};const result=await window.X1PublicAgent?.ask?.(text,{surface:'Writing Desk',currentDraft:current});if(generation!==socialX1Generation){thinking?.remove();return}if(!result?.text)throw new Error('x1 did not return an answer.');thinking?.remove();const answer=socialX1Message('assistant',result.text);if(answer&&(current.title||current.content)){const use=document.createElement('button');use.type='button';use.className='fd-mini-btn';use.innerHTML='<i class="fas fa-file-import"></i> Insert answer into current editor';use.addEventListener('click',()=>{setSocialEditorOpen(true,'x1-assisted publication');const editor=$('#fdArticleEditor');if(editor)editor.innerHTML=sanitiseHTML(`<p>${escapeHTML(result.text).replace(/\n\n/g,'</p><p>').replace(/\n/g,'<br>')}</p>`);setStatus('fdArticleStatus','x1 answer inserted for your review. It has not been saved or published.','success')});answer.append(document.createElement('br'),use)}setStatus('fdSocialX1Status',result.provider==='openai'?'OpenAI and x1 completed the response.':result.provider==='ollama'?'Local Ollama completed the response.':'Embedded x1 knowledge completed the response.','success')}catch(error){thinking?.remove();if(generation!==socialX1Generation)return;socialX1Message('assistant',error.message||'x1 is temporarily unavailable.');setStatus('fdSocialX1Status','The local x1 interface remains available from Developments.','error')}}
      ...
      $('#fdSocialAskX1')?.addEventListener('click',()=>window.toggleModal?.('fdSocialX1Modal',true));$('#fdSocialX1Close')?.addEventListener('click',()=>window.toggleModal?.('fdSocialX1Modal',false));$('#fdSocialX1Clear')?.addEventListener('click',clearSocialX1Conversation);
      $('#fdSocialX1Form')?.addEventListener('submit',event=>{event.preventDefault();const input=$('#fdSocialX1Input'),value=input?.value||'';if(input)input.value='';askSocialX1(value)});$$('[data-social-x1-prompt]').forEach(button=>button.addEventListener('click',()=>{if($('#fdSocialX1Input'))$('#fdSocialX1Input').value=button.dataset.socialX1Prompt||'';$('#fdSocialX1Input')?.focus()}));
      ...
      $('#fdNetworkSearchInput')?.addEventListener('input',()=>{socialNetworkPage=1;if(socialActiveTab==='network')renderSocialNetworkDirectory()});
      $('#fdNetworkSearchButton')?.addEventListener('click',()=>{socialNetworkPage=1;renderSocialNetworkDirectory();$('#fdNetworkSearchInput')?.focus()});
      $('#fdSocialChatClear')?.addEventListener('click',clearCurrentSocialChat);
      ...
      document.addEventListener('click',async event=>{
        ...
        const networkPage=event.target.closest('[data-network-page]');if(networkPage){event.preventDefault();if(networkPage.disabled)return;socialNetworkPage=Math.max(1,Number(networkPage.dataset.networkPage)||1);renderSocialNetworkDirectory();$('#fdSocialFeed')?.scrollIntoView({behavior:'smooth',block:'start'});return}
      ...
      document.addEventListener('click',async event=>{
        const articleAudio=event.target.closest('[data-article-audio]');if(articleAudio){event.preventDefault();event.stopPropagation();openReader(articleAudio.dataset.articleAudio);setTimeout(toggleReaderSpeech,120);return}
      ...
      $('#fdReaderAudioToggle')?.addEventListener('click',toggleReaderSpeech);$('#fdReaderAudioStop')?.addEventListener('click',stopReaderSpeech);$('#fdReaderAudioVolume')?.addEventListener('input',event=>{readerSpeech.volume=Math.max(.2,Math.min(1,Number(event.target.value)||.9));if(readerSpeech.playing){readerSpeech.run+=1;speechSynthesis.cancel();speakReaderChunk(readerSpeech.run)}});
...
(()=>{
    ...
    const openAssuranceRegent=(event)=>{event?.stopPropagation?.();const configured=String(window.ASSURANCE_REGENT_URL||'').trim();const target=configured||new URL('adra_recovery_work/',document.baseURI).href;const opened=window.open(target,'_blank','noopener,noreferrer');if(!opened)window.location.href=target;};
    document.querySelectorAll('[data-open-assurance-regent]').forEach(button=>button.addEventListener('click',openAssuranceRegent));
    document.querySelector('[data-assurance-regent-details]')?.addEventListener('click',event=>{event.stopPropagation();document.querySelector('[data-open-assurance-regent]')?.focus()});
...
(()=>{
  ...
  const openAI={functionName:String(window.FINDAT_AUTH_CONFIG?.x1OpenAiFunction||'findat-x1-openai'),available:null,checking:false,model:'gpt-5-mini-2025-08-07',lastFailureAt:0};
  ...
  async function testOpenAI({silent=true}={}){
    ...
    try{
      const result=await invokeOpenAIService({action:'health'});openAI.available=Boolean(result?.ready);openAI.model=result?.model||openAI.model;openAI.lastFailureAt=0;
  ...
  async function callOpenAI(prompt,plan,matches){
    ...
    const segments=[...collectEvidenceSegments(prompt,matches||[],12),...collectReconciliationEvidence(prompt)],evidence=segments.slice(0,16).map((segment,index)=>({
    ...
    const recentContext=conversation.slice(0,-1).slice(-10).map(turn=>({role:turn.role,text:turn.text}));
    updateOpenAIUi('OpenAI is preparing the financial response…','busy');
    try{
      const result=await invokeOpenAIService({prompt,plan,evidence,conversation:recentContext,styleProfile:adaptiveWritingProfile()});
      if(!result?.text)return null;openAI.available=true;openAI.model=result.model||openAI.model;openAI.lastFailureAt=0;
  ...
  async function respond(raw){
    ...
    const externalEligible=!result.action&&!['calculation','greeting','gratitude','identity','app_state'].includes(result.intent);
    if(externalEligible){
