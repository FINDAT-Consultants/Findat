/* FINDAT Cloud remote-first virtual drive and desktop operations */
const fsState = {
  entries: new Map(),
  selected: new Set(),
  clipboard: null,
...
let initialCloudSyncPromise;

// File bytes are staged in memory only for the duration of a Supabase upload.
// IndexedDB contains lightweight desktop metadata, never the document payload.
const pendingCloudPayloads = new Map();

function remoteDocumentsOnly() {
  const config = window.FINDAT_CLOUD_CONFIG || {};
  return String(config.documentStorage || '').toLowerCase() === 'supabase-only' || config.cacheFileBytes === false;
}

function requireRemoteDocumentStorage() {
  if (!remoteDocumentsOnly()) return;
  if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled())) {
    const detail = typeof sharedCloudConfigurationError === 'function'
      ? sharedCloudConfigurationError()
      : 'FINDAT Cloud storage is not configured';
    throw new Error(detail || 'FINDAT Cloud storage is required for document uploads');
  }
}

function stagePendingCloudPayload(path, blob) {
  if (!(blob instanceof Blob)) throw new Error('The selected document could not be read');
  pendingCloudPayloads.set(normalizeVirtualPath(path), blob);
}

function peekPendingCloudPayload(path) {
  return pendingCloudPayloads.get(normalizeVirtualPath(path)) || null;
}

function releasePendingCloudPayload(path) {
  pendingCloudPayloads.delete(normalizeVirtualPath(path));
}
  ...
  icon.addEventListener('pointerdown', event => {
    ...
    const finish = finishEvent => {
      ...
      if (moved) {
        ...
        const otherPositions = qsa('.desktop-icon', area)
          .filter(other => other !== icon && !other.classList.contains('system-icon-hidden'))
          .map(other => ({ x: Number.parseFloat(other.style.left), y: Number.parseFloat(other.style.top) }))
          .filter(position => Number.isFinite(position.x) && Number.isFinite(position.y));
        const candidates = desktopGridPositions([], otherPositions.length + 1);
        if (otherPositions.some(position => desktopPositionsOverlap(nextPosition, position))) {
          nextPosition = nearestFreeDesktopPosition(nextPosition, otherPositions, candidates);
        }
...
const MIME_TYPES = {
  ...
  yml: 'text/yaml', yaml: 'text/yaml', log: 'text/plain', rtf: 'application/rtf', pdf: 'application/pdf',
...
async function restoreDatabaseSnapshot(snapshot = []) {
  const previous = Array.isArray(snapshot) ? snapshot : [];
  const previousPaths = new Set(previous.map(node => node.path));
  const current = await databaseAll();
  const deletes = current.map(node => node.path).filter(path => !previousPaths.has(path));
  await databaseCommit(previous, deletes);
}

function driveMime(name, provided = '') {
  const supplied = String(provided || '').split(';')[0].trim().toLowerCase();
  const extension = String(name || '').split('.').pop().toLowerCase();
  const inferred = MIME_TYPES[extension] || '';
  // Mobile browsers frequently report Office and PDF files as a generic binary
  // stream. Prefer the extension mapping in that case so the viewer can render
  // the document instead of treating a tap as a download-only file.
  if (supplied && supplied !== 'application/octet-stream' && supplied !== 'binary/octet-stream') return supplied;
  return inferred || supplied || 'application/octet-stream';
}
...
async function initializeDrive() {
  ...
  if (typeof sharedCloudInitialSync === 'function') {
    // Render cached icon metadata immediately, especially on phones, instead of
    // holding the whole desktop behind a network round trip. The Supabase index
    // refreshes the view as soon as current cloud metadata arrives. Mutations
    // still await this promise so an early upload cannot overwrite a newer
    // remote index before the initial reconciliation has completed.
    initialCloudSyncPromise = Promise.resolve().then(() => sharedCloudInitialSync());
    initialCloudSyncPromise.then(() => {
      ...
      console.warn('FINDAT Cloud storage is unavailable; document uploads remain paused until it reconnects.', error);
    });
  }
  if (remoteDocumentsOnly()) {
    purgeLocalDocumentBytes().then(count => {
      if (count > 0) refreshUsage();
    }).catch(() => { /* old cached payloads can be removed on the next start */ });
  } else {
...
async function purgeLocalDocumentBytes() {
  const nodes = await databaseAll();
  const cleaned = nodes
    .filter(node => node.type === 'file' && node.blob instanceof Blob && node.cloudSynced && !node.cloudPending)
    .map(node => ({
      ...node,
      blob: undefined,
      storedSize: 0,
  ...
  if (cleaned.length) await databaseCommit(cleaned, []);
...
async function localUpload(blobValue, parent, name, relativePath = '', mime = '') {
  ...
  if (remoteDocumentsOnly()) {
    ...
    stagePendingCloudPayload(path, blob);
    try {
      await databasePut(node);
    } catch (error) {
      releasePendingCloudPayload(path);
      throw error;
    }
    return publicEntry(node);
  }
...
async function localTransfer(source, destination, moving) {
  ...
  const puts = descendants.map(node => {
    ...
    if (moving) {
      ...
      next.cloudSourceObjectPath = node.type === 'file' ? (node.cloudObjectPath || node.cloudSourceObjectPath || '') : '';
      next.cloudObjectPath = '';
      next.cloudModified = '';
      next.cloudPreviousPath = '';
      next.cloudPreviousObjectPath = '';
      if (remoteDocumentsOnly() && next.type === 'file') {
        next.blob = undefined;
        next.storedSize = 0;
        next.compression = '';
        next.compressionVersion = 0;
        next.compressionChecked = 1;
      }
    }
    return next;
  });
...
async function readLocalFile(path) {
  const normalized = normalizeVirtualPath(path);
  const staged = peekPendingCloudPayload(normalized);
  if (staged instanceof Blob) return staged;
  const node = await databaseGet(normalized);
  if (!node || node.type !== 'file') throw new Error('File not found');
  if (node.blob instanceof Blob) return restoreStoredBlob(node);
  const sourceObjectPath = node.cloudObjectPath || node.cloudSourceObjectPath || '';
  if (sourceObjectPath && typeof sharedCloudDownloadObject === 'function') {
    // Remote-only mode downloads into memory for this open/export operation and
    // deliberately does not write the document bytes into browser storage.
    return sharedCloudDownloadObject({
      name: node.name,
      object_path: sourceObjectPath,
      cloudModified: node.cloudModified || node.modified || ''
    });
  }
  throw new Error(`${node.name} is not available in FINDAT Cloud`);
}
...
async function fsRequest(route, options = {}, params = {}) {
  ...
  try {
    ...
    if (mutating && typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) {
      // Let the visual desktop start immediately, but never begin a write until
      // the first remote metadata reconciliation has finished.
      if (initialCloudSyncPromise) {
        try { await initialCloudSyncPromise; }
        catch (_) { /* the direct write below will report the actionable error */ }
      }
      // A pre-sync failure must not hide the actual Storage upload result.
      // The direct Supabase Storage request is the authority for persistence.
      try {
        await sharedCloudEnsureFresh();
      } catch (error) {
        console.warn('Pre-operation cloud refresh failed; continuing with direct cloud write.', error);
      }
    ...
    if (mutating && beforeNodes && typeof sharedCloudPushDiff === 'function') {
      ...
      try {
        ...
        if (window.FINDAT_CLOUD_CONFIG?.requireRemoteCommit !== false) {
          try {
            await restoreDatabaseSnapshot(beforeNodes);
            if (typeof refreshAllFileViews === 'function') refreshAllFileViews();
          } catch (rollbackError) {
            console.error('Could not roll back the failed cloud operation.', rollbackError);
          }
        }
        throw cloudError;
      }
...
function setCloudBusy(busy, failed = false) {
  ...
  indicator.title = failed || !fsState.cloudOnline
    ? shared ? 'FINDAT Cloud storage unavailable; document uploads are paused' : 'FINDAT Cloud storage unavailable'
    : busy ? shared ? 'Syncing with FINDAT Cloud' : 'Saving to FINDAT Cloud'
      : shared ? 'FINDAT Cloud storage is connected' : 'FINDAT Cloud workspace ready';
}
...
function fileGlyph(entry) {
  ...
  const glyphs = {
    pdf: ['PDF', 'pdf'], doc: ['DOC', 'doc'], docx: ['DOC', 'doc'], docm: ['DOC', 'doc'], rtf: ['RTF', 'doc'], odt: ['ODT', 'doc'],
    xls: ['XLS', 'sheet'], xlsx: ['XLS', 'sheet'], xlsm: ['XLS', 'sheet'], ods: ['ODS', 'sheet'],
    ppt: ['PPT', 'slides'], pptx: ['PPT', 'slides'], pptm: ['PPT', 'slides'], odp: ['ODP', 'slides'],
    aurlink: ['↗', 'shortcut'], zip: ['ZIP', 'archive'], rar: ['RAR', 'archive'], '7z': ['7Z', 'archive'],
...
function makeEntryButton(entry, surface = 'finder') {
  ...
  let touchOpenLocked = false;
  let lastPointerType = '';
  button.addEventListener('pointerdown', event => { lastPointerType = event.pointerType || ''; }, { capture: true });
  button.addEventListener('click', event => {
    selectEntry(entry, button, event);
    // Phones and tablets do not provide a dependable double-click gesture.
    // A single tap opens the item there, while mouse users retain the normal
    // desktop pattern of one click to select and a double-click to open.
    if (!touchFirstOpenEnabled(lastPointerType) || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
    if (touchOpenLocked) return;
    touchOpenLocked = true;
    setTimeout(() => { touchOpenLocked = false; }, 450);
    openEntry(entry);
  });
  button.addEventListener('dblclick', event => {
    event.preventDefault();
    if (!touchFirstOpenEnabled(lastPointerType)) openEntry(entry);
  });
...
function viewerMarkup() {
  return `<div class="viewer-app"><div class="viewer-toolbar"><div data-viewer-meta></div><div class="viewer-toolbar-actions"><button data-viewer-action="share" class="hidden">Copy Shared Link</button><button data-viewer-action="open-external" class="hidden">Open full screen</button><button data-viewer-action="save" class="hidden">Save</button><button data-viewer-action="download">Download</button></div></div><div class="viewer-content" data-viewer-content><div class="viewer-loading">Opening file…</div></div></div>`;
}
...
function releaseViewerObjectUrl() {
  if (!fsState.viewerUrl) return;
  URL.revokeObjectURL(fsState.viewerUrl);
  fsState.viewerUrl = null;
}

async function objectUrlForEntry(entry, suppliedBlob = null, forcedMime = '') {
  releaseViewerObjectUrl();
  let blob = suppliedBlob instanceof Blob ? suppliedBlob : await readLocalFile(entry.path);
  const desiredMime = driveMime(entry.name, forcedMime || entry.mime || blob.type);
  if (desiredMime && blob.type !== desiredMime) blob = new Blob([blob], { type: desiredMime });
...
function entryExtension(entryOrName) {
...
function isPdfEntry(entry) {
  return driveMime(entry?.name || '', entry?.mime || '') === 'application/pdf' || entryExtension(entry) === 'pdf';
}

function isOfficePreviewEntry(entry) {
  return OFFICE_PREVIEW_EXTENSIONS.has(entryExtension(entry));
}

function officePreviewUrlForEntry(entry) {
  const sourceUrl = typeof sharedCloudPublicUrl === 'function' ? sharedCloudPublicUrl(entry) : '';
  if (!sourceUrl) return '';
  return `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(sourceUrl)}`;
}

function touchFirstOpenEnabled(pointerType = '') {
...
function openViewerExternalUrl() {
  const url = fsState.viewerExternalUrl || '';
  if (!url) return;
  const opened = window.open(url, '_blank', 'noopener,noreferrer');
  if (!opened) toast('Allow pop-ups to open this document in a separate tab');
}

async function renderViewer(win, entry) {
  const resolvedMime = driveMime(entry.name, entry.mime || '');
  entry = { ...entry, mime: resolvedMime };
  fsState.viewerEntry = entry;
  fsState.viewerExternalUrl = '';
  ...
  const openExternal = qs('[data-viewer-action="open-external"]', win);
  const publicUrl = typeof sharedCloudPublicUrl === 'function' ? sharedCloudPublicUrl(entry) : '';
  save.classList.add('hidden');
  share?.classList.toggle('hidden', !publicUrl);
  openExternal?.classList.add('hidden');
  if (openExternal) openExternal.textContent = 'Open full screen';
  content.innerHTML = '<div class="viewer-loading">Opening file…</div>';

  try {
    if (entry.mime?.startsWith('image/')) {
      const url = await previewUrlForEntry(entry);
      content.innerHTML = `<div class="media-preview"><img src="${escapeHtml(url)}" alt="${escapeHtml(entry.name)}" loading="eager"></div>`;
    } else if (entry.mime?.startsWith('audio/')) {
      const url = await previewUrlForEntry(entry);
      content.innerHTML = `<div class="media-preview audio-preview">${fileGlyph(entry)}<h2>${escapeHtml(entry.name)}</h2><audio controls autoplay preload="metadata" src="${escapeHtml(url)}"></audio></div>`;
    } else if (entry.mime?.startsWith('video/')) {
      const url = await previewUrlForEntry(entry);
      content.innerHTML = `<div class="media-preview"><video controls playsinline preload="metadata" src="${escapeHtml(url)}"></video></div>`;
    } else if (isPdfEntry(entry)) {
      // Supabase object URLs may be treated as attachments by some mobile
      // browsers. A typed in-memory Blob URL forces an inline PDF preview while
      // keeping the Download button as a separate, deliberate action.
      const blob = await readLocalFile(entry.path);
      const url = await objectUrlForEntry(entry, blob, 'application/pdf');
      content.innerHTML = `<div class="pdf-preview-shell"><iframe class="pdf-preview" src="${escapeHtml(url)}#toolbar=1&view=FitH" title="${escapeHtml(entry.name)}"></iframe></div>`;
      if (publicUrl) {
        fsState.viewerExternalUrl = publicUrl;
        openExternal?.classList.remove('hidden');
        if (openExternal) openExternal.textContent = 'Open original';
      }
    } else if (isOfficePreviewEntry(entry)) {
      const officeUrl = officePreviewUrlForEntry(entry);
      if (!officeUrl) {
        content.innerHTML = `<div class="generic-preview">${fileGlyph(entry)}<h2>${escapeHtml(entry.name)}</h2><p>This Office document needs a shareable Cloud link before it can be previewed.</p><div class="generic-preview-actions"><button data-generic-download>Download</button></div></div>`;
        qs('[data-generic-download]', content)?.addEventListener('click', () => downloadEntry(entry));
      } else {
        releaseViewerObjectUrl();
        fsState.viewerExternalUrl = officeUrl;
        openExternal?.classList.remove('hidden');
        content.innerHTML = `<div class="office-preview-shell"><div class="office-preview-status"><span class="office-preview-spinner"></span><b>Opening ${escapeHtml(entry.name)}</b><small>Loading the document viewer…</small></div><iframe class="office-preview" src="${escapeHtml(officeUrl)}" title="${escapeHtml(entry.name)}" loading="eager" allowfullscreen></iframe></div>`;
        const shell = qs('.office-preview-shell', content);
        const iframe = qs('.office-preview', content);
        iframe?.addEventListener('load', () => shell?.classList.add('loaded'), { once: true });
        setTimeout(() => {
          if (!shell?.classList.contains('loaded')) shell?.classList.add('slow');
        }, 6500);
      }
      ...
      content.innerHTML = `<div class="generic-preview">${fileGlyph(entry)}<h2>${escapeHtml(entry.name)}</h2><p>${escapeHtml(entry.mime || 'Unknown file type')} · ${formatBytes(entry.size)}</p><p>This format is stored safely in FINDAT Cloud. Your browser does not provide an embedded preview for it, so it will not download unless you press Download.</p><div class="generic-preview-actions">${publicUrl ? '<button data-generic-open>Open original</button>' : ''}<button data-generic-download>Download</button></div></div>`;
      qs('[data-generic-open]', content)?.addEventListener('click', openViewerExternalUrl);
      qs('[data-generic-download]', content)?.addEventListener('click', () => downloadEntry(entry));
    }
  } catch (error) {
    content.innerHTML = `<div class="generic-preview"><h2>Could not open file</h2><p>${escapeHtml(error.message)}</p><div class="generic-preview-actions"><button data-generic-download>Download</button></div></div>`;
    qs('[data-generic-download]', content)?.addEventListener('click', () => downloadEntry(entry));
  }
}
...
function initViewer(win) {
  ...
  qs('[data-viewer-action="open-external"]', win).addEventListener('click', openViewerExternalUrl);
...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const progress = typeof showSystemProgress === 'function'
    ? showSystemProgress({
        ...
        state: 'Preparing secure Cloud upload…',
        progress: 3
      })
    : null;
  ...
  const cloudProgressHandler = event => {
    ...
    progress?.update(value, {
      title: supabase ? 'Uploading to FINDAT Cloud' : 'Uploading to GitHub',
      message: detail.name || (supabase ? 'Transferring document bytes' : 'Committing document to the repository'),
      state: supabase
        ? (detail.phase === 'uploaded' ? 'Cloud upload confirmed; applying green tick…' : 'Uploading document bytes directly…')
  ...
  try {
    for (const item of list) {
      ...
      progress?.update(progressPercent(transferredBytes, totalBytes, 3, 9), {
        title: 'Uploading documents',
        message: item.name,
        state: `Uploading ${completed + 1} of ${list.length} to FINDAT Cloud…`
      });
      try {
  ...
  if (!completed) {
    progress?.fail({
      ...
      state: 'Not saved: FINDAT Cloud rejected the upload',
      progress: 100
    });
  ...
  progress?.complete({
    title: githubActive ? 'Stored in GitHub' : (supabaseActive ? 'Saved in FINDAT Cloud' : 'Upload complete'),
    message: githubActive
      ? `${completed} item${completed === 1 ? '' : 's'} committed to the repository`
      : (supabaseActive
        ? `${completed} item${completed === 1 ? '' : 's'} saved in FINDAT Cloud`
...
async function exportDrive() {
  try {
    ...
    for (const node of nodes) {
      ...
      delete item.blob;
      delete item.cloudSourceObjectPath;
      exported.push(item);
    }
...
async function importDriveFile(file) {
  ...
  try {
    ...
    for (const item of payload.nodes) {
      ...
      delete node.data;
      delete node.blob;
      delete node.cloudSourceObjectPath;
      if (item.type === 'file') {
        const blob = item.data ? await dataUrlToBlob(item.data) : new Blob([], { type: item.mime || 'application/octet-stream' });
        if (remoteDocumentsOnly()) {
          requireRemoteDocumentStorage();
          node.size = blob.size;
          node.storedSize = 0;
          node.compression = '';
          node.compressionVersion = 0;
          node.compressionChecked = 1;
          node.cloudSynced = 0;
          node.cloudPending = true;
          node.cloudObjectPath = '';
          node.cloudModified = '';
          stagePendingCloudPayload(node.path, blob);
        } else {
          node.blob = blob;
        }
      }
    ...
    if (remoteDocumentsOnly() && typeof sharedCloudPushDiff === 'function') {
      await sharedCloudPushDiff(existing, nodes);
    }
    driveReadyPromise = initializeDrive();
    await driveReadyPromise;
    if (!remoteDocumentsOnly()) await migrateExistingDriveCompression();
    clearSelection();
    refreshAllFileViews();
    toast(remoteDocumentsOnly() ? 'Drive image restored to FINDAT Cloud' : 'Drive image imported');
  } catch (error) {
    for (const path of [...pendingCloudPayloads.keys()]) releasePendingCloudPayload(path);
    try { await restoreDatabaseSnapshot(existing); } catch (_) { /* preserve original import error */ }
    toast(error.message);
  }
}
...
function bindFilesystemUI() {
  ...
  const persistVisibleDesktopPositions = () => {
    qsa('.desktop-icon', qs('#desktopIcons')).forEach(icon => {
      ...
      desktopLayout[key] = desktopStoredPosition(x, y);
    });
    saveDesktopLayout();
  };
...
const DESKTOP_LAYOUT_POLICY_VERSION = 2;
...
const DEFAULT_WALLPAPER = 'monterey-dark';
const BUILT_IN_WALLPAPERS = [
  ['monterey-dark', 'Monterey Dark'],
  ['monterey-light', 'Monterey Light'],
  ['findat-office-light', 'FINDAT Office Light'],
  ['findat-office-dark', 'FINDAT Office Dark'],
  ['sunrise', 'Sunrise'],
  ['ocean', 'Ocean'],
  ['aurora', 'Aurora'],
  ['graphite', 'Graphite'],
...
let wallpaperCloudSyncTimer = null;
let wallpaperCloudSyncInFlight = false;
let wallpaperLastCloudCheck = 0;

function loadDesktopView() {
  const defaults = {
    iconSize: 'medium', autoArrange: false, alignGrid: false, showIcons: true, sortBy: 'custom',
    layoutPolicyVersion: DESKTOP_LAYOUT_POLICY_VERSION,
    systemIcons: { drive: true, applications: true, trash: true }
  };
  try {
    const stored = JSON.parse(localStorage.getItem(DESKTOP_VIEW_PREFS_KEY) || '{}') || {};
    const view = { ...defaults, ...stored, systemIcons: { ...defaults.systemIcons, ...(stored.systemIcons || {}) } };

    // One-time migration for existing FINDAT installations: arrange documents
    // in horizontal rows once, then preserve every manual position from then on.
    if (Number(stored.layoutPolicyVersion || 0) < DESKTOP_LAYOUT_POLICY_VERSION) {
      view.autoArrange = false;
      view.alignGrid = false;
      view.sortBy = 'custom';
      view.layoutPolicyVersion = DESKTOP_LAYOUT_POLICY_VERSION;
      Object.keys(desktopLayout).forEach(key => {
        if (key.startsWith('file:')) delete desktopLayout[key];
      });
      saveDesktopLayout();
      try { localStorage.setItem(DESKTOP_VIEW_PREFS_KEY, JSON.stringify(view)); } catch (_) { /* optional persistence */ }
    }
    return view;
...
function loadPersonalization() {
  const defaults = {
    ...
    wallpaperDefaultsVersion: WALLPAPER_DEFAULT_VERSION,
    wallpaperUpdatedAt: '',
    customWallpaperPath: '',
    customWallpaperVersion: '',
    theme: 'system', accent: '#0a84ff', brightness: 100, scale: 100,
    reduceMotion: false, wallpaperFit: 'cover', transparency: 82
  };
  try {
    const stored = JSON.parse(localStorage.getItem(PERSONALIZATION_KEY) || '{}') || {};
    const merged = { ...defaults, ...stored };
    // Migrate installations that were still using the old implicit Sunrise
    // default. Explicit custom and non-Sunrise choices remain untouched.
    if (Number(stored.wallpaperDefaultsVersion || 0) < WALLPAPER_DEFAULT_VERSION) {
      if (!stored.wallpaper || stored.wallpaper === 'sunrise') merged.wallpaper = DEFAULT_WALLPAPER;
      merged.wallpaperDefaultsVersion = WALLPAPER_DEFAULT_VERSION;
      try { localStorage.setItem(PERSONALIZATION_KEY, JSON.stringify(merged)); } catch (_) { /* optional */ }
    }
    return merged;
  } catch (_) { return defaults; }
}
...
function desktopPositionsOverlap(left, right, metrics = desktopIconMetrics(), clearance = Math.max(4, Math.round(metrics.gap / 3))) {
  if (!left || !right) return false;
  return !(
    left.x + metrics.width + clearance <= right.x ||
    right.x + metrics.width + clearance <= left.x ||
    left.y + metrics.height + clearance <= right.y ||
    right.y + metrics.height + clearance <= left.y
  );
}

function desktopFileGridTop() {
  const area = qs('#desktopIcons');
  const metrics = desktopIconMetrics();
  const visibleSystemIcons = area
    ? qsa('.system-desktop-icon', area).filter(icon => !icon.classList.contains('system-icon-hidden')).length
    : 0;
  return metrics.gap + visibleSystemIcons * (metrics.height + metrics.gap);
}

function desktopGridPositions(reserved = [], minimumCount = 0) {
  const { width } = desktopAreaSize();
  const metrics = desktopIconMetrics();
  const stepX = metrics.width + metrics.gap;
  const stepY = metrics.height + metrics.gap;
  const maxX = Math.max(0, width - metrics.width);
  const leftInset = Math.min(metrics.gap, maxX);
  const startY = Math.max(metrics.gap, desktopFileGridTop());
  const columns = [];
  const positions = [];

  // New documents fill each row from left to right, then continue on the next
  // row. The first row starts below the default FINDAT system shortcuts.
  for (let x = leftInset; x <= maxX + 0.5; x += stepX) columns.push(Math.round(x));
  if (!columns.length) columns.push(0);

  let row = 0;
  while (positions.length < Math.max(minimumCount, columns.length)) {
    const y = Math.round(startY + row * stepY);
    for (const x of columns) {
      const candidate = { x, y };
      if (!reserved.some(position => desktopPositionsOverlap(candidate, position, metrics))) positions.push(candidate);
    }
    row += 1;
    // Prevent a malformed reserved layout from causing an endless loop.
    if (row > Math.max(1000, minimumCount + 10)) break;
  }
  return positions;
}

function desktopStoredPosition(x, y) {
  return {
...
function nearestFreeDesktopPosition(desired, occupied = [], candidates = []) {
  const metrics = desktopIconMetrics();
  const exact = desired ? desktopStoredPosition(desired.x, desired.y) : null;

  // A manually chosen position remains exact until the user moves the icon.
  if (exact && !occupied.some(position => desktopPositionsOverlap(exact, position, metrics))) return exact;

  const free = candidates.filter(candidate => !occupied.some(position => desktopPositionsOverlap(candidate, position, metrics)));
  if (!free.length) return exact || defaultDesktopPosition(occupied.length);
  if (!exact) return free[0];
  return free.reduce((best, candidate) => {
    const bestDistance = (best.x - exact.x) ** 2 + (best.y - exact.y) ** 2;
    const candidateDistance = (candidate.x - exact.x) ** 2 + (candidate.y - exact.y) ** 2;
...
function defaultDesktopPosition(index) {
  ...
  const stepX = metrics.width + metrics.gap;
  const stepY = metrics.height + metrics.gap;
  const columns = Math.max(1, Math.floor((width - metrics.gap) / stepX));
  const column = index % columns;
  const row = Math.floor(index / columns);
  return desktopStoredPosition(
    metrics.gap + column * stepX,
    desktopFileGridTop() + row * stepY
  );
}
...
function systemDesktopPosition(index) {
  const metrics = desktopIconMetrics();
  const leftInset = Math.max(10, metrics.gap);
  const topInset = Math.max(10, metrics.gap);
  return clampDesktopPosition(leftInset, topInset + index * (metrics.height + metrics.gap));
}
...
function layoutDesktopIcons() {
  ...
  // System shortcuts use their standard left column until the user moves them.
  // Their saved positions are then preserved just like document positions.
  systemIcons.forEach((icon, index) => {
    bindDesktopIconPositioning(icon);
    const key = desktopIconKey(icon);
    const saved = key ? desktopLayout[key] : null;
    const desired = saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)
      ? desktopStoredPosition(saved.x, saved.y)
      : systemDesktopPosition(index);
    const candidates = desktopGridPositions([], systemIcons.length + fileIcons.length + 4);
    const position = nearestFreeDesktopPosition(desired, occupied, candidates);
  ...
  const candidates = desktopGridPositions(occupied, fileIcons.length + occupied.length + 4);

  if (desktopView.autoArrange || desktopView.sortBy !== 'custom') {
    // Explicit auto-arrange commands use the same horizontal, left-to-right grid.
    fileIcons.forEach(icon => {
      bindDesktopIconPositioning(icon);
      const key = desktopIconKey(icon);
      const position = nearestFreeDesktopPosition(null, occupied, candidates);
      occupied.push(position);
      icon.style.left = `${position.x}px`;
      icon.style.top = `${position.y}px`;
      if (key) desktopLayout[key] = position;
    ...
    // Place existing saved icons first so adding a new document never moves an
    // icon that the user has already positioned.
    const savedIcons = [];
    const newIcons = [];
    fileIcons.forEach(icon => {
      bindDesktopIconPositioning(icon);
      const key = desktopIconKey(icon);
      const saved = key ? desktopLayout[key] : null;
      if (saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) savedIcons.push({ icon, key, saved });
      else newIcons.push({ icon, key });
    });

    savedIcons.forEach(({ icon, key, saved }) => {
      const desired = desktopView.alignGrid
        ? snapDesktopPosition(saved.x, saved.y)
        : desktopStoredPosition(saved.x, saved.y);
      const position = nearestFreeDesktopPosition(desired, occupied, candidates);
      occupied.push(position);
      icon.style.left = `${position.x}px`;
...
function customWallpaperUrl() {
  const version = String(personalization.customWallpaperVersion || '');
  const hasCloudWallpaper = Boolean(personalization.customWallpaperPath || version);
  if (hasCloudWallpaper && typeof sharedCloudCustomWallpaperPublicUrl === 'function') {
    const cloudUrl = sharedCloudCustomWallpaperPublicUrl(version);
    if (cloudUrl) return cloudUrl;
  }
  // Backwards-compatible fallback for custom wallpapers created by older
  // FINDAT builds before cloud-synchronised personalization was introduced.
...
function applyWallpaper(name = personalization.wallpaper) {
  ...
  if (!wallpaper) return;
  personalization.wallpaper = name || DEFAULT_WALLPAPER;
  desktop.dataset.wallpaper = personalization.wallpaper;
  wallpaper.style.backgroundImage = '';
  wallpaper.style.backgroundSize = '';
  wallpaper.style.backgroundPosition = '';
  wallpaper.style.backgroundRepeat = '';
  if (personalization.wallpaper === 'custom') {
    ...
    if (url) {
      ...
      personalization.wallpaper = DEFAULT_WALLPAPER;
      desktop.dataset.wallpaper = DEFAULT_WALLPAPER;
    }
  }
  savePersonalization();
}

function wallpaperCloudPayload() {
  return {
    wallpaper: personalization.wallpaper || DEFAULT_WALLPAPER,
    wallpaperFit: personalization.wallpaperFit || 'cover',
    wallpaperDefaultsVersion: WALLPAPER_DEFAULT_VERSION,
    wallpaperUpdatedAt: personalization.wallpaperUpdatedAt || new Date().toISOString(),
    customWallpaperPath: personalization.customWallpaperPath || '',
    customWallpaperVersion: personalization.customWallpaperVersion || ''
  };
}

function wallpaperTimestamp(value) {
  const time = Date.parse(String(value || ''));
  return Number.isFinite(time) ? time : 0;
}

async function saveWallpaperPreferenceToCloud(options = {}) {
  if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) || typeof sharedCloudWritePersonalization !== 'function') {
    if (!options.quiet) toast('Wallpaper saved on this device; Cloud sync is unavailable');
    return false;
  }
  try {
    await sharedCloudWritePersonalization(wallpaperCloudPayload());
    wallpaperLastCloudCheck = Date.now();
    if (!options.quiet) toast('Wallpaper applied and synced');
    return true;
  } catch (error) {
    if (!options.quiet) toast(`Wallpaper applied, but cloud sync failed: ${error.message}`);
    return false;
  }
}

function chooseWallpaper(name, options = {}) {
  personalization.wallpaper = name || DEFAULT_WALLPAPER;
  personalization.wallpaperUpdatedAt = new Date().toISOString();
  personalization.wallpaperDefaultsVersion = WALLPAPER_DEFAULT_VERSION;
  savePersonalization();
  applyWallpaper(personalization.wallpaper);
  saveWallpaperPreferenceToCloud({ quiet: Boolean(options.quiet) });
}

function updateWallpaperFit(value, options = {}) {
  personalization.wallpaperFit = value || 'cover';
  personalization.wallpaperUpdatedAt = new Date().toISOString();
  savePersonalization();
  applyWallpaper(personalization.wallpaper);
  saveWallpaperPreferenceToCloud({ quiet: Boolean(options.quiet) });
...
async function syncWallpaperPreferenceFromCloud(options = {}) {
  if (wallpaperCloudSyncInFlight) return false;
  if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) || typeof sharedCloudReadPersonalization !== 'function') return false;
  wallpaperCloudSyncInFlight = true;
  try {
    const remote = await sharedCloudReadPersonalization();
    wallpaperLastCloudCheck = Date.now();
    if (!remote?.personalization) {
      // First device establishes the current local choice as the shared value.
      await saveWallpaperPreferenceToCloud({ quiet: true });
      return false;
    }
    const incoming = remote.personalization || {};
    const remoteTime = wallpaperTimestamp(incoming.wallpaperUpdatedAt || remote.updatedAt);
    const localTime = wallpaperTimestamp(personalization.wallpaperUpdatedAt);
    if (remoteTime >= localTime || options.force) {
      personalization.wallpaper = incoming.wallpaper || DEFAULT_WALLPAPER;
      personalization.wallpaperFit = incoming.wallpaperFit || 'cover';
      personalization.wallpaperDefaultsVersion = WALLPAPER_DEFAULT_VERSION;
      personalization.wallpaperUpdatedAt = incoming.wallpaperUpdatedAt || remote.updatedAt || new Date().toISOString();
      personalization.customWallpaperPath = incoming.customWallpaperPath || '';
      personalization.customWallpaperVersion = incoming.customWallpaperVersion || '';
      savePersonalization();
      applyWallpaper(personalization.wallpaper);
      qsa('.wallpaper-choice').forEach(button => {
        const id = button.dataset.settingsWallpaper || button.dataset.wallpaperChoice || '';
        const selected = personalization.wallpaper === 'custom'
          ? button.hasAttribute('data-settings-wallpaper-upload') || button.hasAttribute('data-wallpaper-upload')
    ...
    if (!options.quiet) console.warn('Wallpaper cloud synchronization failed.', error);
    return false;
  } finally {
    wallpaperCloudSyncInFlight = false;
  }
}

function startWallpaperCloudSync() {
  if (wallpaperCloudSyncTimer) return;
  syncWallpaperPreferenceFromCloud({ quiet: true });
  const interval = Math.max(15000, Number(window.FINDAT_CLOUD_CONFIG?.refreshIntervalMs) || 30000);
  wallpaperCloudSyncTimer = setInterval(() => {
    if (document.hidden) return;
    syncWallpaperPreferenceFromCloud({ quiet: true });
  }, interval);
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState !== 'visible' || Date.now() - wallpaperLastCloudCheck < 5000) return;
    syncWallpaperPreferenceFromCloud({ quiet: true });
  });
}
...
function applyPersonalization() {
  ...
  applyWallpaper(personalization.wallpaper || DEFAULT_WALLPAPER);
...
function compressWallpaper(file) {
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      try {
        const maxWidth = 2560, maxHeight = 1440;
        ...
        canvas.toBlob(blob => {
          if (!blob) return reject(new Error('The custom wallpaper could not be prepared'));
          resolve(blob);
        }, 'image/jpeg', .86);
...
function desktopSettingsMarkup() {
  const wallpapers = BUILT_IN_WALLPAPERS;
  ...
  return `<div class="settings-section" data-settings-panel="desktop">
    ...
    ${settingsToggleRow('Align icons to grid', 'Optionally snap manually moved icons to the desktop grid.', 'align-grid', desktopView.alignGrid)}
    ${settingsToggleRow('Auto arrange icons', 'Automatically place icons in organised rows from left to right.', 'auto-arrange', desktopView.autoArrange)}
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    <div class="settings-heading"><div><h2>FINDAT Cloud</h2><p>Manage Cloud documents, imports and backups. File bytes are retrieved only when needed.</p></div><span class="settings-heading-icon cloud-heading-logo"></span></div>
    <section class="settings-card cloud-overview-card">
      ...
      <div class="cloud-stat-grid"><div><b data-cloud-files>—</b><small>Files</small></div><div><b data-cloud-folders>—</b><small>Folders</small></div><div><b data-cloud-protection>None</b><small>Local file bytes</small></div></div>
    </section>
    <section class="settings-card"><b>Cloud storage</b><p data-shared-cloud-status>The FINDAT Cloud connection is being checked.</p><div class="settings-button-row"><button data-cloud-action="sync">Sync Now</button><button data-cloud-action="publish-local">Send Pending Files</button></div><small>Documents are saved in FINDAT Cloud and synchronised across authorised devices.</small></section>
    ...
    <section class="settings-card"><b>Device cache</b><p>Only lightweight desktop metadata and preferences are retained on this device. Documents are retrieved from FINDAT Cloud when opened.</p></section>
...
function bindSettingsSection(win, section) {
  ...
  if (section === 'desktop') {
    qsa('[data-settings-wallpaper]', win).forEach(button => button.addEventListener('click', () => {
      chooseWallpaper(button.dataset.settingsWallpaper);
    ...
    qs('[data-settings-wallpaper-fit]', win)?.addEventListener('change', event => updateWallpaperFit(event.target.value));
    ...
    qsa('[data-cloud-action]', win).forEach(button => button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (action === 'sync') {
          ...
          toast('FINDAT Cloud is up to date');
        }
        if (action === 'publish-local') {
          ...
          toast(result.published ? `${result.published} pending item${result.published === 1 ? '' : 's'} saved to FINDAT Cloud` : 'No Cloud uploads are pending');
        }
        if (action === 'refresh' && typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) {
          await sharedCloudDatabaseHealth(true);
          await sharedCloudPull();
          toast('Cloud storage status refreshed');
        }
        if (action === 'persist') {
...
async function refreshCloudSettingsPanel(win) {
  ...
  try {
    ...
    const storageLabel = qs('[data-storage-label]', panel); if (storageLabel) { storageLabel.textContent = `Used ${formatBytes(usage.logicalUsed)} of ${formatBytes(usage.quota)} in FINDAT Cloud · local document cache: ${formatBytes(usage.stored)}`; storageLabel.title = 'Only file and folder metadata is retained in the browser'; }
    ...
    if (sharedStatus.enabled) {
      ...
      if (detail) detail.textContent = sharedStatus.lastError || `${sharedStatus.remoteCount} items in FINDAT Cloud · ${files + folders} desktop metadata records · no local document cache`;
      if (sharedLine) {
        const destination = sharedStatus.provider === 'github'
          ...
          : 'FINDAT Cloud storage';
        const storageHealth = sharedStatus.health?.ok && sharedStatus.health?.bucket_exists
          ? ' Storage check passed.'
          : '';
        sharedLine.textContent = sharedStatus.lastError
          ? `Cloud sync failed and uploads are paused: ${sharedStatus.lastError}`
          : `Connected to ${destination}.${storageHealth} ${sharedStatus.remoteCount} shared items are available to website visitors${sharedStatus.lastSuccessAt ? `; last synced ${new Date(sharedStatus.lastSuccessAt).toLocaleString()}` : ''}.`;
      }
      ...
      if (statusTitle) statusTitle.textContent = 'FINDAT Cloud connection required';
      if (detail) detail.textContent = `The desktop can display metadata, but document uploads require FINDAT Cloud storage`;
      if (sharedLine) {
        ...
        if (selectedProvider === 'supabase') {
          ...
          sharedLine.textContent = !hasUrl
            ? 'Cloud storage is selected, but its connection address is missing or invalid.'
            : /^sb_secret_/i.test(key)
              ...
              : (!key || key.includes('PASTE_') || key.includes('YOUR_'))
                ? 'The Cloud address is configured, but the browser-safe connection key is missing.'
                : 'FINDAT Cloud is configured but currently unavailable. Check access policies and the network connection.';
        } else {
          sharedLine.textContent = 'Shared storage is off. Configure the selected provider in cloud-config.js, then set enabled to true.';
        }
      }
    }
    const protection = qs('[data-cloud-protection]', panel); if (protection) protection.textContent = usage.stored > 0 ? formatBytes(usage.stored) : 'None';
  } catch (error) {
...
function initPersonalize(win) {
  ...
  qsa('[data-wallpaper-choice]', win).forEach(button => button.addEventListener('click', () => {
    chooseWallpaper(button.dataset.wallpaperChoice);
    qsa('.wallpaper-choice', win).forEach(item => item.classList.toggle('selected', item === button));
  }));
  qs('[data-wallpaper-upload]', win)?.addEventListener('click', () => qs('#wallpaperInput')?.click());
  qs('[data-wallpaper-fit]', win)?.addEventListener('change', event => updateWallpaperFit(event.target.value));
...
wallpaperInput?.addEventListener('change', async () => {
  ...
  if (!file) return;
  try {
    if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) || typeof sharedCloudUploadCustomWallpaper !== 'function') {
      throw new Error('FINDAT Cloud storage must be connected before using a custom wallpaper');
    }
    toast('Preparing custom wallpaper…');
    const blob = await compressWallpaper(file);
    const uploaded = await sharedCloudUploadCustomWallpaper(blob);
    personalization.wallpaper = 'custom';
    personalization.wallpaperFit = personalization.wallpaperFit || 'cover';
    personalization.wallpaperDefaultsVersion = WALLPAPER_DEFAULT_VERSION;
    personalization.wallpaperUpdatedAt = new Date().toISOString();
    personalization.customWallpaperPath = uploaded.objectPath || '';
    personalization.customWallpaperVersion = uploaded.version || '';
    try { localStorage.removeItem(CUSTOM_WALLPAPER_KEY); } catch (_) { /* old cache is optional */ }
    savePersonalization();
    applyWallpaper('custom');
    const preferenceSaved = await saveWallpaperPreferenceToCloud({ quiet: true });
    if (!preferenceSaved) throw new Error('The image was uploaded, but the wallpaper selection could not be synchronized');
    ...
    const settingsWindow = qs('.app-window[data-app="settings"]');
    if (settingsWindow && localStorage.getItem(SETTINGS_SECTION_KEY) === 'desktop') renderSettingsSection(settingsWindow, 'desktop');
    toast('Custom wallpaper uploaded and synced across devices');
  } catch (error) {
    toast(error.message || 'The custom wallpaper could not be saved');
  }
});
