...
(()=>{
      ...
      async function refreshDirectory(){if(!supabaseClient||!activeAccount)return;const next=[activeAccount];if(activeAccount.role==='admin'){const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'list'},true);for(const row of response.users||[])next.push(profileFromRow(row))}accountCache=[...new Map(next.filter(Boolean).map(item=>[item.id,item])).values()]}
      ...
      async function loadAuthenticatedWorkspace(){await Promise.all([refreshDirectory(),refreshArticles()]);renderAll()}
      ...
      async function clearSession(){if(supabaseClient)await supabaseClient.auth.signOut();activeAccount=null;accountCache=[];workspaceCache={articles:[],audit:[]}}
      ...
      function articleOwner(article){return accountById(article.ownerId||article.createdBy)||{username:article.authorName||'FINDAT Member'}}
      function visibleWritingArticles(){const state=loadWorkspace();if(!activeAccount)return[];if(isAdmin())return state.articles;return state.articles.filter(article=>(article.ownerId||article.createdBy)===activeAccount.id||article.collaboratorId===activeAccount.id)}
      ...
      async function enterDashboard(account){if(!account||account.active===false)return;activeAccount=account;accountCache=[account];closeAllAuthModals();document.body.classList.add('dashboard-mode');dashboard?.setAttribute('aria-hidden','false');history.replaceState(null,'','#writing');syncPublicAuthBar(account);hydrateUser();applyRoleVisibility();renderAll();try{await loadAuthenticatedWorkspace();await migrateLegacyWorkspaceIfEmpty();populateCollaborators();renderAll();if(activeAccount.role==='consultant'){const assigned=visibleWritingArticles()[0];if(assigned)openEditor(assigned.id);else setStatus('fdArticleStatus','No client article has been assigned to this consultant account yet.','')}}catch(error){console.error(error);setStatus('fdArticleStatus',error.message||'The workspace could not be loaded.','error')}showView('writing')}
      ...
      function renderCover(){const host=$('#fdArticleImagePreview');if(!host)return;host.innerHTML=pendingCover?.data?`<img src="${pendingCover.data}" alt="Selected cover photo">`:'<span><i class="far fa-image"></i><br>No cover photo selected</span>';const canEditMedia=isAdmin()||activeAccount?.role==='consultant';$('#fdArticleAttachmentList').innerHTML=pendingAttachments.length?pendingAttachments.map((file,index)=>canEditMedia?`<button class="fd-attachment-chip" type="button" data-remove-attachment="${index}" title="Remove attachment"><i class="fas fa-paperclip"></i>${escapeHTML(file.name)} <span>×</span></button>`:`<span class="fd-attachment-chip"><i class="fas fa-paperclip"></i>${escapeHTML(file.name)}</span>`).join(''):'<span style="font-size:9px;color:#98a2b0">No documents attached</span>'}
      ...
      function collectArticle(status){const title=$('#fdArticleTitle').value.trim(),content=sanitiseHTML($('#fdArticleEditor').innerHTML);if(!title||!stripHTML(content)){setStatus('fdArticleStatus','Enter an article title and some written content.','error');return null}const current=loadWorkspace().articles.find(a=>a.id===editingArticleId),canEditMedia=isAdmin()||activeAccount?.role==='consultant';return{title,subtitle:$('#fdArticleSubtitle').value.trim(),content,template:$('#fdArticleTemplate').value,category:$('#fdArticleCategory').value,collaboratorId:isAdmin()?$('#fdArticleCollaborator').value:(current?.collaboratorId||''),image:canEditMedia?pendingCover:(current?.image||null),attachments:canEditMedia?pendingAttachments:(current?.attachments||[]),status,updatedAt:nowISO()}}
      async function saveArticle(status='Draft'){if(!requireSupabase('fdArticleStatus'))return;const data=collectArticle(status);if(!data)return;let article=workspaceCache.articles.find(a=>a.id===editingArticleId);if(!article){if(activeAccount?.role==='consultant'){setStatus('fdArticleStatus','Consultants can edit only articles assigned by an administrator.','error');return}article={id:uid(),ownerId:activeAccount.id,createdBy:activeAccount.id,createdAt:nowISO()};editingArticleId=article.id}if(article.status==='Published'&&!isAdmin()){setStatus('fdArticleStatus','A published article can only be changed by an administrator.','error');return}const consultantEdit=activeAccount?.role==='consultant';if(consultantEdit)data.status=article.status||'Draft';if(!consultantEdit&&status==='Draft'&&article.status==='Pending approval')data.reviewNote='';if(!consultantEdit&&status==='Pending approval'){data.submittedAt=nowISO();data.reviewNote=''}Object.assign(article,data);setStatus('fdArticleStatus','Saving securely to Supabase…','');const{data:saved,error}=await supabaseClient.from('findat_articles').upsert(articleToRow(article)).select().single();if(error){setStatus('fdArticleStatus',error.message||'The article could not be saved.','error');return}const converted=articleFromRow(saved),index=workspaceCache.articles.findIndex(a=>a.id===converted.id);if(index>=0)workspaceCache.articles[index]=converted;else workspaceCache.articles.push(converted);const auditAction=consultantEdit?'Consultant editorial changes saved':status==='Pending approval'?'Article submitted for approval':'Article draft saved';await addAudit(auditAction,converted.title,converted.id);setStatus('fdArticleStatus',consultantEdit?'Editorial changes saved securely in Supabase.':status==='Pending approval'?'Article sent to the administrator for approval.':'Draft saved securely in Supabase.','success');renderAll()}
      async function deleteArticle(id){const article=workspaceCache.articles.find(a=>a.id===id);if(!article||!isAdmin())return;if(!confirm(`Delete “${article.title}”?`))return;const{error}=await supabaseClient.from('findat_articles').delete().eq('id',id);if(error){setStatus('fdArticleStatus',error.message||'The article could not be deleted.','error');return}workspaceCache.articles=workspaceCache.articles.filter(a=>a.id!==id);await addAudit('Article deleted',article.title,id);if(editingArticleId===id)clearEditor(false);renderAll()}
      async function approveArticle(id){if(!isAdmin())return;const article=workspaceCache.articles.find(a=>a.id===id);if(!article)return;Object.assign(article,{status:'Published',publishedAt:nowISO(),reviewedAt:nowISO(),reviewedBy:activeAccount.id,reviewNote:'Approved for publication.'});const{data,error}=await supabaseClient.from('findat_articles').update(articleToRow(article)).eq('id',id).select().single();if(error){alert(error.message||'The article could not be approved.');return}Object.assign(article,articleFromRow(data));await addAudit('Article approved and published',article.title,id);renderAll();if(currentReaderArticleId===id)openReader(id)}
