...
## Latest maintained upgrade — Commerce, x1 Training and Cloud Access

The current package adds course pricing and time-limited access, payment and certificate history, a searchable Account Registry, Consultant-only promotion to Administrator, x1 training orchestration, and monthly FINDAT Cloud credentials. Run `FINDAT-COMMERCE-X1-CLOUD-ACCESS-UPGRADE.sql`, deploy the updated `findat-admin-users` and `findat-cloud-login` functions with JWT verification enabled, and follow `FINDAT-COMMERCE-X1-CLOUD-ACCESS-DEPLOYMENT.md`.

The x1 Training Lab organises datasets, assignments, jobs and evaluations. Actual fine-tuning or deep-learning computation requires a connected authorised model provider or dedicated ML worker.
