...
(()=>{
      ...
      let pendingLoginOtpEmail='';
      let pendingSignupOtpEmail='';
      let pendingRecoveryOtpEmail='';
      ...
      function cleanOtp(value){return String(value||'').replace(/\D/g,'').slice(0,6)}
      function validEmail(value){return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(value||'').trim())}
      function setLoginMethod(method='password'){
        const passwordMode=method!=='otp';
        $('#loginPasswordPanel')?.toggleAttribute('hidden',!passwordMode);
        $('#loginOtpPanel')?.toggleAttribute('hidden',passwordMode);
        $('#loginPasswordMode')?.classList.toggle('is-active',passwordMode);
        $('#loginOtpMode')?.classList.toggle('is-active',!passwordMode);
        $('#loginPasswordMode')?.setAttribute('aria-selected',String(passwordMode));
        $('#loginOtpMode')?.setAttribute('aria-selected',String(!passwordMode));
        setStatus('loginStatus','');
        setStatus('loginOtpStatus','');
        setTimeout(()=>$(passwordMode?'#loginIdentifier':'#loginOtpEmail')?.focus(),30)
      }
      function startOtpCooldown(buttonId,seconds=60){
        const button=$(buttonId);if(!button)return;
        const previous=otpCooldowns.get(buttonId);if(previous)clearInterval(previous);
        let remaining=seconds;
        const base=button.dataset.baseLabel||button.textContent.trim()||'Resend';
        button.dataset.baseLabel=base;
        button.disabled=true;
        button.textContent=`${base} (${remaining}s)`;
        const timer=setInterval(()=>{remaining-=1;if(remaining<=0){clearInterval(timer);otpCooldowns.delete(buttonId);button.disabled=false;button.textContent=base}else button.textContent=`${base} (${remaining}s)`},1000);
        otpCooldowns.set(buttonId,timer)
      }
      async function finishOtpLogin(session,statusId='loginOtpStatus'){
        const account=await fetchOwnProfile(session?.user?.id);
        if(!account)throw new Error('Your FINDAT profile could not be found.');
        if(account.active===false){await supabaseClient.auth.signOut();throw new Error('This FINDAT account is suspended. Contact an Administrator.')}
        setStatus(statusId,'Code verified. Opening the writing desk…','success');
        pendingLoginOtpEmail='';
        $('#loginOtpCode').value='';
        $('#loginOtpCodeArea')?.setAttribute('hidden','');
        $('#loginOtpVerify')?.setAttribute('hidden','');
        $('#loginOtpResend')?.setAttribute('hidden','');
        $('#loginOtpSend')?.removeAttribute('hidden');
        await enterDashboard(account)
      }
      async function sendLoginOtp(){
        if(!requireSupabase('loginOtpStatus'))return;
        const email=String($('#loginOtpEmail')?.value||'').trim().toLowerCase();
        if(!validEmail(email)){setStatus('loginOtpStatus','Enter a valid registered email address.','error');return}
        const button=$('#loginOtpSend');button.disabled=true;setStatus('loginOtpStatus','Requesting a secure FINDAT code…','');
        try{
          rememberSupabaseSession=$('#loginOtpRemember')?.checked!==false;
          const{error}=await supabaseClient.auth.signInWithOtp({email,options:{shouldCreateUser:false}});
          if(error)throw error;
          pendingLoginOtpEmail=email;
          $('#loginOtpEmail').value='';
          $('#loginOtpDestination').textContent=email;
          $('#loginOtpCodeArea')?.removeAttribute('hidden');
          $('#loginOtpVerify')?.removeAttribute('hidden');
          $('#loginOtpResend')?.removeAttribute('hidden');
          $('#loginOtpSend')?.setAttribute('hidden','');
          setStatus('loginOtpStatus','If this email belongs to an active FINDAT account, a six-digit code has been sent.','success');
          startOtpCooldown('#loginOtpResend',60);
          $('#loginOtpCode')?.focus()
        }catch(error){setStatus('loginOtpStatus',error.message||'The verification code could not be sent.','error')}
        finally{button.disabled=false}
      }
      async function verifyLoginOtp(){
        if(!requireSupabase('loginOtpStatus'))return;
        const token=cleanOtp($('#loginOtpCode')?.value);
        if(!pendingLoginOtpEmail){setStatus('loginOtpStatus','Request a new verification code.','error');return}
        if(token.length!==6){setStatus('loginOtpStatus','Enter the complete six-digit code.','error');return}
        const button=$('#loginOtpVerify');button.disabled=true;setStatus('loginOtpStatus','Verifying your code…','');
        try{
          rememberSupabaseSession=$('#loginOtpRemember')?.checked!==false;
          authTransitionInProgress=true;
          const{data,error}=await supabaseClient.auth.verifyOtp({email:pendingLoginOtpEmail,token,type:'email'});
          authTransitionInProgress=false;
          if(error||!data?.session)throw error||new Error('The code is invalid or expired.');
          await finishOtpLogin(data.session)
        }catch(error){authTransitionInProgress=false;$('#loginOtpCode').value='';$('#loginOtpCode')?.focus();setStatus('loginOtpStatus',error.message||'The code is incorrect or expired.','error')}
        finally{button.disabled=false}
      }
      async function resendLoginOtp(){
        if(!pendingLoginOtpEmail){setStatus('loginOtpStatus','Enter your email and request a new code.','error');return}
        const button=$('#loginOtpResend');button.disabled=true;setStatus('loginOtpStatus','Sending a new code…','');
        try{const{error}=await supabaseClient.auth.signInWithOtp({email:pendingLoginOtpEmail,options:{shouldCreateUser:false}});if(error)throw error;setStatus('loginOtpStatus','A new code has been sent.','success');startOtpCooldown('#loginOtpResend',60)}catch(error){button.disabled=false;setStatus('loginOtpStatus',error.message||'The code could not be resent.','error')}
      }
      async function verifySignupOtp(){
        const token=cleanOtp($('#signupOtpCode')?.value);
        if(!pendingSignupOtpEmail){setStatus('signupStatus','Create the account again to request a new code.','error');return}
        if(token.length!==6){setStatus('signupStatus','Enter the complete six-digit code.','error');return}
        const button=$('#signupOtpVerify');button.disabled=true;setStatus('signupStatus','Confirming your FINDAT account…','');
        try{authTransitionInProgress=true;const{data,error}=await supabaseClient.auth.verifyOtp({email:pendingSignupOtpEmail,token,type:'signup'});authTransitionInProgress=false;if(error||!data?.session)throw error||new Error('The code is invalid or expired.');const account=await fetchOwnProfile(data.user?.id);if(!account)throw new Error('Your FINDAT profile could not be found.');pendingSignupOtpEmail='';$('#signupOtpCode').value='';$('#signupOtpArea')?.setAttribute('hidden','');setStatus('signupStatus','Account verified. Opening the writing desk…','success');await enterDashboard(account)}catch(error){authTransitionInProgress=false;$('#signupOtpCode').value='';$('#signupOtpCode')?.focus();setStatus('signupStatus',error.message||'The confirmation code is incorrect or expired.','error')}finally{button.disabled=false}
      }
      async function resendSignupOtp(){
        if(!pendingSignupOtpEmail){setStatus('signupStatus','Create the account again to request a new code.','error');return}
        const button=$('#signupOtpResend');button.disabled=true;try{const{error}=await supabaseClient.auth.resend({type:'signup',email:pendingSignupOtpEmail});if(error)throw error;setStatus('signupStatus','A new account-confirmation code has been sent.','success');startOtpCooldown('#signupOtpResend',60)}catch(error){button.disabled=false;setStatus('signupStatus',error.message||'The code could not be resent.','error')}
      }
      async function sendRecoveryOtp(emailOverride=''){
        if(!requireSupabase('recoveryStatus'))return;
        const email=String(emailOverride||$('#recoveryEmail')?.value||'').trim().toLowerCase();
        if(!validEmail(email)){setStatus('recoveryStatus','Enter a valid registered email address.','error');return}
        const button=$('#recoverySendCode');button.disabled=true;setStatus('recoveryStatus','Sending a secure recovery code…','');
        try{const{error}=await supabaseClient.auth.resetPasswordForEmail(email,{redirectTo:`${location.origin}${location.pathname}`});if(error)throw error;pendingRecoveryOtpEmail=email;$('#recoveryEmail').value='';$('#recoveryOtpDestination').textContent=email;$('#recoveryEmailArea')?.setAttribute('hidden','');$('#recoveryOtpArea')?.removeAttribute('hidden');$('#recoveryVerifyCode')?.removeAttribute('hidden');$('#recoveryOtpResend')?.removeAttribute('hidden');$('#recoverySendCode')?.setAttribute('hidden','');setStatus('recoveryStatus','If an account uses that email, a six-digit recovery code has been sent.','success');startOtpCooldown('#recoveryOtpResend',60);$('#recoveryOtpCode')?.focus()}catch(error){setStatus('recoveryStatus',error.message||'The recovery code could not be sent.','error')}finally{button.disabled=false}
      }
      async function verifyRecoveryOtp(){
        const token=cleanOtp($('#recoveryOtpCode')?.value);
        if(!pendingRecoveryOtpEmail){setStatus('recoveryStatus','Request a new recovery code.','error');return}
        if(token.length!==6){setStatus('recoveryStatus','Enter the complete six-digit code.','error');return}
        const button=$('#recoveryVerifyCode');button.disabled=true;setStatus('recoveryStatus','Verifying the recovery code…','');
        try{recoverySessionActive=true;authTransitionInProgress=true;const{data,error}=await supabaseClient.auth.verifyOtp({email:pendingRecoveryOtpEmail,token,type:'recovery'});authTransitionInProgress=false;if(error||!data?.session)throw error||new Error('The recovery code is invalid or expired.');pendingRecoveryOtpEmail='';$('#recoveryOtpCode').value='';window.toggleModal?.('recoveryPopup',false);openAuthModal('passwordResetPopup');setStatus('passwordResetStatus','Code verified. Choose a new password.','success')}catch(error){recoverySessionActive=false;authTransitionInProgress=false;$('#recoveryOtpCode').value='';$('#recoveryOtpCode')?.focus();setStatus('recoveryStatus',error.message||'The recovery code is incorrect or expired.','error')}finally{button.disabled=false}
      }


      $('#loginPasswordMode')?.addEventListener('click',()=>setLoginMethod('password'));
      $('#loginOtpMode')?.addEventListener('click',()=>setLoginMethod('otp'));
      $('#loginOtpSend')?.addEventListener('click',sendLoginOtp);
      $('#loginOtpVerify')?.addEventListener('click',verifyLoginOtp);
      $('#loginOtpResend')?.addEventListener('click',resendLoginOtp);
      $('#loginOtpCode')?.addEventListener('input',event=>{event.target.value=cleanOtp(event.target.value)});
      $('#loginOtpCode')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();verifyLoginOtp()}});
      $('#loginForm')?.addEventListener('submit',async event=>{event.preventDefault();if($('#loginPasswordPanel')?.hasAttribute('hidden'))return;const form=event.currentTarget,identifier=$('#loginIdentifier').value,password=$('#loginPassword').value,remember=$('#loginRemember').checked;if(!identifier||!password){setStatus('loginStatus','Enter your username or email and password.','error');return}form.querySelector('#loginIdentifier').value='';form.querySelector('#loginPassword').value='';await attemptLogin(identifier,password,remember,'loginStatus')});
      ...
      $('#googleLoginButton')?.addEventListener('click',()=>beginGoogleAuthentication($('#loginPasswordPanel')?.hasAttribute('hidden')?'loginOtpStatus':'loginStatus',$('#loginPasswordPanel')?.hasAttribute('hidden')?$('#loginOtpRemember')?.checked!==false:$('#loginRemember')?.checked!==false));
      ...
      $('#signupOtpCode')?.addEventListener('input',event=>{event.target.value=cleanOtp(event.target.value)});
      $('#signupOtpCode')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();verifySignupOtp()}});
      $('#signupOtpVerify')?.addEventListener('click',verifySignupOtp);
      $('#signupOtpResend')?.addEventListener('click',resendSignupOtp);
      $('#signupForm')?.addEventListener('submit',async event=>{event.preventDefault();if(!requireSupabase('signupStatus'))return;const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());setStatus('signupStatus','');if(!data.firstName||!data.lastName||!data.email||!data.username||!data.password||!data.confirmPassword){setStatus('signupStatus','Complete all required fields.','error');return}const username=String(data.username).trim();const email=String(data.email).trim().toLowerCase();if(!/^[A-Za-z0-9][A-Za-z0-9._-]{2,29}$/.test(username)){setStatus('signupStatus','Use 3–30 letters, numbers, dots, underscores or hyphens for the username.','error');return}if(String(data.password).length<8){setStatus('signupStatus','Password must contain at least 8 characters.','error');return}if(data.password!==data.confirmPassword){setStatus('signupStatus','The two passwords do not match.','error');return}form.reset();$('#signupRole').value='client';$('#signupOtpArea')?.setAttribute('hidden','');setStatus('signupStatus','Creating your secure Client account…','');const available=await supabaseClient.rpc('findat_username_available',{candidate:username});if(available.error){setStatus('signupStatus',available.error.message,'error');return}if(available.data!==true){setStatus('signupStatus','This username is already in use.','error');return}rememberSupabaseSession=true;authTransitionInProgress=true;const{data:authData,error}=await supabaseClient.auth.signUp({email,password:String(data.password),options:{emailRedirectTo:`${location.origin}${location.pathname}`,data:{username,first_name:String(data.firstName).trim(),last_name:String(data.lastName).trim(),phone:String(data.phone||'').trim(),organisation:String(data.organisation||'').trim(),country:'',role_request:'client'}}});authTransitionInProgress=false;if(error){setStatus('signupStatus',error.message||'The account could not be created.','error');return}if(authData.session&&authData.user){const account=await fetchOwnProfile(authData.user.id);setStatus('signupStatus','Account created. Opening the writing desk…','success');await enterDashboard(account)}else{pendingSignupOtpEmail=email;$('#signupOtpDestination').textContent=email;$('#signupOtpArea')?.removeAttribute('hidden');setStatus('signupStatus','Account created. Enter the six-digit code sent to your email.','success');startOtpCooldown('#signupOtpResend',60);$('#signupOtpCode')?.focus()}});
      $('#recoveryOtpCode')?.addEventListener('input',event=>{event.target.value=cleanOtp(event.target.value)});
      $('#recoveryOtpCode')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();verifyRecoveryOtp()}});
      $('#recoveryForm')?.addEventListener('submit',async event=>{event.preventDefault();await sendRecoveryOtp()});
      $('#recoveryVerifyCode')?.addEventListener('click',verifyRecoveryOtp);
      $('#recoveryOtpResend')?.addEventListener('click',()=>pendingRecoveryOtpEmail&&sendRecoveryOtp(pendingRecoveryOtpEmail));
      $('#passwordResetForm')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());if(!recoverySessionActive){setStatus('passwordResetStatus','Verify the six-digit recovery code first.','error');return}if(String(data.password||'').length<8||data.password!==data.confirmPassword){setStatus('passwordResetStatus','Use matching passwords of at least 8 characters.','error');return}form.reset();const{error}=await supabaseClient.auth.updateUser({password:String(data.password)});if(error){setStatus('passwordResetStatus',error.message||'The password could not be changed.','error');return}recoverySessionActive=false;setStatus('passwordResetStatus','Password updated. You can continue to your account.','success');setTimeout(async()=>{window.toggleModal?.('passwordResetPopup',false);const session=await getCurrentSession();if(session?.user){const account=await fetchOwnProfile(session.user.id);if(account)await enterDashboard(account)}},500)});
