...
(()=>{
      ...
      function profileFromRow(row){if(!row)return null;return{id:row.id,firstName:row.first_name||'',lastName:row.last_name||'',email:row.email||'',username:row.username||'',phone:row.phone||'',organisation:row.organisation||'',role:normaliseRole(row.role),country:row.country||'',active:row.active!==false,emailConfirmed:row.emailConfirmed===true||Boolean(row.email_confirmed_at),emailConfirmedAt:row.email_confirmed_at||'',lastSignInAt:row.last_sign_in_at||'',createdAt:row.created_at||'',updatedAt:row.updated_at||''}}
