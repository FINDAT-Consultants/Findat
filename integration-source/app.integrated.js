...
(()=>{
  ...
  const openAI={functionName:String(window.FINDAT_AUTH_CONFIG?.x1OpenAiFunction||'findat-x1-openai'),available:null,checking:false,model:'OpenAI',lastFailureAt:0};
  ...
  function updateOpenAIUi(text,state='idle'){
    ...
    if(host)host.dataset.state=state;if(label)label.textContent=text;
  }
  ...
  async function callOpenAI(prompt,plan,matches){
    ...
    const segments=collectEvidenceSegments(prompt,matches||[],12),evidence=segments.map((segment,index)=>({
  ...
  async function respond(raw){
    ...
    const thinking=addThinking(openAI.available!==false?'Preparing secure OpenAI financial synthesis':ollama.enabled&&ollama.available?'Retrieving evidence and synthesising with Ollama':'Planning the instruction and inspecting x1 tools');
    ...
    if(externalEligible){
      ...
      if(generation!==conversationGeneration){thinking?.remove();return}
      if(generated?.text){finalText=reviewAnswer(generated.text,prompt,result.plan);meta=`Secure OpenAI ${generated.model} · ${generated.evidenceCount} selected evidence passage${generated.evidenceCount===1?'':'s'}${generated.usage?.totalTokens?` · ${generated.usage.totalTokens} tokens`:''}`;provider='openai'}
    }
  ...
  loadImportedKnowledge();loadOllamaSettings();buildKnowledgeIndex();initialiseRecentItems();initialiseConversation();updateOpenAIUi('Secure OpenAI synthesis is available after sign-in','idle');setTimeout(()=>testOpenAI({silent:true}),900);
