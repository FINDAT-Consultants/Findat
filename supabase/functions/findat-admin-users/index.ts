    ...
    try {
      ...
      if (action === 'create') {
        ...
        if (!['consultant', 'client'].includes(role)) {
          return jsonResponse({ error: 'Administrators can create only Consultant or Client accounts. Promote an eligible Consultant separately when needed.' }, 400)
        }
      ...
      if (action === 'promote_admin') {
        ...
        if (target.role !== 'consultant') return jsonResponse({ error: 'Only a Consultant account can be promoted to Administrator. Client accounts cannot be assigned the Administrator role.' }, 403)
