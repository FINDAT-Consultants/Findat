...
(()=>{
      ...
      function hydrateUser(){$$('[data-user-fullname]').forEach(el=>el.textContent=accountName(activeAccount));$$('[data-user-firstname]').forEach(el=>el.textContent=activeAccount?.firstName||activeAccount?.username||'Writer');$$('[data-user-role]').forEach(el=>el.textContent=roleLabel(activeAccount?.role));$$('[data-user-initials]').forEach(el=>el.textContent=initials(activeAccount));$('#fdSidebarRole').textContent=`${roleLabel(activeAccount?.role)} workspace`;const intro=$('#fdWritingIntro'),saveButton=$('#fdSaveArticle');if(intro)intro.textContent=isAdmin()?'Write, edit and publish directly, or review submissions waiting for approval. You can edit, return, delete or approve submitted work.':activeAccount?.role==='consultant'?'Proofread assigned client articles, improve the writing, add cover photographs and supporting documents, then save the editorial changes.':'Write articles, essays and reports, add a cover photograph, then send completed work to an administrator for review and publication.';if(saveButton)saveButton.innerHTML=activeAccount?.role==='consultant'?'<i class="fas fa-save"></i> Save editorial changes':'<i class="fas fa-save"></i> Save draft'}
