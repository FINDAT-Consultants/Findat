  ...
  fetch: withSupabase({ auth: 'none' }, async (request, ctx) => {
    ...
    try {
      ...
      if (profile.active === false) {
        return jsonResponse({ error: 'This account is suspended. Contact a FINDAT Administrator.' }, 403)
      }
      ...
      if (error) {
        ...
        if (code === 'user_banned') {
          return jsonResponse({ error: 'This account is suspended. Contact a FINDAT Administrator.', code }, 403)
        }
