...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const progress = typeof showSystemProgress === 'function'
    ? showSystemProgress({
        title: 'Transferring',
        message: `Saving ${list.length} item${list.length === 1 ? '' : 's'} to ${destinationLabel}`,
        state: 'Transferring…',
        progress: 3
      })
    : null;
  ...
  for (const item of list) {
    progress?.update(progressPercent(transferredBytes, totalBytes), {
      title: 'Transferring',
      message: item.name,
      state: `Transferring ${completed + 1} of ${list.length}…`
    });
    try {
      ...
      progress?.update(progressPercent(transferredBytes, totalBytes), {
        title: 'Transferring',
        message: item.name,
        state: `Transferring ${completed} of ${list.length}…`
      });
    } catch (error) {
  ...
  if (!completed) {
    progress?.fail({
      title: 'Transferring',
      message: failures[0] || 'No items were transferred',
      state: 'Failed',
      progress: 100
    });
  ...
  progress?.complete({
    title: 'Transferring',
    message: `${completed} item${completed === 1 ? '' : 's'} transferred to ${destinationLabel}`,
    state: 'Complete'
  });
}
...
function findatGitHubSettingsMarkup() {
  const config = findatGitHubDrive.config || findatLoadGitHubConfig();
  const access = findatGitHubDrive.token ? 'Write token active for this page' : 'No token — public repositories are read-only';
  return `<div class="settings-section" data-settings-panel="cloud">
    ...
    <section class="settings-card github-storage-card">
      ...
      <p>Use one repository and branch on every device. Repository settings may be remembered locally; the token is held only in memory until this page closes.</p>
      <div class="github-settings-grid">
        ...
        <label class="github-token-field"><span>Fine-grained token</span><input data-github-token type="password" autocomplete="off" placeholder="Contents: Read and write"></label>
      </div>
...
async function findatConnectFromSettings(win) {
  ...
  findatGitHubDrive.token = String(qs('[data-github-token]', panel)?.value || '').trim();
...
refreshCloudSettingsPanel = async function(win) {
  ...
  try {
    ...
    const access = qs('[data-github-access]', panel); if (access) access.textContent = findatGitHubDrive.token ? 'Read and write access is active for this page' : 'Read-only access; enter a token to upload or change files';
  } catch (error) {
    qs('.cloud-status-dot', panel)?.classList.add('error');
...
apps.about.html = () => `<div class="about"><div class="about-logo"><span class="aurelia-symbol"></span></div><h1>FINDAT Cloud</h1><p>Version 5.0 GitHub Cloud Edition</p><p>Documents are stored in a GitHub repository so multiple devices can access the same workspace.</p><small>Browser storage is used only for interface preferences and repository settings; document contents remain on GitHub.</small></div>`;
