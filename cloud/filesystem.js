...
async function restoreLocalSnapshot(snapshotNodes) {
  const snapshot = Array.isArray(snapshotNodes) ? snapshotNodes : [];
  const current = await databaseAll();
  const snapshotPaths = new Set(snapshot.map(node => node.path));
  const removePaths = current
    .map(node => node.path)
    .filter(path => !snapshotPaths.has(path));
  await databaseCommit(snapshot, removePaths);
}
...
async function fsRequest(route, options = {}, params = {}) {
  ...
  try {
    ...
    if (mutating && typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) {
      // Do not let a stale optional health function block a real upload or
      // deletion. Continue with the local transaction, then let the direct
      // Storage/PostgreSQL write determine whether cloud persistence succeeds.
      try {
    ...
    if (mutating && beforeNodes && typeof sharedCloudPushDiff === 'function') {
      ...
      try {
        ...
        // In cloud-authoritative mode, never leave a failed upload/delete as a
        // normal local document. Roll the browser cache back to the exact state
        // that existed before the operation.
        if (window.FINDAT_CLOUD_CONFIG?.cloudAuthoritative !== false) {
          await restoreLocalSnapshot(beforeNodes);
...
function setCloudBusy(busy, failed = false) {
  ...
  indicator.title = failed || !fsState.cloudOnline
    ? shared ? 'Shared FINDAT Cloud unavailable; cloud changes are not saved' : 'FINDAT Cloud storage unavailable'
...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const cloudProgressHandler = event => {
    ...
    if (detail.provider === 'supabase' && detail.phase === 'database-fallback') {
      progress?.update(98, {
        title: 'Repairing Supabase SQL write',
        message: detail.name || 'Document metadata',
        state: detail.message || 'Retrying through the validated PostgreSQL function…'
      });
      return;
    }
  ...
  if (!completed) {
    progress?.fail({
      ...
      state: 'Not stored in Supabase; local transaction rolled back',
      progress: 100
    });
