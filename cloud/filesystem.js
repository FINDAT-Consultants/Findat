...
const DESKTOP_LAYOUT_POLICY_VERSION = 1;
...
function loadDesktopView() {
  const defaults = {
    iconSize: 'medium', autoArrange: true, alignGrid: true, showIcons: true, sortBy: 'name',
  ...
  try {
    ...
    // One-time migration for existing FINDAT installations: enable the compact,
    // collision-free desktop arrangement without removing any other preference.
...
function desktopGridPositions(reserved = [], minimumCount = 0) {
  ...
  const stepX = metrics.width + metrics.gap;
  const stepY = metrics.height + metrics.gap;
  const maxY = Math.max(0, height - metrics.height);
  const maxX = Math.max(0, width - metrics.width);
  const topInset = Math.min(metrics.gap, maxY);
  const rightInset = Math.min(metrics.gap, maxX);
  const rows = [];
  const positions = [];

  for (let y = topInset; y <= maxY + 0.5; y += stepY) rows.push(Math.round(y));
  if (!rows.length) rows.push(0);

  // Arrange user documents vertically, then continue in compact columns from
  // right to left. Reserved system-icon cells are skipped automatically.
  for (let x = maxX - rightInset; x >= 0; x -= stepX) {
    for (const y of rows) {
      const candidate = { x: Math.round(x), y };
      if (!reserved.some(position => desktopPositionsOverlap(candidate, position, metrics))) positions.push(candidate);
    }
  }

  // A normal desktop has ample room, but for unusually large workspaces add
  // scrollable overflow columns rather than ever stacking icons on each other.
  let overflowColumn = 0;
  while (positions.length < minimumCount) {
    const x = Math.round(width + metrics.gap + overflowColumn * stepX);
    for (const y of rows) positions.push({ x, y });
    overflowColumn += 1;
  }
  return positions;
}
...
function nearestFreeDesktopPosition(desired, occupied = [], candidates = []) {
  ...
  if (!free.length) return desired ? clampDesktopPosition(desired.x, desired.y) : defaultDesktopPosition(occupied.length);
  if (!desired) return free[0];
  return free.reduce((best, candidate) => {
    const bestDistance = (best.x - desired.x) ** 2 + (best.y - desired.y) ** 2;
    const candidateDistance = (candidate.x - desired.x) ** 2 + (candidate.y - desired.y) ** 2;
...
function layoutDesktopIcons() {
  ...
  // Keep workstation controls in one compact left-hand column.
  systemIcons.forEach((icon, index) => {
    bindDesktopIconPositioning(icon);
    const position = systemDesktopPosition(index);
  ...
  const candidates = desktopGridPositions(reserved, fileIcons.length);
  const occupied = [];

  // Every document receives a unique grid cell. Existing custom positions are
  // respected when possible; collisions are moved to the nearest free cell.
  fileIcons.forEach(icon => {
    ...
    if (!desktopView.autoArrange && desktopView.sortBy === 'custom' && saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) {
      desired = desktopView.alignGrid ? snapDesktopPosition(saved.x, saved.y) : clampDesktopPosition(saved.x, saved.y);
    }
