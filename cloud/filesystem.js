...
function requireRemoteDocumentStorage() {
  ...
  if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled())) {
    ...
    throw new Error(detail || 'Supabase Storage is required for document uploads');
  }
}
...
async function initializeDrive() {
  ...
  if (typeof sharedCloudInitialSync === 'function') {
    ...
    initialCloudSyncPromise.then(() => {
      ...
      console.warn('Supabase Storage is unavailable; document uploads remain blocked until it reconnects.', error);
    });
  }
...
async function readLocalFile(path) {
  ...
  throw new Error(`${node.name} is not available in Supabase Storage`);
}
...
function setCloudBusy(busy, failed = false) {
  ...
  indicator.title = failed || !fsState.cloudOnline
    ? shared ? 'FINDAT Cloud is temporarily unavailable; document uploads are paused' : 'FINDAT Cloud storage unavailable'
    : busy ? shared ? 'Synchronising with FINDAT Cloud' : 'Saving to FINDAT Cloud'
...
async function renderViewer(win, entry) {
  ...
  try {
    if (entry.mime?.startsWith('image/')) {
      ...
      if (!officeUrl) {
        content.innerHTML = `<div class="generic-preview">${fileGlyph(entry)}<h2>${escapeHtml(entry.name)}</h2><p>This Office document needs a public Supabase link before it can be previewed in the browser.</p><div class="generic-preview-actions"><button data-generic-download>Download</button></div></div>`;
...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const cloudProgressHandler = event => {
    ...
    progress?.update(value, {
      ...
      state: supabase
        ? (detail.phase === 'uploaded' ? 'Cloud storage confirmed the upload; applying green tick…' : 'Uploading document…')
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    <div class="settings-heading"><div><h2>FINDAT Cloud</h2><p>Manage Cloud documents, imports and backups.</p></div><span class="settings-heading-icon cloud-heading-logo"></span></div>
    ...
    <section class="settings-card"><b>Cloud storage</b><p data-shared-cloud-status>Cloud storage is being checked.</p><div class="settings-button-row"><button data-cloud-action="sync">Sync Now</button><button data-cloud-action="publish-local">Send Pending Files</button></div><small>Documents are saved securely in FINDAT Cloud and remain available across authorised devices.</small></section>
    ...
    <section class="settings-card"><b>Device settings</b><p>Only desktop preferences are kept on this device. Documents remain in FINDAT Cloud.</p></section>
    ...
    qsa('[data-cloud-action]', win).forEach(button => button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (action === 'publish-local') {
          ...
          toast(result.published ? `${result.published} pending item${result.published === 1 ? '' : 's'} saved to Supabase` : 'No Cloud uploads are pending');
        }
...
async function refreshCloudSettingsPanel(win) {
  ...
  try {
    ...
    const storageLabel = qs('[data-storage-label]', panel); if (storageLabel) { storageLabel.textContent = `Used ${formatBytes(usage.logicalUsed)} of ${formatBytes(usage.quota)} in Supabase · document bytes cached locally: ${formatBytes(usage.stored)}`; storageLabel.title = 'Only file and folder metadata is retained in the browser'; }
    ...
    if (sharedStatus.enabled) {
      ...
      if (detail) detail.textContent = sharedStatus.lastError || `${sharedStatus.remoteCount} items in Supabase · ${files + folders} desktop metadata records · no local document cache`;
      if (sharedLine) {
        const destination = sharedStatus.provider === 'github'
          ...
          : `Supabase Storage bucket ${sharedStatus.bucket || 'findat-documents'}`;
        ...
        sharedLine.textContent = sharedStatus.lastError
          ? `Supabase sync failed and uploads are paused; no document bytes are kept locally: ${sharedStatus.lastError}`
      ...
      if (statusTitle) statusTitle.textContent = 'Supabase connection required';
      if (detail) detail.textContent = `The desktop can display metadata, but document uploads require Supabase Storage`;
      if (sharedLine) {
          ...
          sharedLine.textContent = !hasUrl
            ...
            : /^sb_secret_/i.test(key)
              ...
              : (!key || key.includes('PASTE_') || key.includes('YOUR_'))
                ...
                : 'Supabase is configured but currently unavailable. Check the Storage bucket policies and network connection.';
        } else {
...
wallpaperInput?.addEventListener('change', async () => {
  ...
  try {
    if (!(typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) || typeof sharedCloudUploadCustomWallpaper !== 'function') {
      throw new Error('Supabase Storage must be connected before using a custom wallpaper');
    }
