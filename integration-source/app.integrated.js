...
const mainNav = document.getElementById('mainNav');
    ...
    const findatProtectedPosterPath=window[String.fromCharCode(95,95,70,73,78,68,65,84,95,80,82,79,84,69,67,84,69,68,95,80,79,83,84,69,82,95,95)]||'';
    if(recordingPlayer){
...
(()=>{
      ...
      let workstationCache=[];
      let sitePatchCache=[];
      let sitePatchChannel=null;
      let developerPythonWorker=null;
      let developerPythonRequest=0;
      const platformVisitorId=(()=>{try{let value=sessionStorage.getItem('findat-platform-visitor-v1');if(!value){value=globalThis.crypto?.randomUUID?.()||`visitor-${Date.now()}-${Math.random().toString(16).slice(2)}`;sessionStorage.setItem('findat-platform-visitor-v1',value)}return value}catch{return`visitor-${Date.now()}-${Math.random().toString(16).slice(2)}`}})();
      ...
      async function trackPlatformEvent(eventType,detail={}){
        if(!supabaseClient)return;
        const payload={p_event_type:String(eventType||'activity').slice(0,80),p_page_path:String(detail.page||location.hash||location.pathname).slice(0,240),p_page_title:String(detail.title||document.title).slice(0,180),p_entity_type:String(detail.entityType||'').slice(0,80),p_entity_id:String(detail.articleId||detail.entityId||'').slice(0,120)||null,p_detail:detail,p_visitor_id:platformVisitorId,p_referrer:String(document.referrer||'').slice(0,500),p_timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'',p_language:navigator.language||'',p_exit_reason:String(detail.exitReason||'').slice(0,120)};
        const result=await supabaseClient.rpc('findat_track_platform_event',payload);if(result.error&&result.error.code!=='PGRST202')console.warn('Platform event tracking unavailable',result.error)
      }
      window.FINDAT_TRACK_PLATFORM_EVENT=trackPlatformEvent;
      function groupCounts(rows,key){const map=new Map();rows.forEach(row=>{const value=String(typeof key==='function'?key(row):row[key]||'Unknown').trim()||'Unknown';map.set(value,(map.get(value)||0)+1)});return[...map].sort((a,b)=>b[1]-a[1])}
      function renderAdminChart(hostId,rows){const host=$(hostId);if(!host)return;const max=Math.max(1,...rows.map(item=>item[1]));host.innerHTML=rows.length?rows.slice(0,10).map(([label,value])=>`<div class="fd-admin-chart-row"><span title="${escapeHTML(label)}">${escapeHTML(label)}</span><i><b style="width:${Math.max(3,value/max*100).toFixed(1)}%"></b></i><strong>${value.toLocaleString()}</strong></div>`).join(''):'<div class="fd-activity-empty">No data has been recorded yet.</div>'}
      async function refreshAdminActivity(){
        if(!isAdmin())return;setStatus('fdActivityDashboardStatus','Loading protected activity data…','');
        const [eventsResult,auditResult]=await Promise.all([supabaseClient.from('findat_platform_events').select('*').order('occurred_at',{ascending:false}).limit(2000),supabaseClient.from('findat_audit_log').select('*').order('occurred_at',{ascending:false}).limit(1000)]);
        if(eventsResult.error)throw eventsResult.error;if(auditResult.error)throw auditResult.error;const events=eventsResult.data||[],audit=auditResult.data||[];
        $('#fdMetricVisitors').textContent=String(new Set(events.map(row=>row.visitor_id).filter(Boolean)).size);$('#fdMetricPageViews').textContent=String(events.filter(row=>['page_view','workspace_view'].includes(row.event_type)).length);$('#fdMetricSignedIn').textContent=String(new Set(events.map(row=>row.actor_id).filter(Boolean)).size);$('#fdMetricAudit').textContent=String(audit.length+events.length);if($('#fdActivityNavCount'))$('#fdActivityNavCount').textContent=String(events.length);
        renderAdminChart('#fdTopPagesChart',groupCounts(events.filter(row=>['page_view','workspace_view','article_open','information_retrieved'].includes(row.event_type)),row=>row.page_title||row.page_path||'Unknown'));
        renderAdminChart('#fdVisitorOriginsChart',groupCounts(events,row=>row.country_signal||row.timezone||row.language||'Unknown'));
        renderAdminChart('#fdExitChart',groupCounts(events.filter(row=>row.event_type==='session_exit'||row.exit_reason),row=>row.exit_reason||'Page closed'));
        const activityRows=[
          ...audit.map(row=>({time:row.occurred_at,user:row.actor_name||'System',role:row.actor_role||'',action:row.action||'Audit event',entity:row.page_path||row.article_id||row.entity_id||'—',detail:row.detail||''})),
          ...events.map(row=>({time:row.occurred_at,user:row.actor_name||'Visitor',role:row.actor_role||'',action:String(row.event_type||'activity').replaceAll('_',' '),entity:row.page_title||row.page_path||row.entity_id||'—',detail:row.detail&&Object.keys(row.detail).length?JSON.stringify(row.detail):row.exit_reason||''}))
        ].sort((a,b)=>new Date(b.time)-new Date(a.time)).slice(0,750);
        const body=$('#fdAdminAuditBody');if(body)body.innerHTML=activityRows.length?activityRows.map(row=>`<tr><td>${escapeHTML(formatTime(row.time))}</td><td><strong>${escapeHTML(row.user)}</strong><small>${escapeHTML(row.role)}</small></td><td>${escapeHTML(row.action)}</td><td>${escapeHTML(row.entity)}</td><td class="fd-audit-detail">${escapeHTML(row.detail)}</td></tr>`).join(''):'<tr><td colspan="5">No activity has been recorded.</td></tr>';
        setStatus('fdActivityDashboardStatus',`Loaded ${events.length} activity events and ${audit.length} audit records.`,'success')
      }
      async function refreshWorkstations(){
        if(!isAdmin())return;const body=$('#fdWorkstationTableBody');if(body)body.innerHTML='<tr><td colspan="4">Loading workstations…</td></tr>';const result=await supabaseClient.rpc('findat_admin_list_workstations');if(result.error)throw result.error;workstationCache=result.data||[];populateWorkstationUsers();if(body)body.innerHTML=workstationCache.length?workstationCache.map(row=>`<tr><td><strong>Work Station No.${Number(row.workstation_no)}</strong><small>${escapeHTML(row.name||'FINDAT Cloud')}</small></td><td>${escapeHTML(row.assigned_name||row.assigned_username||'Unassigned')}</td><td><span class="fd-account-status ${row.active?'is-active':'is-suspended'}">${row.active?'Active':'Disabled'}</span></td><td><div class="fd-account-controls"><button class="fd-mini-btn" data-workstation-edit="${row.id}" type="button"><i class="fas fa-edit"></i> Edit</button><button class="fd-mini-btn is-danger" data-workstation-delete="${row.id}" type="button"><i class="fas fa-trash"></i> Delete</button></div></td></tr>`).join(''):'<tr><td colspan="4">No managed workstations exist yet.</td></tr>'
      }
      async function saveWorkstationForm(event){event.preventDefault();if(!isAdmin())return;const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());form.reset();setStatus('fdWorkstationStatus','Saving workstation and hashing its password…','');const result=await supabaseClient.rpc('findat_admin_save_workstation',{p_user_id:String(data.userId||''),p_workstation_no:Number(data.workstationNo),p_name:String(data.name||'').trim(),p_password:String(data.password||'')});if(result.error){setStatus('fdWorkstationStatus',result.error.message||'The workstation could not be saved.','error');return}setStatus('fdWorkstationStatus','Workstation assigned and password saved securely.','success');await refreshWorkstations();await trackPlatformEvent('workstation_assigned',{entityId:String(result.data||''),userId:data.userId,workstationNo:Number(data.workstationNo)})}
      async function deleteWorkstation(id){if(!confirm('Delete this workstation assignment? Cloud documents are not deleted.'))return;const result=await supabaseClient.rpc('findat_admin_delete_workstation',{p_workstation_id:id});if(result.error){setStatus('fdWorkstationStatus',result.error.message||'The workstation could not be deleted.','error');return}await refreshWorkstations();setStatus('fdWorkstationStatus','Workstation assignment deleted.','success')}
      async function loadAssignedWorkstations(){if(!supabaseClient||!activeAccount)return[];const result=await supabaseClient.rpc('findat_my_workstations');if(result.error){console.warn('Managed workstations unavailable',result.error);return[]}return result.data||[]}
      window.FINDAT_GET_CLOUD_CONTEXT=async()=>{const session=await getCurrentSession();return{supabaseUrl:SUPABASE_URL,publishableKey:SUPABASE_KEY,accessToken:session?.access_token||'',user:activeAccount?{id:activeAccount.id,name:accountName(activeAccount),role:activeAccount.role}:null,workstations:activeAccount?await loadAssignedWorkstations():[]}}
      function applySitePatchRows(rows){
        sitePatchCache=rows||[];const htmlPatch=sitePatchCache.find(row=>row.language==='html'&&row.active),cssPatch=sitePatchCache.find(row=>row.language==='css'&&row.active),jsPatch=sitePatchCache.find(row=>row.language==='javascript'&&row.active);const host=$('#findatLivePatchHost');if(host)host.innerHTML=htmlPatch?sanitiseHTML(htmlPatch.code||''):'';let styleTag=$('#findatLivePatchStyle');if(cssPatch){if(!styleTag){styleTag=document.createElement('style');styleTag.id='findatLivePatchStyle';document.head.appendChild(styleTag)}styleTag.textContent=cssPatch.code||''}else styleTag?.remove();if(jsPatch&&window.__FINDAT_LIVE_JS_VERSION__!==jsPatch.updated_at){window.__FINDAT_LIVE_JS_VERSION__=jsPatch.updated_at;try{Function('"use strict";\n'+String(jsPatch.code||''))()}catch(error){console.error('FINDAT live JavaScript patch failed',error)}}
      }
      async function refreshSitePatches(){if(!supabaseClient)return;const result=await supabaseClient.from('findat_site_patches').select('id,name,language,code,active,updated_at').eq('active',true).order('updated_at',{ascending:false});if(result.error){if(result.error.code!=='42P01')console.warn('Site patches unavailable',result.error);return}applySitePatchRows(result.data||[])}
      function startSitePatchRealtime(){if(!supabaseClient||typeof supabaseClient.channel!=='function')return;if(sitePatchChannel)try{supabaseClient.removeChannel(sitePatchChannel)}catch{}sitePatchChannel=supabaseClient.channel('findat-site-patches').on('postgres_changes',{event:'*',schema:'public',table:'findat_site_patches'},()=>refreshSitePatches()).subscribe()}
      function selectedDeveloperPatch(){const language=$('#fdDeveloperLanguage')?.value||'html',name=String($('#fdDeveloperPatchName')?.value||'main').trim()||'main';return sitePatchCache.find(row=>row.language===language&&row.name===name)||null}
      async function loadDeveloperEditor(){if(!isAdmin())return;await refreshSitePatches();const patch=selectedDeveloperPatch();if($('#fdDeveloperCode'))$('#fdDeveloperCode').value=patch?.code||({html:'<section class="custom-announcement"><h2>Custom FINDAT update</h2><p>Edit this controlled HTML region.</p></section>',css:'.custom-announcement { padding: 24px; }',javascript:'console.info("FINDAT live patch loaded");',python:'print("FINDAT Python workspace ready")',java:'public class FindatTool {\n  public static void main(String[] args) {\n    System.out.println("FINDAT Java source workspace");\n  }\n}'}[$('#fdDeveloperLanguage')?.value||'html']||'');setStatus('fdDeveloperStatus',patch?'Loaded the current Supabase source patch.':'No saved patch exists for this name and language.','')}
      function validateDeveloperSource(){const language=$('#fdDeveloperLanguage')?.value||'html',source=$('#fdDeveloperCode')?.value||'',out=$('#fdDeveloperOutput');let message='';if(!source.trim())message='Source is empty.';else if(language==='html'){const template=document.createElement('template');template.innerHTML=source;message=`HTML parsed with ${template.content.querySelectorAll('*').length} element(s).`}else if(language==='css'){try{const sheet=new CSSStyleSheet();sheet.replaceSync(source);message=`CSS parsed with ${sheet.cssRules.length} rule(s).`}catch(error){message=`CSS error: ${error.message}`}}else if(language==='javascript'){try{Function('"use strict";\n'+source);message='JavaScript syntax parsed successfully.'}catch(error){message=`JavaScript error: ${error.message}`}}else if(language==='python')message='Python source is ready to run in the browser sandbox.';else{const opens=(source.match(/\{/g)||[]).length,closes=(source.match(/\}/g)||[]).length;message=opens===closes?'Java source passed basic structural validation. Use an isolated Java build service for compilation.':'Java source has unbalanced braces.'}if(out)out.textContent=message;setStatus('fdDeveloperStatus',message,message.includes('error')||message.includes('unbalanced')?'error':'success')}
      async function saveDeveloperPatch(){if(!isAdmin())return;const language=$('#fdDeveloperLanguage')?.value||'html',name=String($('#fdDeveloperPatchName')?.value||'main').trim()||'main',source=$('#fdDeveloperCode')?.value||'';setStatus('fdDeveloperStatus','Saving the versioned source in Supabase…','');const result=await supabaseClient.rpc('findat_admin_save_site_patch',{p_name:name,p_language:language,p_code:source,p_active:true});if(result.error){setStatus('fdDeveloperStatus',result.error.message||'The patch could not be saved.','error');return}await refreshSitePatches();setStatus('fdDeveloperStatus',language==='python'||language==='java'?'Source saved in Supabase. Python and Java are not applied to the public page.':'Patch saved and applied live across FINDAT.','success');await trackPlatformEvent('developer_patch_applied',{entityId:String(result.data||''),language,name})}
      function getDeveloperPythonWorker(){if(developerPythonWorker)return developerPythonWorker;developerPythonWorker=new Worker(new URL('assets/js/developer-python-worker.mjs',document.baseURI).href,{type:'module'});return developerPythonWorker}
      async function runDeveloperPython(){const source=$('#fdDeveloperCode')?.value||'',language=$('#fdDeveloperLanguage')?.value,out=$('#fdDeveloperOutput');if(language!=='python'){setStatus('fdDeveloperStatus','Select Python before running browser code.','error');return}setStatus('fdDeveloperStatus','Running Python in the browser sandbox…','');const worker=getDeveloperPythonWorker(),id=`devpy-${Date.now()}-${++developerPythonRequest}`;const result=await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('Python timed out.')),60000);const handler=event=>{if(event.data?.id!==id)return;clearTimeout(timer);worker.removeEventListener('message',handler);event.data.ok?resolve(event.data):reject(new Error(event.data.error||'Python failed.'))};worker.addEventListener('message',handler);worker.postMessage({id,code:source})});if(out)out.textContent=result.stdout||'(No output)';setStatus('fdDeveloperStatus','Python completed in the browser sandbox.','success')}
      async function triggerNetlifyDeploy(){if(!isAdmin())return;setStatus('fdDeveloperStatus','Requesting a Netlify production build…','');try{const result=await invokeFunction('findat-developer-deploy',{reason:'Administrator Developer Studio'},true);setStatus('fdDeveloperStatus',result.message||'Netlify build hook triggered.','success')}catch(error){setStatus('fdDeveloperStatus',error.message||'The build hook could not be triggered. Configure FINDAT_NETLIFY_BUILD_HOOK in Edge Function secrets.','error')}}
      ...
      async function logout(){await trackPlatformEvent('logout',{exitReason:'User signed out'}).catch(()=>{});await clearSession();editingArticleId='';leaveDashboard(false);syncPublicAuthBar(null);await refreshPublicArticles();openAuthModal('loginPopup')}
      function showView(view){
        const delegated=activeAccount?.role==='consultant'&&workspaceCache.articles.some(article=>article.status==='Pending approval'&&article.publisherId===activeAccount.id);
        if(['accounts','activity','developer'].includes(view)&&!isAdmin())view='writing';
        if(view==='review'&&!isAdmin()&&!delegated)view='writing';
        $$('[data-view-panel]').forEach(panel=>panel.classList.toggle('is-active',panel.dataset.viewPanel===view));
        $$('[data-dashboard-view]').forEach(button=>button.classList.toggle('is-active',button.dataset.dashboardView===view));
        const titles={writing:'Writing Desk',review:'Review & Approval',publications:'Published Articles',accounts:'User Accounts',activity:'Activity Dashboard',developer:'Developer Studio'};
        $('#fdTopbarTitle').textContent=titles[view]||'Writing Desk';
        $('#fdDashboardMain')?.scrollTo({top:0,behavior:'smooth'});
        if(view==='accounts'&&isAdmin())Promise.all([refreshAdminAccounts(),refreshWorkstations()]).catch(console.warn);
        if(view==='activity'&&isAdmin())refreshAdminActivity().catch(console.warn);
        if(view==='developer'&&isAdmin())loadDeveloperEditor().catch(console.warn);
        trackPlatformEvent('workspace_view',{page:view,title:titles[view]||view}).catch(()=>{});
        closeSidebar()
      }
      ...
      function renderCollaborationInbox(){
        ...
        list.innerHTML=rows.length?rows.map(request=>{
          ...
          const open=request.article_id?`<button class="fd-mini-btn fd-notification-open" data-notification-open="${request.article_id}" data-notification-kind="${escapeHTML(request.kind||'')}" type="button"><i class="fas ${request.kind==='article_comment'?'fa-comments':'fa-arrow-right'}"></i> ${request.kind==='article_comment'?'Open discussion':'Open article'}</button>`:'';
          return `<article class="fd-collaboration-request is-${escapeHTML(request.status||'none')} ${request.is_read===false?'is-unread':''}" data-notification-article="${escapeHTML(request.article_id||'')}">${avatarMarkup(person,'fd-collaboration-avatar')}<div><p>${sentence}</p><div class="fd-collaboration-meta"><span class="fd-collaboration-status is-${escapeHTML(request.status||'none')}">${escapeHTML(collaborationStatusLabel(request.status))}</span><time>${escapeHTML(formatDate(request.updated_at||request.created_at))}</time></div>${actions}${open}</div></article>`
      ...
      async function respondCollaboration(articleId,accept,button=null){
        ...
        try{
          ...
          if(result.error)throw result.error;
          await Promise.all([refreshCollaborationState(),refreshArticles()]);
          await trackPlatformEvent(accept?'collaboration_accepted':'collaboration_rejected',{articleId,entityType:'collaboration'});
          renderAll();
      ...
      async function addArticleComment(){
        ...
        try{const result=await supabaseClient.rpc('findat_add_article_comment',{p_article_id:article.id,p_body:body,p_parent_id:articleCommentReplyId||null});if(result.error)throw result.error;if(input)input.value='';const wasReply=Boolean(articleCommentReplyId);setCommentReply();await refreshArticleActivity(article.id);await refreshCollaborationState();await trackPlatformEvent(wasReply?'article_comment_reply':'article_comment',{articleId:article.id,entityType:'article_comment',title:article.title});setStatus('fdArticleActivityStatus','Comment saved and collaborators notified.','success')}
      ...
      function selectedDataSeries(grid){
        const categoryIndex=Math.max(0,Math.min(grid.headers.length-1,Number($('#fdChartCategoryColumn')?.value)||0));
        let valueIndex=Math.max(0,Math.min(grid.headers.length-1,Number($('#fdChartValueColumn')?.value)||1));
        if(valueIndex===categoryIndex&&grid.headers.length>1)valueIndex=categoryIndex===0?1:0;
        const raw=grid.rows.map((row,index)=>({label:String(row[categoryIndex]||`Row ${index+1}`),value:Number(String(row[valueIndex]||'').replace(/[^0-9+.\-]/g,''))})).filter(point=>Number.isFinite(point.value));
        if(raw.length<2)throw new Error('The selected value column needs at least two numeric values.');
        const mode=$('#fdChartAggregation')?.value||'none';
        if(mode==='none')return{label:grid.headers[valueIndex],points:raw};
        const groups=new Map();raw.forEach(point=>{const list=groups.get(point.label)||[];list.push(point.value);groups.set(point.label,list)});
        const points=[...groups].map(([label,values])=>{let value=values[0];if(mode==='sum')value=values.reduce((a,b)=>a+b,0);else if(mode==='average')value=values.reduce((a,b)=>a+b,0)/values.length;else if(mode==='count')value=values.length;else if(mode==='min')value=Math.min(...values);else if(mode==='max')value=Math.max(...values);return{label,value}});
        return{label:`${mode} of ${grid.headers[valueIndex]}`,points}
      }
      function articleNumericSeries(grid){return selectedDataSeries(grid)}
      function syncVisualColumns(){
        const category=$('#fdChartCategoryColumn'),value=$('#fdChartValueColumn');if(!category||!value)return;
        try{const grid=parseArticleData($('#fdDataInput')?.value||'');const oldCategory=Number(category.value)||0,oldValue=Number(value.value)||1;const options=grid.headers.map((header,index)=>`<option value="${index}">${escapeHTML(header)}</option>`).join('');category.innerHTML=options;value.innerHTML=options;category.value=String(Math.min(oldCategory,grid.headers.length-1));value.value=String(Math.min(oldValue,grid.headers.length-1));if(category.value===value.value&&grid.headers.length>1)value.value=category.value==='0'?'1':'0'}catch{category.innerHTML='<option value="0">First column</option>';value.innerHTML='<option value="1">Second column</option>'}
      }
      ...
      function dataChartHTML(grid,type){
        const series=selectedDataSeries(grid),title=$('#fdChartTitle')?.value.trim()||`${series.label} by ${grid.headers[Number($('#fdChartCategoryColumn')?.value)||0]}`,points=series.points.slice(0,30),values=points.map(point=>point.value),min=Math.min(0,...values),max=Math.max(0,...values),range=max-min||1;
        if(type==='pie'||type==='doughnut'){
          const positives=points.map(point=>({...point,value:Math.max(0,point.value)})).filter(point=>point.value>0),total=positives.reduce((sum,point)=>sum+point.value,0);if(!total)throw new Error('A pie or doughnut chart needs positive values.');let cursor=0;const shades=['#d7651b','#274362','#ef9a45','#5e7895','#9d4e1f','#7f97ad','#f0bd7d','#415a73'];const stops=positives.map((point,index)=>{const start=cursor;cursor+=point.value/total*100;return`${shades[index%shades.length]} ${start.toFixed(2)}% ${cursor.toFixed(2)}%`});return`<figure class="fd-article-data-block fd-article-chart"><figcaption>${escapeHTML(title)}</figcaption><div class="fd-pie-layout"><div class="fd-article-pie ${type==='doughnut'?'is-doughnut':''}" style="background:conic-gradient(${stops.join(',')})"></div><ul>${positives.map((point,index)=>`<li><i style="background:${shades[index%shades.length]}"></i><span>${escapeHTML(point.label)}</span><strong>${escapeHTML(point.value.toLocaleString())}</strong></li>`).join('')}</ul></div></figure>`
        }
        if(type==='line'||type==='area'||type==='scatter'){
          const coords=points.map((point,index)=>{const x=points.length===1?50:index/(points.length-1)*100,y=94-(point.value-min)/range*84;return{x,y,label:point.label,value:point.value}});
          const poly=coords.map(point=>`${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(' '),area=`0,94 ${poly} 100,94`;
          return`<figure class="fd-article-data-block fd-article-chart"><figcaption>${escapeHTML(title)}</figcaption><svg class="fd-line-chart" viewBox="0 0 100 100" role="img" aria-label="${escapeHTML(title)}"><line x1="0" y1="94" x2="100" y2="94"></line>${type==='area'?`<polygon points="${area}" fill="rgba(215,101,27,.18)" stroke="none"></polygon>`:''}${type!=='scatter'?`<polyline points="${poly}"></polyline>`:''}${coords.map(point=>`<circle cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="${type==='scatter'?2.8:2}"><title>${escapeHTML(point.label)}: ${escapeHTML(point.value.toLocaleString())}</title></circle>`).join('')}</svg><div class="fd-chart-label-row">${points.map(point=>`<span>${escapeHTML(point.label)}</span>`).join('')}</div></figure>`
        }
        const maximum=Math.max(...values.map(value=>Math.abs(value)),1);return`<figure class="fd-article-data-block fd-article-chart"><figcaption>${escapeHTML(title)}</figcaption><div class="fd-bar-chart">${points.map(point=>`<div class="fd-bar-row"><span>${escapeHTML(point.label)}</span><i><b style="width:${Math.max(2,Math.abs(point.value)/maximum*100).toFixed(2)}%"></b></i><strong>${escapeHTML(point.value.toLocaleString())}</strong></div>`).join('')}</div></figure>`
      }
      function previewDataChart(){try{const grid=parseArticleData($('#fdDataInput')?.value),html=dataChartHTML(grid,$('#fdChartType')?.value||'bar'),host=$('#fdDataChartPreview');if(host)host.innerHTML=html;setStatus('fdDataStatus','Visual preview updated.','success')}catch(error){setStatus('fdDataStatus',error.message||'The data could not be previewed.','error')}}
      ...
      async function saveArticle(status='Draft'){
        ...
        Object.assign(article,data);setStatus('fdArticleStatus',status==='Published'?'Publishing securely to Supabase…':collaborativeEdit?'Saving editorial changes securely to Supabase…':'Saving securely to Supabase…','');
        const row=articleToRow(article),existingIndex=workspaceCache.articles.findIndex(a=>a.id===article.id);
        const request=existingIndex>=0
          ? supabaseClient.from('findat_articles').update(row).eq('id',article.id).select().single()
          : supabaseClient.from('findat_articles').insert(row).select().single();
        const{data:saved,error}=await request;if(error){setStatus('fdArticleStatus',error.message||'The article could not be saved.','error');return}
        ...
        const auditAction=collaborativeEdit?'Collaborative editorial changes saved':status==='Published'?'Article published by administrator':status==='Pending approval'?'Article submitted for approval':'Article draft saved';await addAudit(auditAction,converted.title,converted.id);await trackPlatformEvent('article_save',{articleId:converted.id,title:converted.title,status:converted.status,collaborativeEdit});
      ...
      function openReader(id,{updateHash=true}={}){
        ...
        currentReaderArticleId=id;trackPlatformEvent('information_retrieved',{articleId:id,title:article.title,entityType:'published_article'}).catch(()=>{});
      ...
      function populateWorkstationUsers(){const select=$('#fdWorkstationUser');if(!select)return;const current=select.value;select.innerHTML=accountCache.filter(user=>user.active!==false).sort((a,b)=>accountName(a).localeCompare(accountName(b))).map(user=>`<option value="${user.id}">${escapeHTML(accountName(user))}</option>`).join('');if([...select.options].some(option=>option.value===current))select.value=current}
      async function refreshAdminAccounts(){if(!isAdmin())return;const body=$('#fdAccountTableBody');if(body)body.innerHTML='<tr><td colspan="4">Loading accounts from Supabase…</td></tr>';try{const response=await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'list'},true),users=(response.users||[]).map(profileFromRow);accountCache=[...new Map([activeAccount,...users].filter(Boolean).map(item=>[item.id,item])).values()];if($('#fdAccountNavCount'))$('#fdAccountNavCount').textContent=String(users.length);populateWorkstationUsers();if(!body)return;body.innerHTML=users.length?users.map(user=>`<tr class="${user.role==='admin'?'is-admin-row':''}"><td><strong>${escapeHTML(accountName(user))}</strong><small>@${escapeHTML(user.username)} · ${escapeHTML(user.email)}</small></td><td>${adminRoleBadge(user.role)}</td><td><div class="fd-account-status-stack">${accountStateBadges(user)}</div></td><td><div class="fd-account-controls">${user.role==='admin'?'<button class="fd-mini-btn is-locked" disabled type="button"><i class="fas fa-shield-alt"></i> Administrator</button>':`${user.emailConfirmed?'':`<button class="fd-mini-btn is-success" data-account-confirm="${user.id}" type="button"><i class="fas fa-envelope-circle-check"></i> Confirm email</button>`}<button class="fd-mini-btn" data-account-password="${user.id}" type="button"><i class="fas fa-key"></i> Password</button><button class="fd-mini-btn is-promote" data-account-promote="${user.id}" type="button"><i class="fas fa-user-shield"></i> Promote to admin</button><button class="fd-mini-btn ${user.active?'is-danger':'is-success'}" data-account-active="${user.id}" data-next-active="${user.active?'false':'true'}" type="button">${user.active?'Suspend':'Activate'}</button>`}</div></td></tr>`).join(''):'<tr><td colspan="4">No accounts found.</td></tr>'}catch(error){if(body)body.innerHTML=`<tr><td colspan="4">${escapeHTML(error.message||'Accounts could not be loaded.')}</td></tr>`}}
      ...
      $('#fdAccountTableBody')?.addEventListener('click',async event=>{
        const confirmButton=event.target.closest('[data-account-confirm]'),passwordButton=event.target.closest('[data-account-password]'),promoteButton=event.target.closest('[data-account-promote]'),activeButton=event.target.closest('[data-account-active]');
        if(confirmButton){if(!confirm('Confirm this account email so the user can log in?'))return;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'confirm_email',userId:confirmButton.dataset.accountConfirm},true);alert('Email confirmed.');await refreshAdminAccounts()}catch(error){alert(error.message||'The email could not be confirmed.')}return}
        if(passwordButton){const password=prompt('Enter the new password (at least 8 characters):');if(!password)return;if(password.length<8){alert('Password must contain at least 8 characters.');return}try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'change_password',userId:passwordButton.dataset.accountPassword,password},true);alert('Password changed and email confirmed.');await refreshAdminAccounts()}catch(error){alert(error.message||'The password could not be changed.')}return}
        if(promoteButton){if(!confirm('Promote this Client or Consultant to Administrator? This grants full platform privileges.'))return;try{await invokeFunction(FINDAT_AUTH_CONFIG.adminUsersFunction||'findat-admin-users',{action:'set_role',userId:promoteButton.dataset.accountPromote,role:'admin'},true);alert('The account is now an Administrator. The user should sign out and sign in again.');await refreshAdminAccounts();await trackPlatformEvent('account_role_changed',{userId:promoteButton.dataset.accountPromote,role:'admin'})}catch(error){alert(error.message||'The account could not be promoted.')}return}
      ...
      $('#fdRefreshActivityDashboard')?.addEventListener('click',()=>refreshAdminActivity().catch(error=>setStatus('fdActivityDashboardStatus',error.message||'Activity data could not be loaded.','error')));
      $('#fdWorkstationForm')?.addEventListener('submit',saveWorkstationForm);
      $('#fdRefreshWorkstations')?.addEventListener('click',()=>refreshWorkstations().catch(error=>setStatus('fdWorkstationStatus',error.message||'Workstations could not be loaded.','error')));
      $('#fdWorkstationTableBody')?.addEventListener('click',event=>{const edit=event.target.closest('[data-workstation-edit]'),del=event.target.closest('[data-workstation-delete]');if(edit){const row=workstationCache.find(item=>item.id===edit.dataset.workstationEdit);if(!row)return;$('#fdWorkstationUser').value=row.assigned_user_id||'';$('#fdWorkstationNumber').value=String(row.workstation_no||1);$('#fdWorkstationName').value=row.name||'';$('#fdWorkstationPassword').focus();setStatus('fdWorkstationStatus','Enter a new password to update this workstation.','')}if(del)deleteWorkstation(del.dataset.workstationDelete)});
      $('#fdDeveloperLanguage')?.addEventListener('change',loadDeveloperEditor);
      $('#fdDeveloperPatchName')?.addEventListener('change',loadDeveloperEditor);
      $('#fdDeveloperValidate')?.addEventListener('click',validateDeveloperSource);
      $('#fdDeveloperSave')?.addEventListener('click',saveDeveloperPatch);
      $('#fdDeveloperRunPython')?.addEventListener('click',()=>runDeveloperPython().catch(error=>setStatus('fdDeveloperStatus',error.message||'Python failed.','error')));
      $('#fdDeveloperTriggerDeploy')?.addEventListener('click',triggerNetlifyDeploy);
      $('#fdVisualStudioTab')?.addEventListener('click',()=>{$('#fdVisualStudioPanel').hidden=false;$('#fdPythonStudioPanel').hidden=true;$('#fdVisualStudioTab').classList.add('is-active');$('#fdPythonStudioTab').classList.remove('is-active');$('#fdVisualStudioTab').setAttribute('aria-selected','true');$('#fdPythonStudioTab').setAttribute('aria-selected','false')});
      $('#fdPythonStudioTab')?.addEventListener('click',()=>{$('#fdVisualStudioPanel').hidden=true;$('#fdPythonStudioPanel').hidden=false;$('#fdVisualStudioTab').classList.remove('is-active');$('#fdPythonStudioTab').classList.add('is-active');$('#fdVisualStudioTab').setAttribute('aria-selected','false');$('#fdPythonStudioTab').setAttribute('aria-selected','true')});
      $('#fdDataInput')?.addEventListener('input',()=>{syncVisualColumns();if(($('#fdDataInput')?.value||'').trim().split(/\r?\n/).length>2)previewDataChart()});
      ['fdChartType','fdChartCategoryColumn','fdChartValueColumn','fdChartAggregation'].forEach(id=>$('#'+id)?.addEventListener('change',previewDataChart));
      $('#fdPreviewDataChart')?.addEventListener('click',previewDataChart);
      ...
      document.addEventListener('click',async event=>{
        ...
        const response=event.target.closest('[data-collaboration-response]');if(response)respondCollaboration(response.dataset.collaborationArticle,response.dataset.collaborationResponse==='accept',response);
        const notificationOpen=event.target.closest('[data-notification-open]');if(notificationOpen){event.preventDefault();window.toggleModal?.('fdCollaborationInboxModal',false);const articleId=notificationOpen.dataset.notificationOpen,kind=notificationOpen.dataset.notificationKind||'';openEditor(articleId);await trackPlatformEvent('notification_open',{articleId,kind});setTimeout(()=>{const target=kind==='article_comment'?$('#fdArticleActivity'):$('#fdArticleEditor');target?.scrollIntoView({behavior:'smooth',block:'center'});if(kind==='article_comment')$('#fdArticleCommentInput')?.focus()},180)}
      ...
      async function bootstrap(){if($('#fdTodayLabel'))$('#fdTodayLabel').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',day:'numeric',month:'long',year:'numeric'});if(!supabaseClient){syncPublicAuthBar(null);renderHomepagePublications();console.error('FINDAT Supabase Auth is not configured.');return}supabaseClient.auth.onAuthStateChange(async(event,session)=>{if(event==='PASSWORD_RECOVERY'){recoverySessionActive=true;openAuthModal('passwordResetPopup');return}if(event==='SIGNED_OUT'){activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];workspaceCache={articles:[],audit:[]};clearInterval(collaborationRefreshTimer);syncPublicAuthBar(null);renderHomepagePublications()}if(event==='SIGNED_IN'&&session?.user&&!activeAccount&&!recoverySessionActive&&!authTransitionInProgress){try{const account=await fetchOwnProfile(session.user.id);if(account?.active)await enterDashboard(account)}catch(error){console.warn(error)}}});await refreshSitePatches();startSitePatchRealtime();await trackPlatformEvent('page_view',{page:location.hash||location.pathname,title:document.title});await refreshPublicArticles();const session=await getCurrentSession();if(session?.user){try{const account=await fetchOwnProfile(session.user.id);if(account?.active){activeAccount=account;accountCache=[account];syncPublicAuthBar(account)}else if(account){await clearSession()}}catch(error){console.warn('FINDAT session profile unavailable',error)}}else syncPublicAuthBar(null);const publicationMatch=location.hash.match(/^#publication-(.+)$/);if(publicationMatch){openReader(publicationMatch[1],{updateHash:false});return}if(location.hash==='#recordings'){window.openRecordingsPanel?.();return}if(location.hash==='#live-training'){window.openLiveTrainingPanel?.();return}if(location.hash==='#publications'){window.openPublicationsPanel?.();return}if(activeAccount&&(location.hash==='#writing'||location.hash==='#workspace'||location.hash==='#dashboard'||location.hash==='#accounts'||location.hash==='#review'||location.hash==='#activity'||location.hash==='#developer'))await enterDashboard(activeAccount);clearInterval(publicRefreshTimer);publicRefreshTimer=setInterval(refreshPublicArticles,30000)}
      bootstrap();
    })();
...
(()=>{
  ...
  const sendCloudContext=async()=>{
    if(!frame?.contentWindow||typeof window.FINDAT_GET_CLOUD_CONTEXT!=='function')return;
    try{const context=await window.FINDAT_GET_CLOUD_CONTEXT();frame.contentWindow.postMessage({type:'findat-cloud:session',...context},location.origin)}catch(error){console.warn('FINDAT Cloud session context could not be sent.',error)}
  };
  ...
  const setOpen=(open,{returnFocus=false}={})=>{
    ...
    if(open){
      sendCloudContext();
      window.FINDAT_TRACK_PLATFORM_EVENT?.('cloud_open',{page:'cloud',title:'FINDAT Cloud'}).catch?.(()=>{});
  ...
  frame?.addEventListener('load',()=>{connectFrameControls();sendCloudContext()});

  window.addEventListener('message',event=>{
    if(event.source!==frame?.contentWindow)return;
    if(event.data?.type==='findat-cloud:ready')sendCloudContext();
...
(()=>{
  ...
  const protectedPoster=window[String.fromCharCode(95,95,70,73,78,68,65,84,95,80,82,79,84,69,67,84,69,68,95,80,79,83,84,69,82,95,95)]||'';
