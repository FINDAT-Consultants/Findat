...
| Consultant | Created by an Administrator | Edit/proofread text and change media only on client articles assigned by an Administrator; cannot create, delete, change workflow status, approve or publish | No |
...
The browser app adds `#reset-password` after the allowed page URL when processing
