...
function makeEntryButton(entry, surface = 'finder') {
  ...
  button.innerHTML = `${fileGlyph(entry)}${entry.shared ? '<span class="shared-cloud-badge" title="Published to shared cloud">☁</span>' : ''}<small class="entry-name">${escapeHtml(entry.name)}</small>`;
  button.title = `${entry.name}${entry.type === 'file' ? ` · ${formatBytes(entry.size)}` : ''}${entry.shared ? ' · Shared' : ''}`;
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    ...
    <section class="settings-card"><b>Shared website storage</b><p data-shared-cloud-status>The shared cloud configuration is being checked.</p><div class="settings-button-row"><button data-cloud-action="sync">Sync Now</button><button data-cloud-action="publish-local">Publish Local Files</button></div><small>Published files are visible to every visitor using the same website and cloud configuration. Supported files open inside the built-in preview and can be shared by public link.</small></section>
...
async function refreshCloudSettingsPanel(win) {
  ...
  try {
    ...
    if (sharedStatus.enabled) {
      ...
      if (sharedLine) sharedLine.textContent = sharedStatus.lastError
        ? `The browser is using its local cache because sync failed: ${sharedStatus.lastError}`
        : `Connected to Supabase. ${sharedStatus.remoteCount} shared items are available to website visitors${sharedStatus.lastSuccessAt ? `; last synced ${new Date(sharedStatus.lastSuccessAt).toLocaleString()}` : ''}.`;
      ...
      if (sharedLine) sharedLine.textContent = 'Shared storage is off. Add the Supabase URL and anon key in cloud-config.js, then set enabled to true.';
    }
    let persisted = false;
