...
8. Drop several documents and confirm each occupies a separate, aligned desktop grid cell without covering another icon.
9. Confirm the object appears under `findat-v1/files` in Storage.
10. In browser DevTools, inspect IndexedDB: file records should contain metadata
   but no document `blob` payload.
11. Refresh another device and confirm the document appears from Supabase.
12. Permanently delete the document and confirm its Storage object disappears.
