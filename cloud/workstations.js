...
(() => {
  ...
  function normalizeState(value) {
    ...
    const normalized = items
      ...
      .map(item => ({
        ...item,
        name: item.name || `Work Station No.${item.id}`,
  ...
  const managedAccess = () => window.FINDATCloudAccess?.isManaged?.() ? window.FINDATCloudAccess : null;
  ...
  function renderTabs() {
    tabsRoot.replaceChildren(...state.items.map(item => {
      ...
      tab.innerHTML = `
        ...
        ${managedAccess() || item.id === 1 ? '' : `<button class="workstation-remove" type="button" title="Remove ${item.name}" aria-label="Remove ${item.name}">−</button>`}`;
    ...
    addButton.hidden = Boolean(managedAccess());
    addButton.disabled = Boolean(managedAccess()) || state.items.length >= MAX_WORKSTATIONS;
    addButton.title = managedAccess() ? 'Workstations are assigned by a FINDAT Administrator' : addButton.disabled ? `Maximum of ${MAX_WORKSTATIONS} work stations reached` : 'Add work station';
  }
  ...
  async function switchWorkstation(id, announce = true) {
    id = Number(id);
    const requested = state.items.find(item => item.id === id);
    if (!requested) return;
    const access = managedAccess();
    if (access && announce && requested.dbId) {
      const password = await systemPrompt(`Enter the password for ${requested.name}:`, '', { title: 'Open Managed Workstation', okLabel: 'Open', placeholder: 'Workstation password' });
      if (password === null) return;
      let accepted = false;
      try { accepted = await access.verifyPassword(requested.dbId, password); }
      catch (error) { return toast(error.message || 'The workstation password could not be verified'); }
      if (!accepted) return toast('The workstation password is incorrect');
    }
    state.activeId = id;
    saveState();
  ...
  async function addWorkstation() {
    if (managedAccess()) return toast('Workstations are assigned by a FINDAT Administrator');
  ...
  async function removeWorkstation(id) {
    if (managedAccess()) return toast('Workstations are managed by a FINDAT Administrator');
    id = Number(id);
    if (id === 1) return;
  ...
  async function applyManagedWorkstations() {
    const access = managedAccess();
    if (!access) return;
    const assigned = access.getWorkstations();
    state.items = assigned.length ? assigned.map(row => ({
      id: Number(row.workstation_no),
      dbId: String(row.id || ''),
      name: String(row.name || `Work Station No.${row.workstation_no}`),
      path: Number(row.workstation_no) === 1 ? '/Desktop' : `/Workstations/Work Station No.${Number(row.workstation_no)}`,
    })).sort((a,b)=>a.id-b.id) : [{ id: 1, dbId: '', name: 'No workstation assigned', path: '/Desktop' }];
    if (!state.items.some(item => item.id === state.activeId)) state.activeId = state.items[0].id;
    renderTabs();
    await ensureAllWorkstationFolders();
    await switchWorkstation(state.activeId, false);
  }

  window.addEventListener('findat-cloud:access-ready', () => applyManagedWorkstations().catch(error => toast(error.message || 'Managed workstations could not be loaded')));
  renderTabs();
  if (managedAccess()) applyManagedWorkstations().catch(console.warn);
  else ensureAllWorkstationFolders().finally(() => switchWorkstation(state.activeId, false));
})();
