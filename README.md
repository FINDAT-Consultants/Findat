...
## FINDAT Cloud GitHub storage

The Cloud System now stores documents in a configured GitHub repository rather than browser file storage. See `GITHUB-CLOUD-SETUP.md` and `cloud/README.md` for repository, token, cross-device, upload, download, and drag-out instructions.


## Cloud visibility fix (v5.1)

New uploads are force-refreshed from GitHub, revealed in their destination, and highlighted. The drop overlay now displays the destination path, and the tab-scoped token survives page reloads without storing document bytes locally.
