...
(()=>{
      ...
      function renderCollaborationInbox(){
        ...
        list.innerHTML=rows.length?rows.map(request=>{
          ...
          return `<article class="fd-collaboration-request is-${escapeHTML(request.status||'none')} ${request.is_read===false?'is-unread':''}">${avatarMarkup(person,'fd-collaboration-avatar')}<div><p>${sentence}</p><div class="fd-collaboration-meta"><span class="fd-collaboration-status is-${escapeHTML(request.status||'none')}">${escapeHTML(collaborationStatusLabel(request.status))}</span><time>${escapeHTML(formatDate(request.updated_at||request.created_at))}</time></div>${actions}</div></article>`
