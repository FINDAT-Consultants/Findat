...
(()=>{
      ...
      async function refreshCollaborationState(){
        if(!supabaseClient||!activeAccount){collaborationCache=[];collaborationInbox=[];collaborationActivity=[];return}
        const [rowsResult,inboxResult,activityResult]=await Promise.all([
          supabaseClient.from('findat_article_collaborators').select('article_id,user_id,invited_by,status,created_at,responded_at').order('created_at',{ascending:false}),
      ...
      function startCollaborationRealtime(){
        ...
        collaborationChannel=supabaseClient
          ...
          .on('postgres_changes',{event:'*',schema:'public',table:'findat_article_collaborators'},async()=>{
            try{await Promise.all([refreshCollaborationState(),refreshArticles()]);renderAll()}catch(error){console.warn('Realtime collaboration refresh failed',error)}
          })
      ...
      function collaborationStatusLabel(status){return({pending:'Waiting for response',accepted:'Accepted',rejected:'Rejected',cancelled:'Cancelled'})[status]||status||'Unknown'}
      function renderCollaborationInbox(){
        const count=$('#fdCollaborationInboxCount'),list=$('#fdCollaborationInboxList');
        const pendingIncoming=collaborationActivity.filter(item=>item.direction==='incoming'&&item.status==='pending');
        if(count){count.textContent=String(pendingIncoming.length);count.dataset.count=String(pendingIncoming.length)}
        if(!list)return;
        const rows=collaborationActivity.length?collaborationActivity:collaborationInbox.map(request=>({...request,direction:'incoming',other_name:request.inviter_name,other_avatar_url:request.inviter_avatar_url,status:'pending',updated_at:request.created_at}));
        list.innerHTML=rows.length?rows.map(request=>{
          const incoming=request.direction==='incoming',pending=request.status==='pending';
          const person={displayName:request.other_name||request.inviter_name,avatarUrl:request.other_avatar_url||request.inviter_avatar_url};
          const sentence=incoming
          ...
          const actions=incoming&&pending?`<div class="fd-collaboration-request-actions"><button class="fd-mini-btn is-success" data-collaboration-response="accept" data-collaboration-article="${request.article_id}" type="button"><i class="fas fa-check"></i> Accept</button><button class="fd-mini-btn is-danger" data-collaboration-response="reject" data-collaboration-article="${request.article_id}" type="button"><i class="fas fa-times"></i> Reject</button></div>`:'';
          return `<article class="fd-collaboration-request is-${escapeHTML(request.status||'pending')}">${avatarMarkup(person,'fd-collaboration-avatar')}<div><p>${sentence}</p><div class="fd-collaboration-meta"><span class="fd-collaboration-status is-${escapeHTML(request.status||'pending')}">${escapeHTML(collaborationStatusLabel(request.status))}</span><time>${escapeHTML(formatDate(request.responded_at||request.updated_at||request.created_at))}</time></div></div>${actions}</article>`
        }).join(''):'<div class="fd-empty-request"><i class="fas fa-user-group"></i>No collaboration requests or response history is available yet.</div>'
      }
      ...
      async function saveCollaborators(){
        ...
        if(article&&!canManageCollaborators(article)){setStatus('fdCollaboratorStatus','Only the paper author or an Administrator can change this collaboration group.','error');return}
        if(!article){
        ...
        if(result.error){setStatus('fdCollaboratorStatus',result.error.message||'Requests could not be sent. Run the collaboration reliability SQL update, then try again.','error');return}
        const activeRows=(result.data||[]).filter(row=>row.status==='pending'||row.status==='accepted');
        await Promise.all([refreshCollaborationState(),refreshArticles()]);article=workspaceCache.articles.find(row=>row.id===article.id)||article;article.contributorLayout=normaliseContributorLayout(article);contributorLayoutDraft=article.contributorLayout.map(item=>({...item}));contributorLayoutArticleId=article.id;contributorLayoutDirty=true;renderContributorBoard(article);window.toggleModal?.('fdCollaboratorModal',false);setStatus('fdArticleStatus',`${activeRows.length} collaboration request${activeRows.length===1?'':'s'} saved in Supabase. Recipients can accept or reject from their workspace.`,activeRows.length?'success':'')
      }
      async function respondCollaboration(articleId,accept,button=null){
        ...
        if(result.error){if(button)button.disabled=false;alert(result.error.message||'The request could not be updated.');return}
        await Promise.all([refreshCollaborationState(),refreshArticles()]);renderAll();setStatus('fdCollaborationInboxStatus',accept?'Collaboration request accepted. The paper is now available in your Writing Desk.':'Collaboration request rejected. The response has been saved.','success');if(accept){const article=workspaceCache.articles.find(row=>row.id===articleId);if(article){window.toggleModal?.('fdCollaborationInboxModal',false);openEditor(article.id)}}
      }
      ...
      $('#fdCollaborationInboxButton')?.addEventListener('click',async()=>{setStatus('fdCollaborationInboxStatus','Refreshing collaboration requests…','');window.toggleModal?.('fdCollaborationInboxModal',true);try{await refreshCollaborationState();setStatus('fdCollaborationInboxStatus',collaborationActivity.length?'Requests and response history are up to date.':'No collaboration activity is available yet.','')}catch(error){setStatus('fdCollaborationInboxStatus',error.message||'Collaboration requests could not be refreshed.','error')}});
      $('#fdCollaborationInboxClose')?.addEventListener('click',()=>window.toggleModal?.('fdCollaborationInboxModal',false));
      $('#fdCollaborationInboxRefresh')?.addEventListener('click',async()=>{setStatus('fdCollaborationInboxStatus','Refreshing…','');try{await refreshCollaborationState();setStatus('fdCollaborationInboxStatus','Collaboration activity refreshed.','success')}catch(error){setStatus('fdCollaborationInboxStatus',error.message||'Refresh failed.','error')}});
