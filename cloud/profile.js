...
function managedCloudAccess() {
  const access = window.FINDATCloudAccess;
  return access?.isManaged?.() ? access : null;
}

function cloudDisplayName() {
  return managedCloudAccess()?.user?.()?.name || aureliaAccount.username;
}

async function verifyAccountPassword(password) {
  const managed = managedCloudAccess();
  if (managed) {
    const workstation = window.FINDATWorkstations?.activeItem?.();
    if (!workstation?.dbId) return false;
    try { return await managed.verifyPassword(workstation.dbId, password); }
    catch (error) { toast(error.message || 'The managed workstation password could not be verified.'); return false; }
  }
...
function accountHasPassword() {
  return Boolean(managedCloudAccess() || (aureliaAccount.passwordHash && aureliaAccount.passwordSalt));
}
...
function renderSystemAccountMenu() {
  ...
  card.innerHTML = `${profileAvatarMarkup('account-avatar-menu')}<span><b>${accountEscape(cloudDisplayName())}</b><small>Account Settings</small></span><i>›</i>`;
  ...
  signOut.textContent = `Sign Out ${cloudDisplayName()}…`;
...
function updateAuthScreen(mode = 'lock') {
  ...
  if (username) username.textContent = cloudDisplayName();
  if (description) {
...
function profileSettingsMarkup() {
  ...
  const passwordPanel = managed
    ? `<section class="settings-card"><b>Managed workstation password</b><p>This Cloud workstation password is controlled by a FINDAT Administrator from User Accounts. Local password changes are disabled.</p></section>`
  ...
  return `<div class="settings-section" data-settings-panel="profile">
    ...
    <section class="settings-card profile-overview-card">
      ...
      <div class="profile-overview-copy"><b>${accountEscape(displayName)}</b><small>${managed ? 'Administrator-managed Cloud access' : accountHasPassword() ? 'Password protected account' : 'Account has no password'}</small></div>
    ...
    <section class="settings-card">
      <b>Account name</b><p>${managed ? 'This name comes from your FINDAT profile.' : 'This system account name is fixed and cannot be changed.'}</p>
      <div class="account-settings-form">
        <label><span>Account name</span><input type="text" value="${accountEscape(displayName)}" readonly aria-readonly="true" tabindex="-1"></label>
      </div>
    </section>
    ${passwordPanel}
    ...
    <p class="account-storage-note">${managed ? 'Workstation assignments and password hashes are stored securely in Supabase Postgres.' : 'This browser-only profile is stored on this device. Passwords are stored as a salted hash, not as readable text.'}</p>
  </div>`;
}
...
window.addEventListener('findat-cloud:access-ready', () => { refreshAureliaAccountUI(); const screen=qs('#lockScreen'); if(screen&&!screen.classList.contains('hidden')) updateAuthScreen(screen.dataset.mode||'lock'); });
