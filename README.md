...
The Financial Assistant now requires the authenticated `findat-x1-openai` Supabase Edge Function for every conversational answer. FINDAT Knowledge and verified application state are supplied to OpenAI as context; no Ollama or embedded chatbot fallback is used.

No OpenAI secret is included in this package. Store a newly generated key only in Supabase Edge Function Secrets. See `SUPABASE-DASHBOARD-SETUP.md` and `X1-OPENAI-ONLY-ACTIVATION.md`.
...
## Professional Network, Profile Analytics and x1 Upgrade — 2 August 2026

Run `FINDAT-PROFILE-SOCIAL-ANALYTICS-X1-UPGRADE.sql`, redeploy `supabase/functions/findat-admin-users/index.ts`, and then publish the complete website package. See `PROFESSIONAL-NETWORK-X1-UPGRADE-DEPLOYMENT.md`.


## x1 OpenAI-only activation

This package requires OpenAI for every x1 conversational response. The Writing Desk x1 controls open x1 directly and submit their context to the authenticated `findat-x1-openai` Supabase Edge Function. Knowledge retrieval and verified in-system actions remain connected, but local chatbot and Ollama fallback responses are disabled. See `X1-OPENAI-ONLY-ACTIVATION.md`.
