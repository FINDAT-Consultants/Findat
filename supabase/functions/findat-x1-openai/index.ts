...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (action === 'health') {
      ...
      const timeout = setTimeout(() => controller.abort(), Math.min(requestTimeoutMs, 20000))
      let modelResponse: Response
      try {
        modelResponse = await fetch(`https://api.openai.com/v1/models/${encodeURIComponent(openAIModel)}`, {
      ...
      if (!modelResponse.ok) {
        const message = cleanText(modelPayload?.error?.message ?? `OpenAI model verification failed (${modelResponse.status}).`, 700)
        console.error('findat-x1-openai health verification failed', modelResponse.status, message)
        if (modelResponse.status === 401 || modelResponse.status === 403) return jsonResponse({ error: 'The OpenAI API key or project permissions must be checked.' }, 502)
        if (modelResponse.status === 404) return jsonResponse({ error: `The configured OpenAI model ${openAIModel} is unavailable to this project.` }, 502)
        return jsonResponse({ error: 'The live OpenAI connection could not be verified.' }, 502)
      }
      return jsonResponse({ ready: true, provider: 'OpenAI', model: cleanText(modelPayload?.id ?? openAIModel, 120), verified: true })
    }
...
- OpenAI is the mandatory conversational response provider for x1. FINDAT may supply verified runtime outcomes and retrieved Knowledge as context, but do not imply that a local chatbot generated the answer.
- When a verified FINDAT action outcome is supplied, accurately acknowledge what completed or failed and never claim an unverified action.
