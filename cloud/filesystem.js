...
async function fsRequest(route, options = {}, params = {}) {
  ...
  try {
    ...
    if (mutating && beforeNodes && typeof sharedCloudPushDiff === 'function') {
      ...
      await sharedCloudPushDiff(beforeNodes, await databaseAll());
      if (result?.path) {
...
function makeEntryButton(entry, surface = 'finder') {
  ...
  const cloudBadge = entry.pending
    ? '<span class="pending-cloud-badge" title="Uploading to GitHub repository"><i></i></span>'
    : entry.shared ? '<span class="shared-cloud-badge" title="Stored in shared repository">☁</span>' : '';
  button.innerHTML = `${fileGlyph(entry)}${cloudBadge}<small class="entry-name">${escapeHtml(entry.name)}</small>`;
  button.title = `${entry.name}${entry.type === 'file' ? ` · ${formatBytes(entry.size)}` : ''}${entry.pending ? ' · Uploading to GitHub' : entry.shared ? ' · Stored in shared repository' : ''}`;
...
async function uploadFiles(files, targetPath = activeDirectory()) {
  ...
  const cloudProgressHandler = event => {
    ...
    if (detail.provider !== 'github') return;
    ...
    progress?.update(value, {
      ...
      message: detail.name || 'Committing document to the repository',
      state: detail.phase === 'commit' ? 'Creating repository commit…' : 'Uploading document…'
    });
  };
  ...
  const githubActive = typeof sharedCloudStatus === 'function' && sharedCloudStatus().provider === 'github';
  progress?.complete({
    title: githubActive ? 'Stored in GitHub' : 'Upload complete',
...
function cloudSettingsMarkup() {
  return `<div class="settings-section" data-settings-panel="cloud">
    ...
    <section class="settings-card"><b>Shared repository storage</b><p data-shared-cloud-status>The shared storage configuration is being checked.</p><div class="settings-button-row"><button data-cloud-action="sync">Sync Now</button><button data-cloud-action="publish-local">Publish Local Files</button></div><small>GitHub mode commits dropped documents and the shared manifest to your configured repository. Visitors using the website see the same repository-backed files.</small></section>
    ...
    qsa('[data-cloud-action]', win).forEach(button => button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (action === 'sync') {
          ...
          toast('Shared cloud is up to date');
        }
        if (action === 'publish-local') {
          ...
          toast(result.published ? `${result.published} local item${result.published === 1 ? '' : 's'} published` : 'All local files are already published');
        }
        if (action === 'refresh' && typeof sharedCloudEnabled === 'function' && sharedCloudEnabled()) {
          await sharedCloudPull();
          toast('Shared cloud status refreshed');
        }
        if (action === 'persist') {
    ...
    if (sharedStatus.enabled) {
      ...
      if (sharedLine) {
        ...
        sharedLine.textContent = sharedStatus.lastError
          ...
          : `Connected to ${destination}. ${sharedStatus.remoteCount} shared items are available to website visitors${sharedStatus.lastSuccessAt ? `; last synced ${new Date(sharedStatus.lastSuccessAt).toLocaleString()}` : ''}.`;
      }
      ...
      if (sharedLine) sharedLine.textContent = 'Shared storage is off. Configure the GitHub repository and secure API endpoint in cloud-config.js, then set enabled to true.';
    }
    let persisted = false;
