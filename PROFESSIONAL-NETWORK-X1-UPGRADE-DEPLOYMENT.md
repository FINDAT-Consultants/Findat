...
- Uses OpenAI as the mandatory x1 conversational response provider. FINDAT Knowledge/RAG remains connected as retrieved evidence, but local Ollama and embedded chatbot answers are disabled.
...
x1 learns from user-approved FINDAT content through retrieval and aggregate writing-pattern memory. It does not reverse-engineer, duplicate or retrain itself from proprietary ChatGPT internals. When OpenAI is unavailable, x1 shows a clear connection error and does not generate a local chatbot response. FINDAT Knowledge remains available for retrieval after OpenAI service is restored.
