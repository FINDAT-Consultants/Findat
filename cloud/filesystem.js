/* FINDAT Cloud shared/local mirrored virtual drive and desktop operations */
const fsState = {
  entries: new Map(),
  selected: new Set(),
  clipboard: null,
...
async function databasePut(node) {
  ...
  await window.FINDATSharedStorage?.commitNodes?.([node], []);
  return node;
}
...
async function databaseCommit(puts = [], deletes = []) {
  ...
  await window.FINDATSharedStorage?.commitNodes?.(puts, deletes);
}
...
async function fsRequest(route, options = {}, params = {}) {
  ...
  try {
    ...
    fsState.cloudOnline = window.FINDATSharedStorage?.enabled ? Boolean(window.FINDATSharedStorage.online) : true;
    return result;
  } catch (error) {
...
window.addEventListener('findat-sync-status', event => {
  const detail = event.detail || {};
  fsState.cloudOnline = detail.enabled ? Boolean(detail.online) : true;
  setCloudBusy(false, detail.enabled && !detail.online);
  const indicator = qs('#cloudStatus');
  if (indicator && detail.status) indicator.title = detail.status;
});
...
async function refreshCloudSettingsPanel(win) {
  ...
  try {
    ...
    const shared = window.FINDATSharedStorage;
    const sharedEnabled = Boolean(shared?.enabled);
    const storageLabel = qs('[data-storage-label]', panel);
    if (storageLabel) {
      storageLabel.textContent = sharedEnabled
        ? `Used ${formatBytes(usage.logicalUsed)} of ${formatBytes(usage.quota)} · shared server plus local mirror`
        : `Used ${formatBytes(usage.logicalUsed)} of ${formatBytes(usage.quota)} · ${formatBytes(usage.stored)} compressed locally`;
      storageLabel.title = sharedEnabled
        ? 'Data is retained by the FINDAT server and mirrored in this browser'
        : (usage.browserQuota ? `${formatBytes(usage.browserFree)} local browser space available` : 'Physical capacity is controlled by the browser and device');
    }
    ...
    if (title) title.textContent = sharedEnabled ? (shared.online ? 'Shared FINDAT Cloud is synchronized' : 'Shared FINDAT Cloud is offline') : 'FINDAT Cloud is ready';
    const detail = qs('[data-cloud-status-detail]', panel);
    if (detail) detail.textContent = sharedEnabled
      ? `${shared.status} · ${files + folders} items · changes checked automatically`
      : `4 TB virtual workspace · ${files + folders} items · ${formatBytes(usage.saved)} saved by compression`;
    let persisted = false;
    try { persisted = Boolean(await navigator.storage?.persisted?.()); } catch (_) { /* optional */ }
    const protection = qs('[data-cloud-protection]', panel);
    if (protection) protection.textContent = sharedEnabled ? (shared.online ? 'Server + mirror' : 'Queued locally') : (persisted ? 'Protected' : 'Standard');
  } catch (error) {
