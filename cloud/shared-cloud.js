...
async function sharedCloudStoreRemoteNode(remoteNode) {
  if (remoteNode.type === 'folder') {
    ...
    await databasePut({ ...current, ...remoteNode, blob: undefined });
    return;
  }
  const current = await databaseGet(remoteNode.path);
  if (
    ...
    current.cloudObjectPath === remoteNode.cloudObjectPath &&
    current.cloudModified === remoteNode.cloudModified &&
    current.blob instanceof Blob
  ) return;

  const blob = await sharedCloudDownloadObject(remoteNode);
  const packed = await compressBlobForStorage(blob, remoteNode.name, remoteNode.mime || blob.type);
...
async function sharedCloudPull(options = {}) {
  ...
  try {
    ...
    const pending = (await databaseAll()).filter(node =>
      sharedCloudShareable(node) &&
...
function sharedCloudStartPolling() {
  ...
  sharedCloudRuntime.timer = setInterval(() => {
    ...
    }).catch(() => { /* do not replace a successful local cache on network failure */ });
  }, interval);
