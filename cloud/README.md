...
- **GitHub token:** memory only for the current open page. It is not written to localStorage, source files, or GitHub.
...
Use the same owner, repository, branch, and storage folder on every device. Each device can then list and download the same files. A write token must be entered separately in each page session that needs to upload, rename, move, or delete files.
...
- Drop files onto the desktop, Finder area, or a folder to upload them to GitHub.
- Use **Upload Files** to select files normally.
