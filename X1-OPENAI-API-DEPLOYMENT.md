...
The website is wired to the authenticated Supabase Edge Function named `findat-x1-openai`. In this build, OpenAI is mandatory for every x1 conversational answer; embedded Knowledge is evidence retrieval only and there is no local chatbot fallback.
...
Sign in to FINDAT, open x1 | ProATR, and submit any question or instruction. The browser sends the signed-in user's access token to Supabase. Supabase validates the user, verifies the live OpenAI key and configured model, checks the active FINDAT profile, enforces usage limits, calls the OpenAI Responses API, and returns only the OpenAI-generated answer and usage metadata. When OpenAI is unavailable, x1 displays a connection error and does not generate a local chatbot answer.
