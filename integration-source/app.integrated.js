...
const mainNav = document.getElementById('mainNav');
    ...
    if(recordingPlayer){
      ...
      if(findatProtectedPosterPath) recordingPlayer.poster=findatProtectedPosterPath;
...
(()=>{
      'use strict';
      const ACCOUNTS_KEY='findat.accounts.v2';
      const LEGACY_ACCOUNTS_KEY='findat.accounts.v1';
      const WORKSPACE_KEY='findat.workspace.v2';
      const LOCAL_SESSION_KEY='findat.session.v2';
      const TEMP_SESSION_KEY='findat.session.temp.v2';
      const ADMIN_ID='findat-admin';
      ...
      const accountName=account=>`${account?.firstName||''} ${account?.lastName||''}`.trim()||account?.username||'FINDAT Member';
      const initials=account=>`${account?.firstName?.[0]||account?.username?.[0]||'F'}${account?.lastName?.[0]||'M'}`.toUpperCase();
      ...
      async function passwordDigest(value){
        ...
        if(window.crypto?.subtle&&window.TextEncoder){const bytes=new TextEncoder().encode(input);const digest=await window.crypto.subtle.digest('SHA-256',bytes);return[...new Uint8Array(digest)].map(byte=>byte.toString(16).padStart(2,'0')).join('')}
        let hash=2166136261;for(let i=0;i<input.length;i++){hash^=input.charCodeAt(i);hash=Math.imul(hash,16777619)}return`local-${(hash>>>0).toString(16)}`;
      }
      function migrateAccount(account){return{...account,role:normaliseRole(account.role),active:account.active!==false}}
      function loadAccounts(){let accounts=safeParse(localStorage.getItem(ACCOUNTS_KEY),null);if(!Array.isArray(accounts)){accounts=safeParse(localStorage.getItem(LEGACY_ACCOUNTS_KEY),[]).map(migrateAccount);localStorage.setItem(ACCOUNTS_KEY,JSON.stringify(accounts))}return accounts.map(migrateAccount)}
      function saveAccounts(accounts){localStorage.setItem(ACCOUNTS_KEY,JSON.stringify(accounts))}
      function loadWorkspace(){const state=safeParse(localStorage.getItem(WORKSPACE_KEY),{});return{...state,articles:Array.isArray(state.articles)?state.articles:[],audit:Array.isArray(state.audit)?state.audit:[]}}
      function saveWorkspace(state){localStorage.setItem(WORKSPACE_KEY,JSON.stringify(state))}
      function findAccount(identifier){const needle=normal(identifier);return loadAccounts().find(account=>normal(account.username)===needle||normal(account.email)===needle)}
      function accountById(id){return loadAccounts().find(account=>account.id===id)}
      async function ensureAdmin(){const accounts=loadAccounts(),hash=await passwordDigest(ADMIN_PASSWORD);let admin=accounts.find(account=>account.id===ADMIN_ID||normal(account.username)===ADMIN_USERNAME);if(!admin){admin={id:ADMIN_ID,firstName:'System',lastName:'Administrator',email:'admin@findat.local',username:ADMIN_USERNAME,phone:'',organisation:'FINDAT Consultants',role:'admin',country:'Zambia',active:true,passwordHash:hash,createdAt:nowISO()};accounts.unshift(admin)}else{Object.assign(admin,{id:ADMIN_ID,username:ADMIN_USERNAME,email:'admin@findat.local',role:'admin',organisation:admin.organisation||'FINDAT Consultants',active:true,passwordHash:hash})}saveAccounts(accounts)}
      function getSessionUsername(){return sessionStorage.getItem(TEMP_SESSION_KEY)||localStorage.getItem(LOCAL_SESSION_KEY)||''}
      function saveSession(username,remember=true){sessionStorage.removeItem(TEMP_SESSION_KEY);localStorage.removeItem(LOCAL_SESSION_KEY);(remember?localStorage:sessionStorage).setItem(remember?LOCAL_SESSION_KEY:TEMP_SESSION_KEY,username)}
      function clearSession(){sessionStorage.removeItem(TEMP_SESSION_KEY);localStorage.removeItem(LOCAL_SESSION_KEY)}
      ...
      function addAudit(action,detail,articleId=''){const state=loadWorkspace();state.audit.push({id:uid(),date:nowISO(),actorId:activeAccount?.id||'system',actorName:accountName(activeAccount),role:activeAccount?.role||'system',action,area:'Writing & Publication',detail,entityId:articleId});saveWorkspace(state)}
      function closeAllAuthModals(){['loginPopup','signupPopup','recoveryPopup'].forEach(id=>window.toggleModal?.(id,false))}
      ...
      function articleOwner(article){return accountById(article.ownerId||article.createdBy)}
      function visibleWritingArticles(){const state=loadWorkspace();if(!activeAccount)return[];if(isAdmin())return state.articles.filter(article=>(article.ownerId||article.createdBy)===activeAccount.id);return state.articles.filter(article=>(article.ownerId||article.createdBy)===activeAccount.id||article.collaboratorId===activeAccount.id)}
      function statusTag(status){const cls=status==='Published'?'is-green':status==='Pending approval'?'is-orange':status==='Rejected'?'is-red':'';return `<span class="fd-workspace-tag ${cls}">${escapeHTML(status||'Draft')}</span>`}

      function syncPublicAuthBar(account=null){const form=$('#utilityLoginForm'),user=$('#utilityUsername'),pass=$('#utilityPassword'),button=form?.querySelector('button[type="submit"]'),register=$('#utilityRegister');if(!form||!user||!pass||!button)return;form.classList.toggle('has-session',Boolean(account));if(account){user.value=account.username;user.readOnly=true;pass.value='';pass.placeholder='Signed in';button.textContent='Writing Desk';if(register)register.textContent='My writing desk'}else{user.value='';user.readOnly=false;pass.value='';pass.placeholder='Password';button.textContent='Login';if(register)register.textContent='Register'}}
      function applyRoleVisibility(){$$('[data-role-allow]').forEach(el=>{const roles=String(el.dataset.roleAllow||'').split(',').map(normal);el.hidden=!roles.includes(activeAccount?.role)})}
      function hydrateUser(){$$('[data-user-fullname]').forEach(el=>el.textContent=accountName(activeAccount));$$('[data-user-firstname]').forEach(el=>el.textContent=activeAccount?.firstName||activeAccount?.username||'Writer');$$('[data-user-role]').forEach(el=>el.textContent=roleLabel(activeAccount?.role));$$('[data-user-initials]').forEach(el=>el.textContent=initials(activeAccount));$('#fdSidebarRole').textContent=`${roleLabel(activeAccount?.role)} workspace`}
      function enterDashboard(account){if(!account||account.active===false)return;activeAccount=account;closeAllAuthModals();document.body.classList.add('dashboard-mode');dashboard?.setAttribute('aria-hidden','false');history.replaceState(null,'','#writing');syncPublicAuthBar(account);hydrateUser();applyRoleVisibility();populateCollaborators();renderAll();showView('writing')}
      ...
      function logout(){clearSession();activeAccount=null;editingArticleId='';leaveDashboard(false);syncPublicAuthBar(null);openAuthModal('loginPopup')}
      function showView(view){if(view==='review'&&!isAdmin())view='writing';$$('[data-view-panel]').forEach(panel=>panel.classList.toggle('is-active',panel.dataset.viewPanel===view));$$('[data-dashboard-view]').forEach(button=>button.classList.toggle('is-active',button.dataset.dashboardView===view));const titles={writing:'Writing Desk',review:'Review & Approval',publications:'Published Articles'};$('#fdTopbarTitle').textContent=titles[view]||'Writing Desk';$('#fdDashboardMain')?.scrollTo({top:0,behavior:'smooth'});closeSidebar()}
      function populateCollaborators(){const select=$('#fdArticleCollaborator');if(!select)return;const selected=select.value;select.innerHTML='<option value="">None</option>'+loadAccounts().filter(account=>account.active!==false&&account.id!==activeAccount?.id).map(account=>`<option value="${escapeHTML(account.id)}">${escapeHTML(accountName(account))} · ${escapeHTML(roleLabel(account.role))}</option>`).join('');select.value=selected}
      function renderCounters(){const state=loadWorkspace(),mine=visibleWritingArticles();$('#fdDraftCount').textContent=String(mine.filter(a=>(a.status||'Draft')==='Draft'||a.status==='Rejected').length);$('#fdPendingCount').textContent=String(mine.filter(a=>a.status==='Pending approval').length);$('#fdPublishedCount').textContent=String(mine.filter(a=>a.status==='Published').length);$('#fdDraftNavCount').textContent=String(mine.filter(a=>a.status!=='Published').length);$('#fdReviewNavCount').textContent=String(state.articles.filter(a=>a.status==='Pending approval').length);$('#fdPublicationNavCount').textContent=String(state.articles.filter(a=>a.status==='Published').length)}
      function renderStatusBanner(article){const host=$('#fdArticleStatusBanner');if(!host)return;if(!article){host.innerHTML='';return}if(article.status==='Rejected')host.innerHTML=`<div class="fd-status-banner is-rejected"><strong>Returned for revision.</strong><br>${escapeHTML(article.reviewNote||'Please revise the article and submit it again.')}</div>`;else if(article.status==='Pending approval')host.innerHTML='<div class="fd-status-banner"><strong>Awaiting administrator approval.</strong><br>You can still open the article, but saving changes returns it to draft status.</div>';else if(article.status==='Published')host.innerHTML='<div class="fd-status-banner" style="background:#eaf7ef;color:#26774f"><strong>This article is published.</strong><br>Only an administrator can republish changes.</div>';else host.innerHTML=''}
      function renderCover(){const host=$('#fdArticleImagePreview');if(!host)return;host.innerHTML=pendingCover?.data?`<img src="${pendingCover.data}" alt="Selected cover photo">`:'<span><i class="far fa-image"></i><br>No cover photo selected</span>';$('#fdArticleAttachmentList').innerHTML=pendingAttachments.length?pendingAttachments.map((file,index)=>`<button class="fd-attachment-chip" type="button" data-remove-attachment="${index}" title="Remove attachment"><i class="fas fa-paperclip"></i>${escapeHTML(file.name)} <span>×</span></button>`).join(''):'<span style="font-size:9px;color:#98a2b0">No documents attached</span>'}
      function renderLibrary(){const query=normal($('#fdDashboardSearch')?.value),articles=visibleWritingArticles().slice().sort((a,b)=>new Date(b.updatedAt||b.createdAt)-new Date(a.updatedAt||a.createdAt)).filter(a=>!query||normal(`${a.title} ${a.subtitle} ${a.category} ${stripHTML(a.content)}`).includes(query));$('#fdArticleLibrary').innerHTML=articles.length?articles.map(article=>`<article class="fd-library-item ${editingArticleId===article.id?'is-selected':''}"><div class="fd-library-item-head"><div><h4>${escapeHTML(article.title||'Untitled article')}</h4><p>${escapeHTML(article.category||'Article')} · ${escapeHTML(formatTime(article.updatedAt||article.createdAt))}</p></div>${statusTag(article.status||'Draft')}</div><p style="font-size:8px;color:#7e8a9b;line-height:1.5;margin:8px 0">${escapeHTML((article.subtitle||stripHTML(article.content)).slice(0,105))}${(article.subtitle||stripHTML(article.content)).length>105?'…':''}</p><div class="fd-library-item-actions"><button class="fd-mini-btn" type="button" data-article-open-editor="${article.id}">Open</button><button class="fd-mini-btn" type="button" data-article-preview="${article.id}">Preview</button>${article.status!=='Published'||isAdmin()?`<button class="fd-mini-btn is-danger" type="button" data-article-delete="${article.id}">Delete</button>`:''}</div></article>`).join(''):'<div class="fd-empty-compact"><i class="fas fa-feather-alt"></i>No articles yet. Start your first piece above.</div>'}
      function renderReview(){const pending=loadWorkspace().articles.filter(a=>a.status==='Pending approval').sort((a,b)=>new Date(a.submittedAt)-new Date(b.submittedAt));$('#fdApprovalQueue').innerHTML=pending.length?pending.map(article=>`<article class="fd-review-item"><div class="fd-review-thumb">${article.image?.data?`<img src="${article.image.data}" alt="">`:'<i class="fas fa-file-alt"></i>'}</div><div class="fd-review-copy"><h3>${escapeHTML(article.title)}</h3><p>${escapeHTML(article.subtitle||stripHTML(article.content).slice(0,150))}</p><p style="margin-top:5px">By ${escapeHTML(accountName(articleOwner(article)))} · submitted ${escapeHTML(formatTime(article.submittedAt))}</p></div><div class="fd-review-actions"><button class="fd-mini-btn" type="button" data-article-preview="${article.id}">Review</button><button class="fd-mini-btn is-success" type="button" data-article-approve="${article.id}">Approve</button><button class="fd-mini-btn is-danger" type="button" data-article-return="${article.id}">Return</button></div></article>`).join(''):'<div class="fd-empty-large"><i class="fas fa-check-circle"></i>No articles are waiting for approval.</div>'}
      function renderPublications(){const published=loadWorkspace().articles.filter(a=>a.status==='Published').sort((a,b)=>new Date(b.publishedAt)-new Date(a.publishedAt));$('#fdPublicationGrid').innerHTML=published.length?published.map(article=>`<article class="fd-publication-card"><div class="fd-publication-cover">${article.image?.data?`<img src="${article.image.data}" alt="${escapeHTML(article.title)}">`:'<i class="fas fa-newspaper"></i>'}</div><div class="fd-publication-body"><span class="fd-workspace-tag is-green">${escapeHTML(article.category||'Published')}</span><h3>${escapeHTML(article.title)}</h3><p>${escapeHTML(article.subtitle||stripHTML(article.content).slice(0,180))}${(article.subtitle||stripHTML(article.content)).length>180?'…':''}</p><p style="margin-top:9px;font-size:8px">By ${escapeHTML(accountName(articleOwner(article)))} · ${escapeHTML(formatDate(article.publishedAt))}</p><div class="fd-library-item-actions"><button class="fd-mini-btn" type="button" data-article-preview="${article.id}">Read article</button>${isAdmin()?`<button class="fd-mini-btn is-danger" type="button" data-article-unpublish="${article.id}">Unpublish</button>`:''}</div></div></article>`).join(''):'<div class="fd-empty-large" style="grid-column:1/-1"><i class="fas fa-newspaper"></i>No articles have been published yet.</div>'}
      ...
      function renderHomepagePublications(){
        ...
        const visible=published.filter(article=>{
          const owner=accountName(articleOwner(article));
        ...
        list.innerHTML=visible.length?visible.map(article=>{
          ...
          return `<article class="home-publication-card" tabindex="0" role="button" aria-label="Open ${escapeHTML(article.title)}" data-article-preview="${article.id}"><div class="home-publication-cover">${article.image?.data?`<img src="${article.image.data}" alt="Cover for ${escapeHTML(article.title)}">`:'<i class="fas fa-file-alt" aria-hidden="true"></i>'}</div><div class="home-publication-copy"><span class="home-publication-type"><i class="fas fa-book-open" aria-hidden="true"></i>${escapeHTML(article.category||'Article')}</span><h3>${escapeHTML(article.title||'Untitled publication')}</h3><p class="home-publication-meta">${escapeHTML(formatDate(article.publishedAt))} · ${escapeHTML(accountName(articleOwner(article)))}</p><p class="home-publication-excerpt">${escapeHTML(excerpt)}${excerpt.length>=245?'…':''}</p><div class="home-publication-tags">${topics.map(topic=>`<span>${escapeHTML(topic)}</span>`).join('')}</div></div><span class="home-publication-arrow" aria-hidden="true"><i class="fas fa-arrow-right"></i></span></article>`
      ...
      function renderAll(){renderHomepagePublications();if(!activeAccount)return;renderCounters();renderLibrary();renderReview();renderPublications();renderCover();const current=loadWorkspace().articles.find(a=>a.id===editingArticleId);renderStatusBanner(current)}

      function clearEditor(withStarter=false){editingArticleId='';pendingCover=null;pendingAttachments=[];$('#fdArticleTitle').value='';$('#fdArticleSubtitle').value='';$('#fdArticleTemplate').value='classic';$('#fdArticleCategory').value='Research';$('#fdArticleCollaborator').value='';$('#fdArticleEditor').className='fd-editor-canvas template-classic';$('#fdArticleEditor').innerHTML=withStarter?templateStarters.classic:'';$('#fdTemplateHint').textContent=templateHints.classic;setStatus('fdArticleStatus','');renderStatusBanner(null);renderCover();renderLibrary();$('#fdArticleTitle').focus()}
      function openEditor(id){const article=loadWorkspace().articles.find(a=>a.id===id);if(!article)return;const canOpen=isAdmin()||(article.ownerId||article.createdBy)===activeAccount.id||article.collaboratorId===activeAccount.id;if(!canOpen)return;editingArticleId=article.id;pendingCover=article.image||null;pendingAttachments=Array.isArray(article.attachments)?article.attachments:[];$('#fdArticleTitle').value=article.title||'';$('#fdArticleSubtitle').value=article.subtitle||'';$('#fdArticleTemplate').value=article.template||'classic';$('#fdArticleCategory').value=article.category||'Research';$('#fdArticleCollaborator').value=article.collaboratorId||'';$('#fdArticleEditor').className=`fd-editor-canvas template-${article.template||'classic'}`;$('#fdArticleEditor').innerHTML=article.content||'';$('#fdTemplateHint').textContent=templateHints[article.template||'classic'];setStatus('fdArticleStatus','');renderStatusBanner(article);renderCover();renderLibrary();showView('writing')}
      function collectArticle(status){const title=$('#fdArticleTitle').value.trim(),content=sanitiseHTML($('#fdArticleEditor').innerHTML);if(!title||!stripHTML(content)){setStatus('fdArticleStatus','Enter an article title and some written content.','error');return null}return{title,subtitle:$('#fdArticleSubtitle').value.trim(),content,template:$('#fdArticleTemplate').value,category:$('#fdArticleCategory').value,collaboratorId:$('#fdArticleCollaborator').value,image:pendingCover,attachments:pendingAttachments,status,updatedAt:nowISO()}}
      function saveArticle(status='Draft'){const data=collectArticle(status);if(!data)return;const state=loadWorkspace();let article=state.articles.find(a=>a.id===editingArticleId);if(!article){article={id:uid(),ownerId:activeAccount.id,createdBy:activeAccount.id,createdAt:nowISO()};state.articles.push(article);editingArticleId=article.id}if(article.status==='Published'&&!isAdmin()){setStatus('fdArticleStatus','A published article can only be changed by an administrator.','error');return}if(status==='Draft'&&article.status==='Pending approval')data.reviewNote='';if(status==='Pending approval'){data.submittedAt=nowISO();data.reviewNote=''}Object.assign(article,data);saveWorkspace(state);addAudit(status==='Pending approval'?'Article submitted for approval':'Article draft saved',article.title,article.id);setStatus('fdArticleStatus',status==='Pending approval'?'Article sent to the administrator for approval.':'Draft saved successfully.','success');renderAll()}
      function deleteArticle(id){const state=loadWorkspace(),article=state.articles.find(a=>a.id===id);if(!article)return;if(article.status==='Published'&&!isAdmin())return;if(!confirm(`Delete “${article.title}”?`))return;state.articles=state.articles.filter(a=>a.id!==id);saveWorkspace(state);addAudit('Article deleted',article.title,id);if(editingArticleId===id)clearEditor(false);renderAll()}
      function approveArticle(id){if(!isAdmin())return;const state=loadWorkspace(),article=state.articles.find(a=>a.id===id);if(!article)return;article.status='Published';article.publishedAt=nowISO();article.reviewedAt=nowISO();article.reviewedBy=activeAccount.id;article.reviewNote='Approved for publication.';saveWorkspace(state);addAudit('Article approved and published',article.title,id);renderAll();if(currentReaderArticleId===id)openReader(id)}
      function returnArticle(id,note=''){if(!isAdmin())return;const state=loadWorkspace(),article=state.articles.find(a=>a.id===id);if(!article)return;const reason=String(note||prompt('Enter a short revision note for the writer:')||'').trim();if(!reason)return;article.status='Rejected';article.reviewedAt=nowISO();article.reviewedBy=activeAccount.id;article.reviewNote=reason;saveWorkspace(state);addAudit('Article returned for revision',`${article.title}: ${reason}`,id);renderAll();if(currentReaderArticleId===id)openReader(id)}
      function unpublishArticle(id){if(!isAdmin())return;const state=loadWorkspace(),article=state.articles.find(a=>a.id===id);if(!article||!confirm(`Remove “${article.title}” from publications?`))return;article.status='Draft';article.publishedAt='';article.reviewNote='Unpublished by administrator.';saveWorkspace(state);addAudit('Article unpublished',article.title,id);renderAll();closeReader()}

      function openReader(id,{updateHash=true}={}){
        const article=loadWorkspace().articles.find(a=>a.id===id);if(!article)return;
        const canRead=article.status==='Published'||isAdmin()||(article.ownerId||article.createdBy)===activeAccount?.id||article.collaboratorId===activeAccount?.id;if(!canRead)return;
        ...
        $('#fdReaderMeta').textContent=`${article.category||'Article'} · ${accountName(articleOwner(article))} · Published ${formatDate(article.publishedAt||article.submittedAt||article.updatedAt||article.createdAt)}`;
        ...
        if(isAdmin()&&article.status==='Pending approval'){review.innerHTML=`<h3 style="font-size:13px;color:#33445d;margin:0">Administrator decision</h3><textarea class="fd-review-note-box" id="fdReaderReviewNote" placeholder="Add a note when returning the article for revision"></textarea><div class="fd-review-actions" style="justify-content:flex-start"><button class="fd-mini-btn is-success" type="button" data-reader-approve="${article.id}"><i class="fas fa-check"></i> Approve & publish</button><button class="fd-mini-btn is-danger" type="button" data-reader-return="${article.id}"><i class="fas fa-undo"></i> Return for revision</button></div>`}else if(article.reviewNote&&article.status!=='Published'){review.innerHTML=`<div class="fd-status-banner ${article.status==='Rejected'?'is-rejected':''}"><strong>Review note</strong><br>${escapeHTML(article.reviewNote)}</div>`}else review.innerHTML='';
      ...
      async function attemptLogin(identifier,password,remember=true,statusId='loginStatus'){const account=findAccount(identifier);if(!account){setStatus(statusId,'No account matches that username or email.','error');return false}if(account.active===false){setStatus(statusId,'This account is currently suspended.','error');return false}const digest=await passwordDigest(password);if(digest!==account.passwordHash){setStatus(statusId,'The password is incorrect.','error');return false}saveSession(account.username,remember);setStatus(statusId,'Login successful. Opening the writing desk…','success');enterDashboard(account);return true}

      $('#loginForm')?.addEventListener('submit',async event=>{event.preventDefault();const identifier=$('#loginIdentifier').value,password=$('#loginPassword').value,remember=$('#loginRemember').checked;if(!identifier||!password){setStatus('loginStatus','Enter your username or email and password.','error');return}await attemptLogin(identifier,password,remember,'loginStatus')});
      $('#utilityLoginForm')?.addEventListener('submit',async event=>{event.preventDefault();const session=findAccount(getSessionUsername());if(session){enterDashboard(session);return}const identifier=$('#utilityUsername')?.value,password=$('#utilityPassword')?.value;const ok=await attemptLogin(identifier,password,false,'loginStatus');if(!ok){openAuthModal('loginPopup');$('#loginIdentifier').value=identifier||''}});
      $('#signupForm')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());setStatus('signupStatus','');if(!data.firstName||!data.lastName||!data.email||!data.username||!data.role||!data.password||!data.confirmPassword){setStatus('signupStatus','Complete all required fields, including the role.','error');return}if(String(data.password).length<8){setStatus('signupStatus','Password must contain at least 8 characters.','error');return}if(data.password!==data.confirmPassword){setStatus('signupStatus','The two passwords do not match.','error');return}if(data.role==='admin'&&data.password!==ADMIN_PASSWORD){setStatus('signupStatus','Admin registration requires the administrator password admin@123.','error');return}const accounts=loadAccounts();if(accounts.some(a=>normal(a.email)===normal(data.email))){setStatus('signupStatus','This email address is already registered.','error');return}if(accounts.some(a=>normal(a.username)===normal(data.username))){setStatus('signupStatus','This username is already in use.','error');return}const account={id:uid(),firstName:String(data.firstName).trim(),lastName:String(data.lastName).trim(),email:String(data.email).trim(),username:String(data.username).trim(),phone:String(data.phone||'').trim(),organisation:String(data.organisation||'').trim(),role:normaliseRole(data.role),country:'',active:true,passwordHash:await passwordDigest(data.password),createdAt:nowISO()};accounts.push(account);saveAccounts(accounts);activeAccount=account;addAudit('Account registered',`${accountName(account)} registered as ${roleLabel(account.role)}.`);saveSession(account.username,true);form.reset();setStatus('signupStatus','Account created. Opening the writing desk…','success');setTimeout(()=>enterDashboard(account),180)});
      $('#recoveryForm')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,data=Object.fromEntries(new FormData(form).entries());if(!data.identifier||!data.password||!data.confirmPassword){setStatus('recoveryStatus','Complete all fields.','error');return}if(data.password!==data.confirmPassword||String(data.password).length<8){setStatus('recoveryStatus','Use matching passwords of at least 8 characters.','error');return}const accounts=loadAccounts(),index=accounts.findIndex(a=>normal(a.username)===normal(data.identifier)||normal(a.email)===normal(data.identifier));if(index===-1){setStatus('recoveryStatus','No registered account matches that username or email.','error');return}if(accounts[index].id===ADMIN_ID){setStatus('recoveryStatus','The built-in administrator password is fixed as admin@123 in this prototype.','error');return}accounts[index].passwordHash=await passwordDigest(data.password);saveAccounts(accounts);form.reset();setStatus('recoveryStatus','Password updated. You can now log in.','success')});
      ...
      $('#homePublicationsWrite')?.addEventListener('click',event=>{event.preventDefault();const account=findAccount(getSessionUsername());account?enterDashboard(account):openAuthModal('loginPopup')});
      ...
      window.addEventListener('storage',event=>{if(event.key===WORKSPACE_KEY)renderHomepagePublications()});

      document.addEventListener('click',event=>{const nav=event.target.closest('[data-dashboard-view]');if(nav){event.preventDefault();showView(nav.dataset.dashboardView)}const open=event.target.closest('[data-article-open-editor]');if(open)openEditor(open.dataset.articleOpenEditor);const preview=event.target.closest('[data-article-preview]');if(preview)openReader(preview.dataset.articlePreview);const del=event.target.closest('[data-article-delete]');if(del)deleteArticle(del.dataset.articleDelete);const approve=event.target.closest('[data-article-approve]');if(approve)approveArticle(approve.dataset.articleApprove);const ret=event.target.closest('[data-article-return]');if(ret)returnArticle(ret.dataset.articleReturn);const unpublish=event.target.closest('[data-article-unpublish]');if(unpublish)unpublishArticle(unpublish.dataset.articleUnpublish);const readerApprove=event.target.closest('[data-reader-approve]');if(readerApprove)approveArticle(readerApprove.dataset.readerApprove);const readerReturn=event.target.closest('[data-reader-return]');if(readerReturn)returnArticle(readerReturn.dataset.readerReturn,$('#fdReaderReviewNote')?.value)});
      ...
      $('#utilityRegister')?.addEventListener('click',event=>{event.preventDefault();const account=findAccount(getSessionUsername());account?enterDashboard(account):openAuthModal('signupPopup')});
      $('#openLogin')?.addEventListener('click',event=>{event.preventDefault();const account=findAccount(getSessionUsername());account?enterDashboard(account):openAuthModal('loginPopup')});
      ...
      async function bootstrap(){await ensureAdmin();$('#fdTodayLabel').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',day:'numeric',month:'long',year:'numeric'});renderHomepagePublications();const sessionAccount=findAccount(getSessionUsername());syncPublicAuthBar(sessionAccount||null);const publicationMatch=location.hash.match(/^#publication-(.+)$/);if(publicationMatch){openReader(publicationMatch[1],{updateHash:false});return}if(location.hash==='#recordings'){window.openRecordingsPanel?.();return}if(location.hash==='#live-training'){window.openLiveTrainingPanel?.();return}if(location.hash==='#publications'){window.openPublicationsPanel?.();return}if(sessionAccount&&sessionAccount.active!==false&&(location.hash==='#writing'||location.hash==='#workspace'||location.hash==='#dashboard'||localStorage.getItem(LOCAL_SESSION_KEY)))enterDashboard(sessionAccount)}
      bootstrap();
    })();
