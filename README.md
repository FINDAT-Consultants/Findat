...
## x1 | ProATR public OpenAI integration

The Financial Assistant sends normal conversational requests to the OpenAI Responses API through the public `findat-x1-openai` Supabase Edge Function. A FINDAT login is optional: signed-in users are recognized automatically, while visitors use anonymous server-side quota controls. The OpenAI key remains only in Supabase Secrets, and the browser does not silently substitute embedded RAG or Ollama answers when OpenAI fails.

Run `FINDAT-X1-OPENAI-API-UPGRADE.sql`, deploy `findat-x1-openai` with JWT gateway verification disabled, and follow `X1-OPENAI-API-DEPLOYMENT.md` or `SUPABASE-DASHBOARD-SETUP.md`.
