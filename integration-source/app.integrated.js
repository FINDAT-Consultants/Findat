...
(()=>{
      ...
      function requireSupabase(statusId='loginStatus'){if(supabaseClient)return true;setStatus(statusId,'Supabase authentication is not configured in this deployment.','error');return false}
      ...
      async function clearSession(){stopCollaborationRealtime();if(supabaseClient)await supabaseClient.auth.signOut();activeAccount=null;accountCache=[];directoryCache=[];collaborationCache=[];collaborationInbox=[];collaborationActivity=[];notificationFeed=[];notificationCommentArticleId='';notificationCommentReplyId='';contributorLayoutDraft=[];contributorLayoutArticleId='';contributorLayoutDirty=false;reviewSelection.clear();workspaceCache={articles:[],audit:[]};window.dispatchEvent(new CustomEvent('findat:account-cleared'))}
      ...
      async function uploadProfilePhoto(file){
        ...
        try{
          ...
          try{
            ...
            if(!publicData?.publicUrl)throw new Error('Supabase did not return the profile picture address.');
          ...
          setStatus('fdProfilePhotoStatus',savedInStorage?'Profile picture saved to Supabase Storage and updated across FINDAT.':`Profile picture saved in Supabase Postgres because Storage was unavailable${storageError?.message?`: ${storageError.message}`:''}.`,'success');
        }catch(error){
      ...
      async function saveCollaborators(){
        ...
        try{
          ...
          setStatus('fdArticleStatus',`${activeRows.length} collaborator${activeRows.length===1?'':'s'} saved in Supabase. New selections received a notification request.`,activeRows.length?'success':'')
        }catch(error){
      ...
      async function saveArticle(status='Draft'){
        ...
        Object.assign(article,data);setStatus('fdArticleStatus',status==='Published'?'Publishing securely to Supabase…':'Saving securely to Supabase…','');
        ...
        const successMessage=collaborativeEdit?'Collaborative changes saved securely in Supabase.':status==='Published'?'Article published successfully.':status==='Pending approval'?'Article sent to the administrator for approval.':'Draft saved securely in Supabase.';clearEditor(false);setStatus('fdArticleStatus',successMessage,'success');await refreshCollaborationState();renderAll()
      }
      ...
      window.FINDAT_APP_CONTEXT={
        get supabase(){return supabaseClient},
        get account(){return activeAccount},
        get directory(){return directoryCache.slice()},
        isAdmin,
        showView,
        enterDashboard,
        leaveDashboard,
...
(()=>{
    ...
    function runDemo(){
      ...
      window.FINDAT_X1_CAPTURE?.({module:'reconciliation',action:'run_reconciliation',prompt:'Reconcile uploaded bank and cash records',success:balanced,confidence:balanced?1:.65,inputPayload:{bankRows:bank.length,cashRows:cash.length,bankClosingBalance,cashClosingBalance,bankPreview:bank.slice(0,40),cashPreview:cash.slice(0,40)},outputPayload:{matchedRows,unmatchedRows,depositTransit,outstandingChecks,bankCollections,serviceCharges,adjustedBank,adjustedCash,balanced,reportReference:currentReportReference,exceptions:{unmatchedCashDebits:unmatchedCashDebits.slice(0,40),unmatchedCashCredits:unmatchedCashCredits.slice(0,40),unmatchedBankCredits:unmatchedBankCredits.slice(0,40),unmatchedBankDebits:unmatchedBankDebits.slice(0,40)}}});
    }
...
(()=>{
  ...
  let knowledgeBase=[];let approvedTrainingKnowledge=[];
  ...
  function buildKnowledgeIndex(){
    knowledgeBase=[...BUILT_IN_KB,...approvedTrainingKnowledge,...importedKnowledge];
  ...
  const FINANCE_SCOPE_TERMS=/\b(account|accounting|accrual|asset|audit|balance|bank|budget|cash|compliance|control|cost|credit|debit|depreciation|evidence|expense|finance|financial|forensic|fraud|income|invoice|journal|ledger|liability|money|payment|payroll|profit|reconcile|reconciliation|report|revenue|risk|tax|transaction|transform|workflow|working capital|variance)\b/i;
  function financeScopeAllowed(prompt,intent){
    if(['greeting','identity','capabilities','privacy','machine_learning','app_state','gratitude','calculation'].includes(intent))return true;
    if(FINANCE_SCOPE_TERMS.test(prompt))return true;
    const retrieved=retrieveKnowledge(prompt,1)[0];
    return Boolean(retrieved&&['finance','accounting','forensic','evidence','fraud','application','training'].includes(retrieved.item.kind)&&retrieved.score>.42);
  }
  function buildResponse(prompt){
    ...
    if(!financeScopeAllowed(prompt,intent))return{localText:'x1 is restricted to finance, accounting, financial reporting, data controls, reconciliation and financial-forensics work. Reframe the request within that professional scope.',intent:'scope',topic:'finance scope',action:null,meta:'Finance-only scope control',plan:{confidence:1},matches:[]};
  ...
  async function respond(raw){
    ...
    if(window.X1AgenticRuntime?.execute){
      try{
        ...
        if(agentResult?.handled){
          ...
          window.FINDAT_X1_CAPTURE?.({module:'financial_assistant',action:'agentic_command',prompt,answer:agentResult.text||'The requested x1 operation is complete.',success:true,confidence:1,inputPayload:{prompt},outputPayload:{meta:agentResult.meta||'Agentic execution',intent:agentResult.intent||'agentic_command'}});
    ...
    window.FINDAT_X1_CAPTURE?.({module:'financial_assistant',action:'answer',prompt,answer:finalText,success:true,confidence:Number(result.plan?.confidence||0),inputPayload:{prompt},outputPayload:{intent:result.intent||'',topic:result.topic||'',meta,evidenceIds:(result.matches||[]).map(item=>item.item?.id).filter(Boolean)}});
  ...
  function loadApprovedTrainingExamples(rows){
    approvedTrainingKnowledge=(Array.isArray(rows)?rows:[]).filter(row=>row&&row.status==='approved'&&row.is_active!==false).map(row=>({
      id:`approved_training_${row.id}`,
      title:`${row.domain||'Finance'} approved training example`,
      tags:[row.domain,row.module,row.intent_label,'approved example','ideal answer'].filter(Boolean),
      text:`Training prompt: ${row.prompt}\nApproved response: ${row.ideal_response}`,
      points:String(row.acceptance_criteria||'').split(/\n|;/).map(item=>item.trim()).filter(Boolean).slice(0,12),
      source:'FINDAT approved x1 training dataset',
      kind:'training'
    }));
    buildKnowledgeIndex();
    return approvedTrainingKnowledge.length;
  }
  window.X1FinanceReasoning={
    answer(prompt){const result=buildResponse(String(prompt||''));return{text:result.localText,intent:result.intent,confidence:Number(result.plan?.confidence||0),meta:result.meta||''}},
    loadApprovedTraining:loadApprovedTrainingExamples,
    getTrainingCount(){return approvedTrainingKnowledge.length},
    rebuild(){return buildKnowledgeIndex()}
  };
...
(()=>{
  ...
  function transformSource(role){
    ...
    window.FINDAT_X1_CAPTURE?.({module:'transform',action:'transform_data',prompt:`Transform ${role==='bank'?'bank statement':'cash ledger'} data`,success:true,confidence:Number(source.stats?.quality||0)/100,inputPayload:{role,fileName:source.fileName||'',format:source.format||'',headers:source.headers||[],rowsReceived:source.raw.length,preview:source.raw.slice(0,40)},outputPayload:{rowsPrepared:finalRows.length,closingBalance:source.closingBalance,stats:source.stats,mining:{patterns:source.mining?.patterns||[],outlierCount:source.mining?.outliers?.length||0,duplicates:source.mining?.duplicates||0,missing:source.mining?.missing||0},preview:finalRows.slice(0,40)}});
    return true;
  }
...
(()=>{
  ...
  async function runWorkflow(){
    ...
    try{
      ...
      window.FINDAT_X1_CAPTURE?.({module:'workflows',action:'run_workflow',prompt:'Execute x1 financial workflow',success:true,confidence:1,inputPayload:{definition:serialise(),nodeCount:state.nodes.length,edgeCount:state.edges.length},outputPayload:{nodes:state.nodes.map(node=>({id:node.id,type:node.type,title:node.title,status:node.status,rowCount:Array.isArray(node.runtime?.output)?node.runtime.output.length:0,preview:Array.isArray(node.runtime?.output)?node.runtime.output.slice(0,20):[]})),logs:state.logs.slice(-50)}});
...
(()=>{
  ...
  function renderCourseCards(){
    ...
    courseGrid.innerHTML=catalogueCourses.map(course=>{
      ...
      return `<article class="findat-course-card"><div class="findat-course-card-cover" ${cover?`style="background-image:url('${escapeHTML(cover)}')"`:''}><span><i class="fas fa-play-circle"></i>${lessonCount} lesson${lessonCount===1?'':'s'}</span></div><div class="findat-course-card-body"><h3>${escapeHTML(course.title)}</h3><p>${escapeHTML(course.short_description||course.course_content||'FINDAT Academy course')}</p><div class="findat-course-card-meta"><span class="findat-course-rating">${stars(course.rating)}</span><span>${escapeHTML(course.instructor_name||'FINDAT Academy')}</span></div><button class="findat-course-card-button" data-course-open="${course.id}" type="button">Open course</button></div></article>`
    }).join('')
  }
  async function loadCatalogue(){
    ...
    if(coursesResult.error){
      catalogueCourses=[{id:'legacy-data-analytics',slug:'data-analytics-foundations',title:'Data Analytics Foundations',short_description:'Learn how data analytics supports evidence-based decisions across professional domains.',instructor_name:'FINDAT Academy',instructor_qualifications:'Data Analytics | Research | Professional Practice',rating:5,is_builtin:true,lessons:[{id:'legacy-lesson',title:'How Data Analytics Work per Domain'}]}];
  ...
  function renderDynamicQuiz(quiz){
    ...
    $('#findatDynamicQuizForm')?.addEventListener('submit',event=>{event.preventDefault();let score=0;quiz.forEach((question,index)=>{const selected=event.currentTarget.querySelector(`input[name="q${index}"]:checked`);if(selected&&Number(selected.value)===Number(question.correct))score++});const percent=Math.round(score/quiz.length*100);$('#findatDynamicQuizResult').textContent=`Score: ${score} / ${quiz.length} (${percent}%).`})
  }
  ...
  function resetCourseForm(){
    const form=$('#fdCourseForm');if(!form)return;form.reset();form.elements.id.value='';form.elements.rating.value='5';form.elements.status.value='draft';managerCourseId='';courseCoverFile=null;$('#fdCourseCoverName').textContent='No picture selected';renderManagerCourseList();resetLessonForm();renderManagerLessonList()
  }
...
(()=>{
  ...
  const $=selector=>document.querySelector(selector);
  const $$=selector=>[...document.querySelectorAll(selector)];
  const state={tasks:[],examples:[],interactions:[],reviews:[],evalRuns:[],snapshots:[],access:false,busy:false,currentTab:'overview'};
  const moduleNames={financial_assistant:'Financial assistant',transform:'Transform Data',reconciliation:'Reconciliation',workflows:'Workflows'};
  const statusNames={assigned:'Assigned',in_progress:'In progress',submitted:'Submitted',approved:'Approved',closed:'Closed',draft:'Draft',rejected:'Rejected'};
  const context=()=>window.FINDAT_APP_CONTEXT||null;
  const account=()=>context()?.account||null;
  const db=()=>context()?.supabase||null;
  const isAdmin=()=>account()?.role==='admin';
  const esc=value=>String(value??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
  const uidName=id=>{const row=context()?.directory?.find?.(item=>item.id===id);return row?[row.firstName,row.lastName].filter(Boolean).join(' ')||row.username||'FINDAT Member':'FINDAT Member'};
  const shortDate=value=>value?new Date(value).toLocaleString(undefined,{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}):'—';
  const setStatus=(id,message,type='')=>{const el=$(`#${id}`);if(!el)return;el.textContent=message||'';el.classList.toggle('is-error',type==='error');el.classList.toggle('is-success',type==='success')};
  const jsonSafe=value=>{try{return JSON.stringify(value??{},null,2)}catch{return'{}'}};
  const financeTokens=value=>String(value||'').toLowerCase().replace(/[^a-z0-9.%+-]+/g,' ').split(/\s+/).filter(token=>token.length>2);
  const tokenScore=(actual,ideal)=>{const a=new Set(financeTokens(actual)),b=new Set(financeTokens(ideal));if(!b.size)return 0;let hit=0;b.forEach(token=>a.has(token)&&hit++);const recall=hit/b.size,precision=hit/Math.max(a.size,1);return precision+recall?2*precision*recall/(precision+recall):0};
  const numberTokens=value=>String(value||'').match(/-?\d+(?:[,.]\d+)*(?:\.\d+)?%?/g)||[];
  function evaluateText(actual,ideal){const semantic=tokenScore(actual,ideal);const expectedNumbers=numberTokens(ideal),actualNumbers=new Set(numberTokens(actual));const numberScore=expectedNumbers.length?expectedNumbers.filter(item=>actualNumbers.has(item)).length/expectedNumbers.length:1;const scope=/\b(financ|account|bank|cash|reconcil|audit|forensic|fraud|ledger|debit|credit|evidence|control|report|transaction)\w*/i.test(`${actual} ${ideal}`)?1:.7;return Math.round((semantic*.65+numberScore*.25+scope*.1)*100)}
  function showTab(name){state.currentTab=name;$$('[data-x1tl-tab]').forEach(button=>button.classList.toggle('is-active',button.dataset.x1tlTab===name));$$('[data-x1tl-panel]').forEach(panel=>panel.classList.toggle('is-active',panel.dataset.x1tlPanel===name));$('#fdDashboardMain')?.scrollTo({top:0,behavior:'smooth'})}
  function activeTasks(){return state.tasks.filter(task=>!['approved','closed'].includes(task.status))}
  function approvedExamples(){return state.examples.filter(example=>example.status==='approved'&&example.is_active!==false)}
  function myTasks(){const me=account();return isAdmin()?state.tasks:state.tasks.filter(task=>task.assigned_to===me?.id)}
  function updateAccess(){const me=account();state.access=Boolean(me&&(me.role==='admin'||(me.role==='consultant'&&state.tasks.some(task=>task.assigned_to===me.id&&!['closed'].includes(task.status)))));const nav=$('#fdX1TrainingNav');if(nav){nav.hidden=!state.access;nav.style.display=state.access?'':'none'}const shortcut=$('#x1TrainingLabShortcut');if(shortcut){shortcut.hidden=!state.access;shortcut.style.display=state.access?'':'none'}$$('.x1tl-admin-only').forEach(el=>el.hidden=!isAdmin());return state.access}
  async function selectData(table,columns='*',configure=query=>query){let query=db().from(table).select(columns);query=configure(query)||query;const{data,error}=await query;if(error)throw error;return data||[]}
  async function refreshAll(){const client=db(),me=account();if(!client||!me)return;state.busy=true;try{
    const [tasks,examples,interactions,reviews,evalRuns,snapshots]=await Promise.all([
      selectData('findat_x1_training_tasks','*',q=>q.order('updated_at',{ascending:false})),
      selectData('findat_x1_training_examples','*',q=>q.order('updated_at',{ascending:false})),
      selectData('findat_x1_interactions','*',q=>q.order('created_at',{ascending:false}).limit(250)),
      selectData('findat_x1_reviews','*',q=>q.order('created_at',{ascending:false}).limit(250)),
      selectData('findat_x1_eval_runs','*',q=>q.order('created_at',{ascending:false}).limit(30)),
      selectData('findat_x1_model_snapshots','*',q=>q.order('created_at',{ascending:false}).limit(20))
    ]);
    Object.assign(state,{tasks,examples,interactions,reviews,evalRuns,snapshots});updateAccess();populateSelectors();renderAll();window.X1FinanceReasoning?.loadApprovedTraining(approvedExamples());
  }catch(error){console.warn('x1 training refresh failed',error);if(String(error.message||'').includes('does not exist')){setStatus('x1tlTaskStatus','Run the supplied x1 AI Training SQL upgrade in Supabase first.','error')}else setStatus('x1tlTaskStatus',error.message||'Training data could not be loaded.','error')}finally{state.busy=false}}
  function populateSelectors(){const consultants=(context()?.directory||[]).filter(item=>item.role==='consultant'&&item.active!==false);const assignee=$('#x1tlTaskAssignee');if(assignee){const current=assignee.value;assignee.innerHTML='<option value="">Select Consultant</option>'+consultants.map(item=>`<option value="${esc(item.id)}">${esc([item.firstName,item.lastName].filter(Boolean).join(' ')||item.username)}</option>`).join('');assignee.value=current}const taskSelect=$('#x1tlExampleTask');if(taskSelect){const current=taskSelect.value;taskSelect.innerHTML=(isAdmin()?'<option value="">Independent example</option>':'<option value="">Select assigned task</option>')+myTasks().filter(task=>!['closed'].includes(task.status)).map(task=>`<option value="${esc(task.id)}">${esc(task.title)}</option>`).join('');taskSelect.value=current}}
  function renderMetrics(){const latest=state.evalRuns[0];$('#x1tlMetricTasks').textContent=String(activeTasks().length);$('#x1tlMetricExamples').textContent=String(approvedExamples().length);$('#x1tlMetricPassRate').textContent=latest?`${Math.round(Number(latest.pass_rate||0))}%`:'—';$('#x1tlMetricInteractions').textContent=String(state.interactions.length);$('#fdX1TrainingNavCount').textContent=String(isAdmin()?activeTasks().length:myTasks().filter(task=>!['approved','closed'].includes(task.status)).length)}
  function compactTask(task){return`<div class="x1tl-compact-item"><i class="fas fa-tasks"></i><div><strong>${esc(task.title)}</strong><small>${esc(moduleNames[task.module]||task.module)} · ${esc(statusNames[task.status]||task.status)}</small></div><time>${esc(shortDate(task.updated_at))}</time></div>`}
  function compactInteraction(row){return`<div class="x1tl-compact-item"><i class="fas fa-${row.module==='financial_assistant'?'comment-dots':row.module==='transform'?'magic':row.module==='reconciliation'?'balance-scale':'project-diagram'}"></i><div><strong>${esc(row.prompt||row.action)}</strong><small>${esc(moduleNames[row.module]||row.module)} · ${esc(row.success?'Completed':'Review')}</small></div><time>${esc(shortDate(row.created_at))}</time></div>`}
  function renderOverview(){const tasks=myTasks().slice(0,5),interactions=state.interactions.slice(0,5);$('#x1tlOverviewTasks').innerHTML=tasks.length?tasks.map(compactTask).join(''):'<div class="x1tl-empty">No training assignments are currently available.</div>';$('#x1tlOverviewInteractions').innerHTML=interactions.length?interactions.map(compactInteraction).join(''):'<div class="x1tl-empty">No x1 activity has been captured yet.</div>'}
  function statusBadge(status){const tone=status==='approved'||status==='closed'?'is-green':status==='rejected'?'is-red':status==='submitted'||status==='in_progress'?'is-orange':'';return`<span class="x1tl-badge ${tone}">${esc(statusNames[status]||status)}</span>`}
  function renderTasks(){const filter=$('#x1tlTaskFilter')?.value||'';const rows=myTasks().filter(task=>!filter||task.status===filter);$('#x1tlTaskSummary').textContent=`${rows.length} task${rows.length===1?'':'s'}`;$('#x1tlTaskList').innerHTML=rows.length?rows.map(task=>{const controls=[];if(!isAdmin()&&task.status==='assigned')controls.push(`<button class="x1tl-mini-button is-primary" data-x1tl-task-status="in_progress" data-task-id="${task.id}">Start task</button>`);if(!isAdmin()&&task.status==='in_progress')controls.push(`<button class="x1tl-mini-button is-primary" data-x1tl-task-status="submitted" data-task-id="${task.id}">Submit work</button>`);if(isAdmin()&&task.status==='submitted')controls.push(`<button class="x1tl-mini-button is-primary" data-x1tl-task-status="approved" data-task-id="${task.id}">Approve task</button>`);if(isAdmin()&&!['closed'].includes(task.status))controls.push(`<button class="x1tl-mini-button" data-x1tl-task-status="closed" data-task-id="${task.id}">Close</button>`);return`<article class="x1tl-task-card"><div class="x1tl-task-card-head"><div><h3>${esc(task.title)}</h3><div class="x1tl-task-meta"><span class="x1tl-badge is-orange">${esc(moduleNames[task.module]||task.module)}</span>${statusBadge(task.status)}<span class="x1tl-badge">${esc(task.priority||'normal')}</span></div></div><span class="x1tl-badge"><i class="fas fa-user"></i> ${esc(uidName(task.assigned_to))}</span></div><p>${esc(task.instructions||'No additional instructions.')}</p><div class="x1tl-task-meta"><span class="x1tl-badge"><i class="fas fa-calendar"></i> Due ${esc(task.due_at?new Date(task.due_at).toLocaleDateString():'not set')}</span><span class="x1tl-badge">Target ${Number(task.target_examples||0)} examples</span></div><div class="x1tl-task-actions">${controls.join('')}</div></article>`}).join(''):'<div class="x1tl-empty">No training tasks match this filter.</div>'}
  function renderExamples(){const filter=$('#x1tlExampleFilter')?.value||'';const rows=state.examples.filter(example=>!filter||example.status===filter);$('#x1tlExampleSummary').textContent=`${rows.length} example${rows.length===1?'':'s'}`;$('#x1tlExampleList').innerHTML=rows.length?rows.map(example=>{const mine=example.created_by===account()?.id;const buttons=[];if(mine&&['draft','rejected'].includes(example.status)){buttons.push(`<button class="x1tl-mini-button" data-x1tl-edit-example="${example.id}">Edit</button>`,`<button class="x1tl-mini-button is-primary" data-x1tl-submit-example="${example.id}">Submit for review</button>`)}if(isAdmin()&&example.status==='approved')buttons.push(`<button class="x1tl-mini-button" data-x1tl-toggle-example="${example.id}" data-active="${example.is_active!==false?'false':'true'}">${example.is_active!==false?'Disable':'Enable'}</button>`);return`<article class="x1tl-example-card"><div class="x1tl-example-head"><div><h3>${esc(example.prompt)}</h3><small>${esc(example.domain)} · ${esc(moduleNames[example.module]||example.module)} · ${esc(uidName(example.created_by))}</small></div>${statusBadge(example.status)}</div><div class="x1tl-ideal"><strong>Ideal response</strong><p>${esc(example.ideal_response)}</p></div><div class="x1tl-example-actions">${buttons.join('')}</div></article>`}).join(''):'<div class="x1tl-empty">No training examples match this filter.</div>'}
  function renderInteractions(){const filter=$('#x1tlInteractionFilter')?.value||'';const rows=state.interactions.filter(row=>!filter||row.module===filter);$('#x1tlInteractionList').innerHTML=rows.length?rows.map(row=>`<article class="x1tl-interaction-card"><div class="x1tl-interaction-head"><div><h3>${esc(row.prompt||row.action)}</h3><small>${esc(moduleNames[row.module]||row.module)} · ${esc(uidName(row.user_id))} · ${esc(shortDate(row.created_at))}</small></div><span class="x1tl-badge ${row.success?'is-green':'is-red'}">${row.success?'Completed':'Review'}</span></div>${row.answer?`<div class="x1tl-response-box"><strong>x1 output</strong><p>${esc(row.answer)}</p></div>`:''}<details><summary>Structured input and output</summary><pre class="x1tl-json">${esc(jsonSafe({input:row.input_payload,output:row.output_payload}))}</pre></details><div class="x1tl-interaction-actions"><button class="x1tl-mini-button is-primary" data-x1tl-use-interaction="${row.id}"><i class="fas fa-plus"></i> Use as training example</button></div></article>`).join(''):'<div class="x1tl-empty">No captured x1 inputs or outputs match this filter.</div>'}
  function rubricOptions(){return[1,2,3,4,5].map(value=>`<option value="${value}" ${value===4?'selected':''}>${value}</option>`).join('')}
  function renderReviews(){const rows=state.examples.filter(example=>example.status==='submitted');$('#x1tlReviewList').innerHTML=rows.length?rows.map(example=>`<article class="x1tl-review-card" data-review-card="${example.id}"><div class="x1tl-review-head"><div><h3>${esc(example.prompt)}</h3><small>${esc(example.domain)} · submitted by ${esc(uidName(example.created_by))}</small></div><span class="x1tl-badge is-orange">Awaiting review</span></div><div class="x1tl-response-grid"><div class="x1tl-response-box"><strong>Response A</strong><p>${esc(example.response_a||'No candidate response A supplied.')}</p></div><div class="x1tl-response-box"><strong>Response B</strong><p>${esc(example.response_b||'No candidate response B supplied.')}</p></div></div><div class="x1tl-ideal"><strong>Reference / ideal response</strong><p>${esc(example.ideal_response)}</p></div><div class="x1tl-preference"><span>Preferred output:</span><label><input name="preference-${example.id}" type="radio" value="A"> A</label><label><input name="preference-${example.id}" type="radio" value="B"> B</label><label><input name="preference-${example.id}" type="radio" value="tie"> Tie</label><label><input name="preference-${example.id}" checked type="radio" value="ideal"> Ideal answer</label></div><div class="x1tl-review-rubric"><label>Correctness<select data-rubric="correctness">${rubricOptions()}</select></label><label>Relevance<select data-rubric="relevance">${rubricOptions()}</select></label><label>Clarity<select data-rubric="clarity">${rubricOptions()}</select></label><label>Evidence support<select data-rubric="support">${rubricOptions()}</select></label></div><textarea data-review-notes placeholder="Reviewer notes, corrections, unsupported claims or deployment conditions"></textarea><div class="x1tl-task-actions"><button class="x1tl-mini-button is-danger" data-x1tl-review="reject" data-example-id="${example.id}">${isAdmin()?'Reject':'Recommend rejection'}</button><button class="x1tl-mini-button is-primary" data-x1tl-review="approve" data-example-id="${example.id}">${isAdmin()?'Approve training example':'Recommend approval'}</button></div></article>`).join(''):'<div class="x1tl-empty">No submitted examples are waiting for review.</div>'}
  function renderEvaluationRuns(){const latest=state.evalRuns[0];if(!latest){$('#x1tlEvaluationSummary').innerHTML='';$('#x1tlEvaluationResults').innerHTML='<div class="x1tl-empty">No evaluation run has been recorded.</div>';return}$('#x1tlEvaluationSummary').innerHTML=`<article><strong>${Number(latest.dataset_size||0)}</strong><small>Cases</small></article><article><strong>${Number(latest.passed||0)}</strong><small>Passed</small></article><article><strong>${Number(latest.failed||0)}</strong><small>Failed</small></article><article><strong>${Math.round(Number(latest.pass_rate||0))}%</strong><small>Pass rate</small></article>`;const results=Array.isArray(latest.results)?latest.results:[];$('#x1tlEvaluationResults').innerHTML=results.length?results.map(row=>`<article class="x1tl-eval-card"><div class="x1tl-eval-head"><div><h3>${esc(row.prompt)}</h3><small>${esc(row.intent||'finance')} · ${esc(row.meta||'Embedded engine')}</small></div><span class="x1tl-badge ${row.passed?'is-green':'is-red'}">${row.score}% · ${row.passed?'Pass':'Fail'}</span></div><div class="x1tl-response-box"><strong>Generated output</strong><p>${esc(row.answer)}</p><div class="x1tl-score-bar"><i style="width:${Math.max(0,Math.min(100,row.score))}%"></i></div></div></article>`).join(''):'<div class="x1tl-empty">The latest run contains no case details.</div>'}
  function renderSnapshot(){const active=state.snapshots.find(row=>row.is_active)||state.snapshots[0];$('#x1tlSnapshotName').textContent=active?.name||'Built-in baseline';$('#x1tlSnapshotMeta').textContent=active?`${Number(active.training_count||0)} approved examples · created ${shortDate(active.created_at)}`:'No approved training snapshot has been applied yet.'}
  function renderAll(){renderMetrics();renderOverview();renderTasks();renderExamples();renderInteractions();renderReviews();renderEvaluationRuns();renderSnapshot()}
  async function createTask(event){event.preventDefault();if(!isAdmin())return;const form=event.currentTarget,data=new FormData(form),row={title:String(data.get('title')||'').trim(),module:String(data.get('module')||'financial_assistant'),assigned_to:String(data.get('assignedTo')||''),priority:String(data.get('priority')||'normal'),due_at:data.get('dueAt')?new Date(`${data.get('dueAt')}T23:59:59`).toISOString():null,target_examples:Number(data.get('targetExamples')||5),instructions:String(data.get('instructions')||'').trim(),status:'assigned',created_by:account().id};if(!row.title||!row.assigned_to)return setStatus('x1tlTaskStatus','Enter a title and choose a Consultant.','error');setStatus('x1tlTaskStatus','Assigning training task…');const{error}=await db().from('findat_x1_training_tasks').insert(row);if(error)return setStatus('x1tlTaskStatus',error.message,'error');form.reset();setStatus('x1tlTaskStatus','Training task assigned.','success');await refreshAll()}
  async function updateTaskStatus(id,status){const{error}=await db().rpc('findat_x1_set_task_status',{p_task_id:id,p_status:status});if(error)return alert(error.message);await refreshAll()}
  function clearExampleForm(){const form=$('#x1tlExampleForm');form?.reset();if(form?.elements.id)form.elements.id.value='';setStatus('x1tlExampleStatus','')}
  function fillExampleForm(example){const form=$('#x1tlExampleForm');if(!form)return;const map={id:'id',taskId:'task_id',domain:'domain',module:'module',intentLabel:'intent_label',riskLevel:'risk_level',sourceReference:'source_reference',prompt:'prompt',responseA:'response_a',responseB:'response_b',idealResponse:'ideal_response',acceptanceCriteria:'acceptance_criteria'};Object.entries(map).forEach(([name,key])=>{if(form.elements[name])form.elements[name].value=example[key]??''});showTab('examples');form.scrollIntoView({behavior:'smooth',block:'start'})}
  async function saveExample(event){event.preventDefault();const form=event.currentTarget,data=new FormData(form),id=String(data.get('id')||''),row={task_id:String(data.get('taskId')||'')||null,domain:String(data.get('domain')||'Accounting'),module:String(data.get('module')||'financial_assistant'),intent_label:String(data.get('intentLabel')||'').trim(),risk_level:String(data.get('riskLevel')||'low'),source_reference:String(data.get('sourceReference')||'').trim(),prompt:String(data.get('prompt')||'').trim(),response_a:String(data.get('responseA')||'').trim(),response_b:String(data.get('responseB')||'').trim(),ideal_response:String(data.get('idealResponse')||'').trim(),acceptance_criteria:String(data.get('acceptanceCriteria')||'').trim(),created_by:account().id};if(!row.prompt||!row.ideal_response)return setStatus('x1tlExampleStatus','Enter the prompt and ideal response.','error');setStatus('x1tlExampleStatus','Saving training example…');let result;if(id)result=await db().from('findat_x1_training_examples').update(row).eq('id',id);else result=await db().from('findat_x1_training_examples').insert({...row,status:'draft',is_active:false});if(result.error)return setStatus('x1tlExampleStatus',result.error.message,'error');clearExampleForm();setStatus('x1tlExampleStatus','Training example saved.','success');await refreshAll()}
  async function submitExample(id){const{error}=await db().rpc('findat_x1_submit_example',{p_example_id:id});if(error)return alert(error.message);await refreshAll()}
  async function toggleExample(id,active){const{error}=await db().from('findat_x1_training_examples').update({is_active:active}).eq('id',id);if(error)return alert(error.message);await refreshAll()}
  async function reviewExample(id,decision){const card=$(`[data-review-card="${CSS.escape(id)}"]`),preference=card?.querySelector(`input[name="preference-${CSS.escape(id)}"]:checked`)?.value||'ideal',score=name=>Number(card?.querySelector(`[data-rubric="${name}"]`)?.value||4),notes=card?.querySelector('[data-review-notes]')?.value||'';const{error}=await db().rpc('findat_x1_review_example',{p_example_id:id,p_decision:decision,p_preference:preference,p_correctness:score('correctness'),p_relevance:score('relevance'),p_clarity:score('clarity'),p_support:score('support'),p_notes:notes});if(error)return alert(error.message);await refreshAll()}
  function useInteraction(id){const row=state.interactions.find(item=>item.id===id);if(!row)return;fillExampleForm({id:'',task_id:'',domain:row.module==='reconciliation'?'Bank reconciliation':row.module==='transform'?'Data transformation':row.module==='workflows'?'Workflow controls':'Accounting',module:row.module,intent_label:row.output_payload?.intent||row.action,risk_level:'medium',source_reference:`Captured x1 run ${row.id}`,prompt:row.prompt||row.action,response_a:row.answer||jsonSafe(row.output_payload),response_b:'',ideal_response:'',acceptance_criteria:''});setStatus('x1tlExampleStatus','Captured output loaded. Write the ideal answer or expected result before saving.','success')}
  async function runEvaluation(){const examples=approvedExamples();if(!examples.length)return setStatus('x1tlEvaluationStatus','Approve at least one active training example before running an evaluation.','error');const engine=window.X1FinanceReasoning;if(!engine)return setStatus('x1tlEvaluationStatus','The x1 reasoning engine is not ready. Open Developments once, then retry.','error');setStatus('x1tlEvaluationStatus',`Evaluating ${examples.length} approved case${examples.length===1?'':'s'}…`);const results=[];for(const example of examples){const generated=engine.answer(example.prompt),score=evaluateText(generated.text,example.ideal_response);results.push({id:example.id,prompt:example.prompt,answer:generated.text,intent:generated.intent,meta:generated.meta,score,passed:score>=60})}const passed=results.filter(row=>row.passed).length,failed=results.length-passed,passRate=results.length?passed/results.length*100:0,average=results.reduce((sum,row)=>sum+row.score,0)/Math.max(results.length,1);const{error}=await db().from('findat_x1_eval_runs').insert({run_name:`Evaluation ${new Date().toISOString().slice(0,16).replace('T',' ')}`,created_by:account().id,dataset_size:results.length,passed,failed,pass_rate:passRate,average_score:average,results});if(error)return setStatus('x1tlEvaluationStatus',error.message,'error');setStatus('x1tlEvaluationStatus',`Evaluation complete: ${passed} passed and ${failed} failed.`,failed?'error':'success');await refreshAll()}
  async function buildSnapshot(event){event.preventDefault();if(!isAdmin())return;const form=event.currentTarget,data=new FormData(form),name=String(data.get('name')||'').trim(),note=String(data.get('note')||'').trim();if(!name)return setStatus('x1tlSnapshotStatus','Enter a snapshot name.','error');const{data:result,error}=await db().rpc('findat_x1_apply_training_snapshot',{p_name:name,p_release_note:note});if(error)return setStatus('x1tlSnapshotStatus',error.message,'error');form.reset();setStatus('x1tlSnapshotStatus',`Snapshot applied with ${result?.training_count??approvedExamples().length} approved examples.`,'success');await refreshAll()}
  async function capture(payload){const client=db(),me=account();if(!client||!me||!payload)return;const row={user_id:me.id,module:payload.module||'financial_assistant',action:payload.action||'run',prompt:String(payload.prompt||''),input_payload:payload.inputPayload||{},output_payload:payload.outputPayload||{},answer:String(payload.answer||''),success:payload.success!==false,confidence:Number.isFinite(Number(payload.confidence))?Number(payload.confidence):null};const{error}=await client.from('findat_x1_interactions').insert(row);if(error)console.warn('x1 interaction capture failed',error);else if(state.access){state.interactions.unshift({...row,id:crypto.randomUUID(),created_at:new Date().toISOString()});renderMetrics();renderOverview();renderInteractions()}}
  window.FINDAT_X1_CAPTURE=capture;
  window.FINDAT_X1_TRAINING_HAS_ACCESS=()=>state.access;
  window.FINDAT_X1_TRAINING_OPEN=async()=>{if(!account())return alert('Log in to open the x1 Training Lab.');if(!state.access)await refreshAll();if(!state.access)return alert('The x1 Training Lab is available to Administrators and Consultants with an active training assignment.');document.body.classList.add('dashboard-mode');$('#findatDashboard')?.setAttribute('aria-hidden','false');context()?.showView?.('x1-training');showTab(state.currentTab)};
  function openDevelopments(){document.body.classList.remove('dashboard-mode');$('#findatDashboard')?.setAttribute('aria-hidden','true');location.hash='#developments';setTimeout(()=>{$('#developmentsTrigger')?.click();setTimeout(()=>document.querySelector('[data-open-x1-app]')?.click(),120)},80)}
  document.addEventListener('click',event=>{const tab=event.target.closest('[data-x1tl-tab]');if(tab)showTab(tab.dataset.x1tlTab);const go=event.target.closest('[data-x1tl-go]');if(go)showTab(go.dataset.x1tlGo);const task=event.target.closest('[data-x1tl-task-status]');if(task)updateTaskStatus(task.dataset.taskId,task.dataset.x1tlTaskStatus);const edit=event.target.closest('[data-x1tl-edit-example]');if(edit){const row=state.examples.find(item=>item.id===edit.dataset.x1tlEditExample);if(row)fillExampleForm(row)}const submit=event.target.closest('[data-x1tl-submit-example]');if(submit)submitExample(submit.dataset.x1tlSubmitExample);const toggle=event.target.closest('[data-x1tl-toggle-example]');if(toggle)toggleExample(toggle.dataset.x1tlToggleExample,toggle.dataset.active==='true');const use=event.target.closest('[data-x1tl-use-interaction]');if(use)useInteraction(use.dataset.x1tlUseInteraction);const review=event.target.closest('[data-x1tl-review]');if(review)reviewExample(review.dataset.exampleId,review.dataset.x1tlReview);if(event.target.closest('#x1TrainingLabShortcut'))window.FINDAT_X1_TRAINING_OPEN();});
  $('#x1tlTaskForm')?.addEventListener('submit',createTask);$('#x1tlExampleForm')?.addEventListener('submit',saveExample);$('#x1tlExampleClear')?.addEventListener('click',clearExampleForm);$('#x1tlTaskFilter')?.addEventListener('change',renderTasks);$('#x1tlExampleFilter')?.addEventListener('change',renderExamples);$('#x1tlInteractionFilter')?.addEventListener('change',renderInteractions);$('#x1tlRefreshAll')?.addEventListener('click',refreshAll);$('#x1tlRefreshInteractions')?.addEventListener('click',refreshAll);$('#x1tlRunEvaluation')?.addEventListener('click',runEvaluation);$('#x1tlSnapshotForm')?.addEventListener('submit',buildSnapshot);$('#x1tlOpenDevelopments')?.addEventListener('click',openDevelopments);
  window.addEventListener('findat:account-ready',refreshAll);window.addEventListener('findat:account-cleared',()=>{Object.assign(state,{tasks:[],examples:[],interactions:[],reviews:[],evalRuns:[],snapshots:[],access:false});updateAccess();renderAll()});
  setTimeout(()=>{if(account())refreshAll()},600);
  setInterval(()=>{if(account()&&!state.busy)refreshAll()},20000);
})();
